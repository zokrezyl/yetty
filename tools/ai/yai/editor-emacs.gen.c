...
struct yetty_yclass_object_ptr_result yetty_yai_emacs_create(struct yetty_yclass_ctx *ctx)
{
     ...
     * response isn't silently swallowed. */
    {
        ...
        if (YETTY_IS_ERR(translate_class_r)) {
            yetty_ycore_error_print(
                stderr, "yetty_yai_emacs_create: translate_class (degraded — will lazy-resolve)",
    ...
    if (YETTY_IS_ERR(create_call_r)) {
        return YETTY_ERR(yetty_yclass_object_ptr, "yetty_yai_emacs_create: CREATE call failed",
                         create_call_r);
    }
