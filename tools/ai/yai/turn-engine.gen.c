...
struct yetty_yclass_ptr_result yetty_yai_turn_engine_class_get(void)
{
    ...
    static const struct yetty_yclass_op ops[] = {
        {"yetty_yai", "on_child_exit", (yetty_yclass_method_id_t)yetty_yai_on_child_exit,
         (yetty_yclass_impl_t)turn_engine_on_child_exit},
        {"yetty_yai", "on_child_eof", (yetty_yclass_method_id_t)yetty_yai_on_child_eof,
         (yetty_yclass_impl_t)turn_engine_on_child_eof},
        {"yetty_yai", "interrupt", (yetty_yclass_method_id_t)yetty_yai_interrupt,
         (yetty_yclass_impl_t)turn_engine_interrupt},
    };
    struct yetty_yclass_ptr_result parent_class_r = yetty_yai_engine_class_get();
    if (YETTY_IS_ERR(parent_class_r)) {
        yerror("yetty_yai_turn_engine_class_get: parent accessor failed: %s",
               parent_class_r.error.msg);
        return YETTY_ERR(yetty_yclass_ptr,
                         "yetty_yai_turn_engine_class_get: parent accessor failed", parent_class_r);
    }
    struct yetty_yclass_ptr_result register_class_r = yetty_yclass_register(
        &desc, ops, sizeof(ops) / sizeof(ops[0]), parent_class_r.value, NULL, 0);
    if (YETTY_IS_ERR(register_class_r)) {
        yerror("yetty_yai_turn_engine_class_get: class_register failed: %s",
               register_class_r.error.msg);
        return YETTY_ERR(yetty_yclass_ptr, "yetty_yai_turn_engine_class_get: class_register failed",
                         register_class_r);
    }
...
struct yetty_yai_turn_engine_ptr_result yetty_yai_turn_engine_from(struct yetty_yclass_object *obj)
{
    ...
    if (YETTY_IS_ERR(class_r)) {
        return YETTY_ERR(yetty_yai_turn_engine_ptr, "yetty_yai_turn_engine_from: class accessor",
                         class_r);
    }
    struct yetty_yclass_void_ptr_result slice_r = yetty_yclass_object_data(obj, class_r.value);
    if (YETTY_IS_ERR(slice_r)) {
        return YETTY_ERR(yetty_yai_turn_engine_ptr, "yetty_yai_turn_engine_from: object_data",
                         slice_r);
    }
...
struct yetty_yclass_object_ptr_result yetty_yai_turn_engine_to(struct yetty_yai_turn_engine *data)
{
    ...
    YETTY_RETURN_IF_ERR(yetty_yclass_object_ptr, class_r,
                        "yetty_yai_turn_engine_to: class accessor");
...
struct yetty_yclass_object_ptr_result yetty_yai_turn_engine_create(struct yetty_yclass_ctx *ctx)
{
    ...
    if (!proxy) {
        return YETTY_ERR(yetty_yclass_object_ptr,
                         "yetty_yai_turn_engine_create: calloc(proxy) failed");
    }
