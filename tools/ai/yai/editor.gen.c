                                                  ...
                                                  struct yai_app *app, int byte)
{
    ...
    if (method_slot == YETTY_YCLASS_METHOD_SLOT_UNDEFINED) {
        struct yetty_yclass_method_slot_result method_slot_r = yetty_yclass_method_slot_get(
            "yetty_yai", (yetty_yclass_method_id_t)yetty_yai_feed_byte);
        if (YETTY_IS_ERR(method_slot_r)) {
            return YETTY_ERR(yetty_ycore_int, "yetty_yai_feed_byte: method_slot_get failed",
                             method_slot_r);
        }
    ...
    if (!obj) {
        return YETTY_ERR(yetty_ycore_int, "yetty_yai_feed_byte: NULL object");
    }
    ...
    YETTY_RETURN_IF_ERR(yetty_ycore_int, object_class_r,
                        "yetty_yai_feed_byte: object_class failed");
    struct yetty_yclass_impl_t_result dispatch_impl_r =
        yetty_yclass_dispatch_lookup(object_class_r.value, method_slot);
    YETTY_RETURN_IF_ERR(yetty_ycore_int, dispatch_impl_r,
                        "yetty_yai_feed_byte: dispatch_lookup failed");
...
struct yetty_yclass_object_ptr_result yetty_yai_editor_create(struct yetty_yclass_ctx *ctx)
{
     ...
     * response isn't silently swallowed. */
    {
        ...
        if (YETTY_IS_ERR(translate_class_r)) {
            yetty_ycore_error_print(
                stderr, "yetty_yai_editor_create: translate_class (degraded — will lazy-resolve)",
    ...
    if (YETTY_IS_ERR(create_call_r)) {
        return YETTY_ERR(yetty_yclass_object_ptr, "yetty_yai_editor_create: CREATE call failed",
                         create_call_r);
    }
