/*
 * commands.c — ccc's slash-command table.
 */
#include "commands.h"
...
static struct yetty_ycore_void_result table_reserve(struct ccc_command_table *table, size_t wanted)
{
    ...
    struct ccc_command *grown = realloc(table->items, new_capacity * sizeof(struct ccc_command));
    if (!grown) {
...
static struct yetty_ycore_void_result table_add(struct ccc_command_table *table, const char *name,
                                                const char *description, const char *argument_hint,
                                                int local)
{
    if (ccc_command_table_find(table, name, strlen(name))) {
    ...
    struct ccc_command *entry = &table->items[table->count];
...
{
    const struct ccc_command *left = first;
    const struct ccc_command *right = second;
...
struct yetty_ycore_void_result ccc_command_table_init(struct ccc_command_table *table)
{
    ...
    res = table_add(table, "exit", "exit ccc", "", 1);
    YETTY_RETURN_IF_ERR(yetty_ycore_void, res, "command_table_init: exit");
    res = table_add(table, "quit", "exit ccc", "", 1);
