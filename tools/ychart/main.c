 ...
 * legend, and slice labels are measured exactly the way the pane draws them.
 ...
 *   ychart --font-cdb <p> / --font-shader <p> / --no-font
 */
...
/*=============================================================================
 * Font measurement (mirrors tools/ydiagram)
 *===========================================================================*/
                                                  ...
                                                  const char *shader_override)
{
    ...
    if (!path_readable(cdb_path)) {
        fprintf(stderr, "ychart: font CDB not found: %s\n", cdb_path);
        return NULL;
    }
    if (!path_readable(sh_path)) {
        fprintf(stderr, "ychart: msdf shader not found: %s\n", sh_path);
        return NULL;
    }
...
/*=============================================================================
 * OSC / raw emission (mirrors tools/ydiagram)
 *===========================================================================*/
                         ...
                         const void *body, size_t body_len)
{
    ...
    if (YETTY_IS_OK(r) && env.size > 0) {
        ...
        fprintf(stderr, "ychart: yface_emit failed: %s\n", r.error.msg ? r.error.msg : "?");
        rc = 1;
    }
...
static int emit_osc_bin(FILE *out, struct yetty_ydraw_drawable_list *buf)
{
    ...
    if (size == 0 || !raw) {
        fprintf(stderr, "ychart: serialize produced empty buffer\n");
        return 1;
    }
...
static char *slurp_file(const char *path, size_t *out_len)
{
    ...
    if (!f) {
        fprintf(stderr, "ychart: cannot open '%s': %s\n", path, strerror(errno));
        return NULL;
    }
    size_t cap = 8192;
    size_t size = 0;
...
static void usage(const char *prog)
{
    fprintf(stderr,
            ...
            "\n"
            "Options:\n"
            "  --type <kind>         Force chart kind: bar column line area scatter\n"
...
int main(int argc, char **argv)
{
    ...
    enum yetty_ychart_kind kind = YETTY_YCHART_KIND_AUTO;
    float width = 0.0f;
    float height = 0.0f;
    bool no_font = false;
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (strcmp(a, "--type") == 0 || strcmp(a, "--kind") == 0) {
            if (i + 1 >= argc) {
                fprintf(stderr, "ychart: %s requires a value\n", a);
                return 2;
            }
            kind = yetty_ychart_kind_from_name(argv[++i]);
            continue;
        }
        if (strcmp(a, "--width") == 0) {
            if (i + 1 >= argc) {
                fprintf(stderr, "ychart: --width requires a value\n");
                return 2;
            }
        ...
        if (strcmp(a, "--height") == 0) {
            if (i + 1 >= argc) {
                fprintf(stderr, "ychart: --height requires a value\n");
                return 2;
            }
            height = (float)atof(argv[++i]);
            continue;
        }
        if (strcmp(a, "-o") == 0) {
            if (i + 1 >= argc) {
                fprintf(stderr, "ychart: -o requires a path\n");
                return 2;
            }
            out_path = argv[++i];
            continue;
        ...
        if (strcmp(a, "--font-cdb") == 0) {
            if (i + 1 >= argc) {
                fprintf(stderr, "ychart: --font-cdb requires a path\n");
                return 2;
            }
            cdb_override = argv[++i];
            continue;
        ...
        if (strcmp(a, "--font-shader") == 0) {
            if (i + 1 >= argc) {
                fprintf(stderr, "ychart: --font-shader requires a path\n");
                return 2;
            }
            sh_override = argv[++i];
            continue;
        ...
        if (a[0] == '-' && strcmp(a, "-") != 0) {
            fprintf(stderr, "ychart: unknown option '%s'\n", a);
            usage(argv[0]);
            return 2;
        }
        if (input_path) {
            fprintf(stderr, "ychart: multiple inputs\n");
            return 2;
        }
        input_path = a;
    }
    ...
    yetty_ychart_measure_text_fn measure_fn = font ? measure_with_font : NULL;

    struct yetty_ychart_render_options opts = yetty_ychart_default_render_options();
    opts.width = width;
    opts.height = height;

    /* Pass the path (not "-") so format detection can use the extension. */
    const char *path_hint = strcmp(input_path, "-") == 0 ? NULL : input_path;
    struct yetty_ychart_buffer_result br =
        yetty_ychart_render_data_full(in_buf, in_len, path_hint, kind, &opts, measure_fn, font);
    free(in_buf);
    if (YETTY_IS_ERR(br)) {
        fprintf(stderr, "ychart: render failed: %s\n", br.error.msg ? br.error.msg : "?");
