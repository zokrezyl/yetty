...
static void seed_demo(struct yetty_yrich_slides *s)
{
	...
	yetty_yrich_slides_add_textbox(s, 200, 50, 560, 50, "Shape examples",
	...
	yetty_yrich_slides_add_textbox(s, 200, 50, 560, 50, "Features",
				       strlen("Features"));
	const char *bullets[] = {
		"- Multiple shapes",
		"- Text editing",
	...
	for (size_t i = 0; i < sizeof(bullets) / sizeof(bullets[0]); i++) {
		yetty_yrich_slides_add_textbox(s, 100, y, 400, 40, bullets[i],
					       strlen(bullets[i]));
		y += 50.0f;
	}
...
int main(int argc, char **argv)
{
	...
	return yetty_yrich_app_run(argc, argv, &deck->base, YETTY_YRICH_APP_YSLIDE);
}
