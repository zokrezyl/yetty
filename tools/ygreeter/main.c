...
static struct yetty_ydraw_core_draw_list *build_image_buffer(const char *title, const char *path)
{
    ...
    struct yetty_ydraw_core_draw_list_result r = yetty_yimage_render_path(path, &cfg);
    if (YETTY_IS_ERR(r)) {
    ...
    if (title && *title) {
        ...
        struct yetty_ycore_void_result tr =
            yetty_ydraw_core_draw_list_add_text(r.value, 16.0f, 36.0f, &text_view, 22.0f,
