/* Embedded assets for the standalone ygreeter binary.
 *
 ...
 *
 * Compiled into ygreeter only when YETTY_ENABLE_LIB_INCBIN is on AND
 * the embed staging actually produced a manifest (HAS_DATA_MANIFEST). In
 * dev builds without incbin, this becomes a no-op so ygreeter still
 * works via argv0-relative lookup. */

#ifndef YGREETER_EMBEDDED_ASSETS_H
#define YGREETER_EMBEDDED_ASSETS_H
