...
static void compute_diff(const struct glyph *a, const struct glyph *b, float threshold,
                         struct diff_row *row)
{
    int w = a->hdr.width;
    int h = a->hdr.height;
...
int main(int argc, char **argv)
{
    ...
    for (size_t i = 0; i < rows_n; i++) {
        if (rows[i].dim_mismatch) {
            continue;
        }
        if (rows[i].w_a == 0 || rows[i].h_a == 0) {
            continue;
        }
        sum_max += rows[i].max_diff;
        sum_mean += rows[i].mean_diff;
        sum_bad += rows[i].bad_pct;
        if (rows[i].max_diff > global_max) {
            global_max = rows[i].max_diff;
        }
        cmp_n++;
    }
    if (cmp_n > 0) {
    ...
    if (o.sort_by == SORT_BAD) {
        cmp = cmp_row_bad_desc;
        sort_label = "bad_pct";
    }
    if (o.sort_by == SORT_MEAN) {
        cmp = cmp_row_mean_desc;
        sort_label = "mean_diff";
    }
    qsort(rows, rows_n, sizeof(*rows), cmp);

    int top = o.top_n < (int)rows_n ? o.top_n : (int)rows_n;
    printf("=== top %d glyphs by %s ===\n", top, sort_label);
    printf("%-10s  %-9s  %-11s  %-11s  %-9s  %-9s  %-9s\n", "cp", "char", "size_a", "size_b",
           "max_diff", "mean_diff", "bad_pct");
    for (int i = 0; i < top; i++) {
        ...
        if (r->dim_mismatch) {
            printf("0x%06X  %-9s  %4ux%-5u  %4ux%-5u  %-9s  %-9s  %-9s\n", r->cp, cpbuf, r->w_a,
                   r->h_a, r->w_b, r->h_b, "DIM_MIS", "DIM_MIS", "DIM_MIS");
        } else {
            printf("0x%06X  %-9s  %4ux%-5u  %4ux%-5u  %-9.4f  %-9.4f  %-7.2f%%\n", r->cp, cpbuf,
                   r->w_a, r->h_a, r->w_b, r->h_b, (double)r->max_diff, (double)r->mean_diff,
                   (double)r->bad_pct);
        }
    }
    ...
    if (o.preview) {
        ...
        for (int i = 0; i < top; i++) {
            ...
            while (lo < hi) {
                size_t m = lo + (hi - lo) / 2;
                if (sa.items[m].cp < r->cp) {
                    lo = m + 1;
                } else {
                    hi = m;
                }
            }
            if (lo >= sa.n || sa.items[lo].cp != r->cp) {
                continue;
            }
            ...
            while (lo < hi) {
                size_t m = lo + (hi - lo) / 2;
                if (sb.items[m].cp < r->cp) {
                    lo = m + 1;
                } else {
                    hi = m;
                }
            }
            if (lo >= sb.n || sb.items[lo].cp != r->cp) {
                continue;
            }
            ...
            printf("\n--- 0x%06X %s  max_diff=%.3f bad_pct=%.1f%% ---\n", r->cp, cpbuf,
                   (double)r->max_diff, (double)r->bad_pct);
