 ...
 * are ~2:1 tall:wide). */
static void render_preview(const struct yetty_ymsdf_gen_glyph_header *hdr, const uint8_t *px,
                           int target_w, int full)
{
...
static void print_record(uint32_t cp, const uint8_t *value, size_t vlen, const struct opts *o)
{
    char cpbuf[16];
    ...
    printf("cp=0x%06X %-9s "
           ...
           cp, cpbuf, hdr.width, hdr.height, (double)hdr.size_x, (double)hdr.size_y,
           (double)hdr.bearing_x, (double)hdr.bearing_y, (double)hdr.advance, (double)ascent,
           (double)descent, vlen);
...
static void stats_update(struct stats *s, uint32_t cp,
                         const struct yetty_ymsdf_gen_glyph_header *hdr, size_t pixel_bytes)
{
    s->count++;
    if (cp < s->cp_min) {
        s->cp_min = cp;
    ...
    if (hdr->width > 0 && hdr->width < s->w_min) {
        s->w_min = hdr->width;
    }
    if (hdr->width > s->w_max) {
        s->w_max = hdr->width;
    }
    if (hdr->height > 0 && hdr->height < s->h_min) {
        s->h_min = hdr->height;
    }
    if (hdr->height > s->h_max) {
        s->h_max = hdr->height;
    }
    if (hdr->advance > 0.0f) {
        if (hdr->advance < s->adv_min) {
            s->adv_min = hdr->advance;
        }
        if (hdr->advance > s->adv_max) {
            s->adv_max = hdr->advance;
        }
    }
...
int main(int argc, char **argv)
{
    ...
    if (!o.summary_only) {
        printf("# %s\n", o.path);
        printf("# data section: 0x%08X .. 0x%08X (%u bytes)\n", (unsigned)CDB_HEADER_SIZE,
               (unsigned)data_end, (unsigned)(data_end - CDB_HEADER_SIZE));
    }
    ...
    while (pos < data_end) {
        ...
        if (klen != 4) {
            ...
            if (fseek(f, klen, SEEK_CUR) != 0) {
                rc = 1;
                break;
            }
        } else {
            uint8_t kbuf[4];
            if (fread(kbuf, 1, 4, f) != 4) {
                rc = 1;
                break;
            }
            cp = rd_u32_le(kbuf);
