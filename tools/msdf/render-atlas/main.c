...
static void atlas_blit_rgba8(struct atlas *a, int dst_x, int dst_y, const uint8_t *src, int w,
                             int h)
{
    for (int y = 0; y < h; y++) {
...
static int atlas_load_cdb(struct atlas *a, const char *path)
{
    ...
    while (pos < data_end) {
        ...
        if (pix_bytes != (size_t)h.width * h.height * 4) {
            continue;
        }
        ...
        if (needed_h > a->height) {
            ...
            if (target < needed_h + 64) {
                target = needed_h + 64;
            }
            if (atlas_grow_pixels(a, target) < 0) {
                rc = -1;
                break;
            }
        }
    ...
    if (rc == 0) {
        ...
        used_h = (used_h + 7) & ~7;
        if (used_h > a->height) {
            if (atlas_grow_pixels(a, used_h) < 0) {
                rc = -1;
            }
        }
    }
...
static const char *DISPLAY_MODE_NAMES[6] = {"median3", "raw RGB", "R only",
                                            "G only",  "B only",  "real MSDF"};
...
static const char *shaderCode =
    ...
    "    // Sample atlas-texel CENTERS, not edges. Going from texel 0 to\n"
    "    // texel (glyphW-1) at +0.5 offset means innerUV ∈ [0,1] maps\n"
    "    // exactly to the inner span [0.5, glyphW-0.5] of the glyph rect.\n"
    "    // Without this, innerUV=0 lands at the boundary between the glyph\n"
    "    // and the 1-pixel gutter, and bilinear filtering blends 50/50 with\n"
    "    // the gutter's zeros — producing a faint halo around every glyph\n"
    "    // edge. Most visible on letters with strokes touching the bbox\n"
    "    // (E, H, G, etc).\n"
    "    let atlasUV = vec2<f32>(\n"
    "        (glyph.x + 0.5 + clamp(innerUV.x, 0.0, 1.0) * (glyphW - 1.0)) / uniforms.atlasW,\n"
    "        (glyph.y + 0.5 + clamp(innerUV.y, 0.0, 1.0) * (glyphH - 1.0)) / uniforms.atlasH);\n"
    ...
    "    let out_of_bounds = col < 0.0 || col >= uniforms.cols || row < 0.0 || rawIdx >= "
    "u32(uniforms.totalGlyphs);\n"
...
static void onDeviceReady(WGPURequestDeviceStatus status, WGPUDevice result, WGPUStringView message,
                          void *u1, void *u2)
{
    (void)message;
    (void)u1;
...
static void onUncapturedError(WGPUDevice const *d, WGPUErrorType type, WGPUStringView message,
                              void *u1, void *u2)
{
    (void)d;
    (void)u1;
    ...
    fprintf(stderr, "[WebGPU ERROR] %s: %.*s\n", typeStr, message.data ? (int)message.length : 0,
...
static int initAtlasTexture(void)
{
    ...
    wgpuQueueWriteTexture(queue, &dst, g_atlas.pixels, (size_t)g_atlas.width * g_atlas.height * 4,
                          &layout, &ext);
    return 1;
}
...
static void cleanup(void)
{
    ...
    if (atlasView) {
        wgpuTextureViewRelease(atlasView);
    }
    if (atlasTexture) {
        wgpuTextureRelease(atlasTexture);
    }
    atlas_free(&g_atlas);
}
...
static void charCallback(GLFWwindow *window, unsigned int codepoint)
{
    ...
    case 'C':
        ...
        fprintf(stderr, "[mode] %d (%s)  [ch reset: R=1 G=1 B=1 A=1]\n", displayMode,
                DISPLAY_MODE_NAMES[displayMode]);
        break;
    default:
        break;
    }
...
int main(int argc, char *argv[])
{
    ...
    GLFWwindow *window =
        glfwCreateWindow(windowWidth, windowHeight, "MSDF-WGSL Atlas Viewer", NULL, NULL);
