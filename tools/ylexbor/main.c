...
static void term_input_subscribe(uint32_t flags)
{
    ...
    (void)emit_envelope(YETTY_OSC_CS_CLIENT_INPUT_SUB, 0, NULL, 0, &msg, sizeof(msg));
}
                   ...
                   const uint8_t *payload, size_t payload_len)
{
    ...
    if (osc_code == YETTY_OSC_SC_CLIENT_INPUT_RESIZE ||
        ...
        if (r->width > 0 && r->height > 0) {
            yetty_ylexbor_set_viewport(st->yl, (int)r->width, (int)r->height);
            st->dirty = 1;
        }
        return;
    }
...
int main(int argc, char **argv)
{
    ...
    if (probe_terminal_size(&term_w, &term_h)) {
        width = term_w;
        height = term_h;
        ydebug("ylexbor-demo: detected terminal viewport %dx%d", width, height);
    }
    ...
    if (interactive && (!path || !strcmp(path, "-"))) {
        ...
        fprintf(stderr, "ylexbor-demo: interactive mode needs a file path or URL\n"
