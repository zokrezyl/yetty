...
static void handle_mouse_button(struct ev_state *st, const struct yetty_client_input_mouse *m)
{
    if (m->pressed) {
...
static void handle_mouse_move(struct ev_state *st, const struct yetty_client_input_mouse *m)
{
...
static void handle_mouse_wheel(struct ev_state *st, const struct yetty_client_input_mouse *m)
{
    /* Each wheel notch ~ 1.0; positive = up = zoom in (closer). */
    float factor = 1.0f - m->wheel_dy * YMESH_ZOOM_STEP;
    if (factor < 0.1f) {
        factor = 0.1f; /* clamp to avoid crossing the origin */
    }
    st->dist_factor *= factor;
    if (st->dist_factor < 0.2f) {
        st->dist_factor = 0.2f;
    }
    if (st->dist_factor > 50.0f) {
        st->dist_factor = 50.0f;
    }
    st->dirty = 1;
}
...
static void handle_key_codepoint(struct ev_state *st, uint32_t cp)
{
    ...
    case 'Q':
    case 0x03:
    case 0x1b: /* Ctrl-C / ESC */
        st->want_quit = 1;
        break;
    case 'w':
    case 'W':
        st->mode = (st->mode == YETTY_YMESH_MODE_WIREFRAME) ? YETTY_YMESH_MODE_SOLID
                                                            : YETTY_YMESH_MODE_WIREFRAME;
        st->dirty = 1;
        break;
    case 'f':
    case 'F':
                   ...
                   const uint8_t *payload, size_t payload_len)
{
    ...
    if (osc_code == YETTY_OSC_SC_CLIENT_INPUT_FIGURE_MOUSE ||
        osc_code == YETTY_OSC_SC_CLIENT_INPUT_MOUSE) {
        ...
        case YETTY_YMGUI_INPUT_MOUSE_BUTTON:
            handle_mouse_button(st, m);
            break;
        case YETTY_YMGUI_INPUT_MOUSE_POS:
            handle_mouse_move(st, m);
            break;
        case YETTY_YMGUI_INPUT_MOUSE_WHEEL:
            handle_mouse_wheel(st, m);
            break;
        }
        return;
    }
...
static void on_raw(void *user, const char *bytes, size_t n)
{
    ...
    for (size_t i = 0; i < n; i++) {
        ...
        if (c == 0x1b && i + 2 < n && bytes[i + 1] == '[' &&
            (bytes[i + 2] == 'A' || bytes[i + 2] == 'B' || bytes[i + 2] == 'C' ||
             bytes[i + 2] == 'D')) {
            switch (bytes[i + 2]) {
            case 'A':
                st->elevation += 0.1f;
...
static int interactive_loop(struct ev_state *st)
{
    ...
    term_input_subscribe(YETTY_CLIENT_INPUT_SUB_MOUSE_CLICK | YETTY_CLIENT_INPUT_SUB_MOUSE_MOVE |
                         YETTY_CLIENT_INPUT_SUB_MOUSE_WHEEL | YETTY_CLIENT_INPUT_SUB_KEY);
    fflush(stdout);
...
static int oneshot(const char *path, float bounds_w, float bounds_h)
{
    ...
    struct yetty_ydraw_drawable_list_result br = yetty_ymesh_render_path(path, &cfg);
    if (YETTY_IS_ERR(br)) {
...
static int load_glb(const char *path, uint8_t **out_bytes, size_t *out_len)
{
    ...
    if (!f) {
        fprintf(stderr, "ymesh: cannot open %s\n", path);
        return 1;
    }
...
int main(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (!strcmp(a, "-i") || !strcmp(a, "--interactive")) {
            interactive = 1;
        } else if (!strcmp(a, "--once")) {
            interactive = 0;
        } else if (!strcmp(a, "-w") && i + 1 < argc) {
            bounds_w = (float)atof(argv[++i]);
        } else if (!strcmp(a, "-H") && i + 1 < argc) {
            bounds_h = (float)atof(argv[++i]);
        } else if (a[0] == '-') {
