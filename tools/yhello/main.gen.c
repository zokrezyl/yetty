...
struct yetty_yclass_object_ptr_result yetty_yhello_app_create(struct yetty_yclass_ctx *ctx)
{
     ...
     * response isn't silently swallowed. */
    {
        struct yetty_ycore_void_result translate_class_r =
            yetty_yclass_rpc_session_translate_class(ctx->session, "yetty_yhello_app");
        if (YETTY_IS_ERR(translate_class_r)) {
            yetty_ycore_error_print(stderr,
                "yetty_yhello_app_create: translate_class (degraded — will lazy-resolve)",
    ...
    uint64_t handle = 0;
    const char *class_name = "yetty_yhello_app";
    ...
    if (YETTY_IS_ERR(create_call_r))
        return YETTY_ERR(yetty_yclass_object_ptr,
                         "yetty_yhello_app_create: CREATE call failed", create_call_r);
    if (create_call_r.value != sizeof(handle) || !handle)
        return YETTY_ERR(yetty_yclass_object_ptr,
                         "yetty_yhello_app_create: CREATE returned no/invalid handle");
    ...
    if (!proxy)
        return YETTY_ERR(yetty_yclass_object_ptr, "yetty_yhello_app_create: calloc(proxy) failed");
