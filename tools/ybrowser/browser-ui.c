...
static int key_cb(struct yetty_ygui_framework *fw, uint32_t key, int mods, void *ud)
{
	...
	if (a->address_focused) {
		...
		int consumed = yetty_ygui_textinput_handle_key(a->address, key);
		if (consumed) {
		   ...
		   const uint8_t *payload, size_t payload_len)
{
	...
	if (osc_code == YETTY_OSC_SC_CLIENT_INPUT_MOUSE ||
		...
		if (m->kind == YETTY_YMGUI_INPUT_MOUSE_POS) {
			...
			err_ok(yetty_ygui_framework_feed_mouse_button(a->fw, m->x, m->y, m->button,
								      m->pressed, 0));
			if (m->pressed) {
			...
			err_ok(yetty_ygui_framework_feed_mouse_scroll(a->fw, m->x, m->y, 0.0f,
								      m->wheel_dy));
		}
		return;
	}
...
static int build_ui(struct app *a)
{
	...
	err_ok(yetty_ygui_object_subscribe(a->tabbar, YETTY_YGUI_EVENT_VALUE_CHANGED, on_tab_changed,
					   a));
...
static void render_image(struct app *a, struct tab *t)
{
	...
	if (w != t->rendered_w) {
		float ih = (t->img_w > 0 && t->img_h > 0)
				   ? w * (float)t->img_h / (float)t->img_w
				   : w;
	...
	if (ev->type == YETTY_YCORE_RENDER) {
		...
		if (s->wire_sm) {
			struct yetty_ycore_void_result pr =
				yetty_ywire_wire_statemachine_process(s->wire_sm);
			if (YETTY_IS_ERR(pr)) {
				yetty_ycore_error_destroy(pr.error);
			}
		}
		...
		if (s->root_container) {
			...
			struct yetty_ycore_void_result rr = yetty_yfigure_render(
				NULL, (struct yetty_yclass_object *)rf - 1, s->render_target);
			if (YETTY_IS_ERR(rr)) {
				yetty_ycore_error_destroy(rr.error);
			}
				      ...
				      size_t *out_len)
{
	...
	case 257: /* Enter */
	case 335: scratch[0] = '\r'; *out_len = 1; return scratch; /* KP Enter */
	case 258: scratch[0] = '\t'; *out_len = 1; return scratch; /* Tab */
	case 259: scratch[0] = 0x7F; *out_len = 1; return scratch; /* Backspace */
	case 261: *out_len = (size_t)snprintf(scratch, scratch_n, "\x1b[3~"); return scratch; /* Del */
	...
	case 268: *out_len = (size_t)snprintf(scratch, scratch_n, "\x1b[H"); return scratch; /* Home */
	case 269: *out_len = (size_t)snprintf(scratch, scratch_n, "\x1b[F"); return scratch; /* End */
...
static struct yetty_ycore_void_result sa_worker(struct yetty_yinit_runtime *rt, void *user)
{
	...
	/* MSDF font for the receiver-side ygrid. */
	{
		...
		const char *shaders_dir = s->yframework->config->ops->get_string(
			s->yframework->config, "paths/shaders", "");
		char cdb_path[768];
	...
	/* Local container. */
	struct yetty_context ctx = {.runtime = s->yframework,
				    .event_loop = s->yframework->event_loop};
	{
...
int ybrowser_ui_run_standalone(const char *initial_url, float font_size, int argc, char **argv)
{
	...
	return yetty_yinit_run(argc, argv, &cfg, sa_worker, &s);
}
