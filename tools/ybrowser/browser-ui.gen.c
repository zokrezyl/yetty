...
struct yetty_yclass_ptr_result yetty_ybrowser_app_class_get(void)
{
    ...
    static const struct yetty_yclass_op ops[] = {
        {"yetty_yapp", "init", (yetty_yclass_method_id_t)yetty_yapp_init, (yetty_yclass_impl_t)ybrowser_app_init},
        {"yetty_yapp", "run", (yetty_yclass_method_id_t)yetty_yapp_run, (yetty_yclass_impl_t)sa_worker},
    };
    struct yetty_yclass_ptr_result parent_class_r = yetty_yapp_app_class_get();
    if (YETTY_IS_ERR(parent_class_r)) {
        yerror("yetty_ybrowser_app_class_get: parent accessor failed: %s", parent_class_r.error.msg);
        return YETTY_ERR(yetty_yclass_ptr, "yetty_ybrowser_app_class_get: parent accessor failed", parent_class_r);
    }
    struct yetty_yclass_ptr_result register_class_r =
        yetty_yclass_register(&desc, ops, sizeof(ops) / sizeof(ops[0]),
                              parent_class_r.value, NULL, 0);
    if (YETTY_IS_ERR(register_class_r)) {
        yerror("yetty_ybrowser_app_class_get: class_register failed: %s", register_class_r.error.msg);
        return YETTY_ERR(yetty_yclass_ptr, "yetty_ybrowser_app_class_get: class_register failed", register_class_r);
    }
...
struct yetty_ybrowser_app_ptr_result yetty_ybrowser_app_from(struct yetty_yclass_object *obj)
{
    ...
    if (YETTY_IS_ERR(class_r))
        return YETTY_ERR(yetty_ybrowser_app_ptr, "yetty_ybrowser_app_from: class accessor", class_r);
...
struct yetty_yclass_object_ptr_result yetty_ybrowser_app_create(struct yetty_yclass_ctx *ctx)
{
     ...
     * response isn't silently swallowed. */
    {
        struct yetty_ycore_void_result translate_class_r =
            yetty_yclass_rpc_session_translate_class(ctx->session, "yetty_ybrowser_app");
        if (YETTY_IS_ERR(translate_class_r)) {
            yetty_ycore_error_print(stderr,
                "yetty_ybrowser_app_create: translate_class (degraded — will lazy-resolve)",
    ...
    uint64_t handle = 0;
    const char *class_name = "yetty_ybrowser_app";
    ...
    if (YETTY_IS_ERR(create_call_r))
        return YETTY_ERR(yetty_yclass_object_ptr,
                         "yetty_ybrowser_app_create: CREATE call failed", create_call_r);
    if (create_call_r.value != sizeof(handle) || !handle)
        return YETTY_ERR(yetty_yclass_object_ptr,
                         "yetty_ybrowser_app_create: CREATE returned no/invalid handle");
    ...
    if (!proxy)
        return YETTY_ERR(yetty_yclass_object_ptr, "yetty_ybrowser_app_create: calloc(proxy) failed");
