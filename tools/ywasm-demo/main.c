/*
 * ywasm-demo — smoke test for the WebGPU-over-OSC bridge.
 *
 * Run from inside a yetty terminal as `./yetty -e ywasm-demo`. The tool
 * links libyetty_ywasm, talks to the local yetty's ywasm-layer over
...
int main(int argc, char **argv)
{
    ...
    FILE *trace = fopen("/tmp/ywasm-demo.trace", "w");
...
#define DEMO_TRACE(...) do { if (trace) { fprintf(trace, __VA_ARGS__); } } while (0)
    ...
    for (int i = 0; i < 200; ++i) {
        ...
        if (yetty_ywasm_client_connected(c))
            break;
        sleep_ms(10);
    }
    ...
    uint64_t instance_h = ywasm_client_wgpuCreateInstance(c);
    DEMO_TRACE("demo: createInstance -> handle=%lu\n", (unsigned long)instance_h);

    st.adapter_h = ywasm_client_wgpuInstanceRequestAdapter(c, instance_h, on_adapter, &st);
    ...
    if (st.adapter_done && st.adapter_status == 0) {
        st.device_h = ywasm_client_wgpuAdapterRequestDevice(c, st.adapter_h, on_device, &st);
