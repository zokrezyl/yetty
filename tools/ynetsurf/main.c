...
static void wait_for_layout(struct yetty_ynetsurf *ns, int max_ms)
{
    ...
    for (;;) {
        ...
        int elapsed = (int)((now.tv_sec - t0.tv_sec) * 1000 + (now.tv_nsec - t0.tv_nsec) / 1000000);
        if (elapsed >= max_ms) {
            return;
        }
        int budget = max_ms - elapsed;
        int next = yetty_ynetsurf_pump(ns);
        if (next < 0 || next > budget) {
            next = budget;
        }
...
static uint32_t csi_to_ns_key(const char *seq, size_t len)
{
    ...
    if (len == 3) {
        ...
        case 'D':
            return NS_KEY_LEFT;
        case 'H':
            return NS_KEY_LINE_START; /* xterm Home */
        case 'F':
            return NS_KEY_LINE_END; /* xterm End */
        }
        return 0;
    }
    ...
    if (final == '~') {
        ...
        switch (n) {
        case 1:
            return NS_KEY_LINE_START; /* Home (linux console) */
        case 3:
            return NS_KEY_DELETE_RIGHT; /* Delete */
        case 4:
            return NS_KEY_LINE_END; /* End (linux console) */
        case 5:
...
static void on_raw(void *user, const char *bytes, size_t n)
{
    ...
    for (size_t i = 0; i < n;) {
        ...
        if (st->esc_len > 0) {
            ...
            int complete = 0;
            if (st->esc_len >= 3 && st->esc_buf[1] == '[' && c >= 0x40 && c <= 0x7e) {
                complete = 1;
            } else if (st->esc_len >= 2 && st->esc_buf[1] != '[' && st->esc_buf[1] != 'O') {
        ...
        default:
            if (c >= 0x20 && c < 0x80) {
                ...
                consumed = utf8_decode((const unsigned char *)bytes + i, n - i, &key);
                if (consumed == 0) {
                   ...
                   const uint8_t *payload, size_t payload_len)
{
    ...
    if (osc_code == YETTY_OSC_SC_CLIENT_INPUT_FIGURE_MOUSE ||
        ...
        case YETTY_YMGUI_INPUT_MOUSE_BUTTON: {
            ...
            if (m->pressed) {
                yetty_ynetsurf_mouse_click(st->ns, (int)m->x, (int)m->y, btn);
            } else {
                yetty_ynetsurf_mouse_release(st->ns, (int)m->x, (int)m->y, btn);
            }
            st->dirty = 1;
            break;
        }
        case YETTY_YMGUI_INPUT_MOUSE_POS:
            yetty_ynetsurf_mouse_move(st->ns, (int)m->x, (int)m->y);
            st->dirty = 1;
            break;
        case YETTY_YMGUI_INPUT_MOUSE_WHEEL:
            ...
            yetty_ynetsurf_scroll(st->ns, 0, (int)(-m->wheel_dy * 60.0f));
            st->dirty = 1;
            break;
        }
        return;
