...
enum {
    OP_FRAME_BEGIN = 0x01, OP_FRAME_END = 0x02,
    OP_IMAGE_RESOURCE = 0x03, OP_FONT_RESOURCE = 0x04,
    OP_SAVE = 0x10, OP_RESTORE = 0x11, OP_SAVE_LAYER = 0x12, OP_SAVE_LAYER_ALPHA = 0x13,
    OP_TRANSLATE = 0x20, OP_SCALE = 0x21, OP_ROTATE = 0x22, OP_CONCAT = 0x23, OP_SET_MATRIX = 0x24,
    OP_CLIP_RECT = 0x30, OP_CLIP_RRECT = 0x31, OP_CLIP_PATH = 0x32,
    OP_DRAW_COLOR = 0x40, OP_DRAW_RECT = 0x41, OP_DRAW_IRECT = 0x42, OP_DRAW_OVAL = 0x43,
    OP_DRAW_RRECT = 0x44, OP_DRAW_DRRECT = 0x45, OP_DRAW_LINE = 0x46, OP_DRAW_ARC = 0x47,
    OP_DRAW_PATH = 0x48, OP_DRAW_IMAGE = 0x49, OP_DRAW_IMAGE_RECT = 0x4A,
    OP_DRAW_TEXT_BLOB = 0x4B, OP_DRAW_RECORD = 0x4C, OP_UNSUPPORTED = 0x7F,
};
...
{
    if (cursor->off + 1 > cursor->len) { cursor->off = cursor->len + 1; return 0; }
    return cursor->data[cursor->off++];
}

static uint32_t rd_u32(struct cursor *cursor)
{
    if (cursor->off + 4 > cursor->len) { cursor->off = cursor->len + 1; return 0; }
    uint32_t value;
...
static void skip_path(struct cursor *cursor)
{
    ...
    for (uint32_t i = 0; i < verbs; i++) {
        ...
        switch (verb) {
        case 0: case 1: skip(cursor, 8); break;   /* move, line */
        case 2: skip(cursor, 16); break;          /* quad */
        case 3: skip(cursor, 20); break;          /* conic */
        case 4: skip(cursor, 24); break;          /* cubic */
        default: break;                            /* close */
        }
    }
}
...
static void emit_box(struct yetty_ydraw_drawable_list *list, uint32_t *z,
                     float left, float top, float right, float bottom,
                     float corner_radius, const struct paint_flags *flags)
{
...
int main(int argc, char **argv)
{
    ...
    if (argc > 1 && strcmp(argv[1], "-") != 0) {
        in = fopen(argv[1], "rb");
        if (!in) { fprintf(stderr, "ywire: cannot open %s\n", argv[1]); return 1; }
    }
    ...
    struct yetty_ydraw_drawable_list_config config = {
        ...
        .scene_max_x = (float)width, .scene_max_y = (float)height,
    };
    ...
    while (cursor.off < cursor.len) {
        ...
        if (op == OP_FRAME_END) break;
        switch (op) {
        case OP_SAVE: case OP_RESTORE: case OP_DRAW_RECORD:
            break;
        case OP_TRANSLATE: skip(&cursor, 8); break;
        case OP_SCALE: skip(&cursor, 8); break;
        case OP_ROTATE: skip(&cursor, 4); break;
        case OP_CONCAT: case OP_SET_MATRIX: skip(&cursor, 64); break;
        case OP_CLIP_RECT: skip(&cursor, 16 + 2); break;
        case OP_CLIP_RRECT: skip(&cursor, 48 + 2); break;
        case OP_CLIP_PATH: skip_path(&cursor); skip(&cursor, 2); break;
        case OP_DRAW_COLOR: {
            struct paint_flags flags = {rd_f32(&cursor), rd_f32(&cursor), rd_f32(&cursor),
                                        rd_f32(&cursor), 0, 0.0f};
            (void)rd_u8(&cursor);  /* blend */
            emit_box(list, &z, 0, 0, (float)width, (float)height, 0.0f, &flags);
            boxes++;
            break;
        }
        case OP_DRAW_RECT: case OP_DRAW_OVAL: {
            struct paint_flags flags = rd_flags(&cursor);
            float l = rd_f32(&cursor), t = rd_f32(&cursor), r = rd_f32(&cursor), b = rd_f32(&cursor);
        ...
        case OP_DRAW_LINE: {
            ...
            flags.style = 1;
            emit_box(list, &z, x0 < x1 ? x0 : x1, y0 < y1 ? y0 : y1,
                     x0 < x1 ? x1 : x0, y0 < y1 ? y1 : y0, 0.0f, &flags);
            boxes++;
            break;
        }
        case OP_DRAW_ARC: {
            (void)rd_flags(&cursor); skip(&cursor, 16 + 8);
            break;
        }
        case OP_DRAW_PATH: (void)rd_flags(&cursor); skip_path(&cursor); break;
        case OP_DRAW_IMAGE: (void)rd_flags(&cursor); skip(&cursor, 4 + 8); break;
        case OP_DRAW_IMAGE_RECT: (void)rd_flags(&cursor); skip(&cursor, 4 + 16 + 16); break;
        case OP_DRAW_TEXT_BLOB: {
            (void)rd_flags(&cursor);
            (void)rd_u32(&cursor);  /* font_id */
            (void)rd_f32(&cursor); (void)rd_f32(&cursor);  /* x, y */
            uint32_t glyphs = rd_u32(&cursor);
            skip(&cursor, (size_t)glyphs * 10);  /* text: TODO real glyphs */
            break;
        }
        case OP_UNSUPPORTED: (void)rd_u8(&cursor); break;
        default:
