...
int main(int argc, char **argv)
{
    ...
    int c;
    while ((c = getopt_long(argc, argv, "w:H:nh", long_opts, NULL)) != -1) {
        switch (c) {
        case 'w': opts.w = (float)atof(optarg); break;
        case 'H': opts.h = (float)atof(optarg); break;
        case OPT_XRANGE:
            if (parse_range(optarg, &opts.x_min, &opts.x_max) < 0) {
            ...
            break;
        case OPT_YRANGE:
            if (parse_range(optarg, &opts.y_min, &opts.y_max) < 0) {
