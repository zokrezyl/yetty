 ...
 */
clang::SourceLocation effective_loc(clang::SourceManager &sm, clang::SourceLocation loc);

/*
 * Visitor that walks every external decl, applies the four rules, and
 * invokes `handler` once per violation.
 */
class checker_visitor : public clang::RecursiveASTVisitor<checker_visitor>
{
  public:
