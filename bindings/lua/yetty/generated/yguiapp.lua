...
struct yetty_yclass_object_ptr_result yetty_yguiapp_app_root_get(struct yetty_yclass_object *);
...
function App:build(root)
  rt.live(self, "App:build")
  local res = rt.C().yetty_yguiapp_build(self.handle, rt.unwrap(root))
  rt.check(res)
end
...
App.__prop_get.root = function(obj)
  rt.live(obj, "App.root")
  local res = rt.C().yetty_yguiapp_app_root_get(obj.handle)
  rt.check(res)
  return res.value
end
