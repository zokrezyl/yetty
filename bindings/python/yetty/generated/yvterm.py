...
class ShaderGlyph(_yfigure.Figure):
    """yclass yvterm:shader_glyph"""
    __yclass_domain__: ClassVar[str] = 'yvterm'
    __yclass_name__: ClassVar[str] = 'shader_glyph'
    @classmethod
    def yclass(cls) -> _rt.Result[Any]:
        _fn = _rt.cfn("yetty_yvterm_shader_glyph_class_get", _t.yetty_yclass_ptr_result, [])
        return _rt.result_from_c(_fn())
    def __init__(self, _handle: Any = None) -> None:
        if _handle is None:
        ...
        super().__init__(_handle)
    @classmethod
    def create(cls) -> _rt.Result['ShaderGlyph']:
        obj = cls()
        return obj.init_result
