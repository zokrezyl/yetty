...
static struct yetty_ycore_void_result ymaze_path_reset(struct yetty_ymaze *m, uint32_t hint)
{
    m->path_len = 0;
    if (hint > m->path_cap) {
        struct yetty_ymaze_cell_pos *np = realloc(m->path, hint * sizeof(*np));
        if (!np) {
            return YETTY_ERR(yetty_ycore_void, "ymaze: path realloc failed");
        }
        m->path = np;
        m->path_cap = hint;
    }
...
static struct yetty_ycore_void_result ymaze_path_push(struct yetty_ymaze *m, uint32_t col,
                                                      uint32_t row)
{
    if (m->path_len == m->path_cap) {
        uint32_t nc = m->path_cap ? m->path_cap * 2 : 64;
        struct yetty_ymaze_cell_pos *np = realloc(m->path, nc * sizeof(*np));
        if (!np) {
            return YETTY_ERR(yetty_ycore_void, "ymaze: path push realloc failed");
        }
        m->path = np;
        m->path_cap = nc;
    }
...
static struct yetty_ycore_void_result ymaze_generate_maze(struct yetty_ymaze *m)
{
    ...
    if (total != m->grid_len) {
        ...
        if (!ng) {
            return YETTY_ERR(yetty_ycore_void, "ymaze: grid realloc failed");
        }
        m->grid = ng;
        m->grid_len = total;
    }
    memset(m->grid, YMAZE_WALL_N | YMAZE_WALL_S | YMAZE_WALL_E | YMAZE_WALL_W, total);
    ...
    if (!visited || !stack) {
        free(visited);
        free(stack);
        return YETTY_ERR(yetty_ycore_void, "ymaze: gen scratch alloc failed");
    }
    ...
    const uint8_t wall_remove[4] = {YMAZE_WALL_N, YMAZE_WALL_S, YMAZE_WALL_E, YMAZE_WALL_W};
    const uint8_t wall_opposite[4] = {YMAZE_WALL_S, YMAZE_WALL_N, YMAZE_WALL_W, YMAZE_WALL_E};
    ...
    while (sp > 0) {
        ...
        for (int d = 0; d < 4; d++) {
            ...
            if (nx < 0 || nx >= (int)m->config.cols || ny < 0 || ny >= (int)m->config.rows) {
                continue;
            }
...
static struct yetty_ycore_void_result ymaze_solve_maze(struct yetty_ymaze *m)
{
    ...
    if (!parent || !visited || !queue) {
        ...
        free(visited);
        free(queue);
        return YETTY_ERR(yetty_ycore_void, "ymaze: solve scratch alloc failed");
    }
    ...
    const int dx[4] = {0, 0, 1, -1};
    const int dy[4] = {-1, 1, 0, 0};
    const uint8_t wall_check[4] = {YMAZE_WALL_N, YMAZE_WALL_S, YMAZE_WALL_E, YMAZE_WALL_W};
    ...
    while (qhead < qtail) {
        ...
        for (int d = 0; d < 4; d++) {
            ...
            uint32_t n_idx = (uint32_t)ny * m->config.cols + (uint32_t)nx;
            if (visited[n_idx]) {
                continue;
            }
            visited[n_idx] = 1;
                                                        ...
                                                        float time)
{
                ...
                seg = (struct yetty_ysdf_segment){x0, y0, x1, y0};
                {
                    struct yetty_ycore_void_result set_r =
                        yetty_ydraw_drawable_list_add_cmd_add_segment(
                            buf, 0, z++, 0u, m->config.wall_color, m->config.wall_width, &seg);
                    YETTY_RETURN_IF_ERR(yetty_ycore_void, set_r, "ymaze: wall segment");
                }
            }
            if (w & YMAZE_WALL_S) {
    ...
    /* Start marker — circle. */
    {
        ...
        { struct yetty_ycore_void_result set_r = yetty_ydraw_drawable_list_add_cmd_add_circle(buf, 0, z++, m->config.start_color, 0u, 0.0f, &c); YETTY_RETURN_IF_ERR(yetty_ycore_void, set_r, "drop: yetty_ydraw_drawable_list_add_cmd_add_circle"); }
    }
    ...
    /* End marker — 5-point star. */
    {
        ...
        { struct yetty_ycore_void_result set_r = yetty_ydraw_drawable_list_add_cmd_add_star(buf, 0, z++, m->config.end_color, 0u, 0.0f, &s); YETTY_RETURN_IF_ERR(yetty_ycore_void, set_r, "drop: yetty_ydraw_drawable_list_add_cmd_add_star"); }
    }
    ...
    /* Trail — segments along path[0..cells_traversed]. */
    {
        ...
        uint32_t cells_traversed = total_cells_f < 0.0f ? 0u : (uint32_t)total_cells_f;
        if (cells_traversed > m->path_len - 1) {
            cells_traversed = m->path_len - 1;
        }

        uint32_t trail_color = (m->config.actor_color & 0x00FFFFFFu) | 0x60000000u;
        ...
        for (uint32_t i = 0; i < cells_traversed && i + 1 < m->path_len; i++) {
            ...
            {
                struct yetty_ycore_void_result set_r = yetty_ydraw_drawable_list_add_cmd_add_segment(
                    buf, 0, z++, 0u, trail_color, trail_width, &seg);
                YETTY_RETURN_IF_ERR(yetty_ycore_void, set_r, "ymaze: trail segment");
            }
        }
    }
    ...
    /* Actor — circle interpolated between path[idx] and path[idx + 1]. */
    {
        ...
        { struct yetty_ycore_void_result set_r = yetty_ydraw_drawable_list_add_cmd_add_circle(buf, 0, z++, m->config.actor_color, 0u, 0.0f, &c); YETTY_RETURN_IF_ERR(yetty_ycore_void, set_r, "drop: yetty_ydraw_drawable_list_add_cmd_add_circle"); }
    }
...
{
    if (c.cols < YMAZE_COLS_MIN) {
        c.cols = YMAZE_COLS_MIN;
    }
    if (c.cols > YMAZE_COLS_MAX) {
        c.cols = YMAZE_COLS_MAX;
    }
    if (c.rows < YMAZE_ROWS_MIN) {
        c.rows = YMAZE_ROWS_MIN;
    }
    if (c.rows > YMAZE_ROWS_MAX) {
        c.rows = YMAZE_ROWS_MAX;
    }
    if (c.actor_speed < YMAZE_SPEED_MIN) {
        c.actor_speed = YMAZE_SPEED_MIN;
    }
    if (c.actor_speed > YMAZE_SPEED_MAX) {
        c.actor_speed = YMAZE_SPEED_MAX;
    }
    if (c.wall_width < YMAZE_WALL_MIN) {
        c.wall_width = YMAZE_WALL_MIN;
    }
    if (c.wall_width > YMAZE_WALL_MAX) {
        c.wall_width = YMAZE_WALL_MAX;
    }
                                                 ...
                                                 uint32_t seed)
{
    ...
    m->config = ymaze_clamp_config(config ? *config : yetty_ymaze_config_default());
                                                  ...
                                                  bool *out_regenerated)
{
    ...
    if (!maze->initialized) {
        ...
        YETTY_RETURN_IF_ERR(yetty_ycore_void, gr, "ymaze_render: initial generate failed");
        maze->solve_start_time = time;
        maze->initialized = true;
        regenerated = true;
    }

    if (maze->config.auto_regen && maze->path_len > 0 && yetty_ymaze_is_finished(maze, time)) {
        struct yetty_ycore_void_result gr = ymaze_generate(maze);
        YETTY_RETURN_IF_ERR(yetty_ycore_void, gr, "ymaze_render: auto-regen failed");
        maze->solve_start_time = time;
        regenerated = true;
    }
    ...
    YETTY_RETURN_IF_ERR(yetty_ycore_void, br, "ymaze_render: build_prims failed");
