...
static yetty_yplatform_platform_init_fn
    yetty_yplatform_glfw_platform_yetty_yplatform_platform_init_check = glfw_platform_init;
YETTY_MAYBE_UNUSED
static yetty_yplatform_platform_run_fn
    yetty_yplatform_glfw_platform_yetty_yplatform_platform_run_check = glfw_platform_run;
...
struct yetty_yclass_ptr_result yetty_yplatform_glfw_platform_class_get(void)
{
    ...
    static const struct yetty_yclass_op ops[] = {
        {"yetty_yplatform", "platform_init",
         ...
         (yetty_yclass_impl_t)glfw_platform_init},
        {"yetty_yplatform", "platform_run", (yetty_yclass_method_id_t)yetty_yplatform_platform_run,
         (yetty_yclass_impl_t)glfw_platform_run},
    };
    struct yetty_yclass_ptr_result parent_class_r = yetty_yplatform_platform_class_get();
    if (YETTY_IS_ERR(parent_class_r)) {
        yerror("yetty_yplatform_glfw_platform_class_get: parent accessor failed: %s",
               parent_class_r.error.msg);
        return YETTY_ERR(yetty_yclass_ptr,
                         "yetty_yplatform_glfw_platform_class_get: parent accessor failed",
                         parent_class_r);
    }
    struct yetty_yclass_ptr_result register_class_r = yetty_yclass_register(
        &desc, ops, sizeof(ops) / sizeof(ops[0]), parent_class_r.value, NULL, 0);
    if (YETTY_IS_ERR(register_class_r)) {
        yerror("yetty_yplatform_glfw_platform_class_get: class_register failed: %s",
               register_class_r.error.msg);
        return YETTY_ERR(yetty_yclass_ptr,
                         "yetty_yplatform_glfw_platform_class_get: class_register failed",
                         register_class_r);
    }
    ...
    struct yetty_yclass_object *obj)
{
    ...
    if (YETTY_IS_ERR(class_r)) {
        return YETTY_ERR(yetty_yplatform_glfw_platform_ptr,
                         "yetty_yplatform_glfw_platform_from: class accessor", class_r);
    }
    struct yetty_yclass_void_ptr_result slice_r = yetty_yclass_object_data(obj, class_r.value);
    if (YETTY_IS_ERR(slice_r)) {
        return YETTY_ERR(yetty_yplatform_glfw_platform_ptr,
                         "yetty_yplatform_glfw_platform_from: object_data", slice_r);
    }
    return YETTY_OK(yetty_yplatform_glfw_platform_ptr,
                    (struct yetty_yplatform_glfw_platform *)slice_r.value);
}
    ...
    struct yetty_yplatform_glfw_platform *data)
{
    ...
    YETTY_RETURN_IF_ERR(yetty_yclass_object_ptr, class_r,
                        "yetty_yplatform_glfw_platform_to: class accessor");
    struct yetty_ycore_size_result offset_r =
        yetty_yclass_object_data_offset(class_r.value, class_r.value);
    YETTY_RETURN_IF_ERR(yetty_yclass_object_ptr, offset_r,
                        "yetty_yplatform_glfw_platform_to: data offset");
    ...
    struct yetty_yclass_ctx *ctx)
{
    ...
    if (YETTY_IS_ERR(class_accessor_r)) {
        return YETTY_ERR(yetty_yclass_object_ptr,
                         "yetty_yplatform_glfw_platform_create: class accessor failed",
                         class_accessor_r);
    }
     ...
     * response isn't silently swallowed. */
    {
        ...
        if (YETTY_IS_ERR(translate_class_r)) {
            yetty_ycore_error_print(stderr,
                                    "yetty_yplatform_glfw_platform_create: translate_class "
    ...
    if (!proxy) {
        return YETTY_ERR(yetty_yclass_object_ptr,
                         "yetty_yplatform_glfw_platform_create: calloc(proxy) failed");
    }
