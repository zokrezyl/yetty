...
struct yetty_yclass_object_ptr_result yetty_yplatform_window_manager_create(struct yetty_yclass_ctx *ctx)
{
    ...
    if (YETTY_IS_ERR(class_accessor_r))
        return YETTY_ERR(yetty_yclass_object_ptr,
                         "yetty_yplatform_window_manager_create: class accessor failed", class_accessor_r);
     ...
     * response isn't silently swallowed. */
    {
        struct yetty_ycore_void_result translate_class_r =
            yetty_yclass_rpc_session_translate_class(ctx->session, "yetty_yplatform_window_manager");
        if (YETTY_IS_ERR(translate_class_r)) {
            yetty_ycore_error_print(stderr,
                "yetty_yplatform_window_manager_create: translate_class (degraded — will lazy-resolve)",
    ...
    if (YETTY_IS_ERR(create_call_r))
        return YETTY_ERR(yetty_yclass_object_ptr,
                         "yetty_yplatform_window_manager_create: CREATE call failed", create_call_r);
    ...
    if (!proxy)
        return YETTY_ERR(yetty_yclass_object_ptr, "yetty_yplatform_window_manager_create: calloc(proxy) failed");
    proxy->header.klass = klass;
    proxy->handle = handle;
    return YETTY_OK(yetty_yclass_object_ptr, &proxy->header);
}

/* ---- yplatform: class name → accessor (lazy) ---------------------- */

static struct yetty_yclass_ptr_result yetty_yplatform_accessor_lookup(const char *name)
{
    if (strcmp(name, "yetty_yplatform_window_manager") == 0) return yetty_yplatform_window_manager_class_get();
...
struct yetty_ycore_void_result yetty_yplatform_register(void)
{
    ...
    struct yetty_ycore_void_result add_accessor_r =
        yetty_yclass_add_accessor_lookup(yetty_yplatform_accessor_lookup);
    YETTY_RETURN_IF_ERR(yetty_ycore_void, add_accessor_r,
                        "yetty_yplatform_register: add_accessor_lookup");
    registered = true;
    return YETTY_OK_VOID();
}
