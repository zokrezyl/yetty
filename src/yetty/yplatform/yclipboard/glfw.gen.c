    ...
    struct yetty_yclass_object *obj)
{
    ...
    if (YETTY_IS_ERR(class_r)) {
        return YETTY_ERR(yetty_yplatform_glfw_clipboard_ptr,
                         "yetty_yplatform_glfw_clipboard_from: class accessor", class_r);
    }
    struct yetty_yclass_void_ptr_result slice_r = yetty_yclass_object_data(obj, class_r.value);
    if (YETTY_IS_ERR(slice_r)) {
        return YETTY_ERR(yetty_yplatform_glfw_clipboard_ptr,
                         "yetty_yplatform_glfw_clipboard_from: object_data", slice_r);
    }
    return YETTY_OK(yetty_yplatform_glfw_clipboard_ptr,
                    (struct yetty_yplatform_glfw_clipboard *)slice_r.value);
}
    ...
    struct yetty_yplatform_glfw_clipboard *data)
{
    ...
    YETTY_RETURN_IF_ERR(yetty_yclass_object_ptr, class_r,
                        "yetty_yplatform_glfw_clipboard_to: class accessor");
    struct yetty_ycore_size_result offset_r =
        yetty_yclass_object_data_offset(class_r.value, class_r.value);
    YETTY_RETURN_IF_ERR(yetty_yclass_object_ptr, offset_r,
                        "yetty_yplatform_glfw_clipboard_to: data offset");
    ...
    struct yetty_yclass_ctx *ctx)
{
    ...
    if (YETTY_IS_ERR(class_accessor_r)) {
        return YETTY_ERR(yetty_yclass_object_ptr,
                         "yetty_yplatform_glfw_clipboard_create: class accessor failed",
                         class_accessor_r);
    }
     ...
     * response isn't silently swallowed. */
    {
        struct yetty_ycore_void_result translate_class_r = yetty_yclass_rpc_session_translate_class(
            ctx->session, "yetty_yplatform_glfw_clipboard");
        if (YETTY_IS_ERR(translate_class_r)) {
            yetty_ycore_error_print(stderr,
                                    "yetty_yplatform_glfw_clipboard_create: translate_class "
    ...
    if (YETTY_IS_ERR(create_call_r)) {
        return YETTY_ERR(yetty_yclass_object_ptr,
                         "yetty_yplatform_glfw_clipboard_create: CREATE call failed",
                         create_call_r);
    }
    ...
    if (!proxy) {
        return YETTY_ERR(yetty_yclass_object_ptr,
                         "yetty_yplatform_glfw_clipboard_create: calloc(proxy) failed");
    }
