...
struct yetty_yclass_ptr_result yetty_yplatform_android_clipboard_class_get(void)
{
    ...
    static const struct yetty_yclass_op ops[] = {
        {"yetty_yplatform", "clipboard_set_text",
         ...
         (yetty_yclass_impl_t)android_clipboard_set_text},
        {"yetty_yplatform", "clipboard_request_paste",
         (yetty_yclass_method_id_t)yetty_yplatform_clipboard_request_paste,
         (yetty_yclass_impl_t)android_clipboard_request_paste},
    };
    struct yetty_yclass_ptr_result parent_class_r = yetty_yplatform_clipboard_class_get();
    if (YETTY_IS_ERR(parent_class_r)) {
        yerror("yetty_yplatform_android_clipboard_class_get: parent accessor failed: %s",
               parent_class_r.error.msg);
        return YETTY_ERR(yetty_yclass_ptr,
                         "yetty_yplatform_android_clipboard_class_get: parent accessor failed",
                         parent_class_r);
    }
    struct yetty_yclass_ptr_result register_class_r = yetty_yclass_register(
        &desc, ops, sizeof(ops) / sizeof(ops[0]), parent_class_r.value, NULL, 0);
    if (YETTY_IS_ERR(register_class_r)) {
        yerror("yetty_yplatform_android_clipboard_class_get: class_register failed: %s",
               register_class_r.error.msg);
        return YETTY_ERR(yetty_yclass_ptr,
                         "yetty_yplatform_android_clipboard_class_get: class_register failed",
                         register_class_r);
    }
    ...
    struct yetty_yclass_object *obj)
{
    ...
    if (YETTY_IS_ERR(class_r)) {
        return YETTY_ERR(yetty_yplatform_android_clipboard_ptr,
                         "yetty_yplatform_android_clipboard_from: class accessor", class_r);
    }
    struct yetty_yclass_void_ptr_result slice_r = yetty_yclass_object_data(obj, class_r.value);
    if (YETTY_IS_ERR(slice_r)) {
        return YETTY_ERR(yetty_yplatform_android_clipboard_ptr,
                         "yetty_yplatform_android_clipboard_from: object_data", slice_r);
    }
    return YETTY_OK(yetty_yplatform_android_clipboard_ptr,
                    (struct yetty_yplatform_android_clipboard *)slice_r.value);
}
    ...
    struct yetty_yplatform_android_clipboard *data)
{
    ...
    YETTY_RETURN_IF_ERR(yetty_yclass_object_ptr, class_r,
                        "yetty_yplatform_android_clipboard_to: class accessor");
    struct yetty_ycore_size_result offset_r =
        yetty_yclass_object_data_offset(class_r.value, class_r.value);
    YETTY_RETURN_IF_ERR(yetty_yclass_object_ptr, offset_r,
                        "yetty_yplatform_android_clipboard_to: data offset");
    ...
    struct yetty_yclass_ctx *ctx)
{
    ...
    if (YETTY_IS_ERR(class_accessor_r)) {
        return YETTY_ERR(yetty_yclass_object_ptr,
                         "yetty_yplatform_android_clipboard_create: class accessor failed",
                         class_accessor_r);
    }
     ...
     * response isn't silently swallowed. */
    {
        struct yetty_ycore_void_result translate_class_r = yetty_yclass_rpc_session_translate_class(
            ctx->session, "yetty_yplatform_android_clipboard");
        if (YETTY_IS_ERR(translate_class_r)) {
            yetty_ycore_error_print(stderr,
                                    "yetty_yplatform_android_clipboard_create: translate_class "
    ...
    if (YETTY_IS_ERR(create_call_r)) {
        return YETTY_ERR(yetty_yclass_object_ptr,
                         "yetty_yplatform_android_clipboard_create: CREATE call failed",
                         create_call_r);
    }
    ...
    if (!proxy) {
        return YETTY_ERR(yetty_yclass_object_ptr,
                         "yetty_yplatform_android_clipboard_create: calloc(proxy) failed");
    }
