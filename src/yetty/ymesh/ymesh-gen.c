...
static WGPURenderPipeline ymesh_get_pipeline(struct yetty_ydraw_core_concrete_factory *self)
{
...
static struct yetty_ydraw_core_complex_prim_instance_ptr_result ymesh_create_instance(
    ...
    uint32_t rolling_row)
{
    if (!buffer_data || size < sizeof(struct yetty_ydraw_core_complex_prim)) {
        return YETTY_ERR(yetty_ydraw_core_complex_prim_instance_ptr, "ymesh: invalid buffer data");
    }
    ...
    if (isize != 4 && isize != 2) {
        return YETTY_ERR(yetty_ydraw_core_complex_prim_instance_ptr, "ymesh: bad index_size");
    }
    if (vcount == 0 || icount == 0) {
        return YETTY_ERR(yetty_ydraw_core_complex_prim_instance_ptr, "ymesh: empty mesh");
    }
    ...
    if (!inst) {
        return YETTY_ERR(yetty_ydraw_core_complex_prim_instance_ptr, "ymesh: inst alloc failed");
    }
    struct ymesh_instance_data *d = calloc(1, sizeof(struct ymesh_instance_data));
    if (!d) {
        free(inst);
        return YETTY_ERR(yetty_ydraw_core_complex_prim_instance_ptr, "ymesh: data alloc failed");
    }
    ...
    if (!inst->buffer_data) {
        free(d);
        free(inst);
        return YETTY_ERR(yetty_ydraw_core_complex_prim_instance_ptr, "ymesh: wire copy failed");
    }
    ...
    yinfo("ymesh: instance created vcount=%u icount=%u off=%ux%u", vcount, icount, d->off_w,
          d->off_h);
    return YETTY_OK(yetty_ydraw_core_complex_prim_instance_ptr, inst);
}

static void ymesh_destroy_instance(struct yetty_ydraw_core_concrete_factory *self,
...
static struct yetty_ydraw_core_gpu_resource_set *ymesh_get_shared_rs(
...
struct yetty_ydraw_core_concrete_factory *yetty_ymesh_factory_create(void)
{
...
void yetty_ymesh_factory_destroy(struct yetty_ydraw_core_concrete_factory *self)
{
    if (!self) {
        return;
    }
