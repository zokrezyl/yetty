...
//-----------------------------------------------------------------------------
// GlyphInstance - positioned glyph for GPU rendering (48 bytes)
// Same structure as PDFGlyphInstance for consistency
//-----------------------------------------------------------------------------
...
// Used internally after layout, or can be populated directly
//-----------------------------------------------------------------------------
struct TextChar {
    uint32_t codepoint;
...
private:
    ...
    Result<void> createBindGroup(WebGPUContext& ctx);
    ...
    std::vector<GlyphInstance> glyphs_;        // GPU-ready glyph instances
    ...
    //-------------------------------------------------------------------------
    // GPU resources (mirrors PDFLayer)
    //-------------------------------------------------------------------------
