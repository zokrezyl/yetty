...
struct grid_line {
    ...
    struct yetty_ydraw_composite **figures;
                                                     ...
                                                     struct yetty_yfont_cache *cache)
{
    ...
    for (uint32_t i = 0; i < line->figure_count; i++) {
        yetty_ydraw_composite_destroy(line->figures[i]);
    }
    free(line->figures);
    line->figures = NULL;
    line->figure_count = 0;
...
struct yetty_ydraw_composite *yetty_ydraw_scrolling_grid_figure_in_window(
                                                      ...
                                                      uint32_t grid_cols)
{
    ...
    struct yetty_ydraw_composite **saved_cp = line->figures;
    ...
    struct yetty_ydraw_composite *figure)
{
    ...
    if (line->figure_count >= line->figure_capacity) {
        ...
        struct yetty_ydraw_composite **grown =
            realloc(line->figures, new_cap * sizeof(struct yetty_ydraw_composite *));
        if (!grown) {
