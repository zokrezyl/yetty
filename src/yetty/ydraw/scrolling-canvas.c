...
{
    if (want <= m->capacity) return YETTY_OK_VOID();
...
static struct yetty_ycore_void_result attach_list_note(
    struct attach_list *l, yetty_yfont_cache_handle handle, uint32_t row)
{
    if (handle == YETTY_YFONT_CACHE_HANDLE_INVALID) return YETTY_OK_VOID();
    for (uint32_t i = 0; i < l->count; i++) {
        if (l->entries[i].handle == handle) {
            if (row > l->entries[i].max_row) l->entries[i].max_row = row;
            return YETTY_OK_VOID();
        }
    }
...
static struct yetty_ycore_void_result ensure_grid_staging(
    struct scrolling_canvas *c, uint32_t min_size)
{
    if (min_size <= c->grid_staging_capacity) return YETTY_OK_VOID();
    uint32_t new_cap = c->grid_staging_capacity == 0
                          ? YDRAW_STAGING_INITIAL_CAPACITY
                          : c->grid_staging_capacity;
...
static struct yetty_ycore_void_result ensure_prim_staging(
    struct scrolling_canvas *c, uint32_t min_size)
{
    if (min_size <= c->prim_staging_capacity) return YETTY_OK_VOID();
    uint32_t new_cap = c->prim_staging_capacity == 0
                          ? YDRAW_STAGING_INITIAL_CAPACITY
                          : c->prim_staging_capacity;
    while (new_cap < min_size) new_cap *= 2;
    uint32_t *grown = realloc(c->prim_staging, new_cap * sizeof(uint32_t));
    if (!grown) {
        return YETTY_ERR(yetty_ycore_void, "scrolling-canvas: prim_staging realloc");
    }
    c->prim_staging = grown;
    c->prim_staging_capacity = new_cap;
    return YETTY_OK_VOID();
}
...
static struct yetty_yfont_cache_ref_result get_default_font_ref(
    struct scrolling_canvas *c)
{
...
static struct yetty_ycore_void_result ensure_blob_font_cdb(
    struct scrolling_canvas *c, const uint8_t *ttf, uint32_t ttf_len,
    const char *hint_name, char out_hex[17])
{
    ...
    if (blob_is_raster(hint_name, c->font_render_method)) {
        return YETTY_ERR(yetty_ycore_void,
                         "raster blob fonts not yet wired through font cache");
    }
    ...
    if (yetty_yplatform_file_exists(cdb_path)) return YETTY_OK_VOID();
    ...
    if (!yetty_yplatform_file_exists(ttf_path)) {
        ...
        ydebug("scrolling-canvas: cached TTF '%s' (%u bytes) hint='%s'",
    ...
    struct yetty_ycore_void_result gr =
        c->msdf_generator->ops->generate(c->msdf_generator, &gen);
    YETTY_RETURN_IF_ERR(yetty_ycore_void, gr, "msdf generator failed");
    ydebug("scrolling-canvas: generated CDB '%s' via %s generator",
           cdb_path, c->msdf_generator->ops->name(c->msdf_generator));
    return YETTY_OK_VOID();
#else
...
static struct yetty_yfont_cache_ref_result resolve_blob_font_handle(
    struct scrolling_canvas *c, const char *hex)
{
    const char *cache_dir = yetty_yplatform_get_cache_dir();
    if (!cache_dir || !*cache_dir) {
    ...
    const struct yetty_context *context)
{
    ...
    /* Flyweight registry. */
    struct yetty_ydraw_flyweight_registry_ptr_result fw_res =
    ...
    struct yetty_ydraw_figure_factory_ptr_result factory_res =
        yetty_ydraw_figure_factory_create(
    ...
    /* Opaque grid. */
    struct yetty_ydraw_scrolling_grid_ptr_result grid_res =
        yetty_ydraw_scrolling_grid_create();
...
{
    if (!base) return YETTY_ERR(yetty_ycore_void, "scrolling-canvas: NULL");
...
static struct yetty_ycore_void_result scrolling_set_grid_size(
...
static struct yetty_ycore_pixel_size scrolling_get_cell_size(
    const struct yetty_ydraw_canvas *base)
{
    return base ? as_scrolling_const(base)->cell_size
                : (struct yetty_ycore_pixel_size){0, 0};
}

static struct yetty_ycore_grid_size scrolling_get_grid_size(
    const struct yetty_ydraw_canvas *base)
{
    return base ? as_scrolling_const(base)->grid_size
...
static struct yetty_ycore_void_result scrolling_scroll_lines(
    struct yetty_ydraw_canvas *base, uint16_t num_lines)
{
    if (!base) return YETTY_ERR(yetty_ycore_void, "scrolling-canvas: NULL");
    if (num_lines == 0) return YETTY_OK_VOID();
    struct scrolling_canvas *c = as_scrolling(base);
...
static struct yetty_ycore_void_result scrolling_set_view_top(
    ...
    struct scrolling_canvas *c, const struct yetty_ydraw_drawable_flyweight *fw)
{
    ...
    if (aabb.max.y < min_valid_y) aabb.max.y = min_valid_y;
    if (aabb.min.y < min_valid_y) aabb.min.y = min_valid_y;
    ...
    struct uint32_result push_res = yetty_ydraw_scrolling_grid_push_prim(
        c->grid, primitive_grid_line, primitive_rolling_row,
    ...
    uint32_t prim_col_max = (uint32_t)(aabb.max.x / c->cell_size.width);

    int32_t row_min_rel = (int32_t)floorf(aabb.min.y / c->cell_size.height);
    int32_t row_max_rel =
        (int32_t)ceilf(aabb.max.y / c->cell_size.height) - 1;
    if (row_min_rel < 0) row_min_rel = 0;
    if (row_max_rel < 0) row_max_rel = 0;
    if (row_max_rel < row_min_rel) row_max_rel = row_min_rel;
    ...
    if (prim_col_max >= c->grid_size.cols) {
        prim_col_max = c->grid_size.cols - 1;
    }
    ...
    for (uint32_t row = prim_row_min; row <= prim_row_max; row++) {
        ...
        struct yetty_ycore_void_result dr =
            yetty_ydraw_scrolling_grid_dirty_line(c->grid, row);
    ...
    if (yetty_ydraw_is_complex_type(prim_type)) {
        struct yetty_ydraw_figure_instance_ptr_result inst_res =
            yetty_ydraw_figure_factory_create_instance(
                c->figure_factory, fw->data,
                word_count * sizeof(uint32_t), primitive_rolling_row);
        YETTY_RETURN_IF_ERR(uint32, inst_res, "add_primitive: create_instance");
        struct yetty_ycore_void_result fr =
            yetty_ydraw_scrolling_grid_push_figure(c->grid, primitive_grid_line,
                                                   inst_res.value);
        if (YETTY_IS_ERR(fr)) {
...
static struct uint32_result expand_text_span_to_glyphs(
    struct scrolling_canvas *c, const struct yetty_ydraw_text_span_prim_view *ts,
    struct yetty_ydraw_font *font, yetty_yfont_cache_handle font_handle)
{
    ...
    while (ptr < end) {
        ...
        uint32_t col_min = (c->cell_size.width > 0)
                               ? (uint32_t)(gx / c->cell_size.width) : 0;
        uint32_t col_max = (c->cell_size.width > 0)
                               ? (uint32_t)((gx + gw) / c->cell_size.width) : 0;
        ...
        for (uint32_t row = row_min; row <= glyph_row_max; row++) {
            ...
            for (uint32_t col = col_min; col <= col_max; col++) {
                struct yetty_ycore_void_result rp =
                    yetty_ydraw_scrolling_grid_push_ref(
                        c->grid, row, col, lines_ahead, (uint16_t)prim_idx);
                YETTY_RETURN_IF_ERR(uint32, rp, "expand_text_span: push_ref");
            }
        }

        if (glyph_row_max > glyph_max_row) glyph_max_row = glyph_row_max;
        ...
        if (tv.font_id >= 0 && (uint32_t)tv.font_id < c->env_fonts_map.capacity) {
            ...
            if (e->resolved) {
                ...
                struct yetty_yfont_cache_ref_result rr =
                    resolve_blob_font_handle(c, e->hex);
                YETTY_RETURN_IF_ERR(yetty_ycore_void, rr,
                                    "dispatch: resolve_blob_font_handle");
...
static struct yetty_ycore_void_result env_finalize(struct scrolling_canvas *c)
{
    ...
    if (target_cursor_canvas_line > viewport_bottom) {
        ...
        if (c->scroll_callback) {
            struct yetty_ycore_void_result sb = c->scroll_callback(
                c->scroll_callback_user_data, (uint16_t)lines_to_scroll);
            YETTY_RETURN_IF_ERR(yetty_ycore_void, sb,
                                "env_finalize: scroll_callback failed");
        }
    ...
    if (c->cursor_set_callback) {
        struct yetty_ycore_void_result cb = c->cursor_set_callback(
            c->cursor_set_callback_user_data, c->cursor_row);
        YETTY_RETURN_IF_ERR(yetty_ycore_void, cb,
                            "env_finalize: cursor_set_callback");
    }
...
static struct yetty_ycore_void_result scrolling_rebuild_grid(
    struct yetty_ydraw_canvas *base)
{
    if (!base) return YETTY_ERR(yetty_ycore_void, "scrolling-canvas: NULL");
    struct scrolling_canvas *c = as_scrolling(base);
     ...
     * cells than grid_w. */
    uint32_t max_cells = yetty_ydraw_scrolling_grid_max_cell_count_in_window(
        c->grid, window_top, window_top + grid_h);
    if (max_cells > grid_w) grid_w = max_cells;
    ...
    struct yetty_ycore_void_result e =
        ensure_grid_staging(c, num_cells * 4);
    ...
    struct yetty_ycore_void_result rr = yetty_ydraw_scrolling_grid_rebuild_staging(
        c->grid, window_top, grid_h, grid_w,
        &c->grid_staging, &c->grid_staging_capacity, &count);
...
static struct yetty_ydraw_drawable_staging_result scrolling_build_prim_staging(
    struct yetty_ydraw_canvas *base)
{
    if (!base) {
    ...
    struct yetty_ycore_void_result e = ensure_prim_staging(c, YDRAW_STAGING_INITIAL_CAPACITY);
    YETTY_RETURN_IF_ERR(yetty_ydraw_drawable_staging, e,
                        "build_prim_staging: ensure_prim_staging");

    uint32_t count = 0;
    uint32_t prim_count = 0;
    struct yetty_ycore_void_result r = yetty_ydraw_scrolling_grid_build_prim_staging(
        c->grid, &c->prim_staging, &c->prim_staging_capacity, &count, &prim_count);
    YETTY_RETURN_IF_ERR(yetty_ydraw_drawable_staging, r,
                        "build_prim_staging: grid_build");
    c->prim_staging_count = count;

    if (count == 0) {
        struct yetty_ydraw_drawable_staging empty = {.data = NULL, .word_count = 0};
        return YETTY_OK(yetty_ydraw_drawable_staging, empty);
    }
    struct yetty_ydraw_drawable_staging out = {
        .data = c->prim_staging, .word_count = count};
    return YETTY_OK(yetty_ydraw_drawable_staging, out);
}

static uint32_t scrolling_prim_gpu_size(const struct yetty_ydraw_canvas *base)
{
    if (!base) return 0;
    ...
    return c->prim_staging_count * (uint32_t)sizeof(uint32_t);
}
...
{
    return base ? yetty_ydraw_scrolling_grid_total_prim_count(
                      as_scrolling_const(base)->grid)
                : 0;
}
...
static struct yetty_ydraw_font *scrolling_get_font_at(
...
static struct yetty_ydraw_font *scrolling_get_default_font(
    const struct yetty_ydraw_canvas *base)
{
...
static void visible_window(const struct scrolling_canvas *c,
...
static void glyph_adapter(float x, float y, uint32_t glyph_idx, int32_t font_slot, void *user)
{
    ...
    struct yetty_ydraw_glyph_view view = {
        .x = x, .y = y, .glyph_idx = glyph_idx, .font_slot = font_slot,
    };
...
{
    if (!base)    return YETTY_ERR(yetty_ycore_void, "scrolling-canvas: NULL");
    if (!visitor) return YETTY_ERR(yetty_ycore_void, "scrolling-canvas: visitor NULL");
    struct scrolling_canvas *c = as_scrolling(base);
    struct glyph_adapter_ctx ctx = {.visitor = visitor, .user = user};
    yetty_ydraw_scrolling_grid_for_each_glyph(c->grid, c->cell_size.height,
                                              glyph_adapter, &ctx);
    return YETTY_OK_VOID();
}
