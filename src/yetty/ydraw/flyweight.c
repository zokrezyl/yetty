...
{
    struct yetty_ydraw_flyweight_registry_ptr_result res = yetty_ydraw_flyweight_registry_create();
    if (YETTY_IS_ERR(res)) {
        return res;
    }
    ...
    struct yetty_ycore_void_result r_complex = yetty_ydraw_flyweight_registry_add(
        reg, YETTY_YDRAW_COMPLEX_TYPE_BASE, 0xFFFFFFFF, yetty_ydraw_complex_drawable_handler);
