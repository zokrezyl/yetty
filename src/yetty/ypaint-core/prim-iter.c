...
struct yetty_ycore_void_result yetty_ypaint_core_prim_iter_init(
    struct yetty_ypaint_core_prim_iter *iter,
...
void yetty_ypaint_core_prim_iter_destroy(struct yetty_ypaint_core_prim_iter *iter)
{
    if (!iter) {
        return;
    }
...
static struct yetty_ycore_void_result iter_ensure_scratch(
    struct yetty_ypaint_core_prim_iter *iter, uint32_t want)
{
...
struct yetty_ypaint_core_prim_iter_status_result yetty_ypaint_core_prim_iter_next(
    struct yetty_ypaint_core_prim_iter *iter)
{
    if (!iter) {
        return YETTY_ERR(yetty_ypaint_core_prim_iter_status, "iter is NULL");
    }
        ...
        if (iter->header_filled < PRIM_ITER_ENVELOPE_HEADER_BYTES) {
            ...
            if (at_end) {
                if (iter->header_filled == 0) {
                    ...
                    return YETTY_OK(yetty_ypaint_core_prim_iter_status,
    ...
    if (iter->total_size == 0) {
        ...
        struct yetty_ypaint_core_prim_flyweight_ptr_result fw_res =
            yetty_ypaint_core_flyweight_registry_get(
    ...
    iter->filled = 0;
    iter->total_size = 0;
    return YETTY_OK(yetty_ypaint_core_prim_iter_status, YETTY_PRIM_ITER_OK);
}
