...
struct yetty_ypaint_core_flyweight_registry {
    ...
    struct yetty_ypaint_core_handler_reg handlers[FLYWEIGHT_MAX_HANDLERS];
...
{
    static struct yetty_ypaint_core_prim_flyweight fw;

    if (!reg) {
        return YETTY_ERR(yetty_ypaint_core_prim_flyweight_ptr, "registry is NULL");
    }
    if (!prim_data) {
        return YETTY_ERR(yetty_ypaint_core_prim_flyweight_ptr, "prim_data is NULL");
    }
    ...
    if (reg->default_handler) {
        struct yetty_ypaint_core_prim_base_ops_ptr_result ops_res = reg->default_handler(prim_type);
        if (YETTY_IS_OK(ops_res)) {
            ...
            return YETTY_OK(yetty_ypaint_core_prim_flyweight_ptr, &fw);
        }
    }
    ...
    for (size_t i = 0; i < reg->handler_count; i++) {
        ...
        if (prim_type >= reg->handlers[i].type_min && prim_type <= reg->handlers[i].type_max) {
            struct yetty_ypaint_core_prim_base_ops_ptr_result ops_res =
    ...
    return YETTY_ERR(yetty_ypaint_core_prim_flyweight_ptr, "no handler for prim_type");
}
