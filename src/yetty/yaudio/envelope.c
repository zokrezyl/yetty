    ...
    uint32_t hop_samples)
{
    ...
    if (channel >= w->channels) {
        return YETTY_ERR(yetty_yaudio_envelope_ptr, "envelope_create: channel out of range");
    }
    if (frame_samples == 0) {
        frame_samples = 1024;
    }
    ...
    if ((size_t)frame_samples > w->frames) {
        return YETTY_ERR(yetty_yaudio_envelope_ptr, "envelope_create: file shorter than one frame");
    }
    ...
    if (!env) {
        return YETTY_ERR(yetty_yaudio_envelope_ptr, "envelope_create: env alloc failed");
    }
    env->rms = malloc(total_frames * sizeof(float));
    if (!env->rms) {
        free(env);
        return YETTY_ERR(yetty_yaudio_envelope_ptr, "envelope_create: rms alloc failed");
    }
    env->n = total_frames;
    ...
    if (!buf) {
        free(env->rms);
        free(env);
        return YETTY_ERR(yetty_yaudio_envelope_ptr, "envelope_create: buf alloc failed");
    }
    ...
    /* Initial fill */
    {
        ...
        if (YETTY_IS_ERR(r)) {
            ...
            free(env->rms);
            free(env);
            return YETTY_ERR(yetty_yaudio_envelope_ptr, "envelope_create: initial read failed", r);
        }
        buf_fill = r.value;
        src_off += r.value;
    }
    ...
    while (frame_idx < total_frames) {
        ...
        if (need_end > buf_base_sample + buf_fill) {
            ...
            if (YETTY_IS_ERR(r)) {
                ...
                free(env->rms);
                free(env);
                return YETTY_ERR(yetty_yaudio_envelope_ptr, "envelope_create: chunk read failed",
                                 r);
            }
            buf_fill += r.value;
            src_off += r.value;
    ...
    if (frame_idx != total_frames) {
        ywarn("envelope_create: produced %zu / %zu frames", frame_idx, total_frames);
        env->n = frame_idx;
    }
    yinfo("envelope_create: ch=%u frames=%zu (%.1f s)", channel, env->n,
