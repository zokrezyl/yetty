                                    ...
                                    const struct yetty_yvterm_screen *screen)
{
    ...
    return screen->total_scrolled < capacity ? screen->total_scrolled : capacity;
}
                                                        ...
                                                        uint32_t cols, uint32_t rows)
{
     ...
     * switches back: the primary reflows its soft-wrapped text (history is
     * already archived; alt-screen sessions included), the alternate is
     ...
     * width; they re-inflate on the next scrolled-back frame). A scrolled-back
     * view itself snaps to the live tail on resize (vterm does that). */
    struct yetty_ycore_void_result blank_res = grid_blank_line_resize(grid, cols);
    YETTY_RETURN_IF_ERR(yetty_ycore_void, blank_res, "yvterm: resize blank line");
...
{
    struct yetty_yvterm_grid_ptr_result grid_res = yetty_yvterm_grid_from(obj);
    YETTY_RETURN_IF_ERR(yetty_ycore_uint32, grid_res, "yvterm grid_live_anchor: from_obj");
    return YETTY_OK(yetty_ycore_uint32, (uint32_t)grid_live_top_timeline(grid_res.value));
}
...
{
    struct yetty_yvterm_grid_ptr_result grid_res = yetty_yvterm_grid_from(obj);
    YETTY_RETURN_IF_ERR(yetty_ycore_uint32, grid_res, "yvterm grid_history_floor: from_obj");
    struct yetty_yvterm_grid *grid = grid_res.value;
    struct yetty_yvterm_screen *screen = active_screen(grid);
    if (screen != &grid->primary) {
        return YETTY_OK(yetty_ycore_uint32, (uint32_t)grid_live_top_timeline(grid));
    }
    if (!grid_archive_enabled(grid)) {
        /* Pre-tiering ring floor: whatever history the ring still holds. */
        uint32_t history_rows = screen_history_rows(grid, screen);
        return YETTY_OK(yetty_ycore_uint32, screen->total_scrolled - history_rows);
    }
    /* Archive [dropped, archived) + ring [archived, live top) are contiguous:
     * the floor is simply the archive's dropped count. */
    return YETTY_OK(yetty_ycore_uint32, (uint32_t)grid->tiers.dropped_lines);
}
