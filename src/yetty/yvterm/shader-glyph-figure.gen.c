...
[[maybe_unused]]
static yetty_yfigure_render_fn yetty_yvterm_shader_glyph_yetty_yfigure_render_check =
    figure_render_slot;
[[maybe_unused]]
static yetty_yfigure_destroy_fn yetty_yvterm_shader_glyph_yetty_yfigure_destroy_check =
    figure_destroy_slot;
...
struct yetty_yclass_ptr_result yetty_yvterm_shader_glyph_class_get(void)
{
    ...
    ydebug("registering class=yetty_yvterm_shader_glyph");

    static const struct yetty_yclass_descriptor desc = {
        .name = "yetty_yvterm_shader_glyph",
        .type = YETTY_YCLASS_TYPE_REGULAR,
        .data_size = sizeof(struct yetty_yvterm_shader_glyph),
        .data_align = _Alignof(struct yetty_yvterm_shader_glyph),
    };
    ...
    if (YETTY_IS_ERR(parent_class_r)) {
        yerror("yetty_yvterm_shader_glyph_class_get: parent accessor failed: %s",
               parent_class_r.error.msg);
        return YETTY_ERR(yetty_yclass_ptr,
                         "yetty_yvterm_shader_glyph_class_get: parent accessor failed",
                         parent_class_r);
    }
    struct yetty_yclass_ptr_result register_class_r = yetty_yclass_register(
        &desc, ops, sizeof(ops) / sizeof(ops[0]), parent_class_r.value, NULL, 0);
    if (YETTY_IS_ERR(register_class_r)) {
        yerror("yetty_yvterm_shader_glyph_class_get: class_register failed: %s",
               register_class_r.error.msg);
        return YETTY_ERR(yetty_yclass_ptr,
                         "yetty_yvterm_shader_glyph_class_get: class_register failed",
                         register_class_r);
    }
    ...
    struct yetty_yclass_object *obj)
{
    ...
    if (YETTY_IS_ERR(class_r)) {
        return YETTY_ERR(yetty_yvterm_shader_glyph_ptr,
                         "yetty_yvterm_shader_glyph_from: class accessor", class_r);
    }
    struct yetty_yclass_void_ptr_result slice_r = yetty_yclass_object_data(obj, class_r.value);
    if (YETTY_IS_ERR(slice_r)) {
        return YETTY_ERR(yetty_yvterm_shader_glyph_ptr,
                         "yetty_yvterm_shader_glyph_from: object_data", slice_r);
    }
    return YETTY_OK(yetty_yvterm_shader_glyph_ptr,
                    (struct yetty_yvterm_shader_glyph *)slice_r.value);
}
...
struct yetty_yclass_object_ptr_result yetty_yvterm_shader_glyph_create(struct yetty_yclass_ctx *ctx)
{
    ...
    if (YETTY_IS_ERR(class_accessor_r)) {
        return YETTY_ERR(yetty_yclass_object_ptr,
                         "yetty_yvterm_shader_glyph_create: class accessor failed",
                         class_accessor_r);
    }
    ...
    if (!proxy) {
        return YETTY_ERR(yetty_yclass_object_ptr,
                         "yetty_yvterm_shader_glyph_create: calloc(proxy) failed");
    }
...
/* ---- yvterm/shader_glyph_figure: class name -> accessor ---------------------- */
static struct yetty_yclass_ptr_result yetty_yvterm_shader_glyph_figure_accessor_lookup(
    const char *name)
{
    if (strcmp(name, "yetty_yvterm_shader_glyph") == 0) {
        return yetty_yvterm_shader_glyph_class_get();
    }
    return YETTY_OK(yetty_yclass_ptr, NULL);
}

struct yetty_ycore_void_result yetty_yvterm_shader_glyph_figure_register_hooks(void)
{
    struct yetty_ycore_void_result add_accessor_r =
        yetty_yclass_add_accessor_lookup(yetty_yvterm_shader_glyph_figure_accessor_lookup);
    YETTY_RETURN_IF_ERR(yetty_ycore_void, add_accessor_r,
                        "yetty_yvterm_shader_glyph_figure_register_hooks: accessor");
