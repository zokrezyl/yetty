...
struct yetty_ydraw_core_draw_list_result yetty_yimage_render(
    const uint8_t *image_bytes, size_t len, const struct yetty_yimage_render_config *config)
{
    if (!image_bytes || len == 0) {
        return YETTY_ERR(yetty_ydraw_core_draw_list, "yimage: image_bytes is NULL/empty");
    }
    ...
    if (!pixels) {
        return YETTY_ERR(yetty_ydraw_core_draw_list, stbi_failure_reason());
    }
    ...
    if (!prim_buf) {
        ...
        return YETTY_ERR(yetty_ydraw_core_draw_list, "yimage: prim alloc failed");
    }
    ...
    if (YETTY_IS_ERR(ser)) {
        free(prim_buf);
        return YETTY_ERR(yetty_ydraw_core_draw_list, "yimage: serialize failed", ser);
    }
    ...
    if (YETTY_IS_ERR(br)) {
        free(prim_buf);
        return YETTY_ERR(yetty_ydraw_core_draw_list, "yimage: ydraw buffer create failed", br);
    }

    struct yetty_ydraw_core_id_result idr =
        yetty_ydraw_core_draw_list_add_prim(br.value, prim_buf, required);
    free(prim_buf);
    if (YETTY_IS_ERR(idr)) {
        yetty_ydraw_core_draw_list_destroy(br.value);
        return YETTY_ERR(yetty_ydraw_core_draw_list, "yimage: ydraw add_prim failed", idr);
    }

    return YETTY_OK(yetty_ydraw_core_draw_list, br.value);
}

struct yetty_ydraw_core_draw_list_result yetty_yimage_render_path(
    const char *path, const struct yetty_yimage_render_config *config)
{
    if (!path) {
        return YETTY_ERR(yetty_ydraw_core_draw_list, "yimage_render_path: path is NULL");
    }

    FILE *f = fopen(path, "rb");
    if (!f) {
        return YETTY_ERR(yetty_ydraw_core_draw_list, "yimage_render_path: fopen failed");
    }
    if (fseek(f, 0, SEEK_END) != 0) {
        fclose(f);
        return YETTY_ERR(yetty_ydraw_core_draw_list, "yimage_render_path: fseek failed");
    }
    long size = ftell(f);
    if (size <= 0) {
        fclose(f);
        return YETTY_ERR(yetty_ydraw_core_draw_list, "yimage_render_path: empty or unreadable");
    }
    rewind(f);
    uint8_t *bytes = malloc((size_t)size);
    if (!bytes) {
        fclose(f);
        return YETTY_ERR(yetty_ydraw_core_draw_list, "yimage_render_path: malloc failed");
    }
    size_t nread = fread(bytes, 1, (size_t)size, f);
    fclose(f);
    if (nread != (size_t)size) {
        free(bytes);
        return YETTY_ERR(yetty_ydraw_core_draw_list, "yimage_render_path: short read");
    }

    struct yetty_ydraw_core_draw_list_result r = yetty_yimage_render(bytes, (size_t)size, config);
    free(bytes);
    return r;
}
    ...
    const struct yetty_ydraw_core_draw_list *buffer, FILE *out)
{
    ...
    size_t raw_size =
        yetty_ydraw_core_draw_list_serialize((struct yetty_ydraw_core_draw_list *)buffer, &raw);
    if (raw_size == 0 || !raw) {
