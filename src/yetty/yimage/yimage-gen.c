...
static struct yetty_yrender_gpu_resource_set yimage_lib_rs;
...
static void yimage_populate_rs(struct yetty_yrender_gpu_resource_set *rs)
{
    yimage_init_lib_rs();
    ...
    rs->children[0] = (struct yetty_yrender_gpu_resource_set *)&yimage_lib_rs;
    rs->children_count = 1;
...
static struct yetty_ydraw_figure_ptr_result yimage_create_instance(
...
static struct yetty_yrender_gpu_resource_set *yimage_get_shared_rs(
...
void yetty_yimage_factory_destroy(struct yetty_ydraw_concrete_factory *self)
{
    if (!self) {
        return;
    }
