...
struct yetty_yui_debug_window {
    ...
    struct yetty_ygui_old_widget *label_1s;
    struct yetty_ygui_old_widget *label_10s;
    struct yetty_ygui_old_widget *label_60s;
...
struct yetty_yui_debug_window_ptr_result yetty_yui_debug_window_create(
    struct yetty_ygui_old_engine *engine, yetty_ycore_object_id pane_id)
{
    if (!engine) {
    ...
    yetty_ygui_old_widget_set_bg_color(dw->window, DEBUG_WIN_BG);
    yetty_ygui_old_widget_set_visible(dw->window, 1);
     ...
     * a placeholder; real actions land in later steps. */
    {
        ...
        struct yetty_ygui_old_widget *menu =
            yetty_ygui_old_engine_popup_menu(engine, menu_id, 0.0f, 0.0f, 180.0f);
        if (menu) {
            yetty_ygui_old_widget_popup_menu_add_item(menu, "(no actions yet)", NULL, NULL);
            yetty_ygui_old_widget_window_set_menu(dw->window, menu);
        }
    }

    struct yetty_ygui_old_widget *body = yetty_ygui_old_widget_window_body(dw->window);
    if (body) {
        struct {
            const char *suffix;
        ...
        for (int i = 0; i < 3; i++) {
            ...
            struct yetty_ygui_old_widget *lbl =
                yetty_ygui_old_engine_label(engine, lbl_id, 0.0f, 0.0f, "—");
            if (lbl) {
                yetty_ygui_old_widget_set_fg_color(lbl, DEBUG_WIN_FG);
                yetty_ygui_old_widget_label_set_font_size(lbl, DEBUG_WIN_FONT_SIZE);
                yetty_ygui_old_widget_add_child(body, lbl);
            }
            *rows[i].slot = lbl;
        }
    }
...
struct yetty_ycore_void_result yetty_yui_debug_window_destroy(struct yetty_yui_debug_window *dw)
{
    ...
    if (dw->window) {
        yetty_ygui_old_widget_remove(dw->window);
        dw->window = NULL;
    }
    dw->label_1s = NULL;
    dw->label_10s = NULL;
                                                             ...
                                                             float pane_w, float pane_h)
{
    ...
    if (w < 1.0f || h < 1.0f) {
        yetty_ygui_old_widget_set_visible(dw->window, 0);
        return YETTY_OK_VOID();
    }
    yetty_ygui_old_widget_set_visible(dw->window, 1);
    yetty_ygui_old_widget_set_size(dw->window, w, h);
    ...
    if (!dw->placed) {
        ...
        yetty_ygui_old_widget_set_position(dw->window, x, y);
        dw->placed = 1;
    }
    return YETTY_OK_VOID();
}
    ...
    struct yetty_yui_debug_window *dw, const struct yetty_ywire_stats_snapshot *snap)
{
    ...
    if (snap) {
        if (dw->label_1s) {
            ...
            yetty_ygui_old_widget_label_set_text(dw->label_1s, line);
        }
        if (dw->label_10s) {
            ...
            yetty_ygui_old_widget_label_set_text(dw->label_10s, line);
        }
        if (dw->label_60s) {
            ...
            yetty_ygui_old_widget_label_set_text(dw->label_60s, line);
        }
    } else {
        if (dw->label_1s) {
            yetty_ygui_old_widget_label_set_text(dw->label_1s, "(no terminal)");
        }
        if (dw->label_10s) {
            yetty_ygui_old_widget_label_set_text(dw->label_10s, "");
        }
        if (dw->label_60s) {
            yetty_ygui_old_widget_label_set_text(dw->label_60s, "");
        }
    }
    return YETTY_OK_VOID();
}
