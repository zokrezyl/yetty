    ...
    if (split->first) {
        {
            ...
            YETTY_RETURN_IF_ERR(yetty_ycore_void, drop_r, "drop: yetty_yui_tile_set_bounds");
        }
    }
    if (split->second) {
        {
        ...
        if (split->first) {
            {
                ...
                YETTY_RETURN_IF_ERR(yetty_ycore_int, drop_r, "drop: yetty_yui_tile_on_event");
            }
        }
        if (split->second) {
            {
    ...
    if (pane->view_count > 0 && pane->views[pane->view_count - 1]) {
        {
            ...
            YETTY_RETURN_IF_ERR(yetty_ycore_void, drop_r, "drop: yetty_yui_view_set_bounds");
        }
    }
    ...
    const struct yetty_yconfig_config *config, const struct yetty_context *yetty_ctx)
{
    ...
    if (strcmp(type, "split") == 0) {
        ...
        }
        {
            ...
            YETTY_RETURN_IF_ERR(yetty_yui_tile_ptr, drop_r, "drop: yetty_yui_tile_split_set_ratio");
        }
        ...
        if (first_config) {
            ...
            }
            {
                ...
                YETTY_RETURN_IF_ERR(yetty_yui_tile_ptr, drop_r,
                                    "drop: yetty_yui_tile_split_set_first");
            }
        }
        ...
        if (second_config) {
            ...
            }
            {
                ...
                YETTY_RETURN_IF_ERR(yetty_yui_tile_ptr, drop_r,
                                    "drop: yetty_yui_tile_split_set_second");
            }
        }
        ...
        if (desktop_vnc_client && strlen(desktop_vnc_client) > 0) {
...

            {
                struct yetty_ycore_void_result drop_r = yetty_yui_tile_pane_push_view(
                    res.value, yetty_ydvnc_viewer_as_view(dv_res.value));
                YETTY_RETURN_IF_ERR(yetty_yui_tile_ptr, drop_r,
            ...
            {
                struct yetty_ycore_void_result drop_r = yetty_yui_tile_pane_push_view(
                    res.value, yetty_yvnc_viewer_as_view(vnc_res.value));
                YETTY_RETURN_IF_ERR(yetty_yui_tile_ptr, drop_r,
                                    "drop: yetty_yui_tile_pane_push_view");
            }
        } else {
            ...
            if (strcmp(view_type, "terminal") == 0) {
                ...
                if (YETTY_IS_ERR(term_res)) {
                    ...
                    return YETTY_ERR(yetty_yui_tile_ptr,
                                     "tile_create_from_config: terminal_create failed", term_res);
                }
