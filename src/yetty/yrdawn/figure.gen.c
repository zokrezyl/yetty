...
struct yetty_yclass_object_ptr_result yetty_yrdawn_figure_create(struct yetty_yclass_ctx *ctx)
{
     ...
     * response isn't silently swallowed. */
    {
        struct yetty_ycore_void_result translate_class_r =
            yetty_yclass_rpc_session_translate_class(ctx->session, "yetty_yrdawn_figure");
        if (YETTY_IS_ERR(translate_class_r)) {
            yetty_ycore_error_print(
                stderr,
                "yetty_yrdawn_figure_create: translate_class (degraded — will lazy-resolve)",
    ...
    uint64_t handle = 0;
    const char *class_name = "yetty_yrdawn_figure";
    ...
    if (YETTY_IS_ERR(create_call_r)) {
        return YETTY_ERR(yetty_yclass_object_ptr, "yetty_yrdawn_figure_create: CREATE call failed",
                         create_call_r);
    }
    if (create_call_r.value != sizeof(handle) || !handle) {
        return YETTY_ERR(yetty_yclass_object_ptr,
                         "yetty_yrdawn_figure_create: CREATE returned no/invalid handle");
    }
    ...
    if (!proxy) {
        return YETTY_ERR(yetty_yclass_object_ptr,
                         "yetty_yrdawn_figure_create: calloc(proxy) failed");
    }
...
/* ---- yrdawn/figure: class name -> accessor ---------------------- */
static struct yetty_yclass_ptr_result yetty_yrdawn_figure_accessor_lookup(const char *name)
{
...
struct yetty_ycore_void_result yetty_yrdawn_figure_register_hooks(void)
{
    struct yetty_ycore_void_result add_accessor_r =
        yetty_yclass_add_accessor_lookup(yetty_yrdawn_figure_accessor_lookup);
    YETTY_RETURN_IF_ERR(yetty_ycore_void, add_accessor_r,
                        "yetty_yrdawn_figure_register_hooks: accessor");
...
struct yetty_ycore_void_result yetty_yrdawn_register(void)
{
    ...
    {
        struct yetty_ycore_void_result hook_r = yetty_yrdawn_figure_register_hooks();
        YETTY_RETURN_IF_ERR(yetty_ycore_void, hook_r, "yetty_yrdawn_register: figure");
    }
    registered = true;
    return YETTY_OK_VOID();
}
