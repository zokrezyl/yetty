 ...
 *
 * The full definition is header-destined: codegen reproduces it into the
 * generated figure.h from the `expose` annotation below. This TU needs the
 * same definition for the factory + emit code; it does not include its own
 * figure.h, so the single definition here is the source of truth.
 * yframework.h only forward-declares the struct, so this fuller definition
 * is compatible.
 *=========================================================================*/
...
/* Host-constructed factory bundle — consumers fill this in, so the full
 * definition is exposed (not just a forward decl) into the generated
 * figure.h. */
                                                 ...
                                                 size_t body_len)
{
    ...
    if (status != YETTY_YRDAWN_REPLY_OK) {
        char desc[192];
        int desc_len = snprintf(desc, sizeof(desc), "yrdawn server: method %u failed: %s",
                                hdr->method_id, yetty_yrdawn_reply_status_str(status));
        if (desc_len < 0) {
            desc_len = 0;
