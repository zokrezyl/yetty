...
static struct pending_req *pending_find_free(struct yetty_yrdawn_client *c)
{
...
static struct pending_req *pending_take(struct yetty_yrdawn_client *c, uint32_t req_id)
{
    for (size_t i = 0; i < c->pending_cap; ++i) {
...
{
    struct yetty_yrdawn_client *c = (struct yetty_yrdawn_client *)calloc(1, sizeof(*c));
    if (!c)
...
struct yetty_ycore_void_result yetty_yrdawn_client_send_hello(struct yetty_yrdawn_client *c)
{
    ...
    struct yetty_ycore_void_result emit = yetty_yface_emit_to_fd(
        c->out_fd, YETTY_YRDAWN_OSC_CS_HELLO, 0, NULL, 0, &hello, sizeof(hello));
    YETTY_RETURN_IF_ERR(yetty_ycore_void, emit, "yrdawn_client: emit HELLO");
                                               ...
                                               uint32_t req_id, const void *body, size_t body_len)
{
    ...
    struct yetty_ycore_void_result emit = yetty_yface_emit_to_fd(
        c->out_fd, YETTY_YRDAWN_OSC_CS_CMD, 1, NULL, 0, frame, total);
    free(frame);
    YETTY_RETURN_IF_ERR(yetty_ycore_void, emit, "yrdawn_client: emit CMD");
    return YETTY_OK_VOID();
}
    ...
    yetty_yrdawn_reply_cb cb, void *user)
{
    ...
    if (YETTY_IS_ERR(emit_result)) {
        ...
        slot->cb = NULL;
        slot->user = NULL;
        return YETTY_ERR(yetty_ycore_void, "yrdawn_client: emit_cmd async", emit_result);
    }
    return YETTY_OK_VOID();
}
    ...
    uint8_t **out_buf, size_t *out_len, uint32_t *out_status)
{
    ...
    struct yetty_ycore_void_result sr = yetty_yrdawn_client_send_cmd_async(
...
{
    struct yetty_yrdawn_wire_bye bye = {
        .magic = YETTY_YRDAWN_MAGIC_BYE,
        .version = YETTY_YRDAWN_WIRE_VERSION,
        .total_size = (uint32_t)sizeof(bye),
    };
    struct yetty_ycore_void_result emit = yetty_yface_emit_to_fd(
        c->out_fd, YETTY_YRDAWN_OSC_CS_BYE, 0, NULL, 0, &bye, sizeof(bye));
    YETTY_RETURN_IF_ERR(yetty_ycore_void, emit, "yrdawn_client: emit BYE");
    return YETTY_OK_VOID();
}
    ...
    const void *bytes, size_t len)
{
    ...
    do {
        ...
        struct yetty_ycore_void_result emit_r = yetty_yface_emit_to_fd(
            c->out_fd, YETTY_YRDAWN_OSC_CS_BULK, 1, NULL, 0, frame, frame_bytes);
        free(frame);
    ...
    const void *pixels, size_t bytes)
{
    ...
    return yetty_yrdawn_client_send_cmd_sync(c, /* method id */ 100u, &body, sizeof(body));
}
