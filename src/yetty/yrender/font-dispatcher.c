...
static void builder_appendf(struct wgsl_string_builder *builder, const char *fmt, ...)
{
    ...
    int written = vsnprintf(builder->data + builder->size,
                            builder->capacity - builder->size, fmt, args);
    va_end(args);
    if (written < 0) {
        builder->oom = 1;
        return;
    ...
    char **out_wgsl, size_t *out_size)
{
    ...
    builder_appendf(&builder,
        "// auto-generated font dispatcher (slots 0..%zu)\n", slot_count);
    ...
    for (size_t slot = 0; slot < slot_count; slot++) {
        ...
        if (!namespace_name)
            continue;
        builder_appendf(&builder,
            "        case %zuu: { return uniforms.%s_base_size; }\n",
            slot, namespace_name);
    }
    builder_appendf(&builder,
        "        default: { return 1.0; }\n"
    ...
    for (size_t slot = 0; slot < slot_count; slot++) {
        ...
        if (!namespace_name)
            continue;
        builder_appendf(&builder,
            "        case %zuu: { return %s_glyph_size(glyph_index); }\n",
            slot, namespace_name);
    }
    builder_appendf(&builder,
    ...
    if (builder.oom) {
        free(builder.data);
        return YETTY_ERR(yetty_ycore_void,
                         "build_font_dispatcher_wgsl: out of memory");
    }
