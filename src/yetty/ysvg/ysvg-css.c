                          ...
                          size_t *out_end)
{
    ...
    size_t start = p;
    if (p < len && s[p] == '-') {
        if (p + 1 >= len || !css_is_name_start((unsigned char)s[p + 1])) {
            return 0;
        }
        p++;
    ...
    while (p < len && css_is_name_char((unsigned char)s[p])) {
        p++;
    }
    *out_start = start;
    *out_end = p;
...
{
    for (size_t i = 0; i < c->step_count; i++) {
        css_free_compound(&c->steps[i].compound);
    }
    free(c->steps);
    c->steps = NULL;
    c->step_count = 0;
...
static enum yetty_ysvg_css_pseudo_kind css_pseudo_lookup(const char *name)
{
    if (!strcmp(name, "first-child")) {
        return YETTY_YSVG_CSS_PSEUDO_FIRST_CHILD;
    }
    if (!strcmp(name, "last-child")) {
        return YETTY_YSVG_CSS_PSEUDO_LAST_CHILD;
    }
    if (!strcmp(name, "only-child")) {
        return YETTY_YSVG_CSS_PSEUDO_ONLY_CHILD;
    }
    if (!strcmp(name, "root")) {
        return YETTY_YSVG_CSS_PSEUDO_ROOT;
    }
    if (!strcmp(name, "empty")) {
        return YETTY_YSVG_CSS_PSEUDO_EMPTY;
    }
    if (!strcmp(name, "not")) {
        return YETTY_YSVG_CSS_PSEUDO_NOT;
    }
    if (!strcmp(name, "hover")) {
        return YETTY_YSVG_CSS_PSEUDO_HOVER;
    }
    if (!strcmp(name, "focus")) {
        return YETTY_YSVG_CSS_PSEUDO_FOCUS;
    }
    if (!strcmp(name, "active")) {
        return YETTY_YSVG_CSS_PSEUDO_ACTIVE;
    }
    if (!strcmp(name, "link")) {
        return YETTY_YSVG_CSS_PSEUDO_LINK;
    }
    if (!strcmp(name, "visited")) {
        return YETTY_YSVG_CSS_PSEUDO_VISITED;
    }
    if (!strcmp(name, "enabled")) {
        return YETTY_YSVG_CSS_PSEUDO_ENABLED;
    }
    if (!strcmp(name, "disabled")) {
        return YETTY_YSVG_CSS_PSEUDO_DISABLED;
    }
    if (!strcmp(name, "checked")) {
        return YETTY_YSVG_CSS_PSEUDO_CHECKED;
    }
    if (!strcmp(name, "target")) {
        return YETTY_YSVG_CSS_PSEUDO_TARGET;
    }
                            ...
                            struct yetty_ysvg_css_simple *out)
{
    ...
    if (c == '#') {
        (*pos)++;
        size_t a, b;
        if (!css_read_ident(s, len, pos, &a, &b)) {
            return -1;
        }
        ...
        *pos = css_skip_ws(s, len, *pos);
        if (*pos < len && s[*pos] != ']') {
            ...
            if (*pos + 1 < len && s[*pos + 1] == '=') {
                switch (s[*pos]) {
                case '~':
                    out->attr_op = YETTY_YSVG_CSS_ATTR_INCLUDES;
                    break;
                case '|':
                    out->attr_op = YETTY_YSVG_CSS_ATTR_DASHMATCH;
                    break;
                case '^':
                    out->attr_op = YETTY_YSVG_CSS_ATTR_PREFIX;
                    break;
                case '$':
                    out->attr_op = YETTY_YSVG_CSS_ATTR_SUFFIX;
                    break;
                case '*':
                    out->attr_op = YETTY_YSVG_CSS_ATTR_SUBSTR;
                    break;
                default:
                    return -1;
                }
...
static int css_parse_decl(const char *s, size_t len, size_t *pos, struct yetty_ysvg_css_decl *out)
{
    ...
    *pos = css_skip_ws(s, len, *pos);
    size_t pa, pb;
    if (!css_read_ident(s, len, pos, &pa, &pb)) {
        return 0;
    }
    ...
    *pos = css_skip_ws(s, len, *pos);
    ...
    while (vend > vstart && css_is_wsp((unsigned char)s[vend - 1])) {
        vend--;
    }
    ...
    if (vend - vstart >= 10) {
        size_t scan = vend;
        while (scan > vstart &&
               (css_is_name_char((unsigned char)s[scan - 1]) || s[scan - 1] == '!')) {
            scan--;
        }
        if (vend - scan >= 10 && memcmp(s + scan, "!important", 10) == 0) {
            ...
            if (valid) {
                ...
                size_t cut = scan;
                while (cut > vstart && css_is_wsp((unsigned char)s[cut - 1])) {
                    cut--;
                }
                vend = cut;
                out->important = 1;
...
{
    for (size_t i = 0; i < r->selector_count; i++) {
        css_free_complex(&r->selectors[i]);
    }
    free(r->selectors);
    for (size_t i = 0; i < r->decl_count; i++) {
        css_free_decl(&r->decls[i]);
    }
    free(r->decls);
    r->selectors = NULL;
    r->decls = NULL;
...
static void css_skip_at_rule(const char *s, size_t len, size_t *pos)
{
    ...
    (*pos)++;
    while (*pos < len && css_is_name_char((unsigned char)s[*pos])) {
        (*pos)++;
    }
...
int yetty_ysvg_css_parse(struct yetty_ysvg_doc *doc, const char *css, size_t len)
{
    ...
    while (pos < len) {
        ...
        for (;;) {
            ...
            struct yetty_ysvg_css_complex *ns = realloc(sels, (sel_count + 1) * sizeof(*ns));
            if (!ns) {
                css_free_complex(&cx);
                for (size_t i = 0; i < sel_count; i++) {
                    css_free_complex(&sels[i]);
        ...
        for (;;) {
            ...
            if (r == 0) {
                ...
                struct yetty_ysvg_css_decl *nd = realloc(decls, (dcount + 1) * sizeof(*nd));
                if (!nd) {
                    css_free_decl(&d);
                    for (size_t i = 0; i < dcount; i++) {
                        css_free_decl(&decls[i]);
...
static int node_has_class(const struct yetty_ysvg_node *n, const char *cls)
{
    ...
    size_t i = 0;
    while (i < plen) {
        while (i < plen && css_is_wsp((unsigned char)p[i])) {
            i++;
        }
        size_t s = i;
        while (i < plen && !css_is_wsp((unsigned char)p[i])) {
...
static const struct yetty_ysvg_attr *node_attr(const struct yetty_ysvg_node *n, const char *name_lc)
{
...
{
    if (!val) {
        return op == YETTY_YSVG_CSS_ATTR_EXISTS;
    }
    ...
    case YETTY_YSVG_CSS_ATTR_INCLUDES: {
        size_t i = 0;
        while (i < avlen) {
            while (i < avlen && css_is_wsp((unsigned char)av[i])) {
                i++;
            }
            size_t s = i;
            while (i < avlen && !css_is_wsp((unsigned char)av[i])) {
            ...
            if (i - s == vl && memcmp(av + s, val, vl) == 0) {
                return 1;
            }
        }
        return 0;
    ...
    case YETTY_YSVG_CSS_ATTR_DASHMATCH:
        if (avlen == vl && memcmp(av, val, vl) == 0) {
            return 1;
        }
        if (avlen > vl && av[vl] == '-' && memcmp(av, val, vl) == 0) {
            return 1;
        }
        return 0;
...
static int node_position(const struct yetty_ysvg_node *n, int *is_first, int *is_last, int *is_only)
{
...
static int simple_matches(const struct yetty_ysvg_css_simple *sel, const struct yetty_ysvg_node *n)
{
    if (!n) {
        return 0;
    }
...
{
    for (size_t i = 0; i < c->simple_count; i++) {
        if (!simple_matches(&c->simples[i], n)) {
            return 0;
        }
    }
    return 1;
...
static int complex_matches(const struct yetty_ysvg_css_complex *cx, const struct yetty_ysvg_node *n)
{
    if (cx->step_count == 0) {
        return 0;
    }
    /* Rightmost compound must match the candidate node. */
    if (!compound_matches(&cx->steps[cx->step_count - 1].compound, n)) {
        return 0;
    }
    ...
    for (ptrdiff_t i = (ptrdiff_t)cx->step_count - 1; i > 0; i--) {
        ...
        case YETTY_YSVG_CSS_ADJACENT: {
            ...
            if (!prev_sibling || !compound_matches(prev, prev_sibling)) {
                return 0;
            }
            cursor = prev_sibling;
            break;
...
static int match_cmp_norm(const void *a_, const void *b_)
{
    ...
    if (a->spec_a != b->spec_a) {
        return (a->spec_a < b->spec_a) ? -1 : 1;
    }
    if (a->spec_b != b->spec_b) {
        return (a->spec_b < b->spec_b) ? -1 : 1;
    }
    if (a->spec_c != b->spec_c) {
        return (a->spec_c < b->spec_c) ? -1 : 1;
    }
    if (a->order != b->order) {
    ...
    for (size_t i = 0; i < doc->rule_count; i++) {
        ...
        for (size_t j = 0; j < r->selector_count; j++) {
            ...
            if (complex_matches(cx, node)) {
                ...
                if (cx->spec_a > bA || (cx->spec_a == bA && cx->spec_b > bB) ||
