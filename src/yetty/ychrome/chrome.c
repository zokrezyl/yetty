 ...
 * chrome.h publishes for consumers). The configure() FLAG_* values are an
 * `expose`d enum below; codegen reproduces it into chrome.h. */
 ...
 * Codegen reproduces this enum into the generated chrome.h for consumers. */
enum [[clang::annotate("expose")]] yetty_ychrome_flag {
    YETTY_YCHROME_FLAG_DRAG = 0x1u,     /* caption drag moves the window      */
    YETTY_YCHROME_FLAG_RESIZE = 0x2u,   /* right/bottom edges resize          */
    YETTY_YCHROME_FLAG_MAXIMIZE = 0x4u, /* caption double-click toggles max   */
