...
struct yetty_ycore_void_result yetty_yapp_quit(struct yetty_yclass_object *app)
{
    ...
    if (method_slot == YETTY_YCLASS_METHOD_SLOT_UNDEFINED) {
        ...
        if (YETTY_IS_ERR(method_slot_r)) {
            return YETTY_ERR(yetty_ycore_void, "yetty_yapp_quit: method_slot_get failed",
                             method_slot_r);
        }
        method_slot = method_slot_r.value;
    }
    ...
    if (!app) {
        return YETTY_ERR(yetty_ycore_void, "yetty_yapp_quit: NULL object");
    }
    ...
    struct yetty_yclass_impl_t_result dispatch_impl_r =
        yetty_yclass_dispatch_lookup(object_class_r.value, method_slot);
    YETTY_RETURN_IF_ERR(yetty_ycore_void, dispatch_impl_r,
                        "yetty_yapp_quit: dispatch_lookup failed");
