...
static struct yetty_ydraw_draw_list_result render_from_path(const char *path)
{
    struct _pdfio_file_s *pdf = pdfioFileOpen(path, NULL, NULL, NULL, NULL);
    if (!pdf) {
        return YETTY_ERR(yetty_ydraw_draw_list, "pdfioFileOpen failed");
    }
...
struct yetty_ydraw_draw_list_result yetty_ycat_handler_pdf(
    const uint8_t *bytes, size_t len, const char *path_hint, const struct yetty_ycat_config *config)
{
    ...
    if (!bytes || len == 0) {
        return YETTY_ERR(yetty_ydraw_draw_list, "no bytes and no path");
    }

    char tmp_path[256];
    if (spill_to_tempfile(bytes, len, tmp_path, sizeof(tmp_path)) < 0) {
        return YETTY_ERR(yetty_ydraw_draw_list, "failed to spill PDF to temp file");
    }

    struct yetty_ydraw_draw_list_result r = render_from_path(tmp_path);
    unlink(tmp_path);
    return r;
}
