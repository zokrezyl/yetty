...
struct yetty_ydraw_core_draw_list_result yetty_ycat_handler_mermaid(
    ...
    const struct yetty_ycat_config *config)
{
    ...
    struct yetty_ydiagram_buffer_result r =
        yetty_ydiagram_render_mermaid((const char *)bytes, len);
    if (YETTY_IS_ERR(r)) {
        return YETTY_ERR(yetty_ydraw_core_draw_list, r.error.msg);
    }
    return YETTY_OK(yetty_ydraw_core_draw_list, r.value);
}
