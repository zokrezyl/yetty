...
static struct yetty_ycore_void_result emit_runs(struct yetty_ydraw_core_buffer *buf,
    ...
    for (uint32_t i = 0; i <= source_len; i++) {
        if (i == source_len || source[i] == '\n') {
            ...
            while (p < i) {
                ...
                struct yetty_ycore_void_result r = yetty_ydraw_core_buffer_add_text(
                                                            ...
                                                            const struct yetty_ycat_config *config)
{
    ...
    if (!color_map && err) {
        return YETTY_ERR(yetty_ydraw_core_buffer, err);
    }
    ...
    if (YETTY_IS_ERR(er)) {
        ...
        return YETTY_ERR(yetty_ydraw_core_buffer, "ts-highlight: emit_runs failed", er);
    }
    return br;
}
                                                            ...
                                                            const struct yetty_ycat_config *config)
{
    ...
    (void)grammar_name;
    (void)config;
    return YETTY_ERR(yetty_ydraw_core_buffer, "tree-sitter not compiled in");
}
