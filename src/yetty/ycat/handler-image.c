...
struct yetty_ydraw_drawable_list_result yetty_ycat_handler_image(const uint8_t *bytes, size_t len,
                                                             ...
                                                             const struct yetty_ycat_config *config)
{
    ...
    if (!bytes || len == 0) {
        return YETTY_ERR(yetty_ydraw_drawable_list, "ycat image: no bytes");
    }
