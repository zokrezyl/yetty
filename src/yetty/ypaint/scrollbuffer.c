...
static struct yetty_ycore_void_result sb_tail_reserve(struct yetty_ypaint_scrollbuffer *sb,
                                                      size_t additional)
{
...
static inline void sb_tail_write_u32(struct yetty_ypaint_scrollbuffer *sb, uint32_t v)
{
    memcpy(sb->tail_data + sb->tail_size, &v, sizeof(uint32_t));
    sb->tail_size += sizeof(uint32_t);
}

static inline void sb_tail_write_u16_pair(struct yetty_ypaint_scrollbuffer *sb, uint16_t a,
                                          uint16_t b)
{
...
static inline void sb_tail_write_bytes(struct yetty_ypaint_scrollbuffer *sb, const void *src,
                                       size_t n)
{
...
static inline void sb_tail_patch_u32(struct yetty_ypaint_scrollbuffer *sb, size_t tail_offset,
                                     uint32_t v)
{
...
static struct yetty_ycore_void_result sb_seal_tail(struct yetty_ypaint_scrollbuffer *sb)
{
    if (sb->tail_size == 0) {
        return YETTY_OK_VOID();
    }
    ...
    if (sb->chunks_count == sb->chunks_capacity) {
        ...
        struct yetty_ypaint_scrollbuffer_chunk *grown =
    ...
    struct yetty_ypaint_scrollbuffer_chunk *ch = &sb->chunks[sb->chunks_count++];
...
static int32_t sb_find_chunk(const struct yetty_ypaint_scrollbuffer *sb, size_t logical_offset)
{
    ...
    while (lo < hi) {
        ...
        const struct yetty_ypaint_scrollbuffer_chunk *c = &sb->chunks[mid];
...
static const uint8_t *sb_load_chunk(struct yetty_ypaint_scrollbuffer *sb, int32_t idx)
{
    if (sb->decode_scratch_chunk == idx && sb->decode_scratch) {
        return sb->decode_scratch;
    }
    const struct yetty_ypaint_scrollbuffer_chunk *ch = &sb->chunks[idx];
    ...
    uint32_t n_prims)
{
    ...
    if (body_empty) {
        ...
        if (count_cells_with_refs == 0) {
            ...
            return YETTY_OK(yetty_ypaint_scrollbuffer_offset, record_logical_start);
        }
    }
...
struct yetty_ypaint_scrollbuffer_offset_result yetty_ypaint_scrollbuffer_decode_line(
    const struct yetty_ypaint_scrollbuffer *sb_const,
    ...
    const struct yetty_ypaint_scrollbuffer_decode_sinks *sinks)
{
     ...
     * not mutated. */
    struct yetty_ypaint_scrollbuffer *sb = (struct yetty_ypaint_scrollbuffer *)sb_const;

    /* Resolve logical offset → base byte pointer + bound. */
    const uint8_t *base = NULL;
    size_t base_size = 0;
    size_t offset_in_base = 0;
    size_t total_logical = yetty_ypaint_scrollbuffer_logical_size(sb);
    ...
    if (byte_count == 0) {
        return YETTY_OK(yetty_ypaint_scrollbuffer_offset, sb->logical_committed +
    ...
    while (pos + 4 <= body_end) {
        ...
        if (sinks && sinks->on_cell) {
            const struct yetty_ypaint_scrollbuffer_ref *refs =
                (const struct yetty_ypaint_scrollbuffer_ref *)(base + pos);
            struct yetty_ycore_void_result r =
    ...
    if (base == sb->tail_data) {
        ...
        const struct yetty_ypaint_scrollbuffer_chunk *ch = &sb->chunks[sb->decode_scratch_chunk];
        end_logical = (size_t)ch->logical_start + body_end;
    }
    return YETTY_OK(yetty_ypaint_scrollbuffer_offset, end_logical);
}
