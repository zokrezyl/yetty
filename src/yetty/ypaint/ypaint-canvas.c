...
static struct yetty_font_font_result ypaint_canvas_materialize_blob_font(
    const struct yetty_ypaint_canvas *canvas, const uint8_t *ttf, uint32_t ttf_len,
    ...
    bool scrolling_mode, const struct yetty_context *context)
{
    ...
    struct yetty_ypaint_core_concrete_factory *yetty_yetty_yplot_factory = yetty_yetty_yetty_yplot_factory_create();
    if (!yetty_yetty_yplot_factory) {
    ...
    struct yetty_ycore_void_result yplot_reg_res =
        yetty_ypaint_complex_prim_factory_register(canvas->complex_prim_factory, yetty_yetty_yplot_factory);
    if (YETTY_IS_ERR(yplot_reg_res)) {
        yerror("ypaint_canvas: yplot registration failed: %s", yplot_reg_res.error.msg);
        yetty_yetty_yetty_yplot_factory_destroy(yetty_yetty_yplot_factory);
        yetty_ypaint_complex_prim_factory_destroy(canvas->complex_prim_factory);
        yetty_ypaint_flyweight_registry_destroy(canvas->flyweight_registry);
        free(canvas->lines.lines);
        free(canvas);
    ...
    struct yetty_font_font *font)
{
     ...
     * advance, _pad]. */
        ...
        uint32_t packed_gf =
            (glyph_index & 0xFFFF) | (((uint32_t)(ts->font_id + 1) & 0xFFFF) << 16);
