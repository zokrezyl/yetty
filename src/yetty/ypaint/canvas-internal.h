 ...
 * factory, and GPU staging buffers via `struct yetty_ypaint_canvas` —
 * the polymorphic base. Each variant owns a `struct yetty_ypaint_canvas *`
 * created via `ypaint_canvas_create(context, ops, impl=variant)` and freed
 * via `yetty_ypaint_canvas_destroy(base)` (the polymorphic destroy dispatches
 ...
 * src/yetty/ypaint/ include this header.
 */
...
struct yetty_ypaint_canvas_ops {
    ...
    struct yetty_ycore_void_result (*destroy)(struct yetty_ypaint_canvas *);
    ...
    struct yetty_ycore_void_result (*set_grid_size)(
        struct yetty_ypaint_canvas *, struct yetty_ycore_grid_size);
    ...
    struct yetty_ycore_void_result (*process_input)(
        struct yetty_ypaint_canvas *, struct yetty_yterm_osc_statemachine *);

    /* State */
    struct yetty_ycore_void_result (*clear)(struct yetty_ypaint_canvas *);
    uint32_t (*primitive_count)(const struct yetty_ypaint_canvas *);

    /* GPU staging */
    struct yetty_ycore_void_result (*rebuild_grid)(struct yetty_ypaint_canvas *);
    struct yetty_ypaint_prim_staging_result (*build_prim_staging)(
        struct yetty_ypaint_canvas *);
    uint32_t (*prim_gpu_size)(const struct yetty_ypaint_canvas *);

    /* Complex prims */
    uint32_t (*complex_prim_count)(const struct yetty_ypaint_canvas *);
    struct yetty_ypaint_core_complex_prim_instance *(*get_complex_prim)(
        const struct yetty_ypaint_canvas *, uint32_t index);
 ...
 *
 * Public users only see `yetty_ypaint_canvas_destroy` (in canvas.h) which
