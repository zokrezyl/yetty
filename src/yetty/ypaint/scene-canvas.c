 ...
 *
 * See include/yetty/ypaint/scene-canvas.h for the public contract.
 *
 * Layout
 * ------
 *
...
#include <yetty/ypaint/scene-canvas.h>
...
struct yetty_ypaint_scene_canvas {
    ...
    struct yetty_ypaint_scene_entity *entities;
    uint32_t entity_capacity;
    uint32_t free_slot_head;   /* SCENE_INVALID_SLOT = empty */
};

/* Forward decls. */
static const struct yetty_ypaint_canvas_ops scene_canvas_ops;
...
static void entity_init_empty(struct yetty_ypaint_scene_entity *e, uint32_t slot)
{
    memset(e, 0, sizeof(*e));
    e->slot        = slot;
...
static struct yetty_ycore_void_result scene_grow_entities(
    struct yetty_ypaint_scene_canvas *sc, uint32_t need)
{
    ...
    while (new_cap < need) new_cap *= 2;
    struct yetty_ypaint_scene_entity *grown =
        realloc(sc->entities, new_cap * sizeof(struct yetty_ypaint_scene_entity));
    if (!grown) {
...
static uint32_t scene_alloc_slot(struct yetty_ypaint_scene_canvas *sc)
{
...
static void entity_free_storage(struct yetty_ypaint_scene_entity *e)
{
...
static void scene_release_slot(struct yetty_ypaint_scene_canvas *sc, uint32_t slot)
{
    struct yetty_ypaint_scene_entity *e = &sc->entities[slot];
...
static void entity_clear_in_place(struct yetty_ypaint_scene_canvas *sc,
                                  struct yetty_ypaint_scene_entity *e)
{
...
static void scene_free_cells(struct yetty_ypaint_scene_canvas *sc)
{
    if (!sc->cells) return;
...
{
    if (!context) {
        return YETTY_ERR(yetty_ypaint_canvas_ptr, "scene-canvas: context is NULL");
    }

    struct yetty_ypaint_scene_canvas *sc =
        calloc(1, sizeof(struct yetty_ypaint_scene_canvas));
    if (!sc) {
        return YETTY_ERR(yetty_ypaint_canvas_ptr, "scene-canvas: alloc failed");
    }
...
static struct yetty_ycore_void_result scene_destroy_impl(
    struct yetty_ypaint_canvas *base)
{
    struct yetty_ypaint_scene_canvas *sc = base->impl;
    ...
    struct yetty_ypaint_canvas *canvas)
{
    ...
    struct yetty_ypaint_scene_canvas *sc = canvas->impl;
...
static struct yetty_ycore_void_result entity_push_child(
    struct yetty_ypaint_scene_entity *parent, uint32_t child_slot)
{
    struct yetty_ycore_void_result gr =
...
static void entity_remove_child(struct yetty_ypaint_scene_entity *parent,
                                uint32_t child_slot)
{
    ...
    uint64_t external_id)
{
    if (!canvas || !canvas->impl) {
        return YETTY_ERR(yetty_ypaint_scene_entity_ptr,
    ...
    if (external_id != 0 &&
        yetty_ypaint_scene_entity_lookup(canvas, external_id) != NULL) {
        return YETTY_ERR(yetty_ypaint_scene_entity_ptr,
    ...
    return YETTY_OK(yetty_ypaint_scene_entity_ptr, e);
}
    ...
    const struct yetty_ypaint_core_prim_flyweight *fw)
{
    ...
    struct yetty_ypaint_scene_canvas *sc = canvas->impl;
    struct yetty_ypaint_canvas *base = sc->base;
...
static void entity_delete_recursive(struct yetty_ypaint_scene_canvas *sc,
                                    struct yetty_ypaint_scene_entity *e)
{
...
static uint32_t scene_primitive_count_impl(const struct yetty_ypaint_canvas *base)
{
    const struct yetty_ypaint_scene_canvas *sc = base->impl;
    if (!sc) return 0;
    uint32_t total = 0;
    for (uint32_t i = 0; i < sc->entity_capacity; i++) {
...
static struct yetty_ypaint_prim_staging_result scene_build_prim_staging_impl(
    struct yetty_ypaint_canvas *base)
{
    base->prim_staging_count = 0;
    struct yetty_ypaint_prim_staging view = {
        .data = base->prim_staging,
        .word_count = 0,
    };
    return YETTY_OK(yetty_ypaint_prim_staging, view);
}

static uint32_t scene_prim_gpu_size_impl(const struct yetty_ypaint_canvas *base)
{
    return base ? base->prim_staging_count * (uint32_t)sizeof(uint32_t) : 0;
}

static uint32_t scene_complex_prim_count_impl(const struct yetty_ypaint_canvas *base)
{
    (void)base;
    return 0;
}

static struct yetty_ypaint_core_complex_prim_instance *scene_get_complex_prim_impl(
...
static uint32_t scene_rolling_row_0_impl(struct yetty_ypaint_canvas *base)
{
    (void)base;
    return 0;
}

static uint32_t scene_live_rolling_row_0_impl(struct yetty_ypaint_canvas *base)
{
    (void)base;
    return 0;
}
