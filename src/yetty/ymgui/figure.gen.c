...
struct yetty_yclass_ptr_result yetty_ymgui_figure_class_get(void)
{
    ...
    static const struct yetty_yclass_descriptor desc = {
        ...
        .data_align = _Alignof(struct yetty_ymgui_figure),
    };
    static const struct yetty_yclass_op ops[] = {
        {"yetty_yfigure", "render", (yetty_yclass_method_id_t)yetty_yfigure_render,
         (yetty_yclass_impl_t)ymgui_figure_render_slot},
        {"yetty_yfigure", "destroy", (yetty_yclass_method_id_t)yetty_yfigure_destroy,
         (yetty_yclass_impl_t)ymgui_figure_destroy_slot},
        {"yetty_yfigure", "process_input", (yetty_yclass_method_id_t)yetty_yfigure_process_input,
         (yetty_yclass_impl_t)ymgui_figure_process_input_slot},
        {"yetty_yfigure", "process_bytes", (yetty_yclass_method_id_t)yetty_yfigure_process_bytes,
         (yetty_yclass_impl_t)ymgui_figure_process_bytes_slot},
    };
    struct yetty_yclass_ptr_result parent_class_r = yetty_yfigure_figure_class_get();
    if (YETTY_IS_ERR(parent_class_r)) {
        yerror("yetty_ymgui_figure_class_get: parent accessor failed: %s",
               parent_class_r.error.msg);
        return YETTY_ERR(yetty_yclass_ptr, "yetty_ymgui_figure_class_get: parent accessor failed",
                         parent_class_r);
    }
    struct yetty_yclass_ptr_result register_class_r = yetty_yclass_register(
        &desc, ops, sizeof(ops) / sizeof(ops[0]), parent_class_r.value, NULL, 0);
    if (YETTY_IS_ERR(register_class_r)) {
        yerror("yetty_ymgui_figure_class_get: class_register failed: %s",
               register_class_r.error.msg);
        return YETTY_ERR(yetty_yclass_ptr, "yetty_ymgui_figure_class_get: class_register failed",
                         register_class_r);
    }
...
struct yetty_ymgui_figure_ptr_result yetty_ymgui_figure_from(struct yetty_yclass_object *obj)
{
    ...
    if (YETTY_IS_ERR(class_r)) {
        return YETTY_ERR(yetty_ymgui_figure_ptr, "yetty_ymgui_figure_from: class accessor",
                         class_r);
    }
    struct yetty_yclass_void_ptr_result slice_r = yetty_yclass_object_data(obj, class_r.value);
    if (YETTY_IS_ERR(slice_r)) {
        return YETTY_ERR(yetty_ymgui_figure_ptr, "yetty_ymgui_figure_from: object_data", slice_r);
    }
    return YETTY_OK(yetty_ymgui_figure_ptr, (struct yetty_ymgui_figure *)slice_r.value);
}
...
struct yetty_yclass_object_ptr_result yetty_ymgui_figure_create(struct yetty_yclass_ctx *ctx)
{
     ...
     * response isn't silently swallowed. */
    {
        ...
        if (YETTY_IS_ERR(translate_class_r)) {
            yetty_ycore_error_print(
                stderr, "yetty_ymgui_figure_create: translate_class (degraded — will lazy-resolve)",
    ...
    if (YETTY_IS_ERR(create_call_r)) {
        return YETTY_ERR(yetty_yclass_object_ptr, "yetty_ymgui_figure_create: CREATE call failed",
                         create_call_r);
    }
    ...
    if (!proxy) {
        return YETTY_ERR(yetty_yclass_object_ptr,
                         "yetty_ymgui_figure_create: calloc(proxy) failed");
    }
...
/* ---- ymgui/figure: class name -> accessor ---------------------- */
static struct yetty_yclass_ptr_result yetty_ymgui_figure_accessor_lookup(const char *name)
{
    if (strcmp(name, "yetty_ymgui_figure") == 0) {
        return yetty_ymgui_figure_class_get();
    }
    return YETTY_OK(yetty_yclass_ptr, NULL);
}

struct yetty_ycore_void_result yetty_ymgui_figure_register_hooks(void)
{
    struct yetty_ycore_void_result add_accessor_r =
        yetty_yclass_add_accessor_lookup(yetty_ymgui_figure_accessor_lookup);
    YETTY_RETURN_IF_ERR(yetty_ycore_void, add_accessor_r,
                        "yetty_ymgui_figure_register_hooks: accessor");
...
struct yetty_ycore_void_result yetty_ymgui_register(void)
{
    ...
    {
        struct yetty_ycore_void_result hook_r = yetty_ymgui_figure_register_hooks();
        YETTY_RETURN_IF_ERR(yetty_ycore_void, hook_r, "yetty_ymgui_register: figure");
    }
    registered = true;
    return YETTY_OK_VOID();
}
