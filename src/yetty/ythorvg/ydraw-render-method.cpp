...
static float pointToSegmentDistSq(float px, float py, float x0, float y0, float x1, float y1)
{
    float dx = x1 - x0;
    float dy = y1 - y0;
...
static void flattenCubicBezier(float p0x, float p0y, float p1x, float p1y, float p2x, float p2y,
                               float p3x, float p3y, float toleranceSq, std::vector<float> &out,
                               int depth = 0)
{
    ...
    flattenCubicBezier(p0x, p0y, p01x, p01y, p012x, p012y, midx, midy, toleranceSq, out, depth + 1);
    flattenCubicBezier(midx, midy, p123x, p123y, p23x, p23y, p3x, p3y, toleranceSq, out, depth + 1);
}
...
tvg::RenderData YDrawRenderMethod::prepare(const tvg::RenderShape &rshape, tvg::RenderData data,
                                           const tvg::Matrix &transform,
                                           tvg::Array<tvg::RenderData> &clips, uint8_t opacity,
                                           tvg::RenderUpdateFlag flags, bool clipper)
{
    (void)clips;
    (void)flags;
    (void)clipper;
...
tvg::RenderData YDrawRenderMethod::prepare(tvg::RenderSurface *surface, tvg::RenderData data,
                                           const tvg::Matrix &transform,
                                           tvg::Array<tvg::RenderData> &clips, uint8_t opacity,
                                           tvg::RenderUpdateFlag flags)
{
    (void)clips;
    (void)flags;
    ...
    if (surface) {
        rd->bounds = {{0, 0}, {static_cast<int32_t>(surface->w), static_cast<int32_t>(surface->h)}};
    }
...
bool YDrawRenderMethod::renderShape(tvg::RenderData data)
{
    ...
    uint32_t strokeColor = (strokeA > 0 && rd->strokeWidth > 0)
                               ? rgbaToPackedABGR(rd->strokeR, rd->strokeG, rd->strokeB, strokeA)
                               : 0;
    ...
    if (hasGradient && tryRenderAsGradientBox(rd)) {
        return true;
    }

    if (tryRenderAsEllipse(rd)) {
        return true;
    }
    if (tryRenderAsBox(rd)) {
        return true;
    }
    if (tryRenderAsPolygon(rd)) {
        return true;
    }

    if (tryRenderAsFilledPath(rd)) {
        return true;
    }
...
tvg::RenderCompositor *YDrawRenderMethod::target(const tvg::RenderRegion &region,
                                                 tvg::ColorSpace cs, tvg::CompositionFlag flags)
{
    (void)region;
    (void)cs;
    (void)flags;
...
bool YDrawRenderMethod::beginComposite(tvg::RenderCompositor *cmp, tvg::MaskMethod method,
                                       uint8_t opacity)
{
    (void)cmp;
    (void)method;
...
bool YDrawRenderMethod::render(tvg::RenderCompositor *cmp, const tvg::RenderEffect *effect,
                               bool direct)
{
    (void)cmp;
    (void)effect;
...
{
    return {m.e11 * p.x + m.e12 * p.y + m.e13, m.e21 * p.x + m.e22 * p.y + m.e23};
}
...
{
    if (!_buffer || vertexCount < 3 || !vertices) {
        return;
    }
...
void YDrawRenderMethod::emitMeshPolygonGroup(uint32_t totalVertexCount, uint32_t contourCount,
                                             const uint32_t *contourStarts, const float *vertices,
                                             uint32_t fillColor, uint32_t strokeColor,
                                             float strokeWidth)
{
    if (!_buffer || contourCount == 0 || !contourStarts || !vertices) {
        return;
    }
...
bool YDrawRenderMethod::tryRenderAsEllipse(YDrawRenderData *rd)
{
    ...
    if (cmds[0] != tvg::PathCommand::MoveTo) {
        return false;
    }
    if (cmds[1] != tvg::PathCommand::CubicTo) {
        return false;
    }
    if (cmds[2] != tvg::PathCommand::CubicTo) {
        return false;
    }
    if (cmds[3] != tvg::PathCommand::CubicTo) {
        return false;
    }
    if (cmds[4] != tvg::PathCommand::CubicTo) {
        return false;
    }
    if (cmds[5] != tvg::PathCommand::Close) {
        return false;
    }
...
bool YDrawRenderMethod::tryRenderAsBox(YDrawRenderData *rd)
{
    ...
    bool isSimpleRect4 =
        (cmds.size() == 5 && pts.size() == 4 && cmds[0] == tvg::PathCommand::MoveTo &&
         cmds[1] == tvg::PathCommand::LineTo && cmds[2] == tvg::PathCommand::LineTo &&
         cmds[3] == tvg::PathCommand::LineTo && cmds[4] == tvg::PathCommand::Close);

    bool isSimpleRect5 =
        (cmds.size() == 6 && pts.size() == 5 && cmds[0] == tvg::PathCommand::MoveTo &&
         cmds[1] == tvg::PathCommand::LineTo && cmds[2] == tvg::PathCommand::LineTo &&
         cmds[3] == tvg::PathCommand::LineTo && cmds[4] == tvg::PathCommand::LineTo &&
         cmds[5] == tvg::PathCommand::Close);

    bool isSimpleRect = isSimpleRect4 || isSimpleRect5;

    bool isRoundedRect =
        (cmds.size() == 10 && cmds[0] == tvg::PathCommand::MoveTo &&
         cmds[1] == tvg::PathCommand::LineTo && cmds[2] == tvg::PathCommand::CubicTo &&
         cmds[3] == tvg::PathCommand::LineTo && cmds[4] == tvg::PathCommand::CubicTo &&
         cmds[5] == tvg::PathCommand::LineTo && cmds[6] == tvg::PathCommand::CubicTo &&
         cmds[7] == tvg::PathCommand::LineTo && cmds[8] == tvg::PathCommand::CubicTo &&
         cmds[9] == tvg::PathCommand::Close);

    if (!isSimpleRect && !isRoundedRect) {
        return false;
    }
    ...
    if (isSimpleRect) {
        ...
        bool twoUniqueX =
            (checkCount == 4 && std::abs(xVals[0] - xVals[1]) < eps &&
             std::abs(xVals[2] - xVals[3]) < eps && std::abs(xVals[1] - xVals[2]) > eps);
        bool twoUniqueY =
            (checkCount == 4 && std::abs(yVals[0] - yVals[1]) < eps &&
             std::abs(yVals[2] - yVals[3]) < eps && std::abs(yVals[1] - yVals[2]) > eps);
...
bool YDrawRenderMethod::tryRenderAsPolygon(YDrawRenderData *rd)
{
    ...
    for (const auto &cmd : cmds) {
        if (cmd == tvg::PathCommand::MoveTo) {
            moveCount++;
        } else if (cmd == tvg::PathCommand::CubicTo) {
            hasCurves = true;
        } else if (cmd == tvg::PathCommand::Close) {
            isClosed = true;
        }
    }

    if (hasCurves || moveCount > 1 || !isClosed) {
        return false;
    }
...
bool YDrawRenderMethod::tryRenderAsFilledPath(YDrawRenderData *rd)
{
    ...
    for (size_t i = 0; i < cmds.size() && ptIdx < pts.size(); ++i) {
        ...
        case tvg::PathCommand::CubicTo:
            if (ptIdx + 2 < pts.size() && inSubpath) {
                ...
                flattenCubicBezier(current.x, current.y, cp1.x, cp1.y, cp2.x, cp2.y, end.x, end.y,
                                   FLATTEN_TOLERANCE_SQ, allVertices);
                current = end;
            }
            break;
    ...
    if (validContourStarts.size() == 1) {
        emitMeshPolygon(totalVertexCount, allVertices.data(), fillColor, strokeColor,
                        rd->strokeWidth);
    } else {
        emitMeshPolygonGroup(totalVertexCount, static_cast<uint32_t>(validContourStarts.size()),
                             validContourStarts.data(), allVertices.data(), fillColor, strokeColor,
                             rd->strokeWidth);
    }
    return true;
}
...
void YDrawRenderMethod::renderPath(YDrawRenderData *rd)
{
    ...
    for (const auto &cmd : cmds) {
        ...
        case tvg::PathCommand::LineTo: {
            ...
            if (hasDash) {
                renderDashedSegment(current.x, current.y, next.x, next.y, strokeColor,
                                    rd->strokeWidth, rd->dashPattern, dashPos, dashIdx,
                                    dashVisible);
            } else {
        ...
        case tvg::PathCommand::CubicTo: {
            ...
            flattenCubicBezier(current.x, current.y, cp1.x, cp1.y, cp2.x, cp2.y, end.x, end.y, 1.0f,
                               flat);
...
void YDrawRenderMethod::renderDashedSegment(float x0, float y0, float x1, float y1,
                                            ...
                                            const std::vector<float> &dashPattern, float &dashPos,
                                            uint32_t &dashIdx, bool &dashVisible)
{
    float dx = x1 - x0;
    float dy = y1 - y0;
...
bool YDrawRenderMethod::extractGradientInfo(YDrawRenderData *rd, bool &isLinear, float &gx1,
                                            float &gy1, float &gx2, float &gy2, float &gcx,
                                            float &gcy, float &gr, uint32_t &color1,
                                            uint32_t &color2)
{
    if (!rd->fill) {
        return false;
...
bool YDrawRenderMethod::tryRenderAsGradientBox(YDrawRenderData *rd)
{
    ...
    if (cmds.size() < 4 || cmds.size() > 6) {
        return false;
    }
    if (cmds[0] != tvg::PathCommand::MoveTo) {
        return false;
    }
    if (cmds.back() != tvg::PathCommand::Close) {
        return false;
    }

    for (size_t i = 1; i < cmds.size() - 1; ++i) {
        if (cmds[i] != tvg::PathCommand::LineTo) {
            return false;
        }
    }
    ...
    if (!extractGradientInfo(rd, isLinear, gx1, gy1, gx2, gy2, gcx, gcy, gr, gradColor1,
                             gradColor2)) {
        return false;
    }
