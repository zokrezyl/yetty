    ...
    struct yetty_yclass_object *obj)
{
    if (!obj) {
        return YETTY_ERR(yetty_yclass_object_ptr,
                         "yetty_yterminal_figure_root_container: NULL object");
    }

    if (obj->session) {
        struct uint32_result remote_id_r = yetty_yclass_rpc_session_ensure_remote_id_by_name(
            obj->session, "yetty_yterminal_figure_root_container");
        YETTY_RETURN_IF_ERR(
            yetty_yclass_object_ptr, remote_id_r,
            "yetty_yterminal_figure_root_container: ensure_remote_id_by_name failed");
...
#pragma pack(pop)
        ...
        YETTY_RETURN_IF_ERR(yetty_yclass_object_ptr, rpc_call_r,
                            "yetty_yterminal_figure_root_container: RPC call failed");
        if (response_len < 1) {
            free(resp_buf);
            return YETTY_ERR(yetty_yclass_object_ptr,
                             "yetty_yterminal_figure_root_container: short RPC response");
        }
        if (resp_buf[0] != 0) {
            ...
            struct yetty_yclass_object_ptr_result remote_error =
                YETTY_ERR(yetty_yclass_object_ptr,
                          "yetty_yterminal_figure_root_container: remote impl returned error");
        ...
        if (response_len != 1 + sizeof(uint64_t)) {
            free(resp_buf);
            return YETTY_ERR(yetty_yclass_object_ptr,
                             "yetty_yterminal_figure_root_container: truncated RPC payload");
        }
        ...
        if (method_slot == YETTY_YCLASS_METHOD_SLOT_UNDEFINED) {
            struct yetty_yclass_method_slot_result method_slot_r = yetty_yclass_method_slot_get(
                "yetty_yterminal", (yetty_yclass_method_id_t)yetty_yterminal_figure_root_container);
            if (YETTY_IS_ERR(method_slot_r)) {
                return YETTY_ERR(yetty_yclass_object_ptr,
                                 "yetty_yterminal_figure_root_container: method_slot_get failed",
                                 method_slot_r);
            }
        ...
        YETTY_RETURN_IF_ERR(yetty_yclass_object_ptr, object_class_r,
                            "yetty_yterminal_figure_root_container: object_class failed");
        struct yetty_yclass_impl_t_result dispatch_impl_r =
            yetty_yclass_dispatch_lookup(object_class_r.value, method_slot);
        YETTY_RETURN_IF_ERR(yetty_yclass_object_ptr, dispatch_impl_r,
                            "yetty_yterminal_figure_root_container: dispatch_lookup failed");
