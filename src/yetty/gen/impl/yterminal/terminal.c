...
struct yetty_yterminal_terminal_ptr_result yetty_yterminal_terminal_from(struct yetty_yclass_object *obj)
{
    ...
    if (YETTY_IS_ERR(class_r))
        return YETTY_ERR(yetty_yterminal_terminal_ptr, "yetty_yterminal_terminal_from: class accessor", class_r);
    struct yetty_yclass_void_ptr_result slice_r =
        yetty_yclass_object_data(obj, class_r.value);
    if (YETTY_IS_ERR(slice_r))
        return YETTY_ERR(yetty_yterminal_terminal_ptr, "yetty_yterminal_terminal_from: object_data", slice_r);
...
struct yetty_yclass_object_ptr_result yetty_yterminal_terminal_to(struct yetty_yterminal_terminal *data)
{
    ...
    struct yetty_yclass_ptr_result class_r = yetty_yterminal_terminal_class_get();
    YETTY_RETURN_IF_ERR(yetty_yclass_object_ptr, class_r, "yetty_yterminal_terminal_to: class accessor");
    struct yetty_ycore_size_result offset_r =
        yetty_yclass_object_data_offset(class_r.value, class_r.value);
    YETTY_RETURN_IF_ERR(yetty_yclass_object_ptr, offset_r, "yetty_yterminal_terminal_to: data offset");
...
size_t yetty_yterminal_figure_root_container_skel(const void *body, size_t body_len,
...
#pragma pack(pop)
    ...
    if (YETTY_IS_ERR(obj_resolve_r)) {
        yetty_ycore_error_print(stderr,
            "[skel] yetty_yterminal_figure_root_container: handle_resolve", obj_resolve_r.error);
        if (resp_max < 1) {
    ...
    struct yetty_yclass_object_ptr_result call_r = yetty_yterminal_figure_root_container((struct yetty_yclass_object *)obj_resolve_r.value);
    if (resp_max < 1 + sizeof(uint64_t)) return 0;
    if (YETTY_IS_ERR(call_r)) {
        yetty_ycore_error_print(stderr, "[skel] yetty_yterminal_figure_root_container", call_r.error);
        ((uint8_t *)resp)[0] = 1;
        size_t err_bytes =
    ...
    if (call_r.value) {
        ...
        if (YETTY_IS_ERR(handle_r)) {
            yetty_ycore_error_print(stderr, "[skel] yetty_yterminal_figure_root_container: register returned object",
                                    handle_r.error);
            ((uint8_t *)resp)[0] = 1;
...
struct yetty_yclass_object_ptr_result yetty_yterminal_terminal_create(struct yetty_yclass_ctx *ctx)
{
    ...
    if (YETTY_IS_ERR(class_accessor_r))
        return YETTY_ERR(yetty_yclass_object_ptr,
                         "yetty_yterminal_terminal_create: class accessor failed", class_accessor_r);
...
struct yetty_yterminal_skel_row { const char *name; yetty_yclass_rpc_skel_fn fn; };
...
static yetty_yclass_rpc_skel_fn yetty_yterminal_skel_lookup(yetty_yclass_method_slot slot)
{
    ...
    for (size_t i = 0;
         i < sizeof(yetty_yterminal_skel_rows) / sizeof(yetty_yterminal_skel_rows[0]); ++i)
