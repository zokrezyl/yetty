...
struct yetty_yclass_ptr_result yetty_ygui_datepicker_class_get(void)
{
    ...
    if (YETTY_IS_ERR(parent_class_r)) {
        yerror("yetty_ygui_datepicker_class_get: parent accessor failed: %s",
               parent_class_r.error.msg);
        return YETTY_ERR(yetty_yclass_ptr,
                         "yetty_ygui_datepicker_class_get: parent accessor failed", parent_class_r);
    }
    struct yetty_yclass_ptr_result register_class_r = yetty_yclass_register(
        &desc, ops, sizeof(ops) / sizeof(ops[0]), parent_class_r.value, NULL, 0);
    if (YETTY_IS_ERR(register_class_r)) {
        yerror("yetty_ygui_datepicker_class_get: class_register failed: %s",
               register_class_r.error.msg);
        return YETTY_ERR(yetty_yclass_ptr, "yetty_ygui_datepicker_class_get: class_register failed",
                         register_class_r);
    }
...
struct yetty_ygui_datepicker_ptr_result yetty_ygui_datepicker_from(struct yetty_yclass_object *obj)
{
    ...
    if (YETTY_IS_ERR(class_r)) {
        return YETTY_ERR(yetty_ygui_datepicker_ptr, "yetty_ygui_datepicker_from: class accessor",
                         class_r);
    }
    struct yetty_yclass_void_ptr_result slice_r = yetty_yclass_object_data(obj, class_r.value);
    if (YETTY_IS_ERR(slice_r)) {
        return YETTY_ERR(yetty_ygui_datepicker_ptr, "yetty_ygui_datepicker_from: object_data",
                         slice_r);
    }
...
struct yetty_yclass_object_ptr_result yetty_ygui_datepicker_to(struct yetty_ygui_datepicker *data)
{
    ...
    YETTY_RETURN_IF_ERR(yetty_yclass_object_ptr, class_r,
                        "yetty_ygui_datepicker_to: class accessor");
    struct yetty_ycore_size_result offset_r =
        yetty_yclass_object_data_offset(class_r.value, class_r.value);
    YETTY_RETURN_IF_ERR(yetty_yclass_object_ptr, offset_r, "yetty_ygui_datepicker_to: data offset");
...
struct yetty_yclass_object_ptr_result yetty_ygui_datepicker_create(struct yetty_yclass_ctx *ctx)
{
    ...
    if (ctx && ctx->session) {
        return YETTY_ERR(
            ...
            "yetty_ygui_datepicker_create: remote create unsupported for a split-mode class; "
...
/* ---- ygui_datepicker: class name -> accessor (lazy) ---------------------- */

static struct yetty_yclass_ptr_result yetty_ygui_datepicker_accessor_lookup(const char *name)
{
...
/* ---- ygui_datepicker: explicit yclass-RPC hook registration ------------- */
...
struct yetty_ycore_void_result yetty_ygui_datepicker_register(void)
{
    ...
    struct yetty_ycore_void_result add_accessor_r =
        yetty_yclass_add_accessor_lookup(yetty_ygui_datepicker_accessor_lookup);
    YETTY_RETURN_IF_ERR(yetty_ycore_void, add_accessor_r,
                        "yetty_ygui_datepicker_register: add_accessor_lookup");
    registered = true;
    return YETTY_OK_VOID();
}
