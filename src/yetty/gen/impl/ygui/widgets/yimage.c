...
struct yetty_yclass_ptr_result yetty_ygui_yimage_class_get(void)
{
    ...
    if (YETTY_IS_ERR(parent_class_r)) {
        ...
        return YETTY_ERR(yetty_yclass_ptr, "yetty_ygui_yimage_class_get: parent accessor failed",
                         parent_class_r);
    }
    struct yetty_yclass_ptr_result register_class_r = yetty_yclass_register(
        &desc, ops, sizeof(ops) / sizeof(ops[0]), parent_class_r.value, NULL, 0);
    if (YETTY_IS_ERR(register_class_r)) {
        yerror("yetty_ygui_yimage_class_get: class_register failed: %s",
               register_class_r.error.msg);
        return YETTY_ERR(yetty_yclass_ptr, "yetty_ygui_yimage_class_get: class_register failed",
                         register_class_r);
    }
...
struct yetty_yclass_object_ptr_result yetty_ygui_yimage_create(struct yetty_yclass_ctx *ctx)
{
    ...
    if (YETTY_IS_ERR(ctor_r)) {
        ...
        return YETTY_ERR(yetty_yclass_object_ptr, "yetty_ygui_yimage_create: constructor failed",
                         ctor_r);
    }
    return alloc_r;
}
