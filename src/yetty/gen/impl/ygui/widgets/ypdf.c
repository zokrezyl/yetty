...
struct yetty_yclass_ptr_result yetty_ygui_ypdf_class_get(void)
{
    ...
    if (YETTY_IS_ERR(parent_class_r)) {
        ...
        return YETTY_ERR(yetty_yclass_ptr, "yetty_ygui_ypdf_class_get: parent accessor failed",
                         parent_class_r);
    }
    struct yetty_yclass_ptr_result register_class_r =
        yetty_yclass_register(&desc, NULL, 0, parent_class_r.value, NULL, 0);
    if (YETTY_IS_ERR(register_class_r)) {
        yerror("yetty_ygui_ypdf_class_get: class_register failed: %s", register_class_r.error.msg);
        return YETTY_ERR(yetty_yclass_ptr, "yetty_ygui_ypdf_class_get: class_register failed",
                         register_class_r);
    }
...
struct yetty_yclass_object_ptr_result yetty_ygui_ypdf_to(struct yetty_ygui_ypdf *data)
{
    ...
    struct yetty_yclass_ptr_result class_r = yetty_ygui_ypdf_class_get();
    YETTY_RETURN_IF_ERR(yetty_yclass_object_ptr, class_r, "yetty_ygui_ypdf_to: class accessor");
    struct yetty_ycore_size_result offset_r =
        yetty_yclass_object_data_offset(class_r.value, class_r.value);
    YETTY_RETURN_IF_ERR(yetty_yclass_object_ptr, offset_r, "yetty_ygui_ypdf_to: data offset");
...
struct yetty_yclass_object_ptr_result yetty_ygui_ypdf_create(struct yetty_yclass_ctx *ctx)
{
    ...
    if (ctx && ctx->session) {
        return YETTY_ERR(
            ...
            "yetty_ygui_ypdf_create: remote create unsupported for a split-mode class; "
...
/* ---- ygui_ypdf: class name -> accessor (lazy) ---------------------- */

static struct yetty_yclass_ptr_result yetty_ygui_ypdf_accessor_lookup(const char *name)
{
...
/* ---- ygui_ypdf: explicit yclass-RPC hook registration ------------- */
...
struct yetty_ycore_void_result yetty_ygui_ypdf_register(void)
{
    ...
    struct yetty_ycore_void_result add_accessor_r =
        yetty_yclass_add_accessor_lookup(yetty_ygui_ypdf_accessor_lookup);
    YETTY_RETURN_IF_ERR(yetty_ycore_void, add_accessor_r,
                        "yetty_ygui_ypdf_register: add_accessor_lookup");
    registered = true;
    return YETTY_OK_VOID();
}
