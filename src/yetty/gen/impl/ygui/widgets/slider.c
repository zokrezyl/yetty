...
struct yetty_yclass_ptr_result yetty_ygui_slider_class_get(void)
{
    ...
    static const struct yetty_yclass_op ops[] = {
        {"yetty_ygui", "constructor", (yetty_yclass_method_id_t)yetty_ygui_constructor,
         (yetty_yclass_impl_t)slider_constructor},
    ...
    if (YETTY_IS_ERR(parent_class_r)) {
        ...
        return YETTY_ERR(yetty_yclass_ptr, "yetty_ygui_slider_class_get: parent accessor failed",
                         parent_class_r);
    }
    struct yetty_yclass_ptr_result register_class_r = yetty_yclass_register(
        &desc, ops, sizeof(ops) / sizeof(ops[0]), parent_class_r.value, NULL, 0);
    if (YETTY_IS_ERR(register_class_r)) {
        yerror("yetty_ygui_slider_class_get: class_register failed: %s",
               register_class_r.error.msg);
        return YETTY_ERR(yetty_yclass_ptr, "yetty_ygui_slider_class_get: class_register failed",
                         register_class_r);
    }
...
struct yetty_yclass_object_ptr_result yetty_ygui_slider_create(struct yetty_yclass_ctx *ctx)
{
    ...
    if (ctx && ctx->session) {
        return YETTY_ERR(
            ...
            "yetty_ygui_slider_create: remote create unsupported for a split-mode class; "
    ...
    if (YETTY_IS_ERR(ctor_r)) {
        ...
        return YETTY_ERR(yetty_yclass_object_ptr, "yetty_ygui_slider_create: constructor failed",
                         ctor_r);
    }
    return alloc_r;
}
...
/* ---- ygui_slider: class name -> accessor (lazy) ---------------------- */

static struct yetty_yclass_ptr_result yetty_ygui_slider_accessor_lookup(const char *name)
{
...
/* ---- ygui_slider: explicit yclass-RPC hook registration ------------- */
...
struct yetty_ycore_void_result yetty_ygui_slider_register(void)
{
    ...
    struct yetty_ycore_void_result add_accessor_r =
        yetty_yclass_add_accessor_lookup(yetty_ygui_slider_accessor_lookup);
    YETTY_RETURN_IF_ERR(yetty_ycore_void, add_accessor_r,
                        "yetty_ygui_slider_register: add_accessor_lookup");
    registered = true;
    return YETTY_OK_VOID();
}
