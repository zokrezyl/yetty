...
struct yetty_yclass_ptr_result yetty_ygui_progress_class_get(void)
{
    ...
    static const struct yetty_yclass_op ops[] = {
        {"yetty_ygui", "constructor", (yetty_yclass_method_id_t)yetty_ygui_constructor,
         (yetty_yclass_impl_t)progress_constructor},
    ...
    if (YETTY_IS_ERR(parent_class_r)) {
        yerror("yetty_ygui_progress_class_get: parent accessor failed: %s",
               parent_class_r.error.msg);
        return YETTY_ERR(yetty_yclass_ptr, "yetty_ygui_progress_class_get: parent accessor failed",
                         parent_class_r);
    }
    struct yetty_yclass_ptr_result register_class_r = yetty_yclass_register(
        &desc, ops, sizeof(ops) / sizeof(ops[0]), parent_class_r.value, NULL, 0);
    if (YETTY_IS_ERR(register_class_r)) {
        yerror("yetty_ygui_progress_class_get: class_register failed: %s",
               register_class_r.error.msg);
        return YETTY_ERR(yetty_yclass_ptr, "yetty_ygui_progress_class_get: class_register failed",
                         register_class_r);
    }
...
struct yetty_ygui_progress_ptr_result yetty_ygui_progress_from(struct yetty_yclass_object *obj)
{
    ...
    if (YETTY_IS_ERR(class_r)) {
        return YETTY_ERR(yetty_ygui_progress_ptr, "yetty_ygui_progress_from: class accessor",
                         class_r);
    }
...
struct yetty_yclass_object_ptr_result yetty_ygui_progress_create(struct yetty_yclass_ctx *ctx)
{
    ...
    if (YETTY_IS_ERR(ctor_r)) {
        ...
        return YETTY_ERR(yetty_yclass_object_ptr, "yetty_ygui_progress_create: constructor failed",
                         ctor_r);
    }
    return alloc_r;
}
