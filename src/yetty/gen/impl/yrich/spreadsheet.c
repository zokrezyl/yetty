...
struct yetty_yclass_object_ptr_result yetty_yrich_spreadsheet_to(struct yetty_yrich_spreadsheet *data)
{
    if (!data)
        return YETTY_OK(yetty_yclass_object_ptr, NULL);
    struct yetty_yclass_ptr_result class_r = yetty_yrich_spreadsheet_class_get();
    YETTY_RETURN_IF_ERR(yetty_yclass_object_ptr, class_r, "yetty_yrich_spreadsheet_to: class accessor");
    struct yetty_ycore_size_result offset_r =
        yetty_yclass_object_data_offset(class_r.value, class_r.value);
    YETTY_RETURN_IF_ERR(yetty_yclass_object_ptr, offset_r, "yetty_yrich_spreadsheet_to: data offset");
    return YETTY_OK(yetty_yclass_object_ptr,
                    (struct yetty_yclass_object *)((char *)data - offset_r.value));
}
...
struct yetty_yrich_spreadsheet_skel_row { const char *name; yetty_yclass_rpc_skel_fn fn; };
...
static yetty_yclass_rpc_skel_fn yetty_yrich_spreadsheet_skel_lookup(yetty_yclass_method_slot slot)
{
    ...
    for (size_t i = 0;
         i < sizeof(yetty_yrich_spreadsheet_skel_rows) / sizeof(yetty_yrich_spreadsheet_skel_rows[0]); ++i)
