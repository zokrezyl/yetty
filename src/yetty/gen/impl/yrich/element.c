...
struct yetty_yclass_object_ptr_result yetty_yrich_element_create(struct yetty_yclass_ctx *ctx)
{
    ...
    if (YETTY_IS_ERR(ctor_r)) {
        ...
        return YETTY_ERR(yetty_yclass_object_ptr, "yetty_yrich_element_create: constructor failed",
                         ctor_r);
    }
    return alloc_r;
}
...
struct yetty_yrich_element_skel_row {
    const char *name;
...
static yetty_yclass_rpc_skel_fn yetty_yrich_element_skel_lookup(yetty_yclass_method_slot slot)
{
    ...
    for (size_t i = 0;
         i < sizeof(yetty_yrich_element_skel_rows) / sizeof(yetty_yrich_element_skel_rows[0]);
         ++i) {
