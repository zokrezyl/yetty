...
struct yetty_yterm_yrdawn_layer {
...
static const struct yetty_yterm_terminal_layer_ops *yrdawn_ops(void)
{
    static const struct yetty_yterm_terminal_layer_ops ops = {
        ...
        .is_dirty = yrdawn_is_dirty,
        .is_empty = yrdawn_is_empty,
        .on_key = yrdawn_on_key,
        .on_char = yrdawn_on_char,
    };
    return &ops;
}
...
static struct yetty_ycore_void_result emit(struct yetty_yterm_yrdawn_layer *l, int osc_code,
                                           const void *payload, size_t len)
{
    if (!l->base.emit_osc_fn) {
        ywarn("yrdawn-layer: no emit_osc_fn wired, dropping OSC %d", osc_code);
        return YETTY_OK_VOID();
    }
    struct yetty_ycore_void_result r =
        l->base.emit_osc_fn(osc_code, payload, len, l->base.emit_osc_userdata);
    YETTY_RETURN_IF_ERR(yetty_ycore_void, r, "yrdawn-layer: emit_osc_fn");
    return YETTY_OK_VOID();
}
                                                   ...
                                                   uint32_t ref)
{
    ...
    struct yrdawn_bulk_entry *e = &l->bulks[l->bulk_count++];
    e->ref = ref;
    e->next_seq = 0;
...
static void bulk_release(struct yetty_yterm_yrdawn_layer *l, uint32_t ref)
{
    for (size_t i = 0; i < l->bulk_count; ++i) {
        if (l->bulks[i].ref == ref) {
            yetty_ycore_buffer_destroy(&l->bulks[i].buf);
            l->bulks[i] = (struct yrdawn_bulk_entry){0};
            return;
        }
    }
}
                                                  ...
                                                  const uint8_t *payload, size_t len)
{
    ...
    if (hdr->chunk_size) {
        ...
        YETTY_RETURN_IF_ERR(yetty_ycore_void, w, "yrdawn-layer: BULK accum");
    }
    e->next_seq++;

    if (hdr->flags & YETTY_YRDAWN_BULK_FLAG_LAST) {
        /* Payload sits in e->buf until a CMD references it via payload_ref
         * — the dispatcher takes it via yrdawn_server_bulk_take. Mark the
...
static struct yetty_ycore_void_result handle_bye(struct yetty_yterm_yrdawn_layer *l,
                                                 const uint8_t *payload, size_t len)
{
    (void)payload;
    (void)len;
...
static struct yetty_ycore_void_result dispatch(struct yetty_yterm_yrdawn_layer *l, int code,
                                               const uint8_t *payload, size_t len)
{
    switch (code) {
    ...
    default:
        ywarn("yrdawn-layer: unknown OSC code %d", code);
        return YETTY_OK_VOID();
    }
}
    ...
    struct yetty_ywire_wire_statemachine *sm)
{
    ...
    for (;;) {
        ...
        YETTY_RETURN_IF_ERR(yetty_ycore_void, r, "yrdawn-layer: dispatch");
    ...
    struct yetty_ycore_grid_size gs, struct yetty_ycore_pixel_size cs)
{
    ...
    if (l->connected) {
        struct yetty_yrdawn_wire_input_resize msg = {
            .magic = YETTY_YRDAWN_MAGIC_INPUT_RESIZE,
            .version = YETTY_YRDAWN_WIRE_VERSION,
...
static int yrdawn_is_empty(const struct yetty_yrender_terminal_layer *self)
{
    const struct yetty_yterm_yrdawn_layer *l = (const struct yetty_yterm_yrdawn_layer *)self;
     ...
     * (yetty_yrdawn_present_frame) does. */
    return !l->has_frame;
}
...
static int yrdawn_on_key(struct yetty_yrender_terminal_layer *self, int key, int mods)
{
    ...
    struct yetty_yrdawn_wire_input_key msg = {
        ...
        .version = YETTY_YRDAWN_WIRE_VERSION,
        .total_size = (uint32_t)sizeof(msg),
        .kind = YETTY_YRDAWN_INPUT_KEY_DOWN,
        .key = key,
        .mods = mods,
        .codepoint = 0,
    };
...
static int yrdawn_on_char(struct yetty_yrender_terminal_layer *self, uint32_t cp, int mods)
{
    ...
    struct yetty_yrdawn_wire_input_key msg = {
        ...
        .version = YETTY_YRDAWN_WIRE_VERSION,
        .total_size = (uint32_t)sizeof(msg),
        .kind = YETTY_YRDAWN_INPUT_KEY_CHAR,
        .key = -1,
        .mods = mods,
        .codepoint = cp,
    };
...
/*===========================================================================
 * Handle table — backs the yrdawn_server_handle_* hooks the codegen
...
{
    return ((struct yetty_yterm_yrdawn_layer *)ctx)->instance;
}
void *yrdawn_server_get_shared_adapter(void *ctx)
{
    return ((struct yetty_yterm_yrdawn_layer *)ctx)->adapter;
}
void *yrdawn_server_get_shared_device(void *ctx)
{
    return ((struct yetty_yterm_yrdawn_layer *)ctx)->device;
}
void *yrdawn_server_get_shared_queue(void *ctx)
{
    return ((struct yetty_yterm_yrdawn_layer *)ctx)->queue;
}
...
#define YRDAWN_PLACEHOLDER_W 256u
#define YRDAWN_PLACEHOLDER_H 256u

static int build_pipeline(struct yetty_yterm_yrdawn_layer *l)
{
    const char *wgsl_src = yrdawn_layer_wgsl();
...
static int build_placeholder_texture(struct yetty_yterm_yrdawn_layer *l)
{
    WGPUTextureDescriptor td = {0};
    td.size = (WGPUExtent3D){YRDAWN_PLACEHOLDER_W, YRDAWN_PLACEHOLDER_H, 1};
    td.mipLevelCount = 1;
    td.sampleCount = 1;
    ...
    layout.bytesPerRow = YRDAWN_PLACEHOLDER_W * 4u;
    layout.rowsPerImage = YRDAWN_PLACEHOLDER_H;
    WGPUExtent3D extent = {YRDAWN_PLACEHOLDER_W, YRDAWN_PLACEHOLDER_H, 1};
    ...
    const uint8_t *pixels, size_t bytes)
{
    ...
    ydebug("yrdawn-layer: set_frame %ux%u %zu bytes -> texture updated", width, height, bytes);
    ...
    const struct yetty_context *context)
{
    ...
    if (!build_pipeline(l)) {
        release_gpu(l);
        free(l);
        return YETTY_ERR(yetty_yterm_terminal_layer, "yrdawn_layer_create: pipeline build failed");
    }
