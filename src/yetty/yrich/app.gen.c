...
struct yetty_yclass_object *yetty_yrich_app_to(struct yetty_yrich_app *data)
{
    if (!data)
        return NULL;
    struct yetty_yclass_ptr_result class_r = yetty_yrich_app_class_get();
...
struct yetty_yclass_object_ptr_result yetty_yrich_app_create(struct yetty_yclass_ctx *ctx)
{
    ...
    if (!ctx || !ctx->session) {
        ...
        if (YETTY_IS_ERR(ctor_r)) {
            ...
            return YETTY_ERR(yetty_yclass_object_ptr,
                             "yetty_yrich_app_create: constructor failed", ctor_r);
        }
        return alloc_r;
    }
     ...
     * response isn't silently swallowed. */
    {
        ...
        if (YETTY_IS_ERR(translate_class_r)) {
            yetty_ycore_error_print(stderr,
                "yetty_yrich_app_create: translate_class (degraded — will lazy-resolve)",
    ...
    if (YETTY_IS_ERR(create_call_r))
        return YETTY_ERR(yetty_yclass_object_ptr,
                         "yetty_yrich_app_create: CREATE call failed", create_call_r);
