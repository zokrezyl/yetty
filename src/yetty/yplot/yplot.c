...
{
    if (!source && len > 0) {
        return YETTY_ERR(yetty_ydraw_core_draw_list, "source is NULL");
    }
    ...
    if (YETTY_IS_ERR(pr)) {
        return YETTY_ERR(yetty_ydraw_core_draw_list, "yplot: expression parse failed", pr);
    }
    ...
    if (YETTY_IS_ERR(prog)) {
        return YETTY_ERR(yetty_ydraw_core_draw_list, "yplot: yfsvm compile failed", prog);
    }
    uint32_t bc_buf[1024];
    uint32_t bc_len = yetty_yfsvm_program_serialize(&prog.value, bc_buf, 1024);
    if (bc_len == 0) {
        return YETTY_ERR(yetty_ydraw_core_draw_list, "yplot: bytecode serialize failed");
    }
    ...
    if (!prim_buf) {
        return YETTY_ERR(yetty_ydraw_core_draw_list, "yplot: prim alloc failed");
    }
    struct yetty_ycore_size_result ser =
        yetty_yplot_uniforms_serialize(&u, &bufs, prim_buf, required);
    if (YETTY_IS_ERR(ser)) {
        free(prim_buf);
        return YETTY_ERR(yetty_ydraw_core_draw_list, "yplot: serialize failed", ser);
    }
    ...
    if (YETTY_IS_ERR(br)) {
        free(prim_buf);
        return YETTY_ERR(yetty_ydraw_core_draw_list, "yplot: ydraw buffer create failed", br);
    }
    ...
    if (YETTY_IS_ERR(idr)) {
        ...
        return YETTY_ERR(yetty_ydraw_core_draw_list, "yplot: ydraw add_prim failed", idr);
    }
