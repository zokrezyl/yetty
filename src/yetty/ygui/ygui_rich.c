 ...
 * yetty_ydraw_core_draw_list_data() / _size() — these used to be
                                                  ...
                                                  struct yetty_ygui_render_ctx *ctx)
{
    ...
    const uint8_t *src_data =
        (const uint8_t *)yetty_ydraw_core_draw_list_data(self->data.rich.buffer);
    size_t src_size = yetty_ydraw_core_draw_list_size(self->data.rich.buffer);
    ...
    while (remaining >= sizeof(uint32_t)) {
        ...
        struct yetty_ydraw_core_id_result r =
            yetty_ydraw_core_draw_list_add_prim(ctx->buffer, work, s);
        if (YETTY_IS_ERR(r)) {
            free(heap);
...
static void rich_destroy(struct yetty_ygui_widget *self)
{
    if (self->data.rich.buffer) {
        yetty_ydraw_core_draw_list_destroy(self->data.rich.buffer);
                                       ...
                                       struct yetty_ydraw_core_draw_list *buffer)
{
    ...
    if (widget->data.rich.buffer && widget->data.rich.buffer != buffer) {
        yetty_ydraw_core_draw_list_destroy(widget->data.rich.buffer);
    }
