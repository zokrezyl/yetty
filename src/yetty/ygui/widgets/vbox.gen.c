...
struct yetty_ygui_vbox_data_ptr_result yetty_ygui_vbox_data(struct yetty_ygui_object *obj)
{
    ...
    if (YETTY_IS_ERR(class_r)) {
        yerror("yetty_ygui_vbox_data: class accessor failed: %s", class_r.error.msg);
        return YETTY_ERR(yetty_ygui_vbox_data_ptr, "yetty_ygui_vbox_data: class accessor failed",
                         class_r);
    }
    ...
    return YETTY_OK(yetty_ygui_vbox_data_ptr, (struct yetty_ygui_vbox *)data_slice_r.value);
}
