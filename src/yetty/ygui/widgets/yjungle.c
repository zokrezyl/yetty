                                                     ...
                                                     struct yetty_ygui_emit_ctx *ctx)
{
    ...
    return yetty_ygui_emit_ensure_child(ctx, yetty_ygui_object_id(obj), YETTY_YFIGURE_KIND_YJUNGLE,
                                        r.min.x, r.min.y, r.max.x, r.max.y, NULL, 0);
}

struct [[clang::annotate("class@ygui:yjungle")]] [[clang::annotate("parent@ygui:widget")]]
yjungle_data {
    char _empty;
};
