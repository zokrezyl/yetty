...
struct yetty_ygui_breadcrumbs_data_ptr_result yetty_ygui_breadcrumbs_data(struct yetty_ygui_object *obj)
{
    struct yetty_yclass_ptr_result class_r = yetty_ygui_breadcrumbs_class_get();
    if (YETTY_IS_ERR(class_r)) {
        yerror("yetty_ygui_breadcrumbs_data: class accessor failed: %s", class_r.error.msg);
        return YETTY_ERR(yetty_ygui_breadcrumbs_data_ptr, "yetty_ygui_breadcrumbs_data: class accessor failed", class_r);
    }
    struct yetty_ygui_void_ptr_result data_slice_r =
        yetty_ygui_data_get_result(obj, class_r.value);
    if (YETTY_IS_ERR(data_slice_r))
        return YETTY_ERR(yetty_ygui_breadcrumbs_data_ptr, "yetty_ygui_breadcrumbs_data", data_slice_r);
    return YETTY_OK(yetty_ygui_breadcrumbs_data_ptr, (struct bc_data *)data_slice_r.value);
}
