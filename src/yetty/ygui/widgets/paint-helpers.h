...
{
    if (!text || !text[0]) return YETTY_OK_VOID();
    size_t n = strlen(text);
    struct yetty_ycore_buffer tb = {.data = (uint8_t *)text, .capacity = n, .size = n};
    return yetty_ydraw_draw_list_add_text(ctx->ygrid_draw_list, x, y, &tb, font_size, color, 0,
                                          -1, 0.0f);
}

static inline struct yetty_ycore_void_result yguix_circle(struct yetty_ygui_emit_ctx *ctx,
