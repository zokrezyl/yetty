...
static yetty_ygui_constructor_fn _yetty_ygui_button_yetty_ygui_constructor_check = button_constructor;
__attribute__((unused))
static yetty_ygui_destructor_fn _yetty_ygui_button_yetty_ygui_destructor_check = button_destructor;
__attribute__((unused))
static yetty_ygui_widget_paint_fn _yetty_ygui_button_yetty_ygui_widget_paint_check = button_paint;
...
struct yetty_yclass_ptr_result yetty_ygui_button_class_get(void)
{
    ...
    if (YETTY_IS_ERR(_parent_r))
        return YETTY_ERR(yetty_yclass_ptr, "yetty_ygui_button_class_get: parent accessor failed", _parent_r);
    struct yetty_yclass_ptr_result _mixin0_r = yetty_ygui_clickable_mixin_get();
    if (YETTY_IS_ERR(_mixin0_r))
        return YETTY_ERR(yetty_yclass_ptr, "yetty_ygui_button_class_get: mixin0 accessor failed", _mixin0_r);
    const struct yetty_yclass *mixins[] = { _mixin0_r.value };
    struct yetty_yclass_ptr_result _r =
        yetty_yclass_register(&desc, ops, sizeof(ops) / sizeof(ops[0]),
                              _parent_r.value, mixins, 1);
    if (YETTY_IS_ERR(_r))
        return YETTY_ERR(yetty_yclass_ptr, "yetty_ygui_button_class_get: class_register failed", _r);
    cls = _r.value;
    return _r;
}
