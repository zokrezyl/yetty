                                               ...
                                               float stroke_w)
{
    ...
    struct yetty_ysdf_box g = {.center_x = x + w * 0.5f,
                               ...
                               .half_height = h * 0.5f,
                               .corner_radius = 0.0f};
    return yetty_ydraw_drawable_list_add_cmd_add_box(ctx->ygrid_drawable_list, 0, 0, fill, stroke,
                                                     stroke_w, &g);
}
                                            ...
                                            struct yetty_ygui_emit_ctx *ctx)
{
    ...
    struct yetty_ycore_void_result result_131 =
        dp_frame(ctx, r.min.x, r.min.y, w, h, DP_BG, DP_BORDER, 1.0f);
    YETTY_RETURN_IF_ERR(yetty_ycore_void, result_131, "dp: frame");

    /* Header: ◂ Month Year ▸ */
    float hdr_ty = r.min.y + (DP_HEADER_H + fs) * 0.5f - 3.0f;
    struct yetty_ycore_void_result result_136 =
        yguix_text(ctx, "<", r.min.x + DP_PAD, hdr_ty, fs, DP_MUTED);
    ...
    struct yetty_ycore_void_result result_143 =
    ...
    for (int i = 0; i < 7; i++) {
        struct yetty_ycore_void_result result_152 =
            yguix_text(ctx, wd[i], grid_x + (float)i * DP_CELL_W + 6.0f, row_y + fs, fs, DP_MUTED);
    ...
    for (int day = 1; day <= dim; day++) {
        ...
        int is_sel =
            (d->sel_year == d->shown_year && d->sel_month == d->shown_month && d->sel_day == day);
        if (is_sel) {
            struct yetty_ycore_void_result result_171 =
                yguix_box(ctx, cx, cy, DP_CELL_W, DP_CELL_H, DP_ACCENT, 4.0f);
            YETTY_RETURN_IF_ERR(yetty_ycore_void, result_171, "dp: sel");
        }
        char buf[8];
        snprintf(buf, sizeof(buf), "%d", day);
        struct yetty_ycore_void_result result_177 =
            yguix_text(ctx, buf, cx + 7.0f, cy + fs + 2.0f, fs, is_sel ? DP_BG : DP_TEXT);
                                                              ...
                                                              int *day)
{
    int yy = -1, mm = -1, dd = -1;
    if (obj) {
        struct yetty_ygui_datepicker_ptr_result d_dr =
            yetty_ygui_datepicker_from((struct yetty_yclass_object *)obj);
        YETTY_RETURN_IF_ERR(yetty_ycore_void, d_dr, "yetty_ygui_datepicker_get_date: data_get");
        struct yetty_ygui_datepicker *d = d_dr.value;
        yy = d->sel_year;
        mm = d->sel_month;
        dd = d->sel_day;
