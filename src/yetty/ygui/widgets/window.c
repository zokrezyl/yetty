 ...
 * closable). */
static float window_close_x(const struct yetty_ygui_window *d, struct yetty_ycore_rectangle r)
{
    if (!d->closable) {
        return -1.0f;
    }
...
static float window_hamburger_x(const struct yetty_ygui_window *d, struct yetty_ycore_rectangle r)
{
    if (!d->menu) {
        return -1.0f;
    }
