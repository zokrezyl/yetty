...
static yetty_ygui_widget_paint_fn _yetty_ygui_splitter_yetty_ygui_widget_paint_check = paint;
...
struct yetty_yclass_ptr_result yetty_ygui_splitter_class_get(void)
{
    ...
    if (YETTY_IS_ERR(_parent_r))
        return YETTY_ERR(yetty_yclass_ptr, "yetty_ygui_splitter_class_get: parent accessor failed", _parent_r);
    struct yetty_yclass_ptr_result _r =
        yetty_yclass_register(&desc, ops, sizeof(ops) / sizeof(ops[0]),
                              _parent_r.value, NULL, 0);
    if (YETTY_IS_ERR(_r))
        return YETTY_ERR(yetty_yclass_ptr, "yetty_ygui_splitter_class_get: class_register failed", _r);
    cls = _r.value;
    return _r;
}
