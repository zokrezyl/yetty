/* ygui-stepper.c — wizard step indicator. */
#include "paint-helpers.h"
#include <yetty/ygui/primitive-widget.h>
#include <yetty/ygui/widgets/stepper.h>
#include <stdio.h>
#include <stdlib.h>

#define COLOR_LINE 0xFF474A36u
#define COLOR_DONE 0xFF92A86Bu
#define COLOR_TODO 0xFF2C261Eu
#define COLOR_TEXT 0xFFE4E5E0u
#define COLOR_NUM 0xFF14100Bu

struct stepper_data {
    char **labels;
    int n;
    int cap;
    int current;
};

static struct yetty_ycore_void_result ctor(struct yetty_ygui_object *obj)
{
    struct yetty_ycore_void_result sr = yetty_ygui_super_void(
        obj, yetty_ygui_stepper_class_get(), (yetty_ygui_method_id_t)yetty_ygui_constructor);
    YETTY_RETURN_IF_ERR(yetty_ycore_void, sr, "stepper: super");
    struct stepper_data *d = yetty_ygui_data_get(obj, yetty_ygui_stepper_class_get());
    d->labels = NULL;
    d->n = 0;
    d->cap = 0;
    d->current = 0;
    return YETTY_OK_VOID();
}

static struct yetty_ycore_void_result dtor(struct yetty_ygui_object *obj)
{
    struct stepper_data *d = yetty_ygui_data_get(obj, yetty_ygui_stepper_class_get());
    for (int i = 0; i < d->n; i++) free(d->labels[i]);
    free(d->labels);
    return yetty_ygui_super_void(obj, yetty_ygui_stepper_class_get(),
                                 (yetty_ygui_method_id_t)yetty_ygui_destructor);
}

static struct yetty_ycore_void_result paint(struct yetty_ygui_object *obj,
                                            struct yetty_ygui_emit_ctx *ctx)
{
    if (!ctx) return YETTY_ERR(yetty_ycore_void, "stepper paint: NULL ctx");
    struct stepper_data *d = yetty_ygui_data_get(obj, yetty_ygui_stepper_class_get());
    if (d->n <= 0) return YETTY_OK_VOID();
    struct yetty_ycore_rectangle r = yetty_ygui_widget_rect(obj);
    float w = r.max.x - r.min.x, h = r.max.y - r.min.y;
    if (w <= 0 || h <= 0) return YETTY_OK_VOID();
    float cr = 14.0f;
    float cy = r.min.y + cr + 2;
    float gap = (w - 2 * cr * d->n) / (float)(d->n > 1 ? d->n - 1 : 1);
    if (gap < 0) gap = 0;
    float cx = r.min.x + cr;
    /* Lines first */
    for (int i = 0; i < d->n - 1; i++) {
        float x0 = cx + (cr + gap * 0.5f) * (i == 0 ? 1 : 0) + i * (2 * cr + gap) + cr;
        (void)x0;
        float lx = r.min.x + cr + i * (2 * cr + gap) + cr;
        YETTY_RETURN_IF_ERR(yetty_ycore_void,
                            yguix_box(ctx, lx, cy - 1, 2 * cr + gap, 2,
                                      i < d->current ? COLOR_DONE : COLOR_LINE, 1),
                            "stepper: line");
    }
    for (int i = 0; i < d->n; i++) {
        uint32_t col = i <= d->current ? COLOR_DONE : COLOR_TODO;
        YETTY_RETURN_IF_ERR(yetty_ycore_void, yguix_circle(ctx, cx, cy, cr, col), "stepper: c");
        char buf[8];
        snprintf(buf, sizeof(buf), "%d", i + 1);
        YETTY_RETURN_IF_ERR(yetty_ycore_void,
                            yguix_text(ctx, buf, cx - 4, cy + 5, 13.0f, COLOR_NUM),
                            "stepper: num");
        if (d->labels[i])
            YETTY_RETURN_IF_ERR(yetty_ycore_void,
                                yguix_text(ctx, d->labels[i], cx - 24, cy + cr + 14, 12.0f,
                                           COLOR_TEXT),
                                "stepper: label");
        cx += 2 * cr + gap;
    }
    return YETTY_OK_VOID();
}

static int grow(struct stepper_data *d, int n)
{
    if (n <= d->cap) return 1;
    int c = d->cap ? d->cap * 2 : 4;
    while (c < n) c *= 2;
    char **nl = realloc(d->labels, (size_t)c * sizeof(*nl));
    if (!nl) return 0;
    d->labels = nl;
    d->cap = c;
    return 1;
}

struct yetty_ycore_void_result yetty_ygui_stepper_add_step(struct yetty_ygui_object *obj,
                                                           const char *label)
{
    if (!obj) return YETTY_ERR(yetty_ycore_void, "stepper_add_step: NULL");
    struct stepper_data *d = yetty_ygui_data_get(obj, yetty_ygui_stepper_class_get());
    if (!grow(d, d->n + 1)) return YETTY_ERR(yetty_ycore_void, "stepper_add_step: realloc");
    if (label) {
        size_t n = strlen(label);
        d->labels[d->n] = malloc(n + 1);
        if (!d->labels[d->n]) return YETTY_ERR(yetty_ycore_void, "stepper_add_step: malloc");
        memcpy(d->labels[d->n], label, n + 1);
    } else {
        d->labels[d->n] = NULL;
    }
    d->n++;
    return yetty_ygui_object_set_dirty(obj);
}

struct yetty_ycore_void_result yetty_ygui_stepper_set_current(struct yetty_ygui_object *obj,
                                                              int i)
{
    if (!obj) return YETTY_ERR(yetty_ycore_void, "stepper_set_current: NULL");
    struct stepper_data *d = yetty_ygui_data_get(obj, yetty_ygui_stepper_class_get());
    d->current = i;
    return yetty_ygui_object_set_dirty(obj);
}


static const struct yetty_ygui_op stepper_ops[] = {
    YETTY_YGUI_OP(yetty_ygui_constructor, ctor),
    YETTY_YGUI_OP(yetty_ygui_destructor, dtor),
    YETTY_YGUI_OP(yetty_ygui_widget_paint, paint),
};

static const struct yetty_ygui_class_descriptor stepper_desc = {
    .name = "yetty_ygui_stepper",
    .type = YETTY_YGUI_CLASS_TYPE_REGULAR,
    .data_size = sizeof(struct stepper_data),
};

YETTY_YGUI_DEFINE_CLASS(yetty_ygui_stepper_class_get, &stepper_desc, stepper_ops, yetty_ygui_primitive_widget_class_get(), NULL)
