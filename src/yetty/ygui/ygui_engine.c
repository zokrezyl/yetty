...
struct yetty_ycore_void_result yetty_ygui_engine_destroy(struct yetty_ygui_engine *engine)
{
    ...
    if (engine->buffer) {
        yetty_ydraw_core_buffer_destroy(engine->buffer);
    }
...
struct yetty_ycore_void_result yetty_ygui_engine_render(struct yetty_ygui_engine *engine)
{
    ...
    /* 1. Clear buffer */
    yetty_ydraw_core_buffer_clear(engine->buffer);
    ...
    yetty_ydraw_core_buffer_set_scene_bounds(engine->buffer, 0, 0, engine->width, engine->height);
    ...
    uint32_t size = (uint32_t)yetty_ydraw_core_buffer_serialize(engine->buffer, &data);
    if (size == 0 || !data) {
        return YETTY_OK_VOID();
    }
...
struct yetty_ycore_void_result yetty_ygui_engine_show(struct yetty_ygui_engine *engine)
{
    ...
    if (engine->canvas_mode == YETTY_YGUI_CANVAS_FIT && !engine->have_pixel_size) {
        ...
        yetty_ydraw_core_buffer_clear(engine->buffer);
        yetty_ydraw_core_buffer_set_scene_bounds(engine->buffer, 0, 0, 1, 1);
        const uint8_t *data = NULL;
        uint32_t size = (uint32_t)yetty_ydraw_core_buffer_serialize(engine->buffer, &data);
        if (size > 0 && data) {
