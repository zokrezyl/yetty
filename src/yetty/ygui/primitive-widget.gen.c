    ...
    struct yetty_ygui_object *obj)
{
    ...
    if (YETTY_IS_ERR(class_r)) {
        ...
        return YETTY_ERR(yetty_ygui_primitive_widget_data_ptr,
                         "yetty_ygui_primitive_widget_data: class accessor failed", class_r);
    }
    struct yetty_ygui_void_ptr_result data_slice_r = yetty_ygui_data_get_result(obj, class_r.value);
    if (YETTY_IS_ERR(data_slice_r)) {
        return YETTY_ERR(yetty_ygui_primitive_widget_data_ptr, "yetty_ygui_primitive_widget_data",
                         data_slice_r);
    }
    return YETTY_OK(yetty_ygui_primitive_widget_data_ptr,
                    (struct primitive_widget_data *)data_slice_r.value);
}
