 ...
 * buffer + depth-bump flag — see YETTY_YGUI_OLD_GROUP_MARKER_* below) that
 * the caller hands back to close_group. The sentinel
 * YETTY_YGUI_OLD_GROUP_SKIPPED is a legitimate OK value meaning "the
 * widget was clean and skipped" — close_group treats it as a no-op. */
YETTY_YRESULT_DECLARE(yetty_ygui_old_group_marker, uint32_t);
 ...
 *
 * The default-kind sibling, yetty_ygui_old_widget_open_group, is just this
...
typedef void (*ygui_widget_click_fn)(struct yetty_ygui_old_widget *widget, void *userdata);
typedef void (*ygui_widget_change_fn)(struct yetty_ygui_old_widget *widget, float value,
                                      void *userdata);
...
typedef int (*ygui_widget_on_press_fn)(struct yetty_ygui_old_widget *self, float lx, float ly,
                                       ygui_event_t *out);
typedef int (*ygui_widget_on_release_fn)(struct yetty_ygui_old_widget *self, float lx, float ly,
                                         ygui_event_t *out);
typedef int (*ygui_widget_on_drag_fn)(struct yetty_ygui_old_widget *self, float lx, float ly,
                                      ygui_event_t *out);
typedef int (*ygui_widget_on_scroll_fn)(struct yetty_ygui_old_widget *self, float dx, float dy,
                                        ygui_event_t *out);
typedef int (*ygui_widget_on_key_fn)(struct yetty_ygui_old_widget *self, uint32_t key, int mods,
                                     ygui_event_t *out);
typedef void (*ygui_widget_destroy_fn)(struct yetty_ygui_old_widget *self);
 ...
 * baseline. */
typedef float (*ygui_widget_baseline_fn)(const struct yetty_ygui_old_widget *self,
                                         const struct yetty_ygui_old_theme *theme);

struct yetty_ygui_old_widget_vtable {
...
struct yetty_ygui_old_scrollable {
    float (*get_content_h)(const struct yetty_ygui_old_widget *self);
    float (*get_viewport_h)(const struct yetty_ygui_old_widget *self);
    float (*get_scroll)(const struct yetty_ygui_old_widget *self);
    float (*get_max_scroll)(const struct yetty_ygui_old_widget *self);
    void (*scroll_to)(struct yetty_ygui_old_widget *self, float y);
};
...
struct yetty_ygui_old_widget {
     ...
     * by yetty_ygui_old_widget_set_position/set_size. Never mutated by the engine. */
    ...
    struct yetty_ygui_old_layout layout;
    ...
    struct yetty_ygui_old_widget *parent;
    struct yetty_ygui_old_widget *first_child;
    struct yetty_ygui_old_widget *last_child;
    struct yetty_ygui_old_widget *next_sibling;
    struct yetty_ygui_old_widget *prev_sibling;
    ...
    const struct yetty_ygui_old_widget_vtable *vtable;
    ...
    union {
        ...
        struct {
            ...
            struct yetty_ygui_old_widget *selected;
        } radio_group;

        struct {
            char *label;
            /* Back-pointer to the owning RADIO_GROUP. Set by
             * yetty_ygui_old_widget_radio_group_add at construction time. */
            struct yetty_ygui_old_widget *group;
        } radio;
        ...
        struct {
            ...
            struct yetty_ygui_old_widget *children_list;
        ...
        struct {
            ...
            void (*on_select)(struct yetty_ygui_old_widget *table, int row, void *userdata);
            void *on_select_userdata;
        ...
        struct {
            ...
            void (*on_scroll_change)(struct yetty_ygui_old_widget *self, float scroll_y,
        ...
        struct {
            ...
            struct yetty_ygui_old_widget **panels;
            int n_tabs;
            int capacity;
            int active;
             ...
             * into yetty_ygui_old_widget_tabbar_add_tab itself (or into a
...
typedef struct {
    struct yetty_ygui_old_widget **widgets;
    int count;
    int capacity;
} ygui_grid_cell_t;
...
struct yetty_ygui_old_engine {
    ...
    /* Widget storage */
    struct yetty_ygui_old_widget *first_widget;
    struct yetty_ygui_old_widget *last_widget;
    int widget_count;

    /* Interaction state */
    struct yetty_ygui_old_widget *hovered;
    struct yetty_ygui_old_widget *pressed;
    struct yetty_ygui_old_widget *focused;
     ...
     * pointer here in yetty_ygui_old_engine_attach. uv_state_destroy_cb is
     * the corresponding teardown hook called from engine_destroy when
     * non-NULL. */
    void *uv_state;
    void (*uv_state_destroy_cb)(struct yetty_ygui_old_engine *engine);
     ...
     * yetty_ygui_old_engine_notify in the public header for semantics. The
    ...
    struct ygui_notification {
        ...
        int severity;        /* enum yetty_ygui_old_severity */
        uint64_t created_ms; /* clock_gettime(CLOCK_MONOTONIC) */
        uint32_t ttl_ms;     /* 0 = sticky */
        struct yetty_ygui_old_widget *card;
        struct yetty_ygui_old_widget *close_btn;
    } notifications[8];
    ...
    struct yetty_ygui_old_pending_delete {
        uint32_t group_id;
        uint32_t figure_id;
    } *pending_deletes;
     ...
     * _set_statusbar). */
    struct yetty_ygui_old_widget *engine_titlebar;
    struct yetty_ygui_old_widget *engine_menubar;
    struct yetty_ygui_old_widget *engine_statusbar;
};
...
struct yetty_ycore_void_result yetty_ygui_old_render_ctx_render_box_shadow(
...
void yetty_ygui_old_internal_bring_to_front(struct yetty_ygui_old_widget *w);
...
void yetty_ygui_old_engine_notify_tick(struct yetty_ygui_old_engine *engine);
void yetty_ygui_old_engine_notify_on_resize(struct yetty_ygui_old_engine *engine);
void yetty_ygui_old_engine_notify_shutdown(struct yetty_ygui_old_engine *engine);
 ...
 * dependency; called from yetty_ygui_old_engine_create after the engine
 ...
 * the caller can call yetty_ygui_old_engine_destroy normally. */
 ...
 * yetty_ygui_old_engine_render (OSC envelope) and yetty_yui_render
