...
static struct yetty_ycore_void_result ypdf_render(struct yetty_ygui_old_widget *self,
...
static int ypdf_on_scroll(struct yetty_ygui_old_widget *self, float dx, float dy, ygui_event_t *out)
{
    (void)dx;
    ...
    yetty_ygui_old_widget_ypdf_scroll_to(self, prev - dy * speed);
...
static void ypdf_destroy(struct yetty_ygui_old_widget *self)
{
    if (self->data.ypdf.font_header) {
 ...
 * yetty_ygui_old_widget_scrollbar_bind(scrollbar, ypdf_widget).
 *===========================================================================*/

static float ypdf_scrollable_content(const struct yetty_ygui_old_widget *self)
{
    return self->data.ypdf.total_h;
}

static float ypdf_scrollable_viewport(const struct yetty_ygui_old_widget *self)
{
    return self->layout_h;
}

static float ypdf_scrollable_scroll(const struct yetty_ygui_old_widget *self)
{
    return self->data.ypdf.scroll_y;
}

static float ypdf_scrollable_max(const struct yetty_ygui_old_widget *self)
{
    return ygui_max(0.0f, self->data.ypdf.total_h - self->layout_h);
}

static void ypdf_scrollable_scroll_to(struct yetty_ygui_old_widget *self, float y)
{
    yetty_ygui_old_widget_ypdf_scroll_to(self, y);
}

static const struct yetty_ygui_old_scrollable *ypdf_scrollable_ptr(void)
{
                                                           ...
                                                           float w, float h, pdfio_file_t *pdf)
{
    ...
    struct yetty_ygui_old_widget *widget =
        yetty_ygui_old_engine_widget_alloc(engine, YETTY_YGUI_OLD_WIDGET_YPDF, id);
    if (!widget) {
        yetty_ydraw_draw_list_destroy(font_header);
        for (int i = 0; i < col.count; i++) {
            yetty_ydraw_draw_list_destroy(col.pages[i].buf);
    ...
    const uint8_t *data, size_t len)
{
    ...
    pdfio_file_t *pdf = pdfioFileOpen(path, NULL, NULL, pdfio_silent, NULL);
    struct yetty_ygui_old_widget *widget = build_widget_from_pdf(engine, id, x, y, w, h, pdf);
    yetty_yplatform_unlink(path);
    return widget;
}
...
static int is_ypdf(const struct yetty_ygui_old_widget *widget)
{
    return widget && widget->type == YETTY_YGUI_OLD_WIDGET_YPDF;
}
...
void yetty_ygui_old_widget_ypdf_scroll_by(struct yetty_ygui_old_widget *widget, float dy)
{
    ...
    yetty_ygui_old_widget_ypdf_scroll_to(widget, widget->data.ypdf.scroll_y + dy);
}
