 ...
 * embedded `struct yetty_ygui_old_layout`, then writes:
...
static int layout_is_visible(const struct yetty_ygui_old_widget *w)
{
    return w && (w->flags & YETTY_YGUI_OLD_FLAG_VISIBLE);
}

static int layout_is_in_flow(const struct yetty_ygui_old_widget *w)
{
    ...
    return layout_is_visible(w) && w->layout.position != YETTY_YGUI_OLD_POSITION_ABSOLUTE;
}
...
static const struct yetty_ygui_old_theme *layout_theme(const struct yetty_ygui_old_widget *w)
{
    return (w && w->engine) ? w->engine->theme : NULL;
}

/* Forward declarations. */
static void layout_widget(struct yetty_ygui_old_widget *w, float parent_abs_x, float parent_abs_y,
...
static void layout_manual_children(struct yetty_ygui_old_widget *parent)
{
    for (struct yetty_ygui_old_widget *c = parent->first_child; c; c = c->next_sibling) {
        if (!layout_is_visible(c)) {
            continue;
        }
        if (c->layout.position == YETTY_YGUI_OLD_POSITION_ABSOLUTE) {
...
static float flex_main_basis(const struct yetty_ygui_old_widget *c, ygui_flex_direction_t dir,
                             float parent_main_content)
{
    ...
    return (dir == YETTY_YGUI_OLD_FLEX_ROW) ? c->authored_w : c->authored_h;
}
                             ...
                             float parent_cross_content)
{
    ...
    if (dir == YETTY_YGUI_OLD_FLEX_ROW) {
...
static ygui_align_t flex_resolve_cross_align(const struct yetty_ygui_old_widget *parent,
                                             const struct yetty_ygui_old_widget *c)
{
    ygui_align_t a = c->layout.align_self;
    if (a == YETTY_YGUI_OLD_ALIGN_AUTO) {
        a = parent->layout.align_items;
    }
    if (a == YETTY_YGUI_OLD_ALIGN_AUTO) {
        a = YETTY_YGUI_OLD_ALIGN_START;
    }
    return a;
}
...
static float justify_leading(ygui_justify_t j, float free_space, int n)
{
    ...
    switch (j) {
    case YETTY_YGUI_OLD_JUSTIFY_CENTER:
        return free_space * 0.5f;
    case YETTY_YGUI_OLD_JUSTIFY_END:
        return free_space;
    case YETTY_YGUI_OLD_JUSTIFY_SPACE_AROUND:
        return n > 0 ? (free_space / (float)(2 * n)) : 0.0f;
    case YETTY_YGUI_OLD_JUSTIFY_SPACE_EVENLY:
        return n >= 0 ? (free_space / (float)(n + 1)) : 0.0f;
    case YETTY_YGUI_OLD_JUSTIFY_START:
    case YETTY_YGUI_OLD_JUSTIFY_SPACE_BETWEEN:
    default:
        return 0.0f;
    }
}
...
static float align_content_leading(ygui_align_t a, float free_cross, int n_lines)
{
    ...
    switch (a) {
    case YETTY_YGUI_OLD_ALIGN_CENTER:
        return free_cross * 0.5f;
    case YETTY_YGUI_OLD_ALIGN_END:
        return free_cross;
    case YETTY_YGUI_OLD_ALIGN_AUTO:
    case YETTY_YGUI_OLD_ALIGN_START:
    case YETTY_YGUI_OLD_ALIGN_STRETCH:
    case YETTY_YGUI_OLD_ALIGN_BASELINE:
    default:
        (void)n_lines;
        return 0.0f;
    }
...
static float item_baseline(struct yetty_ygui_old_widget *c, ygui_flex_direction_t dir,
...
static void resolve_and_place_line(struct yetty_ygui_old_widget *parent, struct flex_item *items,
                                   struct flex_line *line, ygui_flex_direction_t dir,
                                   float content_main, float main_origin, float cross_origin,
                                   float gap)
{
    int is_row = (dir == YETTY_YGUI_OLD_FLEX_ROW);
    int n = line->count;
    ...
    for (int i = 0; i < n; i++) {
        ...
        struct yetty_ygui_old_widget *c = it->child;
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (a == YETTY_YGUI_OLD_ALIGN_STRETCH) {
        ...
        float cross_offset;
        if (a == YETTY_YGUI_OLD_ALIGN_BASELINE && line->has_baseline) {
...
static int count_in_flow(const struct yetty_ygui_old_widget *parent)
{
    int n = 0;
    for (const struct yetty_ygui_old_widget *c = parent->first_child; c; c = c->next_sibling) {
...
static void layout_flex(struct yetty_ygui_old_widget *parent)
{
    ygui_flex_direction_t dir = parent->layout.direction;
    int is_row = (dir == YETTY_YGUI_OLD_FLEX_ROW);
    const struct yetty_ygui_old_theme *theme = layout_theme(parent);
    ...
    if (total_n == 0) {
        ...
        for (struct yetty_ygui_old_widget *c = parent->first_child; c; c = c->next_sibling) {
            if (layout_is_visible(c) && c->layout.position == YETTY_YGUI_OLD_POSITION_ABSOLUTE) {
                layout_absolute_child(parent, c);
            }
        }
        return;
    ...
    int do_wrap = (parent->layout.wrap == YETTY_YGUI_OLD_FLEX_WRAP);
    if (!do_wrap) {
        n_lines = 1;
    ...
    if (ac == YETTY_YGUI_OLD_ALIGN_AUTO) {
        ac = YETTY_YGUI_OLD_ALIGN_START;
    }
    if (ac == YETTY_YGUI_OLD_ALIGN_STRETCH && n_lines > 1 && free_cross > 0.0f) {
                          ...
                          float rel_x, float rel_y, float resolved_w, float resolved_h)
{
    ...
    if (w->layout.mode == YETTY_YGUI_OLD_LAYOUT_FLEX) {
        layout_flex(w);
    } else {
...
static void preflight_intrinsic_size(struct yetty_ygui_old_widget *w)
{
    ...
    if (w->type == YETTY_YGUI_OLD_WIDGET_TREE_NODE) {
        ...
        if (h <= 0.0f) {
            const struct yetty_ygui_old_theme *theme = layout_theme(w);
            h = theme ? theme->row_height : 24.0f;
            w->layout.padding_top = h;
        }

        if (w->data.tree_node.expanded && w->data.tree_node.children_list) {
            struct yetty_ygui_old_widget *kids = w->data.tree_node.children_list;
            float sum = 0.0f;
            int count = 0;
            for (struct yetty_ygui_old_widget *c = kids->first_child; c; c = c->next_sibling) {
                if (!layout_is_visible(c)) {
                    continue;
                }
                sum += c->authored_h;
    ...
    if (w->type == YETTY_YGUI_OLD_WIDGET_COLLAPSING_HEADER) {
...
struct yetty_ycore_void_result yetty_ygui_old_layout_compute_engine(struct yetty_ygui_old_engine *engine)
{
    ...
    yetty_ygui_old_internal_engine_pin_bars(engine);
