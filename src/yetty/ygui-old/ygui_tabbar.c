 ...
 *     (yetty_ygui_old_widget_tabbar_remove_tab on click). The hit-box size
 *     matches the YETTY_YGUI_OLD_TABBAR_BUTTON_SIZE constant so the window
...
static float tabbar_header_h(const struct yetty_ygui_old_widget *self)
{
...
static float tabbar_pill_width(const struct yetty_ygui_old_widget *self)
{
...
static void tabbar_newtab_rect(const struct yetty_ygui_old_widget *self, float pill_w, float *out_x,
...
static struct yetty_ycore_void_result tabbar_render(struct yetty_ygui_old_widget *self,
                                                    struct yetty_ygui_old_render_ctx *ctx)
{
    ...
    for (int pi = 0; pi < self->data.tabbar.n_tabs; pi++) {
        struct yetty_ygui_old_widget *panel = self->data.tabbar.panels[pi];
        if (panel) {
            ydebug("tabbar_render: panel[%d] id=%s vis=%d layout=(%.1f,%.1f %.1fx%.1f) "
                   "rel=(%.1f,%.1f %.1fx%.1f)",
                   pi, panel->id ? panel->id : "?",
                   (panel->flags & YETTY_YGUI_OLD_FLAG_VISIBLE) ? 1 : 0, panel->layout_x,
    ...
    for (int i = 0; i < self->data.tabbar.n_tabs; i++) {
        ...
        yetty_ygui_old_render_ctx_render_box(ctx, x, y, pw, hh, fill, TABBAR_PILL_RADIUS);
        ...
        if (label && *label) {
            ...
            yetty_ygui_old_render_ctx_render_text(ctx, label, tx, ty, text_color, fs);
        }
        ...
        yetty_ygui_old_render_ctx_render_text(ctx, "x", gx, gy, theme->text_primary, gfs);
    ...
    for (int i = 0; i < self->data.tabbar.n_tabs; i++) {
        if (lx >= x && lx < x + pw) {
            ...
            if (lx >= gx && lx < gx + cw && ly >= cy && ly < cy + ch) {
                 ...
                 * call yetty_ygui_old_widget_tabbar_remove_tab() itself
                ...
                if (self->data.tabbar.on_tab_close) {
                    ...
                    yetty_ygui_old_widget_tabbar_remove_tab(self, i);
                }
                return 1;
            }
            if (i != self->data.tabbar.active) {
                yetty_ygui_old_widget_tabbar_set_active(self, i);
            }
            return 1;
        }
        x += pw + TABBAR_PILL_GAP;
...
static void tabbar_destroy(struct yetty_ygui_old_widget *self)
{
     ...
     * yetty_ygui_old_widget_free; only the parallel index array is ours. */
...
static const struct yetty_ygui_old_widget_vtable tabbar_vtable = {
                                                   ...
                                                   float x, float y, float w, float h)
{
    ...
    t->layout.mode = YETTY_YGUI_OLD_LAYOUT_FLEX;
    t->layout.direction = YETTY_YGUI_OLD_FLEX_COLUMN;
    t->layout.align_items = YETTY_YGUI_OLD_ALIGN_STRETCH;
    t->layout.padding_top = TABBAR_DEFAULT_HEADER_H;

    yetty_ygui_old_engine_attach_widget(engine, t);
    return t;
}

static int tabbar_grow_capacity(struct yetty_ygui_old_widget *self, int need)
{
    ...
    struct yetty_ygui_old_widget **panels = (struct yetty_ygui_old_widget **)realloc(
        self->data.tabbar.panels, (size_t)cap * sizeof(struct yetty_ygui_old_widget *));
    if (!panels) {
                                                           ...
                                                           const char *label)
{
    ...
    panel->layout.mode = YETTY_YGUI_OLD_LAYOUT_FLEX;
    panel->layout.direction = YETTY_YGUI_OLD_FLEX_COLUMN;
    panel->layout.flex_grow = 1.0f;
    panel->layout.flex_shrink = 1.0f;
    panel->layout.align_items = YETTY_YGUI_OLD_ALIGN_STRETCH;
...
void yetty_ygui_old_widget_tabbar_remove_tab(struct yetty_ygui_old_widget *tabbar, int index)
{
    ...
    /* yetty_ygui_old_widget_remove unlinks the panel from its parent (us)
    ...
    struct yetty_ygui_old_widget *panel = tabbar->data.tabbar.panels[index];
    if (panel) {
        ydebug("tabbar_remove_tab: remove+free panel id=%s ptr=%p", panel->id ? panel->id : "?",
               (void *)panel);
        yetty_ygui_old_widget_remove(panel);
    }
    ...
    if (tabbar->data.tabbar.n_tabs == 0) {
        ...
        if (new_active >= 0 && tabbar->data.tabbar.panels[new_active]) {
            yetty_ygui_old_widget_set_visible(tabbar->data.tabbar.panels[new_active], 1);
        }
    } else if (index < tabbar->data.tabbar.active) {
        new_active--;
    }
...
void yetty_ygui_old_widget_tabbar_set_active(struct yetty_ygui_old_widget *tabbar, int index)
{
    ...
    if (prev >= 0 && prev < tabbar->data.tabbar.n_tabs && tabbar->data.tabbar.panels[prev]) {
        yetty_ygui_old_widget_set_visible(tabbar->data.tabbar.panels[prev], 0);
    }
    tabbar->data.tabbar.active = index;
    if (tabbar->data.tabbar.panels[index]) {
        yetty_ygui_old_widget_set_visible(tabbar->data.tabbar.panels[index], 1);
    }
