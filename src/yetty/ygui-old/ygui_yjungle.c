...
static void live_clear(struct yetty_ygui_old_widget *self)
{
...
static void live_remove(struct yetty_ygui_old_widget *self, uint32_t id)
{
    for (size_t i = 0; i < self->data.yjungle.live_count; i++) {
...
static void apply_delta(struct yetty_ygui_old_widget *self)
{
...
static void yj_drop_producer(struct yetty_ygui_old_widget *self)
{
...
static int yj_ensure_attached(struct yetty_ygui_old_widget *self, float w, float h)
{
    if (w <= 0.0f || h <= 0.0f) {
        return 0;
    }
...
static int yj_rebuild_flat(struct yetty_ygui_old_widget *self)
{
    ...
    struct yetty_ycore_void_result fr =
        yetty_ygui_old_flatten_draw_list(self->data.yjungle.flat, self->data.yjungle.acc);
    if (YETTY_IS_ERR(fr)) {
...
static struct yetty_ycore_void_result yj_render(struct yetty_ygui_old_widget *self,
                                                struct yetty_ygui_old_render_ctx *ctx)
{
    ...
    return yetty_ygui_old_internal_emit_buffer_translated(ctx, self->data.yjungle.flat, 0.0f, 0.0f);
}

static void yj_destroy(struct yetty_ygui_old_widget *self)
{
    yj_drop_producer(self);
...
static struct yetty_ycore_void_result yj_render_all(struct yetty_ygui_old_widget *self,
                                                    struct yetty_ygui_old_render_ctx *ctx)
{
    ...
    struct yetty_ygui_old_group_marker_result mr =
        yetty_ygui_old_widget_open_group_as_kind(self, ctx, YETTY_YFIGURE_KIND_YJUNGLE, yj_render);
...
static const struct yetty_ygui_old_widget_vtable *yj_vtable_ptr(void)
{
