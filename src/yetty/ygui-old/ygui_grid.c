...
static void cell_add(ygui_grid_cell_t *cell, struct yetty_ygui_old_widget *widget)
{
    if (cell->count >= cell->capacity) {
        int new_cap = cell->capacity == 0 ? 4 : cell->capacity * 2;
        struct yetty_ygui_old_widget **new_arr = (struct yetty_ygui_old_widget **)realloc(
            cell->widgets, new_cap * sizeof(struct yetty_ygui_old_widget *));
        if (!new_arr) {
            return;
        }
...
static void cell_remove(ygui_grid_cell_t *cell, struct yetty_ygui_old_widget *widget)
{
...
void yetty_ygui_old_grid_insert(ygui_spatial_grid_t *grid, struct yetty_ygui_old_widget *widget)
{
    ...
    for (struct yetty_ygui_old_widget *child = widget->first_child; child;
         child = child->next_sibling) {
        if (child->was_rendered) {
            yetty_ygui_old_grid_insert(grid, child);
        }
    }
}
...
void yetty_ygui_old_grid_remove(ygui_spatial_grid_t *grid, struct yetty_ygui_old_widget *widget)
{
    ...
    for (struct yetty_ygui_old_widget *child = widget->first_child; child;
         child = child->next_sibling) {
        yetty_ygui_old_grid_remove(grid, child);
    }
}
                                                        ...
                                                        float y)
{
    ...
    for (int i = cell->count - 1; i >= 0; i--) {
        struct yetty_ygui_old_widget *w = cell->widgets[i];
