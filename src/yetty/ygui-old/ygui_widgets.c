...
{
    struct yetty_ygui_old_widget *w =
        (struct yetty_ygui_old_widget *)calloc(1, sizeof(struct yetty_ygui_old_widget));
    if (!w) {
        yetty_ygui_old_set_error("Failed to allocate widget");
        return NULL;
    }
    ...
    w->type = type;
    w->engine = engine;
    w->flags = YETTY_YGUI_OLD_FLAG_VISIBLE;
 ...
 * against it). */
static uint32_t widget_top_level_figure_id(const struct yetty_ygui_old_widget *w)
{
    while (w && w->parent) {
        w = w->parent;
    }
...
static int widget_is_deferred_figure(const struct yetty_ygui_old_widget *w)
{
    if (!w) {
        return 0;
    }
    switch (w->type) {
    case YETTY_YGUI_OLD_WIDGET_YPLOT:
    case YETTY_YGUI_OLD_WIDGET_YIMAGE:
    case YETTY_YGUI_OLD_WIDGET_YVIDEO:
    case YETTY_YGUI_OLD_WIDGET_YZOO:
    case YETTY_YGUI_OLD_WIDGET_YJUNGLE:
        return 1;
    default:
        return 0;
    }
...
static void engine_queue_pending_figure_delete_no_recycle(struct yetty_ygui_old_engine *engine,
                                                          uint32_t id)
{
 ...
 * container. */
static void engine_queue_pending_delete(struct yetty_ygui_old_engine *engine,
                                        const struct yetty_ygui_old_widget *w)
{
    ...
    if (engine->pending_delete_count >= engine->pending_delete_cap) {
        ...
        struct yetty_ygui_old_pending_delete *grown = realloc(
            engine->pending_deletes, new_cap * sizeof(struct yetty_ygui_old_pending_delete));
        if (!grown) {
                                              ...
                                              uint32_t figure_id)
{
    ...
    for (uint32_t r = 0; r < engine->pending_delete_count; r++) {
        struct yetty_ygui_old_pending_delete e = engine->pending_deletes[r];
...
void yetty_ygui_old_internal_queue_delete_subtree_rendered(struct yetty_ygui_old_widget *w)
{
    if (!w || !w->engine) {
        return;
    }
    ...
    w->dirty = 1;
    for (struct yetty_ygui_old_widget *c = w->first_child; c; c = c->next_sibling) {
        yetty_ygui_old_internal_queue_delete_subtree_rendered(c);
    }
}
...
void yetty_ygui_old_widget_free(struct yetty_ygui_old_widget *widget)
{
    ...
    struct yetty_ygui_old_widget *child = widget->first_child;
    while (child) {
        struct yetty_ygui_old_widget *next = child->next_sibling;
        yetty_ygui_old_widget_free(child);
        child = next;
    }
...
/* YETTY_YGUI_OLD_GROUP_SKIPPED — defined in ygui_internal.h. */
 ...
 * yetty_ygui_old_widget_close_group with the returned marker.
 *
 * Returns the group marker on success, or YETTY_YGUI_OLD_GROUP_SKIPPED if
...
static int widget_subtree_has_dirty(const struct yetty_ygui_old_widget *w)
{
    if (!w || !(w->flags & YETTY_YGUI_OLD_FLAG_VISIBLE)) {
        return 0;
    }
    if (w->dirty) {
        return 1;
    }
    for (const struct yetty_ygui_old_widget *c = w->first_child; c; c = c->next_sibling) {
...
struct yetty_ygui_old_group_marker_result yetty_ygui_old_widget_open_group_as_kind(
    struct yetty_ygui_old_widget *self, struct yetty_ygui_old_render_ctx *ctx, uint32_t kind,
                                                ...
                                                struct yetty_ygui_old_render_ctx *))
{
    if (!self || !ctx || !ctx->buffer) {
        return YETTY_ERR(yetty_ygui_old_group_marker, "open_group: NULL self/ctx/buffer");
    }
    ...
    if (!ctx->force_full_redraw && !widget_subtree_has_dirty(self)) {
        return YETTY_OK(yetty_ygui_old_group_marker, YETTY_YGUI_OLD_GROUP_SKIPPED);
    }
    ...
    if (kind == YETTY_YFIGURE_KIND_YGRID && ctx->ygrid_depth > 0) {
        ...
        if (!self->dirty && !ctx->force_full_redraw) {
            ...
            marker = YETTY_YGUI_OLD_GROUP_MARKER_NO_RECORD;
            ...
            YETTY_RETURN_IF_ERR(yetty_ygui_old_group_marker, mark_res, "open_group: begin_group");
            marker = (mark_res.value & YETTY_YGUI_OLD_GROUP_MARKER_OFFSET_MASK) |
                     YETTY_YGUI_OLD_GROUP_MARKER_KIND_CMD_GRP;
            depth_bit = YETTY_YGUI_OLD_GROUP_MARKER_BUMPED_DEPTH;
        }
    } else {
        ...
        if (kind != YETTY_YFIGURE_KIND_YGRID && ctx->ygrid_depth > 0 && ctx->deferred_buf) {
            ...
            buf_bit = YETTY_YGUI_OLD_GROUP_MARKER_BUF_DEFERRED;
        }
        ...
        if (route_to_existing) {
            ...
            YETTY_RETURN_IF_ERR(yetty_ygui_old_group_marker, mark_res, "open_group: begin_record");
            marker = (mark_res.value & YETTY_YGUI_OLD_GROUP_MARKER_OFFSET_MASK) | buf_bit |
                     YETTY_YGUI_OLD_GROUP_MARKER_ROUTED;
            depth_bit = YETTY_YGUI_OLD_GROUP_MARKER_BUMPED_DEPTH;
             ...
             * bug surface. */
            yetty_ygui_old_internal_flush_chrome_deletes(self->engine, dst, figure_id);
        } else {
            ...
            YETTY_RETURN_IF_ERR(yetty_ygui_old_group_marker, mark_res,
            ...
            if (kind == YETTY_YFIGURE_KIND_YGRID) {
                ...
                if (self->engine) {
                    ...
                    for (uint32_t r = 0; r < self->engine->pending_delete_count; r++) {
                        struct yetty_ygui_old_pending_delete e =
    ...
    /* Bump depth FOR THE LIFETIME of this group — render_fn, the
     * caller's child-recursion, and the close_group call all run with
     * the bumped depth. close_group decrements it via the marker bit.
     * Only YGRID kinds nest; non-YGRID figures don't accept CMD_GROUP
     * children, so they don't change the depth gate. */
    int we_are_top_level_ygrid = (kind == YETTY_YFIGURE_KIND_YGRID && ctx->ygrid_depth == 0);
    if (depth_bit) {
        ctx->ygrid_depth++;
        marker |= depth_bit;
    }

    /* Coordinate frame for the body:
     *   render_box (and friends) emit prims at (caller_x + ctx->offset).
     *   Callers pass PARENT-LOCAL coords (self->x + dx).
     *   We want the wire to carry FIGURE-LOCAL absolute coords (=
     *   self's position relative to the enclosing top-level ygrid
    ...
    int is_routed = (marker & YETTY_YGUI_OLD_GROUP_MARKER_ROUTED) != 0;
    uint32_t forced_full_bit = 0;
    if (we_are_top_level_ygrid && !is_routed) {
        ctx->force_full_redraw++;
        forced_full_bit = YETTY_YGUI_OLD_GROUP_MARKER_FORCED_FULL;
    ...
    int self_emits_body = !((marker & YETTY_YGUI_OLD_GROUP_MARKER_NO_RECORD) ||
                            (marker & YETTY_YGUI_OLD_GROUP_MARKER_ROUTED));
    if (render_fn && self_emits_body) {
        ...
        if (YETTY_IS_ERR(r)) {
            ...
            return YETTY_ERR(yetty_ygui_old_group_marker, "open_group: render_fn", r);
        }
    }
    return YETTY_OK(yetty_ygui_old_group_marker, marker);
}

/* Convenience wrapper — most widgets ship as YGRID (the default kind for
 * widget chrome: boxes, borders, text). Producer widgets that need their
 * own kind tag on the wire (yplot/yimage/yvideo/yzoo/yjungle) call
 * open_group_as_kind directly. */
struct yetty_ygui_old_group_marker_result yetty_ygui_old_widget_open_group(
    struct yetty_ygui_old_widget *self, struct yetty_ygui_old_render_ctx *ctx,
    struct yetty_ycore_void_result (*render_fn)(struct yetty_ygui_old_widget *,
                                                struct yetty_ygui_old_render_ctx *))
{
    return yetty_ygui_old_widget_open_group_as_kind(self, ctx, YETTY_YFIGURE_KIND_YGRID, render_fn);
}

/* Close a group previously opened by yetty_ygui_old_widget_open_group.
 * NO-OP when `marker == YETTY_YGUI_OLD_GROUP_SKIPPED`. The marker's high
 * bits select which end_ helper to call and which buffer to patch;
 * see YETTY_YGUI_OLD_GROUP_MARKER_* in ygui_internal.h. */
struct yetty_ycore_void_result yetty_ygui_old_widget_close_group(struct yetty_ygui_old_widget *self,
                                                             ...
                                                             uint32_t marker)
{
    if (!ctx || !ctx->buffer || marker == YETTY_YGUI_OLD_GROUP_SKIPPED) {
    ...
    uint32_t offset = marker & YETTY_YGUI_OLD_GROUP_MARKER_OFFSET_MASK;
    int is_cmd_group = (marker & YETTY_YGUI_OLD_GROUP_MARKER_KIND_CMD_GRP) != 0;
    int is_deferred = (marker & YETTY_YGUI_OLD_GROUP_MARKER_BUF_DEFERRED) != 0;
    int bumped_depth = (marker & YETTY_YGUI_OLD_GROUP_MARKER_BUMPED_DEPTH) != 0;
    int forced_full = (marker & YETTY_YGUI_OLD_GROUP_MARKER_FORCED_FULL) != 0;
    int is_routed = (marker & YETTY_YGUI_OLD_GROUP_MARKER_ROUTED) != 0;
    int no_record = (marker & YETTY_YGUI_OLD_GROUP_MARKER_NO_RECORD) != 0;
    ...
    if (bumped_depth && ctx->ygrid_depth > 0) {
         ...
         * reset the figure origin so the next top-level open captures
         * a fresh one from its own widget. */
        if (ctx->ygrid_depth == 0) {
    ...
    YETTY_RETURN_IF_ERR(yetty_ycore_void, er, "close_group: end record");
    return YETTY_OK_VOID();
}
...
struct yetty_ycore_void_result yetty_ygui_old_widget_emit_self_in_group(
    struct yetty_ygui_old_widget *self, struct yetty_ygui_old_render_ctx *ctx,
    struct yetty_ycore_void_result (*render_fn)(struct yetty_ygui_old_widget *,
                                                struct yetty_ygui_old_render_ctx *))
{
    struct yetty_ygui_old_group_marker_result mr =
        yetty_ygui_old_widget_open_group(self, ctx, render_fn);
    YETTY_RETURN_IF_ERR(yetty_ycore_void, mr, "emit_self_in_group: open_group");
    struct yetty_ycore_void_result cr = yetty_ygui_old_widget_close_group(self, ctx, mr.value);
    YETTY_RETURN_IF_ERR(yetty_ycore_void, cr, "emit_self_in_group: close_group");
    return YETTY_OK_VOID();
}
    ...
    struct yetty_ygui_old_widget *self, struct yetty_ygui_old_render_ctx *ctx)
{
    ...
    /* Open this widget's CREATE_CHILD (top-level) or CMD_GROUP (nested)
     * record, emit my own body, recurse into children inside the same
     * scope (so they nest as CMD_GROUP within this widget's body if I'm
     * a YGRID figure), then close. */
    struct yetty_ygui_old_group_marker_result mr = yetty_ygui_old_widget_open_group(
        self, ctx, self->vtable && self->vtable->render ? self->vtable->render : NULL);
    YETTY_RETURN_IF_ERR(yetty_ycore_void, mr, "render_all_default: open_group");

    for (struct yetty_ygui_old_widget *child = self->first_child; child; child = child->next_sibling) {
        if (!(child->flags & YETTY_YGUI_OLD_FLAG_VISIBLE)) {
            continue;
        }
        ...
        if (child->vtable && child->vtable->render_all) {
            ...
            r = yetty_ygui_old_widget_render_all_default(child, ctx);
        }
        if (YETTY_IS_ERR(r)) {
            /* Bail on first child error. Close our open scope first
             * so depth + dirty bookkeeping stay consistent — that's
             * resource cleanup, allowed even on the error path. */
            struct yetty_ycore_void_result cr =
                yetty_ygui_old_widget_close_group(self, ctx, mr.value);
            if (YETTY_IS_ERR(cr)) {
                yetty_ycore_error_destroy(cr.error);
            }
            return YETTY_ERR(yetty_ycore_void, "render_all_default: child", r);
        }
    }

    struct yetty_ycore_void_result cr = yetty_ygui_old_widget_close_group(self, ctx, mr.value);
    YETTY_RETURN_IF_ERR(yetty_ycore_void, cr, "render_all_default: close_group");
    return YETTY_OK_VOID();
}
...
static void engine_push_free_figure_id(struct yetty_ygui_old_engine *engine, uint32_t figure_id)
{
    if (figure_id == 0) {
        return;
    }
...
static void engine_queue_pending_figure_delete(struct yetty_ygui_old_engine *engine,
                                               uint32_t figure_id)
{
...
static void add_to_engine(struct yetty_ygui_old_engine *engine, struct yetty_ygui_old_widget *widget)
{
...
static void reassign_ids_dfs(struct yetty_ygui_old_widget *w, struct yetty_ygui_old_engine *eng)
{
 ...
 * their parent). */
void yetty_ygui_old_internal_bring_to_front(struct yetty_ygui_old_widget *widget)
{
    if (!widget || !widget->engine || widget->parent) {
        return;
    }
    struct yetty_ygui_old_engine *eng = widget->engine;
    ...
    yetty_ygui_old_internal_queue_delete_subtree_rendered(widget);
...
void yetty_ygui_old_widget_add_child(struct yetty_ygui_old_widget *parent, struct yetty_ygui_old_widget *child)
{
    if (!parent || !child) {
        return;
    }
    ...
    struct yetty_ygui_old_engine *engine = parent->engine;
...
void yetty_ygui_old_widget_remove_child(struct yetty_ygui_old_widget *parent,
                                    struct yetty_ygui_old_widget *child)
{
    ...
    if (parent->engine) {
        ...
        yetty_ygui_old_internal_queue_delete_subtree_rendered(child);
    }
}

void yetty_ygui_old_widget_remove(struct yetty_ygui_old_widget *widget)
{
    if (!widget) {
        return;
    }
    ...
    if (widget->parent) {
        yetty_ygui_old_widget_remove_child(widget->parent, widget);
    } else if (widget->engine) {
        struct yetty_ygui_old_engine *engine = widget->engine;
        if (widget->prev_sibling) {
            widget->prev_sibling->next_sibling = widget->next_sibling;
        } else {
    ...
    yetty_ygui_old_widget_free(widget);
}

struct yetty_ygui_old_widget *yetty_ygui_old_widget_parent(struct yetty_ygui_old_widget *widget)
{
    return widget ? widget->parent : NULL;
}

struct yetty_ygui_old_widget *yetty_ygui_old_widget_first_child(struct yetty_ygui_old_widget *widget)
{
    return widget ? widget->first_child : NULL;
}

struct yetty_ygui_old_widget *yetty_ygui_old_widget_next_sibling(struct yetty_ygui_old_widget *widget)
{
...
const char *yetty_ygui_old_widget_id(const struct yetty_ygui_old_widget *widget)
{
    return widget ? widget->id : NULL;
}

ygui_widget_type_t yetty_ygui_old_widget_type(const struct yetty_ygui_old_widget *widget)
{
    return widget ? widget->type : YETTY_YGUI_OLD_WIDGET_CUSTOM;
}

void yetty_ygui_old_widget_set_position(struct yetty_ygui_old_widget *widget, float x, float y)
{
    if (!widget) {
        return;
    }
...
struct pixel_coord_result yetty_ygui_old_widget_get_position(const struct yetty_ygui_old_widget *widget)
{
    if (!widget) {
...
void yetty_ygui_old_widget_set_size(struct yetty_ygui_old_widget *widget, float w, float h)
{
    if (!widget) {
        return;
    }
...
struct pixel_size_result yetty_ygui_old_widget_get_size(const struct yetty_ygui_old_widget *widget)
{
    if (!widget) {
...
struct rectangle_result yetty_ygui_old_widget_get_layout_box(const struct yetty_ygui_old_widget *widget)
{
    if (!widget) {
...
struct rectangle_result yetty_ygui_old_widget_get_content_box(const struct yetty_ygui_old_widget *widget)
{
    if (!widget) {
...
void yetty_ygui_old_widget_set_visible(struct yetty_ygui_old_widget *widget, int visible)
{
    if (!widget) {
        return;
    }
    int was_visible = (widget->flags & YETTY_YGUI_OLD_FLAG_VISIBLE) != 0;
    if (visible) {
        widget->flags |= YETTY_YGUI_OLD_FLAG_VISIBLE;
    } else {
        widget->flags &= ~YETTY_YGUI_OLD_FLAG_VISIBLE;
    }
    if (widget->engine) {
...
int yetty_ygui_old_widget_is_visible(const struct yetty_ygui_old_widget *widget)
{
    return widget ? (widget->flags & YETTY_YGUI_OLD_FLAG_VISIBLE) != 0 : 0;
}

void yetty_ygui_old_widget_set_enabled(struct yetty_ygui_old_widget *widget, int enabled)
{
    if (!widget) {
        return;
    }
    if (enabled) {
        widget->flags &= ~YETTY_YGUI_OLD_FLAG_DISABLED;
    } else {
        widget->flags |= YETTY_YGUI_OLD_FLAG_DISABLED;
    }
    if (widget->engine) {
...
int yetty_ygui_old_widget_is_enabled(const struct yetty_ygui_old_widget *widget)
{
    return widget ? (widget->flags & YETTY_YGUI_OLD_FLAG_DISABLED) == 0 : 0;
}

uint32_t yetty_ygui_old_widget_get_flags(const struct yetty_ygui_old_widget *widget)
{
    return widget ? widget->flags : 0;
}

void yetty_ygui_old_widget_set_bg_color(struct yetty_ygui_old_widget *widget, uint32_t color)
{
    if (!widget) {
        return;
    }
...
void yetty_ygui_old_widget_set_fg_color(struct yetty_ygui_old_widget *widget, uint32_t color)
{
    if (!widget) {
        return;
    }
...
void yetty_ygui_old_widget_set_accent_color(struct yetty_ygui_old_widget *widget, uint32_t color)
{
    if (!widget) {
        return;
    }
...
static void layout_widget_dirty(struct yetty_ygui_old_widget *widget)
{
...
void yetty_ygui_old_widget_set_layout_mode(struct yetty_ygui_old_widget *widget, ygui_layout_mode_t mode)
{
    if (!widget) {
        return;
    }
...
void yetty_ygui_old_widget_set_flex_direction(struct yetty_ygui_old_widget *widget,
...
void yetty_ygui_old_widget_set_justify_content(struct yetty_ygui_old_widget *widget, ygui_justify_t justify)
{
    if (!widget) {
        return;
    }
...
void yetty_ygui_old_widget_set_align_items(struct yetty_ygui_old_widget *widget, ygui_align_t align)
{
    if (!widget) {
        return;
    }
...
void yetty_ygui_old_widget_set_align_self(struct yetty_ygui_old_widget *widget, ygui_align_t align)
{
    if (!widget) {
        return;
    }
...
void yetty_ygui_old_widget_set_flex(struct yetty_ygui_old_widget *widget, float grow, float shrink,
                                float basis)
{
    if (!widget) {
        return;
...
void yetty_ygui_old_widget_set_gap(struct yetty_ygui_old_widget *widget, float gap)
{
    if (!widget) {
        return;
    }
...
void yetty_ygui_old_widget_set_padding(struct yetty_ygui_old_widget *widget, float top, float right,
...
void yetty_ygui_old_widget_set_margin(struct yetty_ygui_old_widget *widget, float top, float right,
                                  float bottom, float left)
{
    if (!widget) {
        return;
...
void yetty_ygui_old_widget_set_min_size(struct yetty_ygui_old_widget *widget, float min_w, float min_h)
{
    if (!widget) {
        return;
    }
...
void yetty_ygui_old_widget_set_max_size(struct yetty_ygui_old_widget *widget, float max_w, float max_h)
{
    if (!widget) {
        return;
    }
...
void yetty_ygui_old_widget_set_flex_wrap(struct yetty_ygui_old_widget *widget, ygui_flex_wrap_t wrap)
{
    if (!widget) {
        return;
    }
...
void yetty_ygui_old_widget_set_align_content(struct yetty_ygui_old_widget *widget, ygui_align_t align)
{
    if (!widget) {
        return;
    }
...
void yetty_ygui_old_widget_set_position_mode(struct yetty_ygui_old_widget *widget, ygui_position_t pos)
{
    if (!widget) {
        return;
    }
...
void yetty_ygui_old_widget_set_flex_basis_percent(struct yetty_ygui_old_widget *widget, float pct)
{
    if (!widget) {
        return;
    }
...
void yetty_ygui_old_widget_set_size_percent(struct yetty_ygui_old_widget *widget, float w_pct, float h_pct)
{
    if (!widget) {
        return;
    }
...
void yetty_ygui_old_widget_set_min_size_percent(struct yetty_ygui_old_widget *widget, float min_w_pct,
                                            float min_h_pct)
{
    if (!widget) {
        return;
...
void yetty_ygui_old_widget_set_max_size_percent(struct yetty_ygui_old_widget *widget, float max_w_pct,
                                            float max_h_pct)
{
    if (!widget) {
        return;
...
static struct yetty_ycore_void_result button_render(struct yetty_ygui_old_widget *self,
                                                    struct yetty_ygui_old_render_ctx *ctx)
{
    const struct yetty_ygui_old_theme *t = ctx->theme;
    int pressed = (self->flags & YETTY_YGUI_OLD_FLAG_PRESSED) != 0;
    int hovered = (self->flags & YETTY_YGUI_OLD_FLAG_HOVER) != 0;
    int focused = (self->flags & YETTY_YGUI_OLD_FLAG_FOCUSED) != 0;
    ...
    if (t->enable_gradient && !pressed) {
        ...
        yetty_ygui_old_render_ctx_render_box_linear_gradient(
        ...
        yetty_ygui_old_render_ctx_render_box(ctx, self->x, self->y + press_offset, self->w, self->h,
    ...
    if (hovered && !pressed) {
        if (focused) {
            ...
            yetty_ygui_old_render_ctx_render_box_outline(ctx, self->x - 2.0f,
                                                     self->y - 2.0f + press_offset, self->w + 4.0f,
                                                     self->h + 4.0f, self->accent_color, r, 2.0f);
        } else {
            yetty_ygui_old_render_ctx_render_box_outline(ctx, self->x, self->y + press_offset, self->w,
    ...
    if (self->data.button.label) {
        yetty_ygui_old_render_ctx_render_text(ctx, self->data.button.label, self->x + t->pad_large,
...
static int button_on_press(struct yetty_ygui_old_widget *self, float lx, float ly, ygui_event_t *out)
{
    (void)lx;
    (void)ly;
    out->widget_id = self->id;
    out->type = YETTY_YGUI_OLD_EVENT_PRESS;
    return 1;
}

static int button_on_release(struct yetty_ygui_old_widget *self, float lx, float ly, ygui_event_t *out)
{
    if (lx >= 0 && lx < self->w && ly >= 0 && ly < self->h) {
        out->widget_id = self->id;
        out->type = YETTY_YGUI_OLD_EVENT_CLICK;
        return 1;
    }
    return 0;
}

static void button_destroy(struct yetty_ygui_old_widget *self)
{
    free(self->data.button.label);
}

static float button_baseline(const struct yetty_ygui_old_widget *self,
...
struct yetty_ygui_old_widget *yetty_ygui_old_engine_button(struct yetty_ygui_old_engine *engine, const char *id,
                                                   float x, float y, float w, float h,
                                                   const char *label)
{
    struct yetty_ygui_old_widget *btn =
        yetty_ygui_old_engine_widget_alloc(engine, YETTY_YGUI_OLD_WIDGET_BUTTON, id);
    if (!btn) {
        return NULL;
    }

    yetty_ygui_old_widget_init_base(btn, x, y, w, h);
    btn->data.button.label = ygui_strdup(label);
    static const struct yetty_ygui_old_widget_vtable button_vtable = {
...
void yetty_ygui_old_widget_button_set_label(struct yetty_ygui_old_widget *widget, const char *label)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_BUTTON) {
        return;
    }
...
const char *yetty_ygui_old_widget_button_get_label(const struct yetty_ygui_old_widget *widget)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_BUTTON) {
        return NULL;
    }
...
static struct yetty_ycore_void_result label_render(struct yetty_ygui_old_widget *self,
                                                   struct yetty_ygui_old_render_ctx *ctx)
{
    if (self->data.label.text) {
        float font_size =
            self->data.label.font_size > 0 ? self->data.label.font_size : ctx->theme->font_size;
        yetty_ygui_old_render_ctx_render_text(ctx, self->data.label.text, self->x, self->y,
                                          self->fg_color, font_size);
    }
    return YETTY_OK_VOID();
}

static void label_destroy(struct yetty_ygui_old_widget *self)
{
    free(self->data.label.text);
}

static float label_baseline(const struct yetty_ygui_old_widget *self,
                            const struct yetty_ygui_old_theme *theme)
{
...
struct yetty_ygui_old_widget *yetty_ygui_old_engine_label(struct yetty_ygui_old_engine *engine, const char *id,
                                                  float x, float y, const char *text)
{
    struct yetty_ygui_old_widget *lbl =
        yetty_ygui_old_engine_widget_alloc(engine, YETTY_YGUI_OLD_WIDGET_LABEL, id);
    if (!lbl) {
        return NULL;
    }

    float h = engine->theme->row_height;
    yetty_ygui_old_widget_init_base(lbl, x, y, 100, h); /* Width is flexible */
    lbl->data.label.text = ygui_strdup(text);
    lbl->data.label.font_size = 0; /* Use theme default */
    static const struct yetty_ygui_old_widget_vtable label_vtable = {
        .render = label_render,
...
void yetty_ygui_old_widget_label_set_text(struct yetty_ygui_old_widget *widget, const char *text)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_LABEL) {
        return;
    }
...
const char *yetty_ygui_old_widget_label_get_text(const struct yetty_ygui_old_widget *widget)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_LABEL) {
        return NULL;
    }
...
void yetty_ygui_old_widget_label_set_font_size(struct yetty_ygui_old_widget *widget, float size)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_LABEL) {
        return;
    }
...
static struct yetty_ycore_void_result slider_render(struct yetty_ygui_old_widget *self,
                                                    struct yetty_ygui_old_render_ctx *ctx)
{
    const struct yetty_ygui_old_theme *t = ctx->theme;
    ...
    /* Track background */
    yetty_ygui_old_render_ctx_render_box(ctx, self->x, track_y, self->w, track_h, self->bg_color,
                                     t->radius_small);
    ...
    yetty_ygui_old_render_ctx_render_box(ctx, self->x, track_y, fill_w, track_h, self->accent_color,
                                     t->radius_small);

    /* Handle */
    float handle_w = t->scrollbar_size;
    float handle_x = self->x + fill_w - handle_w / 2;
    yetty_ygui_old_render_ctx_render_box(ctx, handle_x, self->y, handle_w, self->h, self->accent_color,
                                     handle_w / 2);
    return YETTY_OK_VOID();
}

static void slider_update_value(struct yetty_ygui_old_widget *self, float local_x)
{
...
static int slider_on_press(struct yetty_ygui_old_widget *self, float lx, float ly, ygui_event_t *out)
{
    (void)ly;
    ...
    out->widget_id = self->id;
    out->type = YETTY_YGUI_OLD_EVENT_CHANGE;
    out->data.float_value = self->data.slider.value;
    return 1;
}

static int slider_on_drag(struct yetty_ygui_old_widget *self, float lx, float ly, ygui_event_t *out)
{
    (void)ly;
    slider_update_value(self, lx);
...
static int slider_on_scroll(struct yetty_ygui_old_widget *self, float dx, float dy, ygui_event_t *out)
{
    (void)dx;
    float range = self->data.slider.max_val - self->data.slider.min_val;
...
struct yetty_ygui_old_widget *yetty_ygui_old_engine_slider(struct yetty_ygui_old_engine *engine, const char *id,
                                                   float x, float y, float w, float h,
                                                   float min_val, float max_val, float value)
{
    struct yetty_ygui_old_widget *sld =
        yetty_ygui_old_engine_widget_alloc(engine, YETTY_YGUI_OLD_WIDGET_SLIDER, id);
    if (!sld) {
        return NULL;
    }

    yetty_ygui_old_widget_init_base(sld, x, y, w, h);
    sld->data.slider.min_val = min_val;
    sld->data.slider.max_val = max_val;
    sld->data.slider.value = ygui_clamp(value, min_val, max_val);
    static const struct yetty_ygui_old_widget_vtable slider_vtable = {
...
void yetty_ygui_old_widget_slider_set_value(struct yetty_ygui_old_widget *widget, float value)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_SLIDER) {
        return;
    }
...
float yetty_ygui_old_widget_slider_get_value(const struct yetty_ygui_old_widget *widget)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_SLIDER) {
        return 0;
    }
...
void yetty_ygui_old_widget_slider_set_range(struct yetty_ygui_old_widget *widget, float min_val,
                                        float max_val)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_SLIDER) {
        return;
...
static struct yetty_ycore_void_result checkbox_render(struct yetty_ygui_old_widget *self,
                                                      struct yetty_ygui_old_render_ctx *ctx)
{
    const struct yetty_ygui_old_theme *t = ctx->theme;
    ...
    yetty_ygui_old_render_ctx_render_box(ctx, self->x, box_y, box_size, box_size, box_color,
                                     t->radius_small);
    yetty_ygui_old_render_ctx_render_box_outline(ctx, self->x, box_y, box_size, box_size, t->border,
                                             t->radius_small, 1.5f);
    ...
    if (self->data.checkbox.checked) {
        ...
        yetty_ygui_old_render_ctx_render_box(ctx, cx - s, cy - 1, s * 2, 3, self->fg_color, 1);
    }

    /* Label */
    if (self->data.checkbox.label) {
        float text_x = self->x + box_size + t->pad_medium;
        yetty_ygui_old_render_ctx_render_text(ctx, self->data.checkbox.label, text_x,
                                          self->y + t->pad_medium, self->fg_color, t->font_size);
    }
    return YETTY_OK_VOID();
}

static int checkbox_on_release(struct yetty_ygui_old_widget *self, float lx, float ly,
                               ygui_event_t *out)
{
    if (lx >= 0 && lx < self->w && ly >= 0 && ly < self->h) {
...
static void checkbox_destroy(struct yetty_ygui_old_widget *self)
{
    free(self->data.checkbox.label);
}

struct yetty_ygui_old_widget *yetty_ygui_old_engine_checkbox(struct yetty_ygui_old_engine *engine,
                                                     const char *id, float x, float y, float w,
                                                     float h, const char *label, int checked)
{
    struct yetty_ygui_old_widget *chk =
        yetty_ygui_old_engine_widget_alloc(engine, YETTY_YGUI_OLD_WIDGET_CHECKBOX, id);
    if (!chk) {
        return NULL;
    }

    yetty_ygui_old_widget_init_base(chk, x, y, w, h);
    chk->data.checkbox.label = ygui_strdup(label);
    chk->data.checkbox.checked = checked;
    static const struct yetty_ygui_old_widget_vtable checkbox_vtable = {
...
void yetty_ygui_old_widget_checkbox_set_checked(struct yetty_ygui_old_widget *widget, int checked)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_CHECKBOX) {
        return;
    }
...
int yetty_ygui_old_widget_checkbox_get_checked(const struct yetty_ygui_old_widget *widget)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_CHECKBOX) {
        return 0;
    }
...
void yetty_ygui_old_widget_checkbox_set_label(struct yetty_ygui_old_widget *widget, const char *label)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_CHECKBOX) {
        return;
    }
...
static struct yetty_ycore_void_result panel_render(struct yetty_ygui_old_widget *self,
                                                   struct yetty_ygui_old_render_ctx *ctx)
{
    const struct yetty_ygui_old_theme *t = ctx->theme;
    float radius =
    ...
    yetty_ygui_old_render_ctx_render_box_shadow(ctx, self->x, self->y, self->w, self->h, radius,
                                            t->elevation_medium, t->shadow, t->elevation_alpha);

    /* Background */
    yetty_ygui_old_render_ctx_render_box(ctx, self->x, self->y, self->w, self->h, self->bg_color,
                                     radius);
    ...
    if (content_h > scrollable_h && scrollable_h > 0) {
        ...
        /* Track */
        yetty_ygui_old_render_ctx_render_box(ctx, track_x, track_y, sb_w, track_h, t->bg_secondary,
                                         sb_w / 2);
        ...
        yetty_ygui_old_render_ctx_render_box(ctx, track_x + t->pad_small, thumb_y, sb_w - t->pad_medium,
                                         thumb_h, t->thumb_normal, (sb_w - t->pad_medium) / 2);
    }
    return YETTY_OK_VOID();
}

static struct yetty_ycore_void_result panel_render_all(struct yetty_ygui_old_widget *self,
                                                       struct yetty_ygui_old_render_ctx *ctx)
{
    ...
    /* Flat-wire emission: open this widget's CREATE_CHILD record, emit
     * own body (background + scrollbar) into it, CLOSE IT, then recurse
     * children at the ROOT envelope level. Each child becomes its own
     * top-level CREATE_CHILD at the root container — the receiver
     * preserves z-order via record order (back-to-front) and resolves
     * absolute coords from the figure rect (effective_x/y), not from
     * any parent-chain accumulation.
     *
     * Scroll/header partition: header children stay at their layout
     * coords; scrollable children get their effective_y mutated by
     * -scroll temporarily so the emitted figure rect lands at the
     * on-screen position. Both child sets recurse at root level. */
    struct yetty_ygui_old_group_marker_result mr =
        yetty_ygui_old_widget_open_group(self, ctx, panel_render);
    YETTY_RETURN_IF_ERR(yetty_ycore_void, mr, "panel_render_all: open_group");

    struct yetty_ycore_void_result first_err = YETTY_OK_VOID();

    /* Header children (no scroll). */
    for (struct yetty_ygui_old_widget *child = self->first_child; child && YETTY_IS_OK(first_err);
        ...
        if (child->y < header_h) {
            ...
            if (YETTY_IS_ERR(r)) {
                first_err = YETTY_ERR(yetty_ycore_void, "panel_render_all: header child", r);
            }
        }
    }

    /* Scrollable children — mutate stored x/y by -scroll so the figure
     * rect emitted by open_group reflects the scrolled position. */
    ...
    for (struct yetty_ygui_old_widget *child = self->first_child; child && YETTY_IS_OK(first_err);
        ...
        if (child->y >= header_h) {
            ...
            float saved_ex = child->effective_x;
            float saved_ey = child->effective_y;
            child->x = (int)(saved_x - scroll_x);
            child->y = (int)(saved_y - scroll_y);
            child->effective_x = saved_ex - scroll_x;
            child->effective_y = saved_ey - scroll_y;
            struct yetty_ycore_void_result r;
            if (child->vtable && child->vtable->render_all) {
                r = child->vtable->render_all(child, ctx);
            } else {
            ...
            child->x = (int)saved_x;
            child->y = (int)saved_y;
            child->effective_x = saved_ex;
            child->effective_y = saved_ey;
            if (YETTY_IS_ERR(r)) {
                first_err = YETTY_ERR(yetty_ycore_void, "panel_render_all: scroll child", r);
            }
        }
    }

    /* Always close — depth + dirty cleanup is required even on error. */
    struct yetty_ycore_void_result cr = yetty_ygui_old_widget_close_group(self, ctx, mr.value);
    if (YETTY_IS_ERR(cr)) {
        if (YETTY_IS_OK(first_err)) {
            first_err = YETTY_ERR(yetty_ycore_void, "panel_render_all: close_group", cr);
        } else {
            yetty_ycore_error_destroy(cr.error);
        }
    }
...
static int panel_on_scroll(struct yetty_ygui_old_widget *self, float dx, float dy, ygui_event_t *out)
{
    (void)dx;
    float scrollable_h = self->h - self->data.panel.header_h;
    ...
    out->widget_id = self->id;
    out->type = YETTY_YGUI_OLD_EVENT_SCROLL;
    out->data.scroll.x = self->data.panel.scroll_x;
    out->data.scroll.y = self->data.panel.scroll_y;
    return 1;
}

struct yetty_ygui_old_widget *yetty_ygui_old_engine_panel(struct yetty_ygui_old_engine *engine, const char *id,
                                                  float x, float y, float w, float h)
{
    struct yetty_ygui_old_widget *pnl =
        yetty_ygui_old_engine_widget_alloc(engine, YETTY_YGUI_OLD_WIDGET_PANEL, id);
    if (!pnl) {
        return NULL;
    }

    yetty_ygui_old_widget_init_base(pnl, x, y, w, h);
    ...
    static const struct yetty_ygui_old_widget_vtable panel_vtable = {
        .render = panel_render,
...
void yetty_ygui_old_widget_panel_set_scroll(struct yetty_ygui_old_widget *widget, float x, float y)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_PANEL) {
        return;
    }
...
void yetty_ygui_old_widget_panel_get_scroll(const struct yetty_ygui_old_widget *widget, float *x, float *y)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_PANEL) {
        return;
    }
...
void yetty_ygui_old_widget_panel_set_content_size(struct yetty_ygui_old_widget *widget, float w, float h)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_PANEL) {
        return;
    }
...
void yetty_ygui_old_widget_panel_set_header_height(struct yetty_ygui_old_widget *widget, float h)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_PANEL) {
        return;
    }
...
static struct yetty_ycore_void_result progress_render(struct yetty_ygui_old_widget *self,
                                                      struct yetty_ygui_old_render_ctx *ctx)
{
    const struct yetty_ygui_old_theme *t = ctx->theme;
    ...
    if (self->data.progress.indeterminate) {
        ...
        yetty_ygui_old_render_ctx_render_box(ctx, slug_x, self->y, slug_w, self->h, self->accent_color,
                                         t->radius_small);
    ...
    float fill_w = pct * self->w;
    if (fill_w > 0) {
        yetty_ygui_old_render_ctx_render_box(ctx, self->x, self->y, fill_w, self->h, self->accent_color,
                                         t->radius_small);
    }
    return YETTY_OK_VOID();
}

struct yetty_ygui_old_widget *yetty_ygui_old_engine_progress(struct yetty_ygui_old_engine *engine,
                                                     const char *id, float x, float y, float w,
                                                     float h, float value)
{
    struct yetty_ygui_old_widget *prg =
        yetty_ygui_old_engine_widget_alloc(engine, YETTY_YGUI_OLD_WIDGET_PROGRESS, id);
    if (!prg) {
        return NULL;
    }

    yetty_ygui_old_widget_init_base(prg, x, y, w, h);
    prg->data.progress.value = ygui_clamp(value, 0, 1);
    static const struct yetty_ygui_old_widget_vtable progress_vtable = {
...
void yetty_ygui_old_widget_progress_set_value(struct yetty_ygui_old_widget *widget, float value)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_PROGRESS) {
        return;
    }
...
float yetty_ygui_old_widget_progress_get_value(const struct yetty_ygui_old_widget *widget)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_PROGRESS) {
        return 0;
    }
...
static struct yetty_ycore_void_result separator_render(struct yetty_ygui_old_widget *self,
                                                       struct yetty_ygui_old_render_ctx *ctx)
{
    yetty_ygui_old_render_ctx_render_box(ctx, self->x, self->y, self->w, self->h, ctx->theme->border,
                                     0);
    return YETTY_OK_VOID();
}

struct yetty_ygui_old_widget *yetty_ygui_old_engine_separator(struct yetty_ygui_old_engine *engine,
                                                      const char *id, float x, float y, float w,
                                                      float h)
{
    struct yetty_ygui_old_widget *sep =
        yetty_ygui_old_engine_widget_alloc(engine, YETTY_YGUI_OLD_WIDGET_SEPARATOR, id);
    if (!sep) {
        return NULL;
    }

    yetty_ygui_old_widget_init_base(sep, x, y, w, h);
    static const struct yetty_ygui_old_widget_vtable separator_vtable = {
...
static struct yetty_ycore_void_result textinput_render(struct yetty_ygui_old_widget *self,
                                                       struct yetty_ygui_old_render_ctx *ctx)
{
    const struct yetty_ygui_old_theme *t = ctx->theme;
    ...
    uint32_t border_color =
        (self->flags & YETTY_YGUI_OLD_FLAG_FOCUSED) ? self->accent_color : t->border;
    yetty_ygui_old_render_ctx_render_box_outline(ctx, self->x, self->y, self->w, self->h, border_color,
                                             t->radius_small, 1.5f);
    ...
    if (display_text) {
        yetty_ygui_old_render_ctx_render_text(ctx, display_text, self->x + t->pad_large,
                                          self->y + t->pad_medium, text_color, t->font_size);
    }

    /* Cursor if focused */
    if (self->flags & YETTY_YGUI_OLD_FLAG_FOCUSED) {
        ...
        yetty_ygui_old_render_ctx_render_box(ctx, cursor_x, cursor_y, 2.0f, cursor_h,
                                         self->accent_color, 0);
    }
    return YETTY_OK_VOID();
}
...
static int textinput_on_key(struct yetty_ygui_old_widget *self, uint32_t key, int mods,
                            ygui_event_t *out)
{
     ...
     * separate CHAR event (yetty_ygui_old_engine_text_input). KEY_DOWN is
...
static void textinput_destroy(struct yetty_ygui_old_widget *self)
{
    free(self->data.textinput.text);
    free(self->data.textinput.placeholder);
}

struct yetty_ygui_old_widget *yetty_ygui_old_engine_textinput(struct yetty_ygui_old_engine *engine,
                                                      const char *id, float x, float y, float w,
                                                      float h, const char *placeholder)
{
    struct yetty_ygui_old_widget *txt =
        yetty_ygui_old_engine_widget_alloc(engine, YETTY_YGUI_OLD_WIDGET_TEXTINPUT, id);
    if (!txt) {
        return NULL;
    }

    yetty_ygui_old_widget_init_base(txt, x, y, w, h);
    txt->data.textinput.text = ygui_strdup("");
    txt->data.textinput.placeholder = ygui_strdup(placeholder);
    txt->data.textinput.cursor_pos = 0;
    static const struct yetty_ygui_old_widget_vtable textinput_vtable = {
...
void yetty_ygui_old_widget_textinput_set_text(struct yetty_ygui_old_widget *widget, const char *text)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_TEXTINPUT) {
        return;
    }
...
const char *yetty_ygui_old_widget_textinput_get_text(const struct yetty_ygui_old_widget *widget)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_TEXTINPUT) {
        return NULL;
    }
...
void yetty_ygui_old_widget_textinput_set_placeholder(struct yetty_ygui_old_widget *widget, const char *text)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_TEXTINPUT) {
        return;
    }
...
static void box_apply_theme_layout(struct yetty_ygui_old_widget *box,
                                   const struct yetty_ygui_old_theme *theme,
                                   ygui_flex_direction_t direction)
{
    box->layout.mode = YETTY_YGUI_OLD_LAYOUT_FLEX;
...
struct yetty_ygui_old_widget *yetty_ygui_old_engine_hbox(struct yetty_ygui_old_engine *engine, const char *id,
                                                 float x, float y, float w, float h)
{
    struct yetty_ygui_old_widget *hbox =
        yetty_ygui_old_engine_widget_alloc(engine, YETTY_YGUI_OLD_WIDGET_HBOX, id);
    if (!hbox) {
        return NULL;
    }
    yetty_ygui_old_widget_init_base(hbox, x, y, w, h);
    box_apply_theme_layout(hbox, engine->theme, YETTY_YGUI_OLD_FLEX_ROW);
    add_to_engine(engine, hbox);
    return hbox;
}

struct yetty_ygui_old_widget *yetty_ygui_old_engine_vbox(struct yetty_ygui_old_engine *engine, const char *id,
                                                 float x, float y, float w, float h)
{
    struct yetty_ygui_old_widget *vbox =
        yetty_ygui_old_engine_widget_alloc(engine, YETTY_YGUI_OLD_WIDGET_VBOX, id);
    if (!vbox) {
        return NULL;
    }
    yetty_ygui_old_widget_init_base(vbox, x, y, w, h);
    box_apply_theme_layout(vbox, engine->theme, YETTY_YGUI_OLD_FLEX_COLUMN);
...
static struct yetty_ycore_void_result dropdown_render(struct yetty_ygui_old_widget *self,
                                                      struct yetty_ygui_old_render_ctx *ctx)
{
    const struct yetty_ygui_old_theme *t = ctx->theme;
    ...
    /* Main button area */
    uint32_t bg = (self->flags & YETTY_YGUI_OLD_FLAG_HOVER) ? t->bg_hover : self->bg_color;
    yetty_ygui_old_render_ctx_render_box(ctx, self->x, self->y, self->w, self->h, bg, t->radius_medium);
    yetty_ygui_old_render_ctx_render_box_outline(ctx, self->x, self->y, self->w, self->h, t->border,
    ...
    if (selected_text) {
        yetty_ygui_old_render_ctx_render_text(ctx, selected_text, self->x + t->pad_large,
                                          self->y + t->pad_medium, self->fg_color, t->font_size);
    }
    ...
    if (is_open) {
        /* Up arrow */
        yetty_ygui_old_render_ctx_render_triangle(ctx, arrow_x, arrow_y + 3, arrow_x + 8, arrow_y + 3,
    ...
    if (is_open && self->data.dropdown.options) {
        ...
        /* List background */
        yetty_ygui_old_render_ctx_render_box(ctx, self->x, list_y, self->w, list_h, t->bg_surface,
                                         t->radius_medium);
        yetty_ygui_old_render_ctx_render_box_outline(ctx, self->x, list_y, self->w, list_h, t->border,
                                                 t->radius_medium, 1.0f);
        ...
        for (int i = 0; i < self->data.dropdown.option_count; i++) {
            ...
            if (i == self->data.dropdown.selected) {
                yetty_ygui_old_render_ctx_render_box(ctx, self->x + 2, opt_y + 2, self->w - 4,
                                                 item_h - 4, self->accent_color, t->radius_small);
            }
            yetty_ygui_old_render_ctx_render_text(ctx, self->data.dropdown.options[i],
...
static int dropdown_on_release(struct yetty_ygui_old_widget *self, float lx, float ly,
                               ygui_event_t *out)
{
    const struct yetty_ygui_old_theme *t = self->engine->theme;
...
static void dropdown_free_options(struct yetty_ygui_old_widget *self)
{
...
static void dropdown_copy_options(struct yetty_ygui_old_widget *self, const char **options, int count)
{
...
static void dropdown_destroy(struct yetty_ygui_old_widget *self)
{
    dropdown_free_options(self);
}

struct yetty_ygui_old_widget *yetty_ygui_old_engine_dropdown(struct yetty_ygui_old_engine *engine,
                                                     const char *id, float x, float y, float w,
                                                     float h, const char **options,
                                                     int option_count)
{
    struct yetty_ygui_old_widget *dd =
        yetty_ygui_old_engine_widget_alloc(engine, YETTY_YGUI_OLD_WIDGET_DROPDOWN, id);
    if (!dd) {
        return NULL;
    }
    yetty_ygui_old_widget_init_base(dd, x, y, w, h);
    ...
    static const struct yetty_ygui_old_widget_vtable dropdown_vtable = {
...
void yetty_ygui_old_widget_dropdown_set_options(struct yetty_ygui_old_widget *widget, const char **options,
                                            int count)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_DROPDOWN) {
        return;
    }
...
void yetty_ygui_old_widget_dropdown_set_selected(struct yetty_ygui_old_widget *widget, int index)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_DROPDOWN) {
        return;
    }
...
int yetty_ygui_old_widget_dropdown_get_selected(const struct yetty_ygui_old_widget *widget)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_DROPDOWN) {
        return 0;
    }
...
static struct yetty_ycore_void_result colorpicker_render(struct yetty_ygui_old_widget *self,
                                                         struct yetty_ygui_old_render_ctx *ctx)
{
    const struct yetty_ygui_old_theme *t = ctx->theme;
    ...
    yetty_ygui_old_render_ctx_render_box(ctx, self->x, self->y, sv_w, sv_h, hue_color, t->radius_small);

    /* SV indicator */
    float ind_x = self->x + sat * sv_w;
    float ind_y = self->y + (1 - val) * sv_h;
    yetty_ygui_old_render_ctx_render_circle(ctx, ind_x, ind_y, 6.0f, 0xFFFFFFFF);
    yetty_ygui_old_render_ctx_render_circle(ctx, ind_x, ind_y, 4.0f, 0xFF000000);

    /* Hue slider bar */
    float hue_y = self->y + sv_h + t->pad_medium;
    yetty_ygui_old_render_ctx_render_box(ctx, self->x, hue_y, sv_w, hue_bar_h, t->bg_surface,
                                     t->radius_small);

    /* Hue indicator */
    float hue_ind_x = self->x + hue * sv_w;
    yetty_ygui_old_render_ctx_render_box(ctx, hue_ind_x - 3, hue_y, 6, hue_bar_h, 0xFFFFFFFF,
                                     t->radius_small);
    ...
    yetty_ygui_old_render_ctx_render_box(ctx, preview_x, self->y, preview_w, self->h, preview_color,
                                     t->radius_medium);
    yetty_ygui_old_render_ctx_render_box_outline(ctx, preview_x, self->y, preview_w, self->h, t->border,
                                             t->radius_medium, 1.5f);
    return YETTY_OK_VOID();
}

static int colorpicker_on_press(struct yetty_ygui_old_widget *self, float lx, float ly,
                                ygui_event_t *out)
{
    const struct yetty_ygui_old_theme *t = self->engine->theme;
    float hue_bar_h = 20.0f;
...
static int colorpicker_on_drag(struct yetty_ygui_old_widget *self, float lx, float ly,
                               ygui_event_t *out)
{
    return colorpicker_on_press(self, lx, ly, out);
}

struct yetty_ygui_old_widget *yetty_ygui_old_engine_colorpicker(struct yetty_ygui_old_engine *engine,
                                                        const char *id, float x, float y, float w,
                                                        float h)
{
    struct yetty_ygui_old_widget *cp =
        yetty_ygui_old_engine_widget_alloc(engine, YETTY_YGUI_OLD_WIDGET_COLORPICKER, id);
    if (!cp) {
        return NULL;
    }
    yetty_ygui_old_widget_init_base(cp, x, y, w, h);
    cp->data.colorpicker.hue = 0;
    cp->data.colorpicker.sat = 1;
    cp->data.colorpicker.val = 1;
    cp->data.colorpicker.alpha = 1;
    static const struct yetty_ygui_old_widget_vtable colorpicker_vtable = {
...
void yetty_ygui_old_widget_colorpicker_set_color(struct yetty_ygui_old_widget *widget, float r, float g,
                                             float b, float a)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_COLORPICKER) {
        return;
    }
...
void yetty_ygui_old_widget_colorpicker_get_color(const struct yetty_ygui_old_widget *widget, float *r,
...
static float popup_title_h(const struct yetty_ygui_old_theme *t)
{
...
static struct yetty_ycore_void_result popup_render(struct yetty_ygui_old_widget *self,
                                                   struct yetty_ygui_old_render_ctx *ctx)
{
    ...
    if (self->data.popup.modal) {
        yetty_ygui_old_render_ctx_render_box(ctx, 0, 0, self->data.popup.scene_w,
    ...
    yetty_ygui_old_render_ctx_render_box(ctx, self->x, self->y, self->w, hdr_h, hdr, t->radius_large);
    const char *label = self->data.popup.label;
    if (label && label[0]) {
        yetty_ygui_old_render_ctx_render_text(ctx, label, self->x + t->pad_large,
    ...
    yetty_ygui_old_render_ctx_render_box(ctx, cb_x, cb_y, POPUP_CLOSE_BTN_SIZE, POPUP_CLOSE_BTN_SIZE,
    ...
    yetty_ygui_old_render_ctx_render_box(ctx, gx - g, gy - 1.0f, g * 2, 2.0f, self->fg_color, 1);
    yetty_ygui_old_render_ctx_render_box(ctx, gx - 1.0f, gy - g, 2.0f, g * 2, self->fg_color, 1);
    return YETTY_OK_VOID();
}

static struct yetty_ycore_void_result popup_render_all(struct yetty_ygui_old_widget *self,
                                                       struct yetty_ygui_old_render_ctx *ctx)
{
    ...
    /* Open own group, recurse children inside the same scope so they
     * nest as CMD_GROUP entities in our ygrid body, then close. */
    struct yetty_ygui_old_group_marker_result mr =
        yetty_ygui_old_widget_open_group(self, ctx, popup_render);
    YETTY_RETURN_IF_ERR(yetty_ycore_void, mr, "popup_render_all: open_group");
    ...
    for (struct yetty_ygui_old_widget *child = self->first_child; child && YETTY_IS_OK(first_err);
        ...
        if (YETTY_IS_ERR(r)) {
            first_err = YETTY_ERR(yetty_ycore_void, "popup_render_all: child", r);
        }
    }
    struct yetty_ycore_void_result cr = yetty_ygui_old_widget_close_group(self, ctx, mr.value);
    if (YETTY_IS_ERR(cr)) {
        if (YETTY_IS_OK(first_err)) {
            first_err = YETTY_ERR(yetty_ycore_void, "popup_render_all: close_group", cr);
        } else {
            yetty_ycore_error_destroy(cr.error);
        }
    }
...
static int popup_hit_close_btn(const struct yetty_ygui_old_widget *self, float lx, float ly,
                               float hdr_h)
{
...
static int popup_on_press(struct yetty_ygui_old_widget *self, float lx, float ly, ygui_event_t *out)
{
    const struct yetty_ygui_old_theme *t = self->engine ? self->engine->theme : NULL;
    ...
    if (ly >= 0 && ly < hdr_h && popup_hit_close_btn(self, lx, ly, hdr_h)) {
        int was_open = (self->flags & YETTY_YGUI_OLD_FLAG_OPEN) != 0;
...
static int popup_on_drag(struct yetty_ygui_old_widget *self, float lx, float ly, ygui_event_t *out)
{
...
static int popup_on_release(struct yetty_ygui_old_widget *self, float lx, float ly, ygui_event_t *out)
{
    (void)lx;
    (void)ly;
    (void)out;
...
static void popup_destroy(struct yetty_ygui_old_widget *self)
{
    free(self->data.popup.label);
}

struct yetty_ygui_old_widget *yetty_ygui_old_engine_popup(struct yetty_ygui_old_engine *engine, const char *id,
                                                  float x, float y, float w, float h,
                                                  const char *label)
{
    struct yetty_ygui_old_widget *p =
        yetty_ygui_old_engine_widget_alloc(engine, YETTY_YGUI_OLD_WIDGET_POPUP, id);
    if (!p) {
        return NULL;
    }
    yetty_ygui_old_widget_init_base(p, x, y, w, h);
    ...
    static const struct yetty_ygui_old_widget_vtable popup_vtable = {
        .render = popup_render,
...
void yetty_ygui_old_widget_popup_set_label(struct yetty_ygui_old_widget *widget, const char *label)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_POPUP) {
        return;
    }
...
const char *yetty_ygui_old_widget_popup_get_label(const struct yetty_ygui_old_widget *widget)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_POPUP) {
        return NULL;
    }
...
void yetty_ygui_old_widget_popup_set_modal(struct yetty_ygui_old_widget *widget, int modal)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_POPUP) {
        return;
    }
...
int yetty_ygui_old_widget_popup_is_modal(const struct yetty_ygui_old_widget *widget)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_POPUP) {
        return 0;
    }
...
void yetty_ygui_old_widget_popup_set_open(struct yetty_ygui_old_widget *widget, int open)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_POPUP) {
        return;
    }
    int was_open = (widget->flags & YETTY_YGUI_OLD_FLAG_OPEN) != 0;
    if (open) {
        widget->flags |= YETTY_YGUI_OLD_FLAG_OPEN;
        /* Float above every widget painted earlier this frame so the
         * popup is never occluded by a sibling later in the engine's
         * widget chain. */
        if (!was_open) {
            yetty_ygui_old_internal_bring_to_front(widget);
        }
    } else {
        widget->flags &= ~YETTY_YGUI_OLD_FLAG_OPEN;
    }
...
int yetty_ygui_old_widget_popup_is_open(const struct yetty_ygui_old_widget *widget)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_POPUP) {
        return 0;
    }
    return (widget->flags & YETTY_YGUI_OLD_FLAG_OPEN) ? 1 : 0;
}

void yetty_ygui_old_widget_popup_set_scene_size(struct yetty_ygui_old_widget *widget, float w, float h)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_POPUP) {
        return;
    }
...
void yetty_ygui_old_widget_popup_set_header_color(struct yetty_ygui_old_widget *widget, uint32_t color)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_POPUP) {
        return;
    }
...
static struct yetty_ycore_void_result collapsing_header_render(struct yetty_ygui_old_widget *self,
                                                               struct yetty_ygui_old_render_ctx *ctx)
{
    const struct yetty_ygui_old_theme *t = ctx->theme;
    ...
    if ((self->flags & YETTY_YGUI_OLD_FLAG_OPEN) && self->h > header_h + 0.5f) {
        float body_y = self->y + header_h;
        float body_h = self->h - header_h;
        yetty_ygui_old_render_ctx_render_box(ctx, self->x, body_y, self->w, body_h, t->bg_secondary,
                                         t->radius_medium);
        yetty_ygui_old_render_ctx_render_box_outline(ctx, self->x, body_y, self->w, body_h,
                                                 t->border_muted, t->radius_medium, 1.0f);
    }

    yetty_ygui_old_render_ctx_render_box(ctx, self->x, self->y, self->w, header_h, self->bg_color,
                                     t->radius_medium);
    ...
    if (self->flags & YETTY_YGUI_OLD_FLAG_OPEN) {
         ...
         * expanded. */
        yetty_ygui_old_render_ctx_render_triangle(ctx, arrow_x, arrow_y - arrow_size / 3.0f,
        ...
        yetty_ygui_old_render_ctx_render_triangle(ctx, arrow_x, arrow_y - arrow_size / 2.0f, arrow_x,
                                              arrow_y + arrow_size / 2.0f,
                                              arrow_x + arrow_size * 0.7f, arrow_y, self->fg_color);
    }

    if (self->data.collapsing_header.label) {
        yetty_ygui_old_render_ctx_render_text(ctx, self->data.collapsing_header.label,
                                          self->x + arrow_size + t->pad_large * 2 + 2,
                                          self->y + t->pad_medium, self->fg_color, t->font_size);
    }
    if (self->flags & YETTY_YGUI_OLD_FLAG_HOVER) {
        yetty_ygui_old_render_ctx_render_box_outline(ctx, self->x, self->y, self->w, header_h,
    ...
    struct yetty_ygui_old_widget *self, struct yetty_ygui_old_render_ctx *ctx)
{
    ...
    /* Open own group, recurse children inside the scope so they nest as
     * CMD_GROUP entities in our ygrid body. Children skipped when the
     * collapser is closed. */
    struct yetty_ygui_old_group_marker_result mr =
        yetty_ygui_old_widget_open_group(self, ctx, collapsing_header_render);
    YETTY_RETURN_IF_ERR(yetty_ycore_void, mr, "collapsing_header_render_all: open_group");

    struct yetty_ycore_void_result first_err = YETTY_OK_VOID();
    if ((self->flags & YETTY_YGUI_OLD_FLAG_OPEN) && self->first_child) {
        for (struct yetty_ygui_old_widget *child = self->first_child;
             child && YETTY_IS_OK(first_err); child = child->next_sibling) {
            if (!(child->flags & YETTY_YGUI_OLD_FLAG_VISIBLE)) {
                continue;
            }
            struct yetty_ycore_void_result r;
            ...
            if (YETTY_IS_ERR(r)) {
                first_err = YETTY_ERR(yetty_ycore_void, "collapsing_header_render_all: child", r);
            }
        }
    }
    ...
    if (YETTY_IS_ERR(cr)) {
        if (YETTY_IS_OK(first_err)) {
            first_err = YETTY_ERR(yetty_ycore_void, "collapsing_header_render_all: close_group",
                                  cr);
        } else {
            yetty_ycore_error_destroy(cr.error);
        }
...
static int collapsing_header_on_press(struct yetty_ygui_old_widget *self, float lx, float ly,
                                      ygui_event_t *out)
{
    (void)lx;
    ...
    int was_open = (self->flags & YETTY_YGUI_OLD_FLAG_OPEN) != 0;
    self->flags ^= YETTY_YGUI_OLD_FLAG_OPEN;
    int now_open = (self->flags & YETTY_YGUI_OLD_FLAG_OPEN) != 0;
    if (self->engine) {
        self->engine->dirty = 1;
        self->dirty = 1;
        if (was_open && !now_open) {
            /* Header keeps rendering; only children stopped. */
            for (struct yetty_ygui_old_widget *c = self->first_child; c; c = c->next_sibling) {
                yetty_ygui_old_internal_queue_delete_subtree_rendered(c);
            }
        }
    }
...
static void collapsing_header_destroy(struct yetty_ygui_old_widget *self)
{
    free(self->data.collapsing_header.label);
}

struct yetty_ygui_old_widget *yetty_ygui_old_engine_collapsing_header(struct yetty_ygui_old_engine *engine,
                                                              const char *id, float x, float y,
                                                              float w, float h, const char *label)
{
    struct yetty_ygui_old_widget *c =
        yetty_ygui_old_engine_widget_alloc(engine, YETTY_YGUI_OLD_WIDGET_COLLAPSING_HEADER, id);
    if (!c) {
        return NULL;
    }
    yetty_ygui_old_widget_init_base(c, x, y, w, h);
    ...
    c->layout.mode = YETTY_YGUI_OLD_LAYOUT_FLEX;
    c->layout.direction = YETTY_YGUI_OLD_FLEX_COLUMN;
    c->layout.align_items = YETTY_YGUI_OLD_ALIGN_STRETCH;
    ...
    c->layout.gap = 6.0f;
    static const struct yetty_ygui_old_widget_vtable collapsing_header_vtable = {
...
void yetty_ygui_old_widget_collapsing_header_set_label(struct yetty_ygui_old_widget *widget,
                                                   const char *label)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_COLLAPSING_HEADER) {
        return;
    }
...
const char *yetty_ygui_old_widget_collapsing_header_get_label(const struct yetty_ygui_old_widget *widget)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_COLLAPSING_HEADER) {
        return NULL;
    }
...
void yetty_ygui_old_widget_collapsing_header_set_open(struct yetty_ygui_old_widget *widget, int open)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_COLLAPSING_HEADER) {
        return;
    }
    ...
    if (widget->engine) {
        ...
        if (was_open && !open) {
            for (struct yetty_ygui_old_widget *c = widget->first_child; c; c = c->next_sibling) {
                yetty_ygui_old_internal_queue_delete_subtree_rendered(c);
            }
        }
    }
...
int yetty_ygui_old_widget_collapsing_header_is_open(const struct yetty_ygui_old_widget *widget)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_COLLAPSING_HEADER) {
        return 0;
    }
...
static struct yetty_ycore_void_result tooltip_render(struct yetty_ygui_old_widget *self,
                                                     struct yetty_ygui_old_render_ctx *ctx)
{
    ...
    yetty_ygui_old_render_ctx_render_box_shadow(ctx, self->x, self->y, self->w, self->h,
                                            ...
                                            t->elevation_alpha);
    yetty_ygui_old_render_ctx_render_box(ctx, self->x, self->y, self->w, self->h, t->tooltip_bg,
                                     t->radius_medium);
    yetty_ygui_old_render_ctx_render_box_outline(ctx, self->x, self->y, self->w, self->h,
                                             t->border_muted, t->radius_medium, 1.0f);
    yetty_ygui_old_render_ctx_render_text(ctx, self->data.tooltip.label, self->x + t->pad_large - 2,
                                      self->y + t->pad_medium, self->fg_color, t->font_size);
    return YETTY_OK_VOID();
}

static void tooltip_destroy(struct yetty_ygui_old_widget *self)
{
    free(self->data.tooltip.label);
}

struct yetty_ygui_old_widget *yetty_ygui_old_engine_tooltip(struct yetty_ygui_old_engine *engine,
                                                    const char *id, float x, float y, float w,
                                                    float h, const char *label)
{
    struct yetty_ygui_old_widget *tt =
        yetty_ygui_old_engine_widget_alloc(engine, YETTY_YGUI_OLD_WIDGET_TOOLTIP, id);
    if (!tt) {
        return NULL;
    }
    yetty_ygui_old_widget_init_base(tt, x, y, w, h);
    tt->data.tooltip.label = ygui_strdup(label);
    static const struct yetty_ygui_old_widget_vtable tooltip_vtable = {
...
void yetty_ygui_old_widget_tooltip_set_label(struct yetty_ygui_old_widget *widget, const char *label)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_TOOLTIP) {
        return;
    }
...
const char *yetty_ygui_old_widget_tooltip_get_label(const struct yetty_ygui_old_widget *widget)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_TOOLTIP) {
        return NULL;
    }
...
static struct yetty_ycore_void_result selectable_render(struct yetty_ygui_old_widget *self,
                                                        struct yetty_ygui_old_render_ctx *ctx)
{
    const struct yetty_ygui_old_theme *t = ctx->theme;
    if (self->flags & YETTY_YGUI_OLD_FLAG_CHECKED) {
        yetty_ygui_old_render_ctx_render_box(ctx, self->x, self->y, self->w, self->h,
                                         self->accent_color, t->radius_small);
    } else if (self->flags & YETTY_YGUI_OLD_FLAG_HOVER) {
        yetty_ygui_old_render_ctx_render_box(ctx, self->x, self->y, self->w, self->h, t->bg_hover,
                                         t->radius_small);
    }
    if (self->data.selectable.label) {
        yetty_ygui_old_render_ctx_render_text(ctx, self->data.selectable.label, self->x + t->pad_large,
                                          self->y + t->pad_medium, self->fg_color, t->font_size);
    }
    return YETTY_OK_VOID();
}

static int selectable_on_press(struct yetty_ygui_old_widget *self, float lx, float ly,
                               ygui_event_t *out)
{
    (void)lx;
    (void)ly;
    self->flags ^= YETTY_YGUI_OLD_FLAG_CHECKED;
    out->widget_id = self->id;
    out->type = YETTY_YGUI_OLD_EVENT_CLICK;
    out->data.bool_value = (self->flags & YETTY_YGUI_OLD_FLAG_CHECKED) ? 1 : 0;
    return 1;
}

static void selectable_destroy(struct yetty_ygui_old_widget *self)
{
    free(self->data.selectable.label);
}

struct yetty_ygui_old_widget *yetty_ygui_old_engine_selectable(struct yetty_ygui_old_engine *engine,
                                                       const char *id, float x, float y, float w,
                                                       float h, const char *label)
{
    struct yetty_ygui_old_widget *s =
        yetty_ygui_old_engine_widget_alloc(engine, YETTY_YGUI_OLD_WIDGET_SELECTABLE, id);
    if (!s) {
        return NULL;
    }
    yetty_ygui_old_widget_init_base(s, x, y, w, h);
    s->data.selectable.label = ygui_strdup(label);
    static const struct yetty_ygui_old_widget_vtable selectable_vtable = {
...
void yetty_ygui_old_widget_selectable_set_label(struct yetty_ygui_old_widget *widget, const char *label)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_SELECTABLE) {
        return;
    }
...
const char *yetty_ygui_old_widget_selectable_get_label(const struct yetty_ygui_old_widget *widget)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_SELECTABLE) {
        return NULL;
    }
...
void yetty_ygui_old_widget_selectable_set_checked(struct yetty_ygui_old_widget *widget, int checked)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_SELECTABLE) {
        return;
    }
    if (checked) {
        widget->flags |= YETTY_YGUI_OLD_FLAG_CHECKED;
    } else {
        widget->flags &= ~YETTY_YGUI_OLD_FLAG_CHECKED;
    }
    if (widget->engine) {
...
int yetty_ygui_old_widget_selectable_is_checked(const struct yetty_ygui_old_widget *widget)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_SELECTABLE) {
        return 0;
    }
    return (widget->flags & YETTY_YGUI_OLD_FLAG_CHECKED) ? 1 : 0;
}
...
static void choicebox_free_options(struct yetty_ygui_old_widget *self)
{
...
static void choicebox_copy_options(struct yetty_ygui_old_widget *self, const char **options, int count)
{
...
static struct yetty_ycore_void_result choicebox_render(struct yetty_ygui_old_widget *self,
                                                       struct yetty_ygui_old_render_ctx *ctx)
{
    const struct yetty_ygui_old_theme *t = ctx->theme;
    ...
    for (int i = 0; i < self->data.choicebox.option_count; i++) {
        ...
        yetty_ygui_old_render_ctx_render_circle_outline(
            ctx, center_x, center_y, radio_size * 0.5f,
            is_hovered ? self->accent_color : t->border_muted, 1.5f);
        if (is_selected) {
            yetty_ygui_old_render_ctx_render_circle(ctx, center_x, center_y, radio_size * 0.25f,
                                                self->accent_color);
        } else if (is_hovered) {
            yetty_ygui_old_render_ctx_render_circle(ctx, center_x, center_y, radio_size / 6.0f,
                                                t->thumb_hover);
        }

        if (self->data.choicebox.options[i]) {
            yetty_ygui_old_render_ctx_render_text(ctx, self->data.choicebox.options[i],
...
static int choicebox_on_press(struct yetty_ygui_old_widget *self, float lx, float ly, ygui_event_t *out)
{
    (void)lx;
    const struct yetty_ygui_old_theme *t = self->engine->theme;
    float opt_h = t->row_height;
...
static void choicebox_destroy(struct yetty_ygui_old_widget *self)
{
    choicebox_free_options(self);
}

struct yetty_ygui_old_widget *yetty_ygui_old_engine_choicebox(struct yetty_ygui_old_engine *engine,
                                                      const char *id, float x, float y, float w,
                                                      float h, const char **options,
                                                      int option_count)
{
    struct yetty_ygui_old_widget *c =
        yetty_ygui_old_engine_widget_alloc(engine, YETTY_YGUI_OLD_WIDGET_CHOICEBOX, id);
    if (!c) {
        return NULL;
    }
    yetty_ygui_old_widget_init_base(c, x, y, w, h);
    ...
    static const struct yetty_ygui_old_widget_vtable choicebox_vtable = {
...
void yetty_ygui_old_widget_choicebox_set_options(struct yetty_ygui_old_widget *widget, const char **options,
                                             int count)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_CHOICEBOX) {
        return;
    }
...
void yetty_ygui_old_widget_choicebox_set_selected(struct yetty_ygui_old_widget *widget, int index)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_CHOICEBOX) {
        return;
    }
...
int yetty_ygui_old_widget_choicebox_get_selected(const struct yetty_ygui_old_widget *widget)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_CHOICEBOX) {
        return 0;
    }
...
static float vscrollbar_value(const struct yetty_ygui_old_widget *self)
{
    struct yetty_ygui_old_widget *tgt = self->data.scrollbar.target;
...
static float vscrollbar_thumb_h(const struct yetty_ygui_old_widget *self)
{
...
static struct yetty_ycore_void_result vscrollbar_render(struct yetty_ygui_old_widget *self,
                                                        struct yetty_ygui_old_render_ctx *ctx)
{
    const struct yetty_ygui_old_theme *t = ctx->theme;
    float track_w = self->w > 0 ? self->w : t->scrollbar_size;
    yetty_ygui_old_render_ctx_render_box(ctx, self->x, self->y, track_w, self->h, t->bg_secondary,
                                     track_w * 0.5f);
    ...
    uint32_t thumb_color =
        (self->flags & YETTY_YGUI_OLD_FLAG_PRESSED)
            ? self->accent_color
            : (self->flags & YETTY_YGUI_OLD_FLAG_HOVER ? t->thumb_hover : t->thumb_normal);
    yetty_ygui_old_render_ctx_render_box(ctx, self->x + t->pad_small, thumb_y, track_w - t->pad_medium,
                                     thumb_h, thumb_color, (track_w - t->pad_medium) * 0.5f);
    return YETTY_OK_VOID();
}

static int vscrollbar_update(struct yetty_ygui_old_widget *self, float ly, ygui_event_t *out)
{
    float thumb_h = vscrollbar_thumb_h(self);
    float track_range = self->h - thumb_h;
    if (track_range <= 0) {
...
static int vscrollbar_on_press(struct yetty_ygui_old_widget *self, float lx, float ly,
                               ygui_event_t *out)
{
    (void)lx;
...
static int vscrollbar_on_drag(struct yetty_ygui_old_widget *self, float lx, float ly, ygui_event_t *out)
{
    (void)lx;
    return vscrollbar_update(self, ly, out);
}
...
static int vscrollbar_on_scroll(struct yetty_ygui_old_widget *self, float dx, float dy,
                                ygui_event_t *out)
{
    (void)dx;
    struct yetty_ygui_old_widget *tgt = self->data.scrollbar.target;
...
struct yetty_ygui_old_widget *yetty_ygui_old_engine_vscrollbar(struct yetty_ygui_old_engine *engine,
                                                       const char *id, float x, float y, float w,
                                                       float h)
{
    struct yetty_ygui_old_widget *sb =
        yetty_ygui_old_engine_widget_alloc(engine, YETTY_YGUI_OLD_WIDGET_VSCROLLBAR, id);
    if (!sb) {
        return NULL;
    }
    yetty_ygui_old_widget_init_base(sb, x, y, w, h);
    sb->data.scrollbar.value = 0;
    sb->data.scrollbar.target = NULL;
    static const struct yetty_ygui_old_widget_vtable vscrollbar_vtable = {
...
static struct yetty_ycore_void_result hscrollbar_render(struct yetty_ygui_old_widget *self,
                                                        struct yetty_ygui_old_render_ctx *ctx)
{
    const struct yetty_ygui_old_theme *t = ctx->theme;
    float track_h = self->h > 0 ? self->h : t->scrollbar_size;
    yetty_ygui_old_render_ctx_render_box(ctx, self->x, self->y, self->w, track_h, t->bg_secondary,
                                     track_h * 0.5f);
    ...
    uint32_t thumb_color =
        (self->flags & YETTY_YGUI_OLD_FLAG_PRESSED)
            ? self->accent_color
            : (self->flags & YETTY_YGUI_OLD_FLAG_HOVER ? t->thumb_hover : t->thumb_normal);
    yetty_ygui_old_render_ctx_render_box(ctx, thumb_x, self->y + t->pad_small, thumb_w,
                                     track_h - t->pad_medium, thumb_color,
                                     (track_h - t->pad_medium) * 0.5f);
    return YETTY_OK_VOID();
}

static int hscrollbar_update(struct yetty_ygui_old_widget *self, float lx, ygui_event_t *out)
{
    float thumb_w = ygui_max(20.0f, self->w * 0.2f);
    float track_range = self->w - thumb_w;
    if (track_range <= 0) {
...
static int hscrollbar_on_press(struct yetty_ygui_old_widget *self, float lx, float ly,
                               ygui_event_t *out)
{
    (void)ly;
...
static int hscrollbar_on_drag(struct yetty_ygui_old_widget *self, float lx, float ly, ygui_event_t *out)
{
    (void)ly;
    return hscrollbar_update(self, lx, out);
}

struct yetty_ygui_old_widget *yetty_ygui_old_engine_hscrollbar(struct yetty_ygui_old_engine *engine,
                                                       const char *id, float x, float y, float w,
                                                       float h)
{
    struct yetty_ygui_old_widget *sb =
        yetty_ygui_old_engine_widget_alloc(engine, YETTY_YGUI_OLD_WIDGET_HSCROLLBAR, id);
    if (!sb) {
        return NULL;
    }
    yetty_ygui_old_widget_init_base(sb, x, y, w, h);
    sb->data.scrollbar.value = 0;
    static const struct yetty_ygui_old_widget_vtable hscrollbar_vtable = {
...
void yetty_ygui_old_widget_scrollbar_set_value(struct yetty_ygui_old_widget *widget, float value)
{
    if (!widget) {
        return;
    }
    if (widget->type != YETTY_YGUI_OLD_WIDGET_VSCROLLBAR &&
        widget->type != YETTY_YGUI_OLD_WIDGET_HSCROLLBAR) {
        return;
    }
...
void yetty_ygui_old_widget_scrollbar_bind(struct yetty_ygui_old_widget *scrollbar,
                                      struct yetty_ygui_old_widget *target)
{
    if (!scrollbar) {
        return;
    }
    if (scrollbar->type != YETTY_YGUI_OLD_WIDGET_VSCROLLBAR &&
        scrollbar->type != YETTY_YGUI_OLD_WIDGET_HSCROLLBAR) {
        return;
    }
    ...
    struct yetty_ygui_old_widget *prev = scrollbar->data.scrollbar.target;
...
float yetty_ygui_old_widget_scrollbar_get_value(const struct yetty_ygui_old_widget *widget)
{
    if (!widget) {
        return 0;
    }
...
static struct yetty_ycore_void_result list_render(struct yetty_ygui_old_widget *self,
                                                  struct yetty_ygui_old_render_ctx *ctx)
{
    const struct yetty_ygui_old_theme *t = ctx->theme;
    struct yetty_ygui_old_widget *sel = self->data.list.selected;
    if (sel && (sel->flags & YETTY_YGUI_OLD_FLAG_VISIBLE)) {
        return yetty_ygui_old_render_ctx_render_box(ctx, sel->x, sel->y, sel->w, sel->h,
 ...
 * their own surfaces. */
static struct yetty_ycore_void_result list_render_all(struct yetty_ygui_old_widget *self,
                                                      struct yetty_ygui_old_render_ctx *ctx)
{
    if (!(self->flags & YETTY_YGUI_OLD_FLAG_VISIBLE)) {
        return YETTY_OK_VOID();
    ...
    /* Open own group (selection background), recurse children inside the
     * scope so they nest as CMD_GROUP entities in our ygrid body. */
    struct yetty_ygui_old_group_marker_result mr =
        yetty_ygui_old_widget_open_group(self, ctx, list_render);
    YETTY_RETURN_IF_ERR(yetty_ycore_void, mr, "list_render_all: open_group");
    ...
    for (struct yetty_ygui_old_widget *child = self->first_child; child && YETTY_IS_OK(first_err);
        ...
        if (YETTY_IS_ERR(r)) {
            first_err = YETTY_ERR(yetty_ycore_void, "list_render_all: child", r);
        }
    }
    ...
    if (YETTY_IS_ERR(cr)) {
        if (YETTY_IS_OK(first_err)) {
            first_err = YETTY_ERR(yetty_ycore_void, "list_render_all: close_group", cr);
        } else {
            yetty_ycore_error_destroy(cr.error);
        }
    }
...
static struct yetty_ygui_old_widget *list_child_at(struct yetty_ygui_old_widget *self, float lx, float ly)
{
    /* Children's x/y are relative to parent (self). */
    for (struct yetty_ygui_old_widget *c = self->first_child; c; c = c->next_sibling) {
        if (!(c->flags & YETTY_YGUI_OLD_FLAG_VISIBLE)) {
            continue;
        }
...
static int list_on_press(struct yetty_ygui_old_widget *self, float lx, float ly, ygui_event_t *out)
{
    struct yetty_ygui_old_widget *child = list_child_at(self, lx, ly);
    if (!child) {
        return 0;
    }
...
struct yetty_ygui_old_widget *yetty_ygui_old_engine_list(struct yetty_ygui_old_engine *engine, const char *id,
                                                 float x, float y, float w, float h)
{
    struct yetty_ygui_old_widget *lst =
        yetty_ygui_old_engine_widget_alloc(engine, YETTY_YGUI_OLD_WIDGET_LIST, id);
    if (!lst) {
        return NULL;
    }
    yetty_ygui_old_widget_init_base(lst, x, y, w, h);
    /* Default layout: flex column with theme gap; children stretched on
     * the cross axis so each row spans the list width. */
    lst->layout.mode = YETTY_YGUI_OLD_LAYOUT_FLEX;
    lst->layout.direction = YETTY_YGUI_OLD_FLEX_COLUMN;
    lst->layout.align_items = YETTY_YGUI_OLD_ALIGN_STRETCH;
    lst->layout.gap = engine->theme->pad_small;
    static const struct yetty_ygui_old_widget_vtable list_vtable = {
...
void yetty_ygui_old_widget_list_set_selected(struct yetty_ygui_old_widget *list,
                                         struct yetty_ygui_old_widget *child)
{
    if (!list || list->type != YETTY_YGUI_OLD_WIDGET_LIST) {
        return;
    }
...
struct yetty_ygui_old_widget *yetty_ygui_old_widget_list_get_selected(const struct yetty_ygui_old_widget *list)
{
    if (!list || list->type != YETTY_YGUI_OLD_WIDGET_LIST) {
        return NULL;
    }
...
void yetty_ygui_old_widget_list_on_select(struct yetty_ygui_old_widget *list, ygui_click_callback_t cb,
                                      void *userdata)
{
    if (!list || list->type != YETTY_YGUI_OLD_WIDGET_LIST) {
        return;
...
static void table_resolve_widths(const struct yetty_ygui_old_widget *self, float *out)
{
...
static struct yetty_ycore_void_result table_render(struct yetty_ygui_old_widget *self,
                                                   struct yetty_ygui_old_render_ctx *ctx)
{
    const struct yetty_ygui_old_theme *t = ctx->theme;
    int n = self->data.table.n_columns;
    if (n <= 0) {
        ...
        yetty_ygui_old_render_ctx_render_box(ctx, self->x, self->y, self->w, self->h, t->bg_surface,
                                         t->radius_small);
        return YETTY_OK_VOID();
    }
    ...
    /* Header row. */
    yetty_ygui_old_render_ctx_render_box(ctx, self->x, self->y, self->w, row_h, t->bg_header,
                                     t->radius_small);
    {
        float cx = self->x;
        for (int c = 0; c < n; c++) {
            ...
            yetty_ygui_old_render_ctx_render_text(ctx, name, cx + text_pad_x, self->y + text_y_off,
                                              t->text_primary, font_size);
            cx += widths[c];
            /* Vertical separator after every column except the last. */
            if (c < n - 1) {
                yetty_ygui_old_render_ctx_render_box(ctx, cx, self->y, 1.0f, self->h, t->border_muted,
                                                 0.0f);
            }
        }
    }
    /* Header / data divider. */
    yetty_ygui_old_render_ctx_render_box(ctx, self->x, self->y + row_h, self->w, 1.0f, t->border, 0.0f);
    ...
    for (int r = 0; r < rows; r++) {
        ...
        if (r == sel) {
            yetty_ygui_old_render_ctx_render_box(ctx, self->x, row_y, self->w, row_h, t->selection_bg,
                                             0.0f);
        } else if ((r & 1) == 1 && t->bg_secondary != 0u) {
            /* Zebra striping for odd-indexed rows when the theme provides
             * a distinct secondary surface. */
            yetty_ygui_old_render_ctx_render_box(ctx, self->x, row_y, self->w, row_h, t->bg_secondary,
                                             0.0f);
        }
        float cx = self->x;
        ...
        for (int c = 0; c < n; c++) {
            ...
            if (txt) {
                yetty_ygui_old_render_ctx_render_text(ctx, txt, cx + text_pad_x, row_y + text_y_off,
                                                  t->text_primary, font_size);
            }
            cx += widths[c];
        }
...
static void table_apply_sort(struct yetty_ygui_old_widget *self)
{
...
static void table_header_clicked(struct yetty_ygui_old_widget *self, int col)
{
...
static int table_on_drag(struct yetty_ygui_old_widget *self, float lx, float ly, ygui_event_t *out)
{
    (void)ly;
...
static int table_on_release(struct yetty_ygui_old_widget *self, float lx, float ly, ygui_event_t *out)
{
    (void)lx;
    (void)ly;
    (void)out;
...
static int table_on_press(struct yetty_ygui_old_widget *self, float lx, float ly, ygui_event_t *out)
{
    const struct yetty_ygui_old_theme *t = self->engine ? self->engine->theme : NULL;
    if (!t) {
        return 0;
...
static void table_destroy(struct yetty_ygui_old_widget *self)
{
...
struct yetty_ygui_old_widget *yetty_ygui_old_engine_table(struct yetty_ygui_old_engine *engine, const char *id,
                                                  float x, float y, float w, float h)
{
    struct yetty_ygui_old_widget *tbl =
        yetty_ygui_old_engine_widget_alloc(engine, YETTY_YGUI_OLD_WIDGET_TABLE, id);
    if (!tbl) {
        return NULL;
    }
    yetty_ygui_old_widget_init_base(tbl, x, y, w, h);
    tbl->data.table.selected_row = -1;
    tbl->data.table.sort_column = -1;
    tbl->data.table.sort_order = 0;
    tbl->data.table.resizing_column = -1;
    static const struct yetty_ygui_old_widget_vtable table_vtable = {
        .render = table_render,
...
void yetty_ygui_old_widget_table_set_columns(struct yetty_ygui_old_widget *table, const char *const *names,
                                         const float *widths, int n_columns)
{
    if (!table || table->type != YETTY_YGUI_OLD_WIDGET_TABLE || n_columns <= 0) {
        return;
    }
...
void yetty_ygui_old_widget_table_clear_rows(struct yetty_ygui_old_widget *table)
{
    if (!table || table->type != YETTY_YGUI_OLD_WIDGET_TABLE) {
        return;
    }
    for (int r = 0; r < table->data.table.n_rows; r++) {
...
void yetty_ygui_old_widget_table_add_row(struct yetty_ygui_old_widget *table, const char *const *cells,
                                     int n_cells)
{
    if (!table || table->type != YETTY_YGUI_OLD_WIDGET_TABLE) {
        return;
...
void yetty_ygui_old_widget_table_set_row(struct yetty_ygui_old_widget *table, int row,
...
int yetty_ygui_old_widget_table_row_count(const struct yetty_ygui_old_widget *table)
{
    if (!table || table->type != YETTY_YGUI_OLD_WIDGET_TABLE) {
        return 0;
    }
...
void yetty_ygui_old_widget_table_set_selected(struct yetty_ygui_old_widget *table, int row)
{
    if (!table || table->type != YETTY_YGUI_OLD_WIDGET_TABLE) {
        return;
    }
...
int yetty_ygui_old_widget_table_get_selected(const struct yetty_ygui_old_widget *table)
{
    if (!table || table->type != YETTY_YGUI_OLD_WIDGET_TABLE) {
        return -1;
    }
...
void yetty_ygui_old_widget_table_on_select(struct yetty_ygui_old_widget *table,
                                       yetty_ygui_old_table_select_fn cb, void *userdata)
{
    if (!table || table->type != YETTY_YGUI_OLD_WIDGET_TABLE) {
        return;
    }
...
void yetty_ygui_old_widget_table_set_row_height(struct yetty_ygui_old_widget *table, float h)
{
    if (!table || table->type != YETTY_YGUI_OLD_WIDGET_TABLE) {
        return;
    }
...
void yetty_ygui_old_widget_table_set_sortable(struct yetty_ygui_old_widget *table, int enabled)
{
...
int yetty_ygui_old_widget_table_get_sort_column(const struct yetty_ygui_old_widget *table)
{
    if (!table || table->type != YETTY_YGUI_OLD_WIDGET_TABLE) {
        return -1;
    }
...
int yetty_ygui_old_widget_table_get_sort_order(const struct yetty_ygui_old_widget *table)
{
    if (!table || table->type != YETTY_YGUI_OLD_WIDGET_TABLE) {
        return 0;
    }
...
void yetty_ygui_old_widget_table_sort_by(struct yetty_ygui_old_widget *table, int column, int descending)
{
    if (!table || table->type != YETTY_YGUI_OLD_WIDGET_TABLE) {
        return;
    }
...
static float tree_node_header_h(const struct yetty_ygui_old_widget *self,
                                const struct yetty_ygui_old_theme *theme)
{
    (void)self;
...
static struct yetty_ycore_void_result tree_node_render(struct yetty_ygui_old_widget *self,
                                                       struct yetty_ygui_old_render_ctx *ctx)
{
    const struct yetty_ygui_old_theme *t = ctx->theme;
    ...
    if (hovered || pressed) {
        ...
        yetty_ygui_old_render_ctx_render_box(ctx, self->x, self->y, self->w, header_h, bg,
                                         t->radius_small);
    }
    ...
    if (expanded) {
        yetty_ygui_old_render_ctx_render_triangle(ctx, cx, cy - r * 0.6f, cx + r * 2.0f, cy - r * 0.6f,
                                              cx + r, cy + r * 0.8f, t->text_primary);
    } else {
        yetty_ygui_old_render_ctx_render_triangle(ctx, cx, cy - r, cx, cy + r, cx + r * 1.2f, cy,
                                              t->text_primary);
    }
    ...
    if (self->data.tree_node.label) {
        ...
        yetty_ygui_old_render_ctx_render_text(ctx, self->data.tree_node.label, label_x, label_y,
                                          self->fg_color, t->font_size);
    }
    return YETTY_OK_VOID();
}
...
static struct yetty_ycore_void_result tree_node_render_all(struct yetty_ygui_old_widget *self,
                                                           struct yetty_ygui_old_render_ctx *ctx)
{
    if (!(self->flags & YETTY_YGUI_OLD_FLAG_VISIBLE)) {
        return YETTY_OK_VOID();
    ...
    /* Open own group (header chrome only), recurse the children-list
     * INSIDE the scope when expanded, then close. */
    struct yetty_ygui_old_group_marker_result mr =
        yetty_ygui_old_widget_open_group(self, ctx, tree_node_render);
    YETTY_RETURN_IF_ERR(yetty_ycore_void, mr, "tree_node_render_all: open_group");

    struct yetty_ygui_old_widget *kids = self->data.tree_node.children_list;
    struct yetty_ycore_void_result first_err = YETTY_OK_VOID();
    if (kids && self->data.tree_node.expanded && (kids->flags & YETTY_YGUI_OLD_FLAG_VISIBLE)) {
        if (kids->vtable && kids->vtable->render_all) {
            first_err = kids->vtable->render_all(kids, ctx);
        } else {
            first_err = yetty_ygui_old_widget_render_all_default(kids, ctx);
        }
        if (YETTY_IS_ERR(first_err)) {
            first_err = YETTY_ERR(yetty_ycore_void, "tree_node_render_all: kids", first_err);
        }
    }
    ...
    if (YETTY_IS_ERR(cr)) {
        if (YETTY_IS_OK(first_err)) {
            first_err = YETTY_ERR(yetty_ycore_void, "tree_node_render_all: close_group", cr);
        } else {
            yetty_ycore_error_destroy(cr.error);
        }
    }
...
static int tree_node_on_press(struct yetty_ygui_old_widget *self, float lx, float ly, ygui_event_t *out)
{
    const struct yetty_ygui_old_theme *theme = self->engine ? self->engine->theme : NULL;
    ...
    if (on_chevron) {
        ...
        if (self->data.tree_node.children_list) {
            yetty_ygui_old_widget_set_visible(self->data.tree_node.children_list,
...
static void tree_node_destroy(struct yetty_ygui_old_widget *self)
{
...
struct yetty_ygui_old_widget *yetty_ygui_old_engine_tree_node(struct yetty_ygui_old_engine *engine,
                                                      const char *id, const char *label)
{
    struct yetty_ygui_old_widget *node =
        yetty_ygui_old_engine_widget_alloc(engine, YETTY_YGUI_OLD_WIDGET_TREE_NODE, id);
    if (!node) {
        return NULL;
    }
    ...
    yetty_ygui_old_widget_init_base(node, 0, 0, 200.0f, header_h);
    ...
    node->layout.mode = YETTY_YGUI_OLD_LAYOUT_FLEX;
    node->layout.direction = YETTY_YGUI_OLD_FLEX_COLUMN;
    node->layout.align_items = YETTY_YGUI_OLD_ALIGN_STRETCH;
    node->layout.padding_top = header_h;

    static const struct yetty_ygui_old_widget_vtable tree_node_vtable = {
    ...
    struct yetty_ygui_old_widget *kids = yetty_ygui_old_engine_list(engine, child_id, 0, 0, 0, 0);
    if (kids) {
        ...
        yetty_ygui_old_widget_set_visible(kids, 0);
        yetty_ygui_old_widget_add_child(node, kids);
...
void yetty_ygui_old_widget_tree_node_set_label(struct yetty_ygui_old_widget *node, const char *label)
{
    if (!node || node->type != YETTY_YGUI_OLD_WIDGET_TREE_NODE) {
        return;
    }
...
const char *yetty_ygui_old_widget_tree_node_get_label(const struct yetty_ygui_old_widget *node)
{
    if (!node || node->type != YETTY_YGUI_OLD_WIDGET_TREE_NODE) {
        return NULL;
    }
...
void yetty_ygui_old_widget_tree_node_set_expanded(struct yetty_ygui_old_widget *node, int expanded)
{
    if (!node || node->type != YETTY_YGUI_OLD_WIDGET_TREE_NODE) {
        return;
    }
    ...
    if (node->data.tree_node.children_list) {
        yetty_ygui_old_widget_set_visible(node->data.tree_node.children_list, expanded);
    }
    if (node->engine) {
        node->engine->dirty = 1;
        node->dirty = 1;
...
int yetty_ygui_old_widget_tree_node_is_expanded(const struct yetty_ygui_old_widget *node)
{
    if (!node || node->type != YETTY_YGUI_OLD_WIDGET_TREE_NODE) {
        return 0;
    }
...
struct yetty_ygui_old_widget *yetty_ygui_old_widget_tree_node_children(struct yetty_ygui_old_widget *node)
{
    if (!node || node->type != YETTY_YGUI_OLD_WIDGET_TREE_NODE) {
        return NULL;
    }
...
void yetty_ygui_old_widget_tree_node_on_toggle(struct yetty_ygui_old_widget *node, ygui_check_callback_t cb,
                                           void *userdata)
{
    if (!node || node->type != YETTY_YGUI_OLD_WIDGET_TREE_NODE) {
        return;
...
static int radio_group_index_of(const struct yetty_ygui_old_widget *group,
                                const struct yetty_ygui_old_widget *child)
{
    int i = 0;
    for (struct yetty_ygui_old_widget *c = group->first_child; c; c = c->next_sibling) {
        if (c == child) {
            return i;
        }
        i++;
...
static struct yetty_ygui_old_widget *radio_group_child_at(struct yetty_ygui_old_widget *group, int index)
{
    if (index < 0) {
        return NULL;
    }
...
static void radio_group_select(struct yetty_ygui_old_widget *group, struct yetty_ygui_old_widget *newsel)
{
    struct yetty_ygui_old_widget *prev = group->data.radio_group.selected;
    if (prev == newsel) {
        return;
    }
    if (prev) {
...
static struct yetty_ycore_void_result radio_render(struct yetty_ygui_old_widget *self,
                                                   struct yetty_ygui_old_render_ctx *ctx)
{
    const struct yetty_ygui_old_theme *t = ctx->theme;
    ...
    /* Outline */
    yetty_ygui_old_render_ctx_render_circle_outline(ctx, cx, cy, dia * 0.5f, t->border, 1.5f);

    /* Filled inner dot when selected. */
    struct yetty_ygui_old_widget *group = self->data.radio.group;
    int selected = (group && group->data.radio_group.selected == self);
    if (selected) {
        yetty_ygui_old_render_ctx_render_circle(ctx, cx, cy, dia * 0.30f, self->accent_color);
    }

    if (self->data.radio.label) {
        float text_x = self->x + t->pad_small + dia + t->pad_medium;
        yetty_ygui_old_render_ctx_render_text(ctx, self->data.radio.label, text_x,
                                          self->y + t->pad_medium, self->fg_color, t->font_size);
    }
    return YETTY_OK_VOID();
}

static int radio_on_release(struct yetty_ygui_old_widget *self, float lx, float ly, ygui_event_t *out)
{
    if (lx < 0 || lx >= self->w || ly < 0 || ly >= self->h) {
        return 0;
    }
...
static void radio_destroy(struct yetty_ygui_old_widget *self)
{
    free(self->data.radio.label);
}

struct yetty_ygui_old_widget *yetty_ygui_old_engine_radio_group(struct yetty_ygui_old_engine *engine,
                                                        const char *id, float x, float y, float w,
                                                        float h)
{
    struct yetty_ygui_old_widget *g =
        yetty_ygui_old_engine_widget_alloc(engine, YETTY_YGUI_OLD_WIDGET_RADIO_GROUP, id);
    if (!g) {
        return NULL;
    }
    yetty_ygui_old_widget_init_base(g, x, y, w, h);
    ...
    g->layout.mode = YETTY_YGUI_OLD_LAYOUT_FLEX;
    g->layout.direction = YETTY_YGUI_OLD_FLEX_COLUMN;
    g->layout.gap = 4.0f;
    g->layout.align_items = YETTY_YGUI_OLD_ALIGN_STRETCH;
...
struct yetty_ygui_old_widget *yetty_ygui_old_widget_radio_group_add(struct yetty_ygui_old_widget *group,
                                                            const char *id, const char *label)
{
    if (!group || group->type != YETTY_YGUI_OLD_WIDGET_RADIO_GROUP || !group->engine) {
        return NULL;
    }
    struct yetty_ygui_old_widget *r =
        yetty_ygui_old_engine_widget_alloc(group->engine, YETTY_YGUI_OLD_WIDGET_RADIO, id);
    if (!r) {
        return NULL;
    }
    /* Sensible default size — caller can override via set_size / CSS. */
    yetty_ygui_old_widget_init_base(r, 0, 0, group->w, 24);
    r->data.radio.label = ygui_strdup(label);
    r->data.radio.group = group;
    static const struct yetty_ygui_old_widget_vtable radio_vtable = {
        .render = radio_render,
    ...
    yetty_ygui_old_widget_add_child(group, r);
    return r;
}

void yetty_ygui_old_widget_radio_group_set_selected_index(struct yetty_ygui_old_widget *group, int index)
{
    if (!group || group->type != YETTY_YGUI_OLD_WIDGET_RADIO_GROUP) {
        return;
    }
    radio_group_select(group, index >= 0 ? radio_group_child_at(group, index) : NULL);
}

int yetty_ygui_old_widget_radio_group_get_selected_index(const struct yetty_ygui_old_widget *group)
{
    if (!group || group->type != YETTY_YGUI_OLD_WIDGET_RADIO_GROUP) {
        return -1;
    }
...
void yetty_ygui_old_widget_radio_group_on_change(struct yetty_ygui_old_widget *group,
...
static float spinner_btn_w(const struct yetty_ygui_old_widget *self)
{
    return self->h;
}

static void spinner_format(const struct yetty_ygui_old_widget *self, char *out, size_t cap)
{
...
static void spinner_apply(struct yetty_ygui_old_widget *self, float new_val)
{
...
static struct yetty_ycore_void_result spinner_render(struct yetty_ygui_old_widget *self,
                                                     struct yetty_ygui_old_render_ctx *ctx)
{
    const struct yetty_ygui_old_theme *t = ctx->theme;
    ...
    yetty_ygui_old_render_ctx_render_box(ctx, self->x, self->y, bw, self->h, t->bg_secondary,
                                     t->radius_small);
    yetty_ygui_old_render_ctx_render_box(ctx, self->x + self->w - bw, self->y, bw, self->h,
                                     t->bg_secondary, t->radius_small);
    /* Minus glyph */
    yetty_ygui_old_render_ctx_render_box(ctx, self->x + bw * 0.25f, self->y + self->h * 0.5f - 1.0f,
                                     bw * 0.5f, 2.0f, self->fg_color, 1.0f);
    /* Plus glyph */
    yetty_ygui_old_render_ctx_render_box(ctx, self->x + self->w - bw + bw * 0.25f,
                                     self->y + self->h * 0.5f - 1.0f, bw * 0.5f, 2.0f,
                                     self->fg_color, 1.0f);
    yetty_ygui_old_render_ctx_render_box(ctx, self->x + self->w - bw + bw * 0.5f - 1.0f,
    ...
    yetty_ygui_old_render_ctx_render_text(ctx, buf, self->x + bw + t->pad_medium,
                                      self->y + t->pad_medium, self->fg_color, t->font_size);
    return YETTY_OK_VOID();
}

static int spinner_on_press(struct yetty_ygui_old_widget *self, float lx, float ly, ygui_event_t *out)
{
    (void)ly;
    float bw = spinner_btn_w(self);
    if (lx < bw) {
...
static int spinner_on_scroll(struct yetty_ygui_old_widget *self, float dx, float dy, ygui_event_t *out)
{
    (void)dx;
    if (dy == 0) {
        return 0;
...
static int spinner_on_key(struct yetty_ygui_old_widget *self, uint32_t key, int mods, ygui_event_t *out)
{
    (void)mods;
    float prev = self->data.spinner.value;
    if (key == 0x1000) {
...
struct yetty_ygui_old_widget *yetty_ygui_old_engine_spinner(struct yetty_ygui_old_engine *engine,
                                                    const char *id, float x, float y, float w,
                                                    float h, float min_val, float max_val,
                                                    float step, float value)
{
    struct yetty_ygui_old_widget *s =
        yetty_ygui_old_engine_widget_alloc(engine, YETTY_YGUI_OLD_WIDGET_SPINNER, id);
    if (!s) {
        return NULL;
    }
    yetty_ygui_old_widget_init_base(s, x, y, w, h);
    ...
    static const struct yetty_ygui_old_widget_vtable spinner_vtable = {
...
void yetty_ygui_old_widget_spinner_set_value(struct yetty_ygui_old_widget *widget, float value)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_SPINNER) {
        return;
    }
    spinner_apply(widget, value);
}

float yetty_ygui_old_widget_spinner_get_value(const struct yetty_ygui_old_widget *widget)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_SPINNER) {
        return 0;
    }
...
void yetty_ygui_old_widget_spinner_set_range(struct yetty_ygui_old_widget *widget, float min_val,
                                         float max_val)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_SPINNER) {
        return;
...
void yetty_ygui_old_widget_spinner_set_step(struct yetty_ygui_old_widget *widget, float step)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_SPINNER) {
        return;
    }
...
void yetty_ygui_old_widget_spinner_set_precision(struct yetty_ygui_old_widget *widget, int decimals)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_SPINNER) {
        return;
    }
...
void yetty_ygui_old_widget_spinner_on_change(struct yetty_ygui_old_widget *widget,
                                         ygui_change_callback_t cb, void *userdata)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_SPINNER) {
        return;
...
static int splitter_axis_row(const struct yetty_ygui_old_widget *self)
{
    ...
    return self->parent && self->parent->layout.mode == YETTY_YGUI_OLD_LAYOUT_FLEX
               ? self->parent->layout.direction == YETTY_YGUI_OLD_FLEX_ROW
               : 1; /* default: assume row (vertical bar) */
}

static struct yetty_ycore_void_result splitter_render(struct yetty_ygui_old_widget *self,
                                                      struct yetty_ygui_old_render_ctx *ctx)
{
    const struct yetty_ygui_old_theme *t = ctx->theme;
    uint32_t bar = (self->flags & (YETTY_YGUI_OLD_FLAG_HOVER | YETTY_YGUI_OLD_FLAG_PRESSED))
                       ? t->thumb_hover
                       : t->border;
    yetty_ygui_old_render_ctx_render_box(ctx, self->x, self->y, self->w, self->h, bar, 0);
    return YETTY_OK_VOID();
}

static int splitter_on_drag(struct yetty_ygui_old_widget *self, float lx, float ly, ygui_event_t *out)
{
    ...
    struct yetty_ygui_old_widget *left = self->prev_sibling;
    struct yetty_ygui_old_widget *right = self->next_sibling;
...
static int splitter_on_press(struct yetty_ygui_old_widget *self, float lx, float ly, ygui_event_t *out)
{
    (void)self;
    (void)lx;
    (void)ly;
...
struct yetty_ygui_old_widget *yetty_ygui_old_engine_splitter(struct yetty_ygui_old_engine *engine,
                                                     const char *id, float x, float y, float w,
                                                     float h)
{
    struct yetty_ygui_old_widget *s =
        yetty_ygui_old_engine_widget_alloc(engine, YETTY_YGUI_OLD_WIDGET_SPLITTER, id);
    if (!s) {
        return NULL;
    }
    yetty_ygui_old_widget_init_base(s, x, y, w, h);
    ...
    static const struct yetty_ygui_old_widget_vtable splitter_vtable = {
...
void yetty_ygui_old_widget_splitter_set_min(struct yetty_ygui_old_widget *widget, float min_size)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_SPLITTER) {
        return;
    }
    widget->data.splitter.min_size = min_size;
}

void yetty_ygui_old_widget_splitter_on_change(struct yetty_ygui_old_widget *widget,
                                          ygui_change_callback_t cb, void *userdata)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_SPLITTER) {
        return;
...
void yetty_ygui_old_widget_splitter_set_axis(struct yetty_ygui_old_widget *widget, int row)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_SPLITTER) {
        return;
    }
...
int yetty_ygui_old_widget_splitter_get_axis(const struct yetty_ygui_old_widget *widget)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_SPLITTER) {
        return -1;
    }
...
int yetty_ygui_old_widget_get_type(const struct yetty_ygui_old_widget *widget)
{
...
struct dialog_button_ud {
    struct yetty_ygui_old_widget *dialog;
    int index;
    yetty_ygui_old_dialog_button_fn cb;
    void *userdata;
};

static void dialog_button_click(struct yetty_ygui_old_widget *btn, void *u)
{
    (void)btn;
    struct dialog_button_ud *ud = (struct dialog_button_ud *)u;
    yetty_ygui_old_widget_popup_set_open(ud->dialog, 0);
    if (ud->cb) {
...
struct yetty_ygui_old_widget *yetty_ygui_old_engine_dialog(struct yetty_ygui_old_engine *engine,
                                                   const struct yetty_ygui_old_dialog_args *args)
{
    ...
    struct yetty_ygui_old_widget *dlg = yetty_ygui_old_engine_popup(engine, args->id, dlg_x, dlg_y, dlg_w,
                                                            dlg_h, args->title ? args->title : "");
    if (!dlg) {
        return NULL;
    }
    if (args->modal) {
        yetty_ygui_old_widget_popup_set_modal(dlg, 1);
    }
    ...
    struct yetty_ygui_old_widget *body = yetty_ygui_old_engine_vbox(engine, args->id, /*x*/ 16,
                                                            /*y*/ 56, dlg_w - 32, dlg_h - 72);
    yetty_ygui_old_widget_apply_css(body, "padding: 0; gap: 16; align-items: stretch;");
    yetty_ygui_old_widget_add_child(dlg, body);

    if (args->message) {
        struct yetty_ygui_old_widget *msg =
            yetty_ygui_old_engine_label(engine, args->id, 0, 0, args->message);
        yetty_ygui_old_widget_add_child(body, msg);
    }

    /* Button row, right-aligned. */
    struct yetty_ygui_old_widget *row = yetty_ygui_old_engine_hbox(engine, args->id, 0, 0, dlg_w - 32, 36);
    yetty_ygui_old_widget_apply_css(row,
                                "padding: 0; gap: 8; justify-content: end; align-items: stretch;");
    yetty_ygui_old_widget_add_child(body, row);

    for (int i = 0; i < args->button_count; i++) {
        const char *label = args->buttons[i] ? args->buttons[i] : "OK";
        struct yetty_ygui_old_widget *b =
            yetty_ygui_old_engine_button(engine, args->id, 0, 0, 88, 32, label);
        ...
        yetty_ygui_old_widget_button_on_click(b, dialog_button_click, ud);
...
static int textarea_reserve(struct yetty_ygui_old_widget *w, int new_size)
{
...
void yetty_ygui_old_internal_textarea_insert(struct yetty_ygui_old_widget *w, const char *text);
void yetty_ygui_old_internal_textarea_insert(struct yetty_ygui_old_widget *w, const char *text)
{
    if (!w || !text || w->type != YETTY_YGUI_OLD_WIDGET_TEXTAREA) {
        return;
    }
...
static void textarea_delete_back(struct yetty_ygui_old_widget *w)
{
...
static void textarea_delete_forward(struct yetty_ygui_old_widget *w)
{
    if (w->data.textarea.cursor >= w->data.textarea.length) {
        return;
    }
...
static void textarea_cursor_pos(const struct yetty_ygui_old_widget *w, int *out_line, int *out_col)
{
    int line = 0, col = 0;
...
static int textarea_line_start(const struct yetty_ygui_old_widget *w, int line)
{
    int cur = 0;
    const char *t = w->data.textarea.text;
    int len = w->data.textarea.length;
...
static int textarea_line_len(const struct yetty_ygui_old_widget *w, int line)
{
...
static int textarea_total_lines(const struct yetty_ygui_old_widget *w)
{
    int lines = 1;
    const char *t = w->data.textarea.text;
    int len = w->data.textarea.length;
...
static void textarea_ensure_cursor_visible(struct yetty_ygui_old_widget *w)
{
    int line, col;
...
static int textarea_on_key(struct yetty_ygui_old_widget *self, uint32_t key, int mods,
                           ygui_event_t *out)
{
    (void)mods;
    int handled = 1;
    ...
    case TEXTAREA_KEY_ENTER:
        yetty_ygui_old_internal_textarea_insert(self, "\n");
        break;
    ...
    if (handled) {
        ...
        out->widget_id = self->id;
        out->type = YETTY_YGUI_OLD_EVENT_KEY;
...
static int textarea_on_press(struct yetty_ygui_old_widget *self, float lx, float ly, ygui_event_t *out)
{
 ...
 * self->x + self->w - PAD. */
static void textarea_paint_line(struct yetty_ygui_old_widget *self, struct yetty_ygui_old_render_ctx *ctx,
                                const char *src, int src_len, int max_cols, float row_y)
{
    ...
    tmp[n] = '\0';
    yetty_ygui_old_render_ctx_render_text(ctx, tmp, self->x + TEXTAREA_PAD, row_y, self->fg_color,
                                      ctx->theme->font_size);
}
...
static struct yetty_ycore_void_result textarea_render(struct yetty_ygui_old_widget *self,
                                                      struct yetty_ygui_old_render_ctx *ctx)
{
    const struct yetty_ygui_old_theme *t = ctx->theme;
    yetty_ygui_old_render_ctx_render_box(ctx, self->x, self->y, self->w, self->h, self->bg_color,
                                     t->radius_small);
    yetty_ygui_old_render_ctx_render_box_outline(
        ctx, self->x, self->y, self->w, self->h,
        (self->flags & YETTY_YGUI_OLD_FLAG_FOCUSED) ? self->accent_color : t->border, t->radius_small,
        1.0f);
    ...
    if (!self->data.textarea.text || self->data.textarea.length == 0) {
        ...
        if (self->flags & YETTY_YGUI_OLD_FLAG_FOCUSED) {
            yetty_ygui_old_render_ctx_render_box(ctx, self->x + TEXTAREA_PAD, self->y + TEXTAREA_PAD,
    ...
    if (!self->data.textarea.wrap && (self->flags & YETTY_YGUI_OLD_FLAG_FOCUSED)) {
        int cline, ccol;
        ...
        if (cline >= first && cline < last) {
            ...
            yetty_ygui_old_render_ctx_render_box(ctx, cx, cy, 2.0f, TEXTAREA_LINE_HEIGHT,
                                             self->fg_color, 1.0f);
        }
    }
    return YETTY_OK_VOID();
...
static void textarea_destroy(struct yetty_ygui_old_widget *self)
{
    free(self->data.textarea.text);
    self->data.textarea.text = NULL;
}

struct yetty_ygui_old_widget *yetty_ygui_old_engine_textarea(struct yetty_ygui_old_engine *engine,
                                                     const char *id, float x, float y, float w,
                                                     float h, const char *initial_text)
{
    struct yetty_ygui_old_widget *ta =
        yetty_ygui_old_engine_widget_alloc(engine, YETTY_YGUI_OLD_WIDGET_TEXTAREA, id);
    if (!ta) {
        return NULL;
    }
    yetty_ygui_old_widget_init_base(ta, x, y, w, h);
    ...
    static const struct yetty_ygui_old_widget_vtable textarea_vtable = {
...
struct yetty_ygui_old_widget *yetty_ygui_old_engine_textarea_wrapped(struct yetty_ygui_old_engine *engine,
                                                             const char *id, float x, float y,
                                                             float w, float h,
                                                             const char *initial_text)
{
    struct yetty_ygui_old_widget *ta = yetty_ygui_old_engine_textarea(engine, id, x, y, w, h, initial_text);
    if (ta) {
...
void yetty_ygui_old_widget_textarea_set_wrap(struct yetty_ygui_old_widget *widget, int wrap)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_TEXTAREA) {
        return;
    }
...
void yetty_ygui_old_widget_textarea_set_text(struct yetty_ygui_old_widget *widget, const char *text)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_TEXTAREA) {
        return;
    }
...
const char *yetty_ygui_old_widget_textarea_get_text(const struct yetty_ygui_old_widget *widget)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_TEXTAREA) {
        return NULL;
    }
...
void yetty_ygui_old_widget_textarea_on_change(struct yetty_ygui_old_widget *widget, ygui_text_callback_t cb,
                                          void *userdata)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_TEXTAREA) {
        return;
...
static void scrollarea_shift_subtree_y(struct yetty_ygui_old_widget *w, float dy)
{
    w->layout_y += dy;
    for (struct yetty_ygui_old_widget *c = w->first_child; c; c = c->next_sibling) {
...
static float scrollarea_compute_content_h(const struct yetty_ygui_old_widget *self)
{
...
static float scrollarea_max_scroll(const struct yetty_ygui_old_widget *self)
{
...
void yetty_ygui_old_widget_scrollarea_scroll_to(struct yetty_ygui_old_widget *widget, float y)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_SCROLLAREA) {
        return;
    }
...
void yetty_ygui_old_widget_scrollarea_scroll_by(struct yetty_ygui_old_widget *widget, float dy)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_SCROLLAREA) {
        return;
    }
    yetty_ygui_old_widget_scrollarea_scroll_to(widget, widget->data.scrollarea.scroll_y + dy);
}

float yetty_ygui_old_widget_scrollarea_get_scroll(const struct yetty_ygui_old_widget *widget)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_SCROLLAREA) {
        return 0.0f;
    }
...
static float scrollarea_get_content(const struct yetty_ygui_old_widget *w)
{
    return scrollarea_compute_content_h(w);
}
static float scrollarea_get_viewport(const struct yetty_ygui_old_widget *w)
{
    return w->layout_h;
}
static float scrollarea_get_scroll(const struct yetty_ygui_old_widget *w)
{
    return w->data.scrollarea.scroll_y;
}
static float scrollarea_get_max_scroll(const struct yetty_ygui_old_widget *w)
{
    return scrollarea_max_scroll(w);
}
static void scrollarea_scroll_to_iface(struct yetty_ygui_old_widget *w, float y)
{
    yetty_ygui_old_widget_scrollarea_scroll_to(w, y);
}

static const struct yetty_ygui_old_scrollable *scrollarea_iface(void)
{
    static const struct yetty_ygui_old_scrollable ops = {
...
static int scrollarea_on_scroll(struct yetty_ygui_old_widget *self, float dx, float dy,
                                ygui_event_t *out)
{
    (void)dx;
    const float speed = 40.0f;
    float prev = self->data.scrollarea.scroll_y;
    yetty_ygui_old_widget_scrollarea_scroll_to(self, prev - dy * speed);
...
static struct yetty_ycore_void_result scrollarea_render(struct yetty_ygui_old_widget *self,
                                                        struct yetty_ygui_old_render_ctx *ctx)
{
...
static struct yetty_ycore_void_result scrollarea_render_all(struct yetty_ygui_old_widget *self,
                                                            struct yetty_ygui_old_render_ctx *ctx)
{
    if (!(self->flags & YETTY_YGUI_OLD_FLAG_VISIBLE)) {
        return YETTY_OK_VOID();
    ...
    /* Open the scrollarea's own group (chrome only — bg/scrollbar),
     * recurse children INSIDE the scope so they nest as CMD_GROUP
     * entities in our ygrid body. Children's figure rects are shifted
     * by -scroll via temporary effective_y mutation so they land
     * on-screen. */
    struct yetty_ygui_old_group_marker_result mr =
        yetty_ygui_old_widget_open_group(self, ctx, scrollarea_render);
    YETTY_RETURN_IF_ERR(yetty_ycore_void, mr, "scrollarea_render_all: open_group");
    ...
    for (struct yetty_ygui_old_widget *child = self->first_child; child && YETTY_IS_OK(first_err);
        ...
        child->effective_y = saved_ey - scroll;
        struct yetty_ycore_void_result r;
        if (child->vtable && child->vtable->render_all) {
            r = child->vtable->render_all(child, ctx);
        } else {
        ...
        child->effective_y = saved_ey;
        if (YETTY_IS_ERR(r)) {
            first_err = YETTY_ERR(yetty_ycore_void, "scrollarea_render_all: child", r);
        }
    }
     ...
     * AFTER render. */
    if (scroll != 0.0f) {
        for (struct yetty_ygui_old_widget *child = self->first_child; child;
             child = child->next_sibling) {
    ...
    if (YETTY_IS_ERR(cr)) {
        if (YETTY_IS_OK(first_err)) {
            first_err = YETTY_ERR(yetty_ycore_void, "scrollarea_render_all: close_group", cr);
        } else {
            yetty_ycore_error_destroy(cr.error);
        }
    }
...
struct yetty_ygui_old_widget *yetty_ygui_old_engine_scrollarea(struct yetty_ygui_old_engine *engine,
                                                       const char *id, float x, float y, float w,
                                                       float h)
{
    struct yetty_ygui_old_widget *s =
        yetty_ygui_old_engine_widget_alloc(engine, YETTY_YGUI_OLD_WIDGET_SCROLLAREA, id);
    if (!s) {
        return NULL;
    }
    yetty_ygui_old_widget_init_base(s, x, y, w, h);
    ...
    s->layout.mode = YETTY_YGUI_OLD_LAYOUT_FLEX;
    s->layout.direction = YETTY_YGUI_OLD_FLEX_COLUMN;
    s->layout.align_items = YETTY_YGUI_OLD_ALIGN_STRETCH;
    s->layout.gap = 4.0f;
    static const struct yetty_ygui_old_widget_vtable scrollarea_vtable = {
...
static struct yetty_ycore_void_result toggle_render(struct yetty_ygui_old_widget *self,
                                                    struct yetty_ygui_old_render_ctx *ctx)
{
    const struct yetty_ygui_old_theme *t = ctx->theme;
    ...
    yetty_ygui_old_render_ctx_render_box(ctx, pill_x, pill_y, pill_w, pill_h, track, pill_h * 0.5f);
    ...
    yetty_ygui_old_render_ctx_render_box_outline(ctx, pill_x, pill_y, pill_w, pill_h, border,
                                             pill_h * 0.5f, 1.5f);
    ...
    yetty_ygui_old_render_ctx_render_box(ctx, thumb_x, thumb_y, thumb_d, thumb_d, thumb_col,
                                     thumb_d * 0.5f);

    if (self->data.toggle.label) {
        yetty_ygui_old_render_ctx_render_text(ctx, self->data.toggle.label,
...
static int toggle_on_release(struct yetty_ygui_old_widget *self, float lx, float ly, ygui_event_t *out)
{
    if (lx < 0 || lx >= self->w || ly < 0 || ly >= self->h) {
        return 0;
    }
...
static void toggle_destroy(struct yetty_ygui_old_widget *self)
{
    free(self->data.toggle.label);
}

struct yetty_ygui_old_widget *yetty_ygui_old_engine_toggle(struct yetty_ygui_old_engine *engine, const char *id,
                                                   float x, float y, float w, float h,
                                                   const char *label, int on)
{
    struct yetty_ygui_old_widget *t =
        yetty_ygui_old_engine_widget_alloc(engine, YETTY_YGUI_OLD_WIDGET_TOGGLE, id);
    if (!t) {
        return NULL;
    }
    yetty_ygui_old_widget_init_base(t, x, y, w, h);
    t->data.toggle.on = on ? 1 : 0;
    t->data.toggle.label = ygui_strdup(label);
    static const struct yetty_ygui_old_widget_vtable toggle_vtable = {
...
void yetty_ygui_old_widget_toggle_set_on(struct yetty_ygui_old_widget *widget, int on)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_TOGGLE) {
        return;
    }
...
int yetty_ygui_old_widget_toggle_get_on(const struct yetty_ygui_old_widget *widget)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_TOGGLE) {
        return 0;
    }
...
void yetty_ygui_old_widget_toggle_on_change(struct yetty_ygui_old_widget *widget, ygui_check_callback_t cb,
                                        void *userdata)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_TOGGLE) {
        return;
...
static float chip_close_w(const struct yetty_ygui_old_widget *self)
{
    return self->data.chip.closable ? self->h : 0.0f;
}

static struct yetty_ycore_void_result chip_render(struct yetty_ygui_old_widget *self,
                                                  struct yetty_ygui_old_render_ctx *ctx)
{
    const struct yetty_ygui_old_theme *t = ctx->theme;
    yetty_ygui_old_render_ctx_render_box(ctx, self->x, self->y, self->w, self->h, t->bg_secondary,
                                     self->h * 0.5f);
    if (self->data.chip.label) {
        yetty_ygui_old_render_ctx_render_text(ctx, self->data.chip.label, self->x + self->h * 0.5f,
    ...
    if (self->data.chip.closable) {
        ...
        float s = cw * 0.25f;
        yetty_ygui_old_render_ctx_render_box(ctx, cx - s, cy - 1.0f, s * 2, 2.0f, self->fg_color, 1);
        yetty_ygui_old_render_ctx_render_box(ctx, cx - 1.0f, cy - s, 2.0f, s * 2, self->fg_color, 1);
    }
    return YETTY_OK_VOID();
}

static int chip_on_release(struct yetty_ygui_old_widget *self, float lx, float ly, ygui_event_t *out)
{
    if (!self->data.chip.closable) {
        return 0;
    }
...
static void chip_destroy(struct yetty_ygui_old_widget *self)
{
    free(self->data.chip.label);
}

struct yetty_ygui_old_widget *yetty_ygui_old_engine_chip(struct yetty_ygui_old_engine *engine, const char *id,
                                                 float x, float y, float w, float h,
                                                 const char *label, int closable)
{
    struct yetty_ygui_old_widget *c =
        yetty_ygui_old_engine_widget_alloc(engine, YETTY_YGUI_OLD_WIDGET_CHIP, id);
    if (!c) {
        return NULL;
    }
    yetty_ygui_old_widget_init_base(c, x, y, w, h);
    ...
    static const struct yetty_ygui_old_widget_vtable chip_vtable = {
        .render = chip_render,
...
void yetty_ygui_old_widget_chip_on_remove(struct yetty_ygui_old_widget *widget, ygui_click_callback_t cb,
                                      void *userdata)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_CHIP) {
        return;
    }
...
static int breadcrumbs_seg_at(const struct yetty_ygui_old_widget *self, float lx)
{
    if (lx < 0) {
        return -1;
    }
...
static struct yetty_ycore_void_result breadcrumbs_render(struct yetty_ygui_old_widget *self,
                                                         struct yetty_ygui_old_render_ctx *ctx)
{
    const struct yetty_ygui_old_theme *t = ctx->theme;
    float cursor = self->x;
    for (int i = 0; i < self->data.breadcrumbs.n; i++) {
        ...
        yetty_ygui_old_render_ctx_render_text(ctx, label, cursor, self->y + t->pad_small, col,
                                          t->font_size);
        cursor += strlen(label) * BREADCRUMB_CHAR_W;
        if (i + 1 < self->data.breadcrumbs.n) {
            yetty_ygui_old_render_ctx_render_text(ctx, BREADCRUMB_SEP, cursor, self->y + t->pad_small,
...
static int breadcrumbs_on_release(struct yetty_ygui_old_widget *self, float lx, float ly,
                                  ygui_event_t *out)
{
    (void)ly;
...
static void breadcrumbs_destroy(struct yetty_ygui_old_widget *self)
{
    for (int i = 0; i < self->data.breadcrumbs.n; i++) {
...
struct yetty_ygui_old_widget *yetty_ygui_old_engine_breadcrumbs(struct yetty_ygui_old_engine *engine,
                                                        const char *id, float x, float y, float w,
                                                        float h, const char *const *labels, int n)
{
    struct yetty_ygui_old_widget *b =
        yetty_ygui_old_engine_widget_alloc(engine, YETTY_YGUI_OLD_WIDGET_BREADCRUMBS, id);
    if (!b) {
        return NULL;
    }
    yetty_ygui_old_widget_init_base(b, x, y, w, h);
    ...
    static const struct yetty_ygui_old_widget_vtable breadcrumbs_vtable = {
...
void yetty_ygui_old_widget_breadcrumbs_on_change(struct yetty_ygui_old_widget *widget,
                                             ygui_change_callback_t cb, void *userdata)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_BREADCRUMBS) {
        return;
    }
    widget->change_callback = cb;
    widget->change_userdata = userdata;
...
void yetty_ygui_old_widget_progress_set_indeterminate(struct yetty_ygui_old_widget *widget,
                                                  int indeterminate)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_PROGRESS) {
        return;
...
void yetty_ygui_old_widget_set_context_menu(struct yetty_ygui_old_widget *widget,
...
static float combo_arrow_w(const struct yetty_ygui_old_widget *self)
{
    return self->h;
}

static struct yetty_ycore_void_result combo_render(struct yetty_ygui_old_widget *self,
                                                   struct yetty_ygui_old_render_ctx *ctx)
{
    const struct yetty_ygui_old_theme *t = ctx->theme;
    yetty_ygui_old_render_ctx_render_box(ctx, self->x, self->y, self->w, self->h, self->bg_color,
    ...
    yetty_ygui_old_render_ctx_render_text(ctx, txt, self->x + 8.0f,
                                      self->y + (self->h - t->font_size) * 0.5f, self->fg_color,
                                      t->font_size);
    /* Arrow button */
    float aw = combo_arrow_w(self);
    float ax = self->x + self->w - aw;
    yetty_ygui_old_render_ctx_render_box(ctx, ax, self->y, aw, self->h, t->bg_secondary,
                                     t->radius_small);
    float cx = ax + aw * 0.5f;
    float cy = self->y + self->h * 0.5f;
    yetty_ygui_old_render_ctx_render_triangle(ctx, cx - 5, cy - 3, cx + 5, cy - 3, cx, cy + 4,
                                          self->fg_color);
    /* Dropdown list (when open) */
    if (self->data.combo.dropdown_open && self->data.combo.option_count > 0) {
        ...
        yetty_ygui_old_render_ctx_render_box(ctx, self->x, dy, self->w, dh, t->bg_dropdown,
                                         t->radius_small);
        yetty_ygui_old_render_ctx_render_box_outline(ctx, self->x, dy, self->w, dh, t->border,
                                                 t->radius_small, 1.0f);
        for (int i = 0; i < self->data.combo.option_count; i++) {
            yetty_ygui_old_render_ctx_render_text(ctx, self->data.combo.options[i], self->x + 8.0f,
...
static int combo_on_release(struct yetty_ygui_old_widget *self, float lx, float ly, ygui_event_t *out)
{
    float aw = combo_arrow_w(self);
...
static void combo_destroy(struct yetty_ygui_old_widget *self)
{
    free(self->data.combo.text);
    for (int i = 0; i < self->data.combo.option_count; i++) {
...
struct yetty_ygui_old_widget *yetty_ygui_old_engine_combo(struct yetty_ygui_old_engine *engine, const char *id,
                                                  float x, float y, float w, float h,
                                                  const char *initial_text,
                                                  const char *const *options, int option_count)
{
    struct yetty_ygui_old_widget *c =
        yetty_ygui_old_engine_widget_alloc(engine, YETTY_YGUI_OLD_WIDGET_COMBO, id);
    if (!c) {
        return NULL;
    }
    yetty_ygui_old_widget_init_base(c, x, y, w, h);
    ...
    static const struct yetty_ygui_old_widget_vtable combo_vtable = {
        .render = combo_render,
...
void yetty_ygui_old_widget_combo_set_text(struct yetty_ygui_old_widget *widget, const char *text)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_COMBO) {
        return;
    }
...
const char *yetty_ygui_old_widget_combo_get_text(const struct yetty_ygui_old_widget *widget)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_COMBO) {
        return NULL;
    }
...
void yetty_ygui_old_widget_combo_on_change(struct yetty_ygui_old_widget *widget, ygui_text_callback_t cb,
                                       void *userdata)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_COMBO) {
        return;
...
static float menubar_item_w(const struct yetty_ygui_old_widget *self, int i)
{
...
static int menubar_item_at(const struct yetty_ygui_old_widget *self, float lx)
{
    float cx = 0;
...
static struct yetty_ycore_void_result menubar_render(struct yetty_ygui_old_widget *self,
                                                     struct yetty_ygui_old_render_ctx *ctx)
{
    const struct yetty_ygui_old_theme *t = ctx->theme;
    yetty_ygui_old_render_ctx_render_box(ctx, self->x, self->y, self->w, self->h, t->bg_header,
                                     t->radius_small);
    float cx = self->x;
    for (int i = 0; i < self->data.menubar.n; i++) {
        float w = menubar_item_w(self, i);
        const char *l = self->data.menubar.labels[i];
        yetty_ygui_old_render_ctx_render_text(ctx, l ? l : "", cx + MENUBAR_ITEM_PAD_X,
                                          self->y + (self->h - t->font_size) * 0.5f, self->fg_color,
                                          t->font_size);
        cx += w;
    }
...
static int menubar_on_release(struct yetty_ygui_old_widget *self, float lx, float ly, ygui_event_t *out)
{
    (void)ly;
    ...
    struct yetty_ygui_old_widget *clicked = self->data.menubar.menus[idx];
    int was_open = yetty_ygui_old_widget_popup_menu_is_open(clicked);
    ...
    for (int i = 0; i < self->data.menubar.n; i++) {
        if (self->data.menubar.menus[i] && self->data.menubar.menus[i] != clicked) {
            yetty_ygui_old_widget_popup_menu_close(self->data.menubar.menus[i]);
        }
    }
    if (was_open) {
        /* Clicking the same menu button again closes it. */
        yetty_ygui_old_widget_popup_menu_close(clicked);
    } else {
        float cx = self->layout_x;
        for (int i = 0; i < idx; i++) {
            cx += menubar_item_w(self, i);
        }
        yetty_ygui_old_widget_popup_menu_open_at(clicked, cx, self->layout_y + self->layout_h);
    }
    out->widget_id = self->id;
    out->type = YETTY_YGUI_OLD_EVENT_CHANGE;
    out->data.int_value = idx;
...
static void menubar_destroy(struct yetty_ygui_old_widget *self)
{
    for (int i = 0; i < self->data.menubar.n; i++) {
...
struct yetty_ygui_old_widget *yetty_ygui_old_engine_menubar(struct yetty_ygui_old_engine *engine,
                                                    const char *id, float x, float y, float w,
                                                    float h)
{
    struct yetty_ygui_old_widget *m =
        yetty_ygui_old_engine_widget_alloc(engine, YETTY_YGUI_OLD_WIDGET_MENUBAR, id);
    if (!m) {
        return NULL;
    }
    yetty_ygui_old_widget_init_base(m, x, y, w, h);
    static const struct yetty_ygui_old_widget_vtable menubar_vtable = {
...
void yetty_ygui_old_widget_menubar_add(struct yetty_ygui_old_widget *menubar, const char *label,
                                   struct yetty_ygui_old_widget *menu)
{
    if (!menubar || menubar->type != YETTY_YGUI_OLD_WIDGET_MENUBAR) {
        return;
    }
    if (menubar->data.menubar.n == menubar->data.menubar.capacity) {
        ...
        struct yetty_ygui_old_widget **gm =
            realloc(menubar->data.menubar.menus, (size_t)nc * sizeof(struct yetty_ygui_old_widget *));
        if (!gl || !gm) {
            free(gl);
            free(gm);
            return;
...
static struct yetty_ycore_void_result stepper_render(struct yetty_ygui_old_widget *self,
                                                     struct yetty_ygui_old_render_ctx *ctx)
{
    const struct yetty_ygui_old_theme *t = ctx->theme;
    ...
    for (int i = 0; i < n; i++) {
        ...
        yetty_ygui_old_render_ctx_render_circle(ctx, cx, cy, STEPPER_CIRCLE_R, fill);
        if (current) {
            yetty_ygui_old_render_ctx_render_circle_outline(ctx, cx, cy, STEPPER_CIRCLE_R + 2,
                                                        self->accent_color, 2.0f);
        }
        /* Step number */
        char buf[8];
        snprintf(buf, sizeof(buf), "%d", i + 1);
        yetty_ygui_old_render_ctx_render_text(ctx, buf, cx - 4, cy - t->font_size * 0.5f + 2,
        ...
        if (i + 1 < n) {
            ...
            float lw = gap;
            yetty_ygui_old_render_ctx_render_box(ctx, lx, cy - 1, lw, 2, line_col, 0);
        }
...
static void stepper_destroy(struct yetty_ygui_old_widget *self)
{
...
struct yetty_ygui_old_widget *yetty_ygui_old_engine_stepper(struct yetty_ygui_old_engine *engine,
                                                    const char *id, float x, float y, float w,
                                                    float h, const char *const *labels, int n_steps)
{
    struct yetty_ygui_old_widget *s =
        yetty_ygui_old_engine_widget_alloc(engine, YETTY_YGUI_OLD_WIDGET_STEPPER, id);
    if (!s) {
        return NULL;
    }
    yetty_ygui_old_widget_init_base(s, x, y, w, h);
    ...
    static const struct yetty_ygui_old_widget_vtable stepper_vtable = {
...
void yetty_ygui_old_widget_stepper_set_current(struct yetty_ygui_old_widget *widget, int step)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_STEPPER) {
        return;
    }
    if (step < 0) {
        step = 0;
...
int yetty_ygui_old_widget_stepper_get_current(const struct yetty_ygui_old_widget *widget)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_STEPPER) {
        return 0;
    }
...
static struct yetty_ycore_void_result datepicker_render(struct yetty_ygui_old_widget *self,
                                                        struct yetty_ygui_old_render_ctx *ctx)
{
    const struct yetty_ygui_old_theme *t = ctx->theme;
    yetty_ygui_old_render_ctx_render_box(ctx, self->x, self->y, self->w, self->h, self->bg_color,
    ...
    /* Header: ← Month Year → */
    /* Left arrow */
    yetty_ygui_old_render_ctx_render_text(ctx, "<", self->x + DATEPICKER_PAD,
                                      self->y + (DATEPICKER_HEADER_H - t->font_size) * 0.5f,
                                      self->fg_color, t->font_size);
    /* Right arrow */
    yetty_ygui_old_render_ctx_render_text(ctx, ">", self->x + self->w - DATEPICKER_NAV_W,
                                      self->y + (DATEPICKER_HEADER_H - t->font_size) * 0.5f,
                                      self->fg_color, t->font_size);
    /* Month + year */
    char hdr[64];
    ...
    yetty_ygui_old_render_ctx_render_text(ctx, hdr, self->x + DATEPICKER_NAV_W + DATEPICKER_PAD,
                                      self->y + (DATEPICKER_HEADER_H - t->font_size) * 0.5f,
                                      self->fg_color, t->font_size);
    ...
    for (int i = 0; i < 7; i++) {
        yetty_ygui_old_render_ctx_render_text(ctx, wd[i], grid_x + i * DATEPICKER_CELL_W + 6, grid_y,
                                          t->text_muted, t->font_size);
    }
    /* Days */
    ...
    for (int d = 1; d <= dim; d++) {
        ...
        if (is_sel) {
            yetty_ygui_old_render_ctx_render_box(ctx, cx, cy, DATEPICKER_CELL_W, DATEPICKER_CELL_H,
                                             self->accent_color, t->radius_small);
        }
        char buf[8];
        snprintf(buf, sizeof(buf), "%d", d);
        yetty_ygui_old_render_ctx_render_text(ctx, buf, cx + 8, cy + 4,
                                          is_sel ? t->text_primary : self->fg_color, t->font_size);
    }
    return YETTY_OK_VOID();
}

static int datepicker_on_release(struct yetty_ygui_old_widget *self, float lx, float ly,
                                 ygui_event_t *out)
{
    /* Header arrows */
...
struct yetty_ygui_old_widget *yetty_ygui_old_engine_datepicker(struct yetty_ygui_old_engine *engine,
                                                       const char *id, float x, float y, float w,
                                                       float h)
{
    struct yetty_ygui_old_widget *d =
        yetty_ygui_old_engine_widget_alloc(engine, YETTY_YGUI_OLD_WIDGET_DATEPICKER, id);
    if (!d) {
        return NULL;
    }
    yetty_ygui_old_widget_init_base(d, x, y, w, h);
    ...
    static const struct yetty_ygui_old_widget_vtable datepicker_vtable = {
...
void yetty_ygui_old_widget_datepicker_set_date(struct yetty_ygui_old_widget *widget, int year,
                                           int month_0_based, int day)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_DATEPICKER) {
        return;
    }
...
void yetty_ygui_old_widget_datepicker_get_date(const struct yetty_ygui_old_widget *widget, int *year,
                                           int *month_0_based, int *day)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_DATEPICKER) {
        if (year) {
...
void yetty_ygui_old_widget_datepicker_on_change(struct yetty_ygui_old_widget *widget,
                                            ygui_change_callback_t cb, void *userdata)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_DATEPICKER) {
        return;
...
static void filepicker_load_dir(struct yetty_ygui_old_widget *self)
{
...
static struct yetty_ycore_void_result filepicker_render(struct yetty_ygui_old_widget *self,
                                                        struct yetty_ygui_old_render_ctx *ctx)
{
    const struct yetty_ygui_old_theme *t = ctx->theme;
    yetty_ygui_old_render_ctx_render_box(ctx, self->x, self->y, self->w, self->h, self->bg_color,
    ...
    yetty_ygui_old_render_ctx_render_box_outline(ctx, self->x, self->y, self->w, self->h, t->border,
                                             t->radius_small, 1.0f);
    /* Header: current path */
    yetty_ygui_old_render_ctx_render_box(ctx, self->x, self->y, self->w, FILEPICKER_HEADER_H,
                                     t->bg_header, t->radius_small);
    if (self->data.filepicker.cwd) {
        yetty_ygui_old_render_ctx_render_text(ctx, self->data.filepicker.cwd, self->x + FILEPICKER_PAD,
    ...
    for (int i = first; i < last; i++) {
        ...
        if (i == self->data.filepicker.selected) {
            yetty_ygui_old_render_ctx_render_box(ctx, self->x + 2, ry, self->w - 4, FILEPICKER_ROW_H,
                                             t->selection_bg, 0);
        }
        const char *e = self->data.filepicker.entries[i];
        uint32_t col = (e && strchr(e, '/')) ? self->accent_color : self->fg_color;
        yetty_ygui_old_render_ctx_render_text(ctx, e ? e : "", self->x + FILEPICKER_PAD,
...
static int filepicker_on_scroll(struct yetty_ygui_old_widget *self, float dx, float dy,
                                ygui_event_t *out)
{
    (void)dx;
    int delta = dy > 0 ? -1 : 1;
...
static int filepicker_on_release(struct yetty_ygui_old_widget *self, float lx, float ly,
                                 ygui_event_t *out)
{
    (void)lx;
...
static void filepicker_destroy(struct yetty_ygui_old_widget *self)
{
    free(self->data.filepicker.cwd);
    for (int i = 0; i < self->data.filepicker.entry_count; i++) {
        free(self->data.filepicker.entries[i]);
...
struct yetty_ygui_old_widget *yetty_ygui_old_engine_filepicker(struct yetty_ygui_old_engine *engine,
                                                       const char *id, float x, float y, float w,
                                                       float h, const char *initial_dir)
{
    struct yetty_ygui_old_widget *f =
        yetty_ygui_old_engine_widget_alloc(engine, YETTY_YGUI_OLD_WIDGET_FILEPICKER, id);
    if (!f) {
        return NULL;
    }
    yetty_ygui_old_widget_init_base(f, x, y, w, h);
    f->data.filepicker.cwd = ygui_strdup(initial_dir && *initial_dir ? initial_dir : "/");
    f->data.filepicker.selected = -1;
    filepicker_load_dir(f);
    static const struct yetty_ygui_old_widget_vtable filepicker_vtable = {
...
const char *yetty_ygui_old_widget_filepicker_get_cwd(const struct yetty_ygui_old_widget *widget)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_FILEPICKER) {
        return NULL;
    }
    return widget->data.filepicker.cwd;
}

const char *yetty_ygui_old_widget_filepicker_get_selected(const struct yetty_ygui_old_widget *widget)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_FILEPICKER) {
        return NULL;
    }
...
void yetty_ygui_old_widget_filepicker_on_change(struct yetty_ygui_old_widget *widget,
...
static struct yetty_ycore_void_result statusbar_render(struct yetty_ygui_old_widget *self,
                                                       struct yetty_ygui_old_render_ctx *ctx)
{
    const struct yetty_ygui_old_theme *t = ctx->theme;
    yetty_ygui_old_render_ctx_render_box(ctx, self->x, self->y, self->w, self->h, t->bg_header, 0.0f);
    /* Top separator line for visual delineation from the body. */
    yetty_ygui_old_render_ctx_render_box(ctx, self->x, self->y, self->w, 1.0f, t->border_muted, 0.0f);
    float ty = self->y + (self->h - t->font_size) * 0.5f;
    if (self->data.statusbar.left_text) {
        yetty_ygui_old_render_ctx_render_text(ctx, self->data.statusbar.left_text,
    ...
    if (self->data.statusbar.right_text) {
        ...
        yetty_ygui_old_render_ctx_render_text(ctx, self->data.statusbar.right_text, tx, ty,
                                          t->text_muted, t->font_size);
    }
    return YETTY_OK_VOID();
}

static void statusbar_destroy(struct yetty_ygui_old_widget *self)
{
    free(self->data.statusbar.left_text);
    free(self->data.statusbar.right_text);
}

struct yetty_ygui_old_widget *yetty_ygui_old_engine_statusbar(struct yetty_ygui_old_engine *engine,
                                                      const char *id, float x, float y, float w,
                                                      float h, const char *left_text)
{
    struct yetty_ygui_old_widget *s =
        yetty_ygui_old_engine_widget_alloc(engine, YETTY_YGUI_OLD_WIDGET_STATUSBAR, id);
    if (!s) {
        return NULL;
    }
    yetty_ygui_old_widget_init_base(s, x, y, w, h);
    ...
    static const struct yetty_ygui_old_widget_vtable statusbar_vtable = {
...
void yetty_ygui_old_widget_statusbar_set_left(struct yetty_ygui_old_widget *widget, const char *text)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_STATUSBAR) {
        return;
    }
...
void yetty_ygui_old_widget_statusbar_set_right(struct yetty_ygui_old_widget *widget, const char *text)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_STATUSBAR) {
        return;
    }
...
void yetty_ygui_old_engine_set_titlebar(struct yetty_ygui_old_engine *engine,
                                    struct yetty_ygui_old_widget *widget)
{
    if (!engine) {
        return;
...
void yetty_ygui_old_engine_set_menubar(struct yetty_ygui_old_engine *engine,
                                   struct yetty_ygui_old_widget *widget)
{
    if (!engine) {
        return;
...
void yetty_ygui_old_engine_set_statusbar(struct yetty_ygui_old_engine *engine,
                                     struct yetty_ygui_old_widget *widget)
{
    if (!engine) {
        return;
 ...
 * canvas. */
void yetty_ygui_old_internal_engine_pin_bars(struct yetty_ygui_old_engine *engine);
void yetty_ygui_old_internal_engine_pin_bars(struct yetty_ygui_old_engine *engine)
{
    if (!engine) {
        return;
    ...
    if (engine->engine_titlebar) {
        struct yetty_ygui_old_widget *w = engine->engine_titlebar;
    ...
    if (engine->engine_menubar) {
        struct yetty_ygui_old_widget *w = engine->engine_menubar;
    ...
    if (engine->engine_statusbar) {
        struct yetty_ygui_old_widget *w = engine->engine_statusbar;
...
static void window_detach_slot(struct yetty_ygui_old_widget *window, struct yetty_ygui_old_widget **slot)
{
    if (!*slot) {
        return;
    }
    yetty_ygui_old_widget_remove_child(window, *slot);
...
void yetty_ygui_old_widget_window_set_menubar(struct yetty_ygui_old_widget *window,
                                          struct yetty_ygui_old_widget *menubar)
{
    ...
    struct yetty_ygui_old_widget *body = window->data.window.body;
    struct yetty_ygui_old_widget *sb = window->data.window.statusbar;
    if (body) {
        yetty_ygui_old_widget_remove_child(window, body);
    }
    if (sb) {
        yetty_ygui_old_widget_remove_child(window, sb);
    }
    if (menubar->parent) {
        yetty_ygui_old_widget_remove_child(menubar->parent, menubar);
    }
    yetty_ygui_old_widget_add_child(window, menubar);
    if (body) {
        yetty_ygui_old_widget_add_child(window, body);
    }
    if (sb) {
        yetty_ygui_old_widget_add_child(window, sb);
    }
...
void yetty_ygui_old_widget_window_set_statusbar(struct yetty_ygui_old_widget *window,
                                            struct yetty_ygui_old_widget *statusbar)
{
    if (!window || window->type != YETTY_YGUI_OLD_WIDGET_WINDOW) {
        return;
    }
    ...
    if (statusbar->parent) {
        yetty_ygui_old_widget_remove_child(statusbar->parent, statusbar);
    }
    yetty_ygui_old_widget_add_child(window, statusbar);
