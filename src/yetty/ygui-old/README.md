...
- **Layout pass** — flexbox-style geometry resolution that runs **before**
  ...
  `struct yetty_ygui_old_layout` and writes resolved `layout_x/y/w/h` (absolute
...
int main(void) {
    ...
    struct ygui_engine_ptr_result r =
        yetty_ygui_old_engine_create("hello", 0, 0, /*cols*/ 80, /*rows*/ 24);
    if (YETTY_IS_ERR(r)) { return 1; }
    struct yetty_ygui_old_engine *engine = r.value;

    struct yetty_ygui_old_widget *btn =
        yetty_ygui_old_engine_button(engine, "ok", 20, 20, 120, 32, "OK");
    yetty_ygui_old_widget_button_on_click(btn, /* callback */ NULL, NULL);

    yetty_ygui_old_engine_show(engine);   /* creates the card on yetty */
    yetty_ygui_old_engine_run(engine);    /* libuv loop, blocks until stop */
...
All public symbols live in `include/yetty/ygui-old/ygui.h`.
...
- `yetty_ygui_old_widget_add_child(parent, child)` — moves `child` from the
  engine's top-level list into `parent->first_child`.
- `yetty_ygui_old_widget_remove_child` / `yetty_ygui_old_widget_remove` — detach
  or destroy.
...
Per-widget `struct yetty_ygui_old_layout` fields:
...
```c
yetty_ygui_old_widget_set_layout_mode(w, YETTY_YGUI_OLD_LAYOUT_FLEX);
yetty_ygui_old_widget_set_flex_direction(w, YETTY_YGUI_OLD_FLEX_ROW);
yetty_ygui_old_widget_set_flex_wrap(w, YETTY_YGUI_OLD_FLEX_WRAP);
yetty_ygui_old_widget_set_justify_content(w, YETTY_YGUI_OLD_JUSTIFY_SPACE_BETWEEN);
yetty_ygui_old_widget_set_align_items(w, YETTY_YGUI_OLD_ALIGN_STRETCH);
yetty_ygui_old_widget_set_align_self(child, YETTY_YGUI_OLD_ALIGN_CENTER);
yetty_ygui_old_widget_set_align_content(w, YETTY_YGUI_OLD_ALIGN_STRETCH);
yetty_ygui_old_widget_set_position_mode(child, YETTY_YGUI_OLD_POSITION_ABSOLUTE);
yetty_ygui_old_widget_set_flex(child, /*grow*/ 1, /*shrink*/ 0, /*basis*/ 0);
yetty_ygui_old_widget_set_flex_basis_percent(child, 25.0f);
yetty_ygui_old_widget_set_size_percent(child, /*w%*/ 50, /*h%*/ 0);
yetty_ygui_old_widget_set_min_size_percent(child, 25, 0);
yetty_ygui_old_widget_set_max_size_percent(child, 75, 0);
yetty_ygui_old_widget_set_gap(w, 12.0f);
yetty_ygui_old_widget_set_padding(w, top, right, bottom, left);
yetty_ygui_old_widget_set_margin(w, top, right, bottom, left);
yetty_ygui_old_widget_set_min_size(w, min_w, min_h);
yetty_ygui_old_widget_set_max_size(w, max_w, max_h);
```
...
yetty_ygui_old_widget_apply_css(child,
...
3. **Layout pass** — `yetty_ygui_old_layout_compute_engine` walks the tree,
...
`yetty_ygui_old_engine_layout(engine)` runs only step 3 — useful in tests and
...
```c
void yetty_ygui_old_engine_set_size(struct yetty_ygui_old_engine *e, float w, float h);
void yetty_ygui_old_engine_get_size(const struct yetty_ygui_old_engine *e,
                                 float *w, float *h);
void yetty_ygui_old_engine_on_resize(struct yetty_ygui_old_engine *e,
...
```c
void on_resize(struct yetty_ygui_old_engine *e,
...
```c
struct yetty_ycore_void_result r = yetty_ygui_old_engine_render(engine);
if (YETTY_IS_ERR(r)) {
...
Public API: `include/yetty/ygui-old/ygui.h`. Don't include `ygui_internal.h`
...
```c
struct yetty_ygui_old_widget *list =
    yetty_ygui_old_engine_list(engine, "left-pane", 8, 8, 240, 600);
yetty_ygui_old_widget_apply_css(list, "padding: 6px; gap: 2px;");
yetty_ygui_old_widget_list_on_select(list, on_row_click, NULL);

yetty_ygui_old_widget_add_child(list, yetty_ygui_old_engine_button(engine, "ok",  0, 0, 0, 32, "OK"));
yetty_ygui_old_widget_add_child(list, yetty_ygui_old_engine_label (engine, "msg", 0, 0, "or click me"));
```
...
- The auto-allocated children list is a regular `list` widget — get it
  with `yetty_ygui_old_widget_tree_node_children(node)` and add anything
...
```c
struct yetty_ygui_old_widget *tree =
    yetty_ygui_old_engine_list(engine, "tree", 8, 8, 300, 600);

struct yetty_ygui_old_widget *src = yetty_ygui_old_engine_tree_node(engine, "src",  "src/");
yetty_ygui_old_widget_add_child(tree, src);

struct yetty_ygui_old_widget *yetty_ = yetty_ygui_old_engine_tree_node(engine, "yetty", "yetty/");
yetty_ygui_old_widget_add_child(yetty_ygui_old_widget_tree_node_children(src), yetty_);

yetty_ygui_old_widget_add_child(yetty_ygui_old_widget_tree_node_children(yetty_),
                             yetty_ygui_old_engine_label(engine, "main_c", 0, 0, "main.c"));

/* Customise the indent and row spacing on this branch only: */
yetty_ygui_old_widget_apply_css(yetty_ygui_old_widget_tree_node_children(src),
                             "padding-left: 32px; gap: 1px;");

yetty_ygui_old_widget_tree_node_set_expanded(src, 1);   /* open by default */
yetty_ygui_old_widget_tree_node_on_toggle(src, on_folder_toggle, NULL);
```
...
1. Pick a `ygui_widget_type_t` value (`include/yetty/ygui-old/ygui.h`) — add a
   new enum entry if needed.
2. Add a constructor `yetty_ygui_old_engine_<type>(engine, id, x, y, w, h, …)`
   that calls `yetty_ygui_old_engine_widget_alloc` + `yetty_ygui_old_widget_init_base`,
   sets `render` (and optionally `render_all`, `on_press`, …), and calls
   `add_to_engine`.
3. Implement the `render` callback. Read `self->x/y/w/h` (already in the
   right coord system after layout) and call `yetty_ygui_old_render_ctx_*`
   helpers.
4. Add type-specific data inside the existing union in
   `struct yetty_ygui_old_widget` if you need state.
