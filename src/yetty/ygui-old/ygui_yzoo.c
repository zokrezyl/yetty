...
static void yzoo_drop_producer(struct yetty_ygui_old_widget *self)
{
...
static int yzoo_ensure_attached(struct yetty_ygui_old_widget *self, float w, float h)
{
    if (w <= 0.0f || h <= 0.0f) {
        return 0;
    }
...
static int yzoo_run_one_frame(struct yetty_ygui_old_widget *self)
{
    ...
    struct yetty_ycore_void_result fl =
        yetty_ygui_old_flatten_draw_list(self->data.yzoo.flat, self->data.yzoo.raw);
    if (YETTY_IS_ERR(fl)) {
...
static struct yetty_ycore_void_result yzoo_render(struct yetty_ygui_old_widget *self,
                                                  struct yetty_ygui_old_render_ctx *ctx)
{
    ...
    return yetty_ygui_old_internal_emit_buffer_translated(ctx, self->data.yzoo.flat, 0.0f, 0.0f);
}

static void yzoo_destroy(struct yetty_ygui_old_widget *self)
{
    yzoo_drop_producer(self);
}

static struct yetty_ycore_void_result yzoo_render_all(struct yetty_ygui_old_widget *self,
                                                      struct yetty_ygui_old_render_ctx *ctx)
{
    ...
    struct yetty_ygui_old_group_marker_result mr =
        yetty_ygui_old_widget_open_group_as_kind(self, ctx, YETTY_YFIGURE_KIND_YZOO, yzoo_render);
    YETTY_RETURN_IF_ERR(yetty_ycore_void, mr, "yzoo_render_all: open_group");
    struct yetty_ycore_void_result cr = yetty_ygui_old_widget_close_group(self, ctx, mr.value);
    YETTY_RETURN_IF_ERR(yetty_ycore_void, cr, "yzoo_render_all: close_group");
    return YETTY_OK_VOID();
}

static const struct yetty_ygui_old_widget_vtable *yzoo_vtable_ptr(void)
{
