...
static void yplot_invalidate_cache(struct yetty_ygui_old_widget *self)
{
...
static void yplot_free_buffers(struct yetty_ygui_old_widget *self)
{
...
static struct yetty_ydraw_draw_list *yplot_build_buffer(struct yetty_ygui_old_widget *self, float w,
                                                        float h)
{
    if (w <= 0.0f || h <= 0.0f) {
        return NULL;
...
static struct yetty_ycore_void_result yplot_render(struct yetty_ygui_old_widget *self,
                                                   struct yetty_ygui_old_render_ctx *ctx)
{
    ...
    return yetty_ygui_old_internal_emit_buffer_translated(ctx, self->data.yplot.cached, 0.0f, 0.0f);
}

static void yplot_destroy(struct yetty_ygui_old_widget *self)
{
...
static struct yetty_ycore_void_result yplot_render_all(struct yetty_ygui_old_widget *self,
                                                       struct yetty_ygui_old_render_ctx *ctx)
{
    ...
    struct yetty_ygui_old_group_marker_result mr =
        yetty_ygui_old_widget_open_group_as_kind(self, ctx, YETTY_YFIGURE_KIND_YPLOT, yplot_render);
    YETTY_RETURN_IF_ERR(yetty_ycore_void, mr, "yplot_render_all: open_group");
    struct yetty_ycore_void_result cr = yetty_ygui_old_widget_close_group(self, ctx, mr.value);
    YETTY_RETURN_IF_ERR(yetty_ycore_void, cr, "yplot_render_all: close_group");
    return YETTY_OK_VOID();
}

static const struct yetty_ygui_old_widget_vtable *yplot_vtable_ptr(void)
{
...
static int yplot_set_source_locked(struct yetty_ygui_old_widget *widget, const char *source,
                                   size_t source_len)
{
    if (!source) {
...
static int yplot_set_buffers_locked(struct yetty_ygui_old_widget *widget,
...
{
    struct yetty_ygui_old_widget *widget = yplot_alloc(engine, id, x, y, w, h);
    if (!widget) {
        return NULL;
    }
    if (config) {
...
struct yetty_ycore_void_result yetty_ygui_old_widget_yplot_set_buffers(
    ...
    const struct yetty_yplot_render_config *config)
{
    if (!widget || widget->type != YETTY_YGUI_OLD_WIDGET_YPLOT) {
