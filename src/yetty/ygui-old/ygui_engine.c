...
static void handle_resize(struct yetty_ygui_old_engine *engine);
...
volatile int yetty_ygui_old_internal_resize_pending = 0;
struct yetty_ygui_old_engine *yetty_ygui_old_internal_active_engine = NULL; /* For resize handler */
...
int yetty_ygui_old_init(void)
{
    ...
    if (tcgetattr(STDIN_FILENO, &ygui_orig_termios) < 0) {
        yetty_ygui_old_set_error("Failed to get terminal attributes");
        return -1;
    }
    ...
    if (tcsetattr(STDIN_FILENO, TCSANOW, &raw) < 0) {
        yetty_ygui_old_set_error("Failed to set raw terminal mode");
        return -1;
    }
    ygui_raw_mode = 1;
...
void yetty_ygui_old_set_error(const char *msg)
{
    if (msg) {
...
{
    /* Pinned to the enum in include/yetty/ygui-old/ygui.h — if someone bumps
    ...
    _Static_assert(YETTY_YGUI_OLD_VERSION_MAJOR == 0 && YETTY_YGUI_OLD_VERSION_MINOR == 2 &&
                                                       ...
                                                       struct yetty_ygui_old_theme *theme)
{
    struct yetty_ygui_old_engine *engine =
        (struct yetty_ygui_old_engine *)calloc(1, sizeof(struct yetty_ygui_old_engine));
    if (!engine) {
        yetty_ygui_old_set_error("Failed to allocate engine");
    ...
    if (!YETTY_IS_OK(br)) {
        yetty_ygui_old_set_error("Failed to create ydraw buffer");
        free(engine);
    ...
    if (theme) {
        ...
        engine->theme = yetty_ygui_old_theme_create_default();
        engine->owns_theme = 1;
    }
    ...
    engine->canvas_mode = YETTY_YGUI_OLD_CANVAS_FIT;
    engine->scale_mode = YETTY_YGUI_OLD_SCALE_OFF;
     ...
     * happen before the child has even called yetty_ygui_old_init (and so
    ...
    yetty_ygui_old_grid_init(&engine->grid, 1.0f, 1.0f, 1.0f);

    return YETTY_OK(ygui_engine_ptr, engine);
}

struct ygui_engine_ptr_result yetty_ygui_old_engine_internal_alloc_for_yui(
...
struct yetty_ycore_void_result yetty_ygui_old_engine_destroy(struct yetty_ygui_old_engine *engine)
{
    ...
    if (engine->clicks_subscribed) {
        struct yetty_ycore_void_result r =
            yetty_ygui_old_osc_subscribe_clicks(engine->output_pty, 0);
        if (YETTY_IS_ERR(r)) {
    ...
    if (engine->moves_subscribed) {
        struct yetty_ycore_void_result r =
            yetty_ygui_old_osc_subscribe_moves(engine->output_pty, 0);
        if (YETTY_IS_ERR(r)) {
    ...
    if (engine->card_shown && engine->buffer) {
        ...
        if (YETTY_IS_ERR(zr)) {
            ...
            if (size > 0 && data) {
                struct yetty_ycore_void_result wr = yetty_ygui_old_osc_update_card(
    ...
    if (engine->card_shown && engine->card_name && !engine->preserve_canvas_on_destroy) {
        struct yetty_ycore_void_result r =
            yetty_ygui_old_osc_kill_card(engine->output_pty, engine->card_name);
        if (YETTY_IS_ERR(r)) {
    ...
    if (engine->figure_id != 0) {
        struct yetty_ycore_void_result r =
            yetty_ygui_old_osc_card_remove(engine->output_pty, engine->figure_id);
        if (YETTY_IS_ERR(r)) {
            first_err = r;
        }
    }
     ...
     * via yetty_ygui_old_engine_attach. ygui-core-only consumers have NULL
    ...
    yetty_ygui_old_engine_notify_shutdown(engine);

    /* Destroy all widgets */
    struct yetty_ygui_old_widget *w = engine->first_widget;
    while (w) {
        struct yetty_ygui_old_widget *next = w->next_sibling;
        yetty_ygui_old_widget_free(w);
        w = next;
    }

    /* Destroy grid */
    yetty_ygui_old_grid_destroy(&engine->grid);

    /* Destroy theme if owned */
    if (engine->owns_theme && engine->theme) {
        yetty_ygui_old_theme_destroy(engine->theme);
    }
...
void yetty_ygui_old_engine_set_size(struct yetty_ygui_old_engine *engine, float width, float height)
{
    ...
    yetty_ygui_old_grid_destroy(&engine->grid);
    yetty_ygui_old_grid_init(&engine->grid, width, height, calc_grid_bucket_size(width, height));
    engine->dirty = 1;
...
static void reset_was_rendered_recursive(struct yetty_ygui_old_widget *w)
{
    if (!w) {
        return;
    }
...
static struct yetty_ycore_void_result engine_rebuild_with_mode(struct yetty_ygui_old_engine *engine,
                                                               int full_redraw)
{
    struct yetty_ycore_void_result first_err = YETTY_OK_VOID();
    ...
    /* Clear the grid */
    yetty_ygui_old_grid_clear(&engine->grid);
    ...
    {
        struct yetty_ycore_void_result lr = yetty_ygui_old_layout_compute_engine(engine);
        if (YETTY_IS_ERR(lr)) {
     ...
     * the render pass. */
    for (struct yetty_ygui_old_widget *w = engine->first_widget; w; w = w->next_sibling) {
    ...
    yetty_ygui_old_render_ctx_init(&ctx, engine->buffer, engine->theme);
    ...
    {
        struct yetty_ydraw_draw_list_result dr = yetty_ydraw_draw_list_config_buffer_create(NULL);
        YETTY_RETURN_IF_ERR(yetty_ycore_void, dr, "engine_rebuild: deferred buffer create failed");
    ...
    for (struct yetty_ygui_old_widget *w = engine->first_widget; w; w = w->next_sibling) {
        ...
        if (w->vtable && w->vtable->render_all) {
            ...
            r = yetty_ygui_old_widget_render_all_default(w, &ctx);
        }
        if (YETTY_IS_ERR(r)) {
            first_err = r;
            break;
    ...
    for (struct yetty_ygui_old_widget *w = engine->first_widget; w; w = w->next_sibling) {
        if (w->was_rendered) {
            yetty_ygui_old_grid_insert(&engine->grid, w);
        }
    }
...
{
    if (!engine) {
        return YETTY_ERR(yetty_ycore_void, "yetty_ygui_old_engine_layout: NULL engine");
    }
    return yetty_ygui_old_layout_compute_engine(engine);
}

struct yetty_ycore_void_result yetty_ygui_old_engine_emit_pending_deletes(
    struct yetty_ygui_old_engine *engine, int full_redraw)
{
    if (!engine || !engine->buffer) {
        return YETTY_ERR(yetty_ycore_void, "engine_emit_pending_deletes: NULL engine or buffer");
    }
    if (full_redraw) {
        struct yetty_ycore_void_result zr =
            yetty_ydraw_draw_list_add_admin_clear_all(engine->buffer);
        YETTY_RETURN_IF_ERR(yetty_ycore_void, zr, "engine_emit_pending_deletes: CLEAR_ALL");
...
struct yetty_ycore_void_result yetty_ygui_old_engine_render(struct yetty_ygui_old_engine *engine)
{
    ...
    yetty_ygui_old_engine_notify_tick(engine);
    ...
    struct yetty_ycore_void_result fd =
        yetty_ygui_old_engine_emit_pending_deletes(engine, full_redraw);
    ...
    if (!engine->card_shown) {
        struct yetty_ycore_void_result r = yetty_ygui_old_osc_create_card(
            engine->output_pty, engine->card_name, engine->card_x, engine->card_y, engine->card_w,
            engine->card_h, data, size);
        engine->card_shown = 1;
        return r;
    }
    return yetty_ygui_old_osc_update_card(engine->output_pty, engine->card_name, data, size);
}
...
struct yetty_ycore_void_result yetty_ygui_old_engine_internal_emit_handshake(
    struct yetty_ygui_old_engine *engine)
{
    if (!engine) {
    ...
    struct yetty_ycore_void_result sc_r =
        yetty_ygui_old_osc_subscribe_clicks(engine->output_pty, 1);
    YETTY_RETURN_IF_ERR(yetty_ycore_void, sc_r, "engine_emit_handshake: subscribe_clicks failed");
    engine->clicks_subscribed = 1;
    struct yetty_ycore_void_result sm_r = yetty_ygui_old_osc_subscribe_moves(engine->output_pty, 1);
    ...
    struct yetty_ycore_void_result place_r = yetty_ygui_old_osc_card_place(
    ...
    struct yetty_ycore_void_result cr =
        yetty_ygui_old_osc_create_card(engine->output_pty, engine->card_name, engine->card_x,
...
static void emit_event(struct yetty_ygui_old_engine *engine, const ygui_event_t *event)
{
    if (engine->event_callback && event->type != YETTY_YGUI_OLD_EVENT_NONE) {
...
void yetty_ygui_old_engine_mouse_move(struct yetty_ygui_old_engine *engine, float x, float y)
{
    ...
    struct yetty_ygui_old_widget *hit = yetty_ygui_old_grid_query(&engine->grid, x, y);
    ...
    if (hit != engine->hovered) {
        if (engine->hovered) {
            engine->hovered->flags &= ~YETTY_YGUI_OLD_FLAG_HOVER;
            engine->hovered->dirty = 1;
            engine->dirty = 1;
        }
        if (hit) {
            hit->flags |= YETTY_YGUI_OLD_FLAG_HOVER;
            hit->dirty = 1;
            engine->dirty = 1;
        }
        engine->hovered = hit;
...
static int point_in_subtree(struct yetty_ygui_old_widget *w, float x, float y)
{
...
static void close_open_overlays_outside(struct yetty_ygui_old_engine *engine, float x, float y)
{
    for (struct yetty_ygui_old_widget *w = engine->first_widget; w; w = w->next_sibling) {
        if (!(w->flags & YETTY_YGUI_OLD_FLAG_OPEN)) {
            continue;
        }
        if (w->type == YETTY_YGUI_OLD_WIDGET_POPUP) {
            if (w->data.popup.modal) {
                continue;
            }
            if (!point_in_subtree(w, x, y)) {
                yetty_ygui_old_widget_popup_set_open(w, 0);
            }
        } else if (w->type == YETTY_YGUI_OLD_WIDGET_POPUP_MENU) {
            if (w->data.popup_menu.modal) {
                continue;
            }
            if (!point_in_subtree(w, x, y)) {
                yetty_ygui_old_widget_popup_menu_close(w);
            }
        }
    }
}
                                      ...
                                      int button)
{
    ...
    if (is_right && hit) {
        for (struct yetty_ygui_old_widget *w = hit; w; w = w->parent) {
            if (w->context_menu) {
                yetty_ygui_old_widget_popup_menu_open_at(w->context_menu, x, y);
                engine->dirty = 1;
                return;
            }
        }
    ...
    if (hit) {
        hit->flags |= YETTY_YGUI_OLD_FLAG_PRESSED;
        engine->pressed = hit;
        hit->dirty = 1;
        engine->dirty = 1;
        ...
        if (engine->focused != hit) {
            if (engine->focused) {
                engine->focused->flags &= ~YETTY_YGUI_OLD_FLAG_FOCUSED;
                engine->focused->dirty = 1;
            }
            hit->flags |= YETTY_YGUI_OLD_FLAG_FOCUSED;
            engine->focused = hit;
        }
                                    ...
                                    int button)
{
    ...
    if (engine->pressed) {
        struct yetty_ygui_old_widget *widget = engine->pressed;
        widget->flags &= ~YETTY_YGUI_OLD_FLAG_PRESSED;
        widget->dirty = 1;
        engine->dirty = 1;
        ...
        if (hit == widget) {
            ...
            /* Legacy event */
            ygui_event_t event = {.widget_id = widget->id, .type = YETTY_YGUI_OLD_EVENT_CLICK};
            emit_event(engine, &event);
        }
...
void yetty_ygui_old_engine_mouse_scroll(struct yetty_ygui_old_engine *engine, float x, float y,
                                        float dx, float dy)
{
    if (!engine) {
        return;
...
void yetty_ygui_old_engine_text_input(struct yetty_ygui_old_engine *engine, const char *text)
{
     ...
     * here too. */
    if (engine->focused->type == YETTY_YGUI_OLD_WIDGET_TEXTAREA) {
        extern void yetty_ygui_old_internal_textarea_insert(struct yetty_ygui_old_widget * w,
                                                            const char *text);
        yetty_ygui_old_internal_textarea_insert(engine->focused, text);
        return;
    }

    /* Only textinput handles text input */
    if (engine->focused->type == YETTY_YGUI_OLD_WIDGET_TEXTINPUT) {
        ...
        if (new_text) {
            ...
            ygui_event_t event = {.widget_id = engine->focused->id,
                                  .type = YETTY_YGUI_OLD_EVENT_CHANGE,
...
static struct yetty_ygui_old_widget *find_recursive(struct yetty_ygui_old_widget *w, const char *id)
{
    if (!w || !id) {
        return NULL;
    }
    ...
    for (struct yetty_ygui_old_widget *child = w->first_child; child; child = child->next_sibling) {
        struct yetty_ygui_old_widget *found = find_recursive(child, id);
        if (found) {
            return found;
        }
    }
                                                         ...
                                                         const char *id)
{
    ...
    for (struct yetty_ygui_old_widget *w = engine->first_widget; w; w = w->next_sibling) {
        struct yetty_ygui_old_widget *found = find_recursive(w, id);
        if (found) {
            return found;
        }
    }
                                                              ...
                                                              float x, float y)
{
    ...
    return yetty_ygui_old_grid_query(&engine->grid, x, y);
}
...
static void scale_coords(struct yetty_ygui_old_engine *engine, float *x, float *y)
{
...
static void handle_resize(struct yetty_ygui_old_engine *engine)
{
    ...
    if (engine->canvas_mode == YETTY_YGUI_OLD_CANVAS_FIT) {
         ...
         *
         * NOTE: YETTY_YGUI_OLD_SCALE_ON is currently a no-op. Issue #41 calls out
        ...
        yetty_ygui_old_grid_destroy(&engine->grid);
        yetty_ygui_old_grid_init(&engine->grid, engine->width, engine->height,
    ...
    yetty_ygui_old_engine_notify_on_resize(engine);
    ...
    while (i < engine->input_len) {
        ...
        /* Try to parse CSI cell size response */
            ...
            if (osc_code == 777777) {
                /* Click event */
                if (press == 1) {
                    yetty_ygui_old_engine_mouse_down(engine, x, y, buttons & 0x7);
                } else {
                    yetty_ygui_old_engine_mouse_up(engine, x, y, buttons & 0x7);
                }
            } else {
                /* Move event */
                yetty_ygui_old_engine_mouse_move(engine, x, y);
            }
            i += consumed;
        } else {
                                          ...
                                          size_t payload_len)
{
    (void)args;
    (void)args_len;
    struct yetty_ygui_old_engine *engine = (struct yetty_ygui_old_engine *)user;
    if (!engine) {
        return;
    }
    ...
    case YETTY_OSC_SC_CLIENT_INPUT_FIGURE_MOUSE: {
        ...
        case YETTY_YMGUI_INPUT_MOUSE_POS:
            yetty_ygui_old_engine_mouse_move(engine, m->x, m->y);
            break;
        case YETTY_YMGUI_INPUT_MOUSE_BUTTON:
            if (m->pressed) {
                yetty_ygui_old_engine_mouse_down(engine, m->x, m->y, m->button);
            } else {
                yetty_ygui_old_engine_mouse_up(engine, m->x, m->y, m->button);
            }
            break;
        case YETTY_YMGUI_INPUT_MOUSE_WHEEL:
            yetty_ygui_old_engine_mouse_scroll(engine, m->x, m->y, 0.0f, m->wheel_dy);
            break;
        }
        break;
    }
    ...
    case YETTY_OSC_SC_CLIENT_INPUT_FIGURE_KEY: {
        ...
        if (k->kind == YETTY_YMGUI_INPUT_KEY_CHAR) {
            ...
            utf8[n] = '\0';
            yetty_ygui_old_engine_text_input(engine, utf8);
        } else if (k->kind == YETTY_YMGUI_INPUT_KEY_DOWN) {
            yetty_ygui_old_engine_key_down(engine, (uint32_t)k->key, k->mods);
        } else if (k->kind == YETTY_YMGUI_INPUT_KEY_UP) {
            yetty_ygui_old_engine_key_up(engine, (uint32_t)k->key, k->mods);
        }
        break;
    }
    default:
...
void yetty_ygui_old_internal_yface_on_raw(void *user, const char *bytes, size_t n)
{
    ...
    yetty_ygui_old_internal_process_input(engine, bytes, (int)n);
}
 ...
 * yetty_ygui_old_engine_attach / _run / _get_loop / _poll all live there;
...
/* yetty_ygui_old_engine_get_loop / yetty_ygui_old_engine_poll are libuv-coupled —
...
{
    if (!button || button->type != YETTY_YGUI_OLD_WIDGET_BUTTON) {
        return;
    }
...
{
    if (!slider || slider->type != YETTY_YGUI_OLD_WIDGET_SLIDER) {
        return;
    }
...
{
    if (!checkbox || checkbox->type != YETTY_YGUI_OLD_WIDGET_CHECKBOX) {
        return;
    }
...
{
    if (!input || input->type != YETTY_YGUI_OLD_WIDGET_TEXTINPUT) {
        return;
    }
...
void yetty_ygui_old_engine_clear(struct yetty_ygui_old_engine *engine)
{
    if (!engine) {
        return;
    }
...
void yetty_ygui_old_engine_subscribe_clicks(struct yetty_ygui_old_engine *engine, int enable)
{
    if (!engine) {
        return;
    }
    ...
    struct yetty_ycore_void_result r =
        yetty_ygui_old_osc_subscribe_clicks(engine->output_pty, want);
    if (YETTY_IS_ERR(r)) {
...
void yetty_ygui_old_engine_subscribe_moves(struct yetty_ygui_old_engine *engine, int enable)
{
    if (!engine) {
        return;
    }
    ...
    struct yetty_ycore_void_result r = yetty_ygui_old_osc_subscribe_moves(engine->output_pty, want);
    if (YETTY_IS_ERR(r)) {
...
struct yetty_ycore_void_result yetty_ygui_old_engine_rebuild(struct yetty_ygui_old_engine *engine)
{
    return engine_rebuild_with_mode(engine, /*full_redraw=*/1);
}

struct yetty_ycore_void_result yetty_ygui_old_engine_rebuild_with_mode(
    struct yetty_ygui_old_engine *engine, int full_redraw)
{
...
void yetty_ygui_old_engine_set_canvas_mode(struct yetty_ygui_old_engine *engine,
                                           ygui_canvas_mode_t mode)
{
    if (engine) {
...
void yetty_ygui_old_engine_set_scale_mode(struct yetty_ygui_old_engine *engine,
                                          ygui_scale_mode_t mode)
{
    if (engine) {
...
void yetty_ygui_old_engine_subscribe_view_changes(struct yetty_ygui_old_engine *engine, int enable)
{
    ...
    struct yetty_ycore_void_result r =
        yetty_ygui_old_osc_subscribe_view_changes(engine->output_pty, want);
    if (YETTY_IS_ERR(r)) {
...
void yetty_ygui_old_engine_set_zoom(struct yetty_ygui_old_engine *engine, float level)
{
    ...
    struct yetty_ycore_void_result r =
        yetty_ygui_old_osc_zoom_card(engine->output_pty, engine->card_name, level);
    if (YETTY_IS_ERR(r)) {
...
void yetty_ygui_old_engine_scroll_to(struct yetty_ygui_old_engine *engine, float x, float y)
{
    ...
    struct yetty_ycore_void_result r =
        yetty_ygui_old_osc_scroll_card(engine->output_pty, engine->card_name, x, y, 1);
    if (YETTY_IS_ERR(r)) {
...
void yetty_ygui_old_engine_scroll_by(struct yetty_ygui_old_engine *engine, float dx, float dy)
{
    ...
    struct yetty_ycore_void_result r =
        yetty_ygui_old_osc_scroll_card(engine->output_pty, engine->card_name, dx, dy, 0);
    if (YETTY_IS_ERR(r)) {
