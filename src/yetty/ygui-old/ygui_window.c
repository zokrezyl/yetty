 ...
 *   close 'x' (yetty_ygui_old_tabbar_button_size) so the two visually
...
static float window_title_h(const struct yetty_ygui_old_widget *self)
{
...
static float window_button_size(void)
{
    ...
    return yetty_ygui_old_tabbar_button_size();
}
...
static void menu_button_rect(const struct yetty_ygui_old_widget *self, float *out_x, float *out_y,
...
static struct yetty_ycore_void_result window_render(struct yetty_ygui_old_widget *self,
                                                    struct yetty_ygui_old_render_ctx *ctx)
{
    ...
    float ay = self->y + cy;
    int menu_open =
        self->data.window.menu && (self->data.window.menu->flags & YETTY_YGUI_OLD_FLAG_OPEN);
    uint32_t bg = menu_open ? theme->bg_hover : theme->bg_header;
    yetty_ygui_old_render_ctx_render_box(ctx, ax, ay, cw, ch, bg, WINDOW_CORNER_RADIUS * 0.5f);
                           ...
                           ygui_event_t *out)
{
    ...
    if (lx >= cx && lx < cx + cw && ly >= cy && ly < cy + ch) {
        if (self->data.window.menu) {
             ...
             * drop the OPEN flag. */
            struct yetty_ygui_old_widget *m = self->data.window.menu;
            if (m->flags & YETTY_YGUI_OLD_FLAG_OPEN) {
                yetty_ygui_old_widget_popup_menu_close(m);
            } else {
                ...
                yetty_ygui_old_widget_popup_menu_open_at(m, menu_x, menu_y);
            }
            return 1;
        }
        ...
        if (self->engine) {
            yetty_ygui_old_engine_close_preserve(self->engine);
        }
        return 1;
    }
...
static int window_on_drag(struct yetty_ygui_old_widget *self, float lx, float ly, ygui_event_t *out)
{
    (void)out;
...
static void window_destroy(struct yetty_ygui_old_widget *self)
{
...
static const struct yetty_ygui_old_widget_vtable window_vtable = {
                                                           ...
                                                           float w, float h, const char *title)
{
    ...
    yetty_ygui_old_widget_init_base(win, x, y, w, h);
    ...
    win->layout.mode = YETTY_YGUI_OLD_LAYOUT_FLEX;
    win->layout.direction = YETTY_YGUI_OLD_FLEX_COLUMN;
    win->layout.align_items = YETTY_YGUI_OLD_ALIGN_STRETCH;
    win->layout.padding_top = WINDOW_DEFAULT_TITLE_H;

    yetty_ygui_old_engine_attach_widget(engine, win);
    ...
    struct yetty_ygui_old_widget *body = yetty_ygui_old_engine_vbox(engine, body_id, 0, 0, 0, 0);
    if (body) {
        body->layout.mode = YETTY_YGUI_OLD_LAYOUT_FLEX;
        body->layout.direction = YETTY_YGUI_OLD_FLEX_COLUMN;
        body->layout.align_items = YETTY_YGUI_OLD_ALIGN_STRETCH;
        body->layout.flex_grow = 1.0f;
        body->layout.flex_shrink = 1.0f;
        yetty_ygui_old_widget_add_child(win, body);
