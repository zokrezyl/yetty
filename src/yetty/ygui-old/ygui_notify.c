...
{
    switch (sev) {
    case YETTY_YGUI_OLD_SEV_INFO:
        return 0xFF6BA892u; /* BRAND_ACCENT mint */
    case YETTY_YGUI_OLD_SEV_WARN:
        return 0xFFD2A45Au; /* warm amber */
    case YETTY_YGUI_OLD_SEV_ERROR:
...
{
    if (sev == YETTY_YGUI_OLD_SEV_ERROR) {
...
static void notify_relayout(struct yetty_ygui_old_engine *engine)
{
    if (!engine) {
        return;
    }
    ...
    for (int i = 0; i < engine->notification_count; i++) {
        ...
        yetty_ygui_old_widget_set_position(n->card, x, y);
...
static int notify_find_by_close_btn(const struct yetty_ygui_old_engine *engine,
                                    const struct yetty_ygui_old_widget *btn)
{
    for (int i = 0; i < engine->notification_count; i++) {
 ...
 * exposes a remove-widget API (see yetty_ygui_old_widget_destroy) — we use
 * it on the top-level card; its children go down with it. */
static void notify_dismiss_at(struct yetty_ygui_old_engine *engine, int idx)
{
    ...
    if (n->card) {
        yetty_ygui_old_widget_remove(n->card);
        n->card = NULL;
        n->close_btn = NULL;
    }
    free(n->message);
...
static void notify_close_clicked(struct yetty_ygui_old_widget *btn, void *userdata)
{
    struct yetty_ygui_old_engine *engine = (struct yetty_ygui_old_engine *)userdata;
    if (!engine || !btn) {
        return;
    }
                                                       ...
                                                       int slot, int severity, const char *message)
{
    ...
    struct yetty_ygui_old_widget *card =
        yetty_ygui_old_engine_vbox(engine, figure_id, 0, 0, NOTIF_CARD_W, 0);
    if (!card) {
        return NULL;
    }
    yetty_ygui_old_widget_set_size(card, NOTIF_CARD_W, 56);
    char card_css[256];
    snprintf(card_css, sizeof(card_css),
             "background:#141A1F;border-radius:8;border:1 #364A47;"
             "padding:%s;",
             NOTIF_CARD_PAD);
    yetty_ygui_old_widget_apply_css(card, card_css);

    struct yetty_ygui_old_widget *row = yetty_ygui_old_engine_hbox(engine, row_id, 0, 0, 0, 0);
    if (!row) {
    ...
    if (stripe) {
        ...
        yetty_ygui_old_widget_apply_css(stripe, css);
        yetty_ygui_old_widget_add_child(row, stripe);
    }
    ...
    struct yetty_ygui_old_widget *lbl = yetty_ygui_old_engine_label(engine, lbl_id, 0, 0, composed);
    if (lbl) {
        yetty_ygui_old_widget_apply_css(lbl, "flex:1 0 0;");
    ...
    struct yetty_ygui_old_widget *close_btn =
        yetty_ygui_old_engine_button(engine, close_id, 0, 0, 22, 22, "x");
    if (close_btn) {
        yetty_ygui_old_widget_apply_css(close_btn, "flex:0 0 22;background:transparent;");
        yetty_ygui_old_widget_button_on_click(close_btn, notify_close_clicked, engine);
        yetty_ygui_old_widget_add_child(row, close_btn);
...
static void notify_push(struct yetty_ygui_old_engine *engine, int severity, uint32_t ttl_ms,
                        const char *message)
{
    if (!engine || !message) {
        return;
