...
```c
/* Exposed helpers — authored in grid.c, published by codegen into
...
/* Generated yclass slot stubs (same header) — work local and over RPC. */
...
ygrid is a yclass module: the public object-API header
`include/yetty/api/ygrid/grid.h`, the API stub TU
`src/yetty/gen/api/ygrid/grid.c`, the impl glue
`src/yetty/gen/impl/ygrid/grid.{c,h}` (the `.c` is appended at the foot of
`grid.c`) and `model.yaml` are all **generated by `make codegen`** from the
`YETTY_ANNOTATE(...)` markers in `grid.c` — never hand-edit them. See
[../yclass/README.md](../yclass/README.md). The hand-written pieces are
`grid.c` and `ygrid.wgsl`. The non-slot helper API (create / factory /
font / composite-factory) is authored in `grid.c` with
`YETTY_ANNOTATE("expose")` and reproduced into the generated API header —
there is no separate hand-written public header.
...
| `../gen/{api,impl}/ygrid/` / `model.yaml` | codegen output — do not edit |
| `ygrid.wgsl` | layer shader (cell walk + SDF/glyph evaluation + post effect); staged to assets by [../yshaders](../yshaders/README.md) |
| `include/yetty/api/ygrid/grid.h` | generated object-API header: slot stubs + exposed create / factory / font helpers |
