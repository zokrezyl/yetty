 ...
 * other modules drive carries `YETTY_ANNOTATE("expose")` below, so
 * codegen publishes it in the generated `yetty/api/ygrid/grid.h`; this
 * TU re-declares only the pieces it defines or calls, so it never pulls
...
/* Hand-written public surface this TU defines. The definitions below
 * carry `YETTY_ANNOTATE("expose")`, so codegen reproduces them —
 * prototypes, the factory-args struct and their doc comments — into the
 * generated object-API header `yetty/api/ygrid/grid.h`, the one header
 * external consumers include. This TU must not include its own generated
 * header (the emitted copy of the exposed struct would clash with the
 * authored one), so the few prototypes it calls before their point of
 * definition are repeated locally below. */
...
struct YETTY_ANNOTATE("expose") yetty_ygrid_factory_args {
...
/* Prototypes for the exposed public API this TU calls before its point
 * of definition. The canonical public declarations are the
 * codegen-emitted copies in `yetty/api/ygrid/grid.h`; these local
 * repeats exist only for in-TU ordering. */
