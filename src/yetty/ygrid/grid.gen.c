...
static size_t yetty_ygrid_add_record_skel(const void *body, size_t body_len, void *resp,
                                          size_t resp_max)
{
/* Byte-exact wire layout — #pragma pack matches the first-party
 * convention (yvnc/ydvnc/libvterm) and compiles on MSVC, unlike a GNU
 ...
 * status wire response at this boundary. */
YETTY_EXTERNAL_CALLBACK
static size_t yetty_ygrid_clear_skel(const void *body, size_t body_len, void *resp, size_t resp_max)
{
/* Byte-exact wire layout — #pragma pack matches the first-party
 * convention (yvnc/ydvnc/libvterm) and compiles on MSVC, unlike a GNU
 * packed attribute. */
 ...
 * status wire response at this boundary. */
YETTY_EXTERNAL_CALLBACK
static size_t yetty_ygrid_destroy_skel(const void *body, size_t body_len, void *resp,
                                       size_t resp_max)
{
/* Byte-exact wire layout — #pragma pack matches the first-party
 * convention (yvnc/ydvnc/libvterm) and compiles on MSVC, unlike a GNU
...
/* ---- ygrid/grid: class name -> accessor ---------------------- */
static struct yetty_yclass_ptr_result yetty_ygrid_grid_accessor_lookup(const char *name)
{
...
struct yetty_ygrid_grid_skel_row {
    const char *name;
    yetty_yclass_rpc_skel_fn fn;
};
static const struct yetty_ygrid_grid_skel_row yetty_ygrid_grid_skel_rows[] = {
...
YETTY_EXTERNAL_CALLBACK
static yetty_yclass_rpc_skel_fn yetty_ygrid_grid_skel_lookup(yetty_yclass_method_slot slot)
{
    ...
    for (size_t i = 0;
         i < sizeof(yetty_ygrid_grid_skel_rows) / sizeof(yetty_ygrid_grid_skel_rows[0]); ++i) {
        if (strcmp(yetty_ygrid_grid_skel_rows[i].name, name) == 0) {
            return yetty_ygrid_grid_skel_rows[i].fn;
        }
    }
    return NULL;
}

struct yetty_ycore_void_result yetty_ygrid_grid_register_hooks(void)
{
    struct yetty_ycore_void_result add_accessor_r =
        yetty_yclass_add_accessor_lookup(yetty_ygrid_grid_accessor_lookup);
    YETTY_RETURN_IF_ERR(yetty_ycore_void, add_accessor_r,
                        "yetty_ygrid_grid_register_hooks: accessor");
    {
        struct yetty_ycore_void_result add_skel_r =
            yetty_yclass_rpc_add_skel_lookup(yetty_ygrid_grid_skel_lookup);
        YETTY_RETURN_IF_ERR(yetty_ycore_void, add_skel_r, "yetty_ygrid_grid_register_hooks: skel");
    }
    return YETTY_OK_VOID();
}
...
struct yetty_ycore_void_result yetty_ygrid_register(void)
{
    ...
    {
        struct yetty_ycore_void_result hook_r = yetty_ygrid_grid_register_hooks();
        YETTY_RETURN_IF_ERR(yetty_ycore_void, hook_r, "yetty_ygrid_register: grid");
    }
    registered = true;
    return YETTY_OK_VOID();
}
