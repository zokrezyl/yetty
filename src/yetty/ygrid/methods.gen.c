...
struct yetty_ycore_void_result yetty_ygrid_process_bytes(struct yetty_yclass_ctx * ctx, struct yetty_yclass_object * obj, struct yetty_ycore_buffer payload)
{
    ...
    if (method_slot == YETTY_YCLASS_METHOD_SLOT_UNDEFINED) {
        ...
        if (YETTY_IS_ERR(method_slot_r))
            return YETTY_ERR(yetty_ycore_void, "yetty_ygrid_process_bytes: method_slot_get failed", method_slot_r);
        method_slot = method_slot_r.value;
    }
    ...
    if (rpc_ctx && rpc_ctx->session) {
        struct uint32_result remote_id_r =
            yetty_yclass_rpc_session_ensure_remote_id(rpc_ctx->session, method_slot);
        YETTY_RETURN_IF_ERR(yetty_ycore_void, remote_id_r, "yetty_ygrid_process_bytes: ensure_remote_id failed");
        uint32_t remote_id = remote_id_r.value;
/* Byte-exact wire layout — #pragma pack matches the first-party
 * convention (yvnc/ydvnc/libvterm) and compiles on MSVC, unlike a GNU
 * packed attribute. */
#pragma pack(push, 1)
        ...
        } wire_args = { container_of((struct yetty_yclass_object *)obj, struct yetty_yclass_proxy, header)->handle, (uint32_t)payload.size };
#pragma pack(pop)
        size_t body_total = sizeof(wire_args) + (size_t)payload.size;
        uint8_t *body_buf = (uint8_t *)malloc(body_total ? body_total : 1);
        if (!body_buf) return YETTY_ERR(yetty_ycore_void, "yetty_ygrid_process_bytes: body buf oom");
        memcpy(body_buf, &wire_args, sizeof(wire_args));
        size_t body_offset = sizeof(wire_args);
        memcpy(body_buf + body_offset, payload.data, payload.size);
        ...
        struct yetty_ycore_size_result rpc_call_r = yetty_yclass_rpc_call(
            ...
            resp_buf, sizeof(resp_buf));
        free(body_buf);
        YETTY_RETURN_IF_ERR(yetty_ycore_void, rpc_call_r, "yetty_ygrid_process_bytes: RPC call failed");
        size_t response_len = rpc_call_r.value;
        if (response_len < 1) return YETTY_ERR(yetty_ycore_void, "yetty_ygrid_process_bytes: short RPC response");
        if (resp_buf[0] != 0) return YETTY_ERR(yetty_ycore_void, "yetty_ygrid_process_bytes: remote impl returned error");
        return YETTY_OK_VOID();
    } else {
        struct yetty_yclass_ptr_result object_class_r =
            yetty_yclass_object_class(obj);
        YETTY_RETURN_IF_ERR(yetty_ycore_void, object_class_r, "yetty_ygrid_process_bytes: object_class failed");
        struct yetty_yclass_impl_t_result dispatch_impl_r =
            yetty_yclass_dispatch_lookup(object_class_r.value, method_slot);
        YETTY_RETURN_IF_ERR(yetty_ycore_void, dispatch_impl_r, "yetty_ygrid_process_bytes: dispatch_lookup failed");
        return ((yetty_ygrid_process_bytes_fn)dispatch_impl_r.value)(ctx, obj, payload);
    }
}
...
struct yetty_ycore_void_result yetty_ygrid_reset_content(struct yetty_yclass_ctx * ctx, struct yetty_yclass_object * obj)
{
    ...
    if (method_slot == YETTY_YCLASS_METHOD_SLOT_UNDEFINED) {
        ...
        if (YETTY_IS_ERR(method_slot_r))
            return YETTY_ERR(yetty_ycore_void, "yetty_ygrid_reset_content: method_slot_get failed", method_slot_r);
        method_slot = method_slot_r.value;
    }
    ...
    if (rpc_ctx && rpc_ctx->session) {
        struct uint32_result remote_id_r =
            yetty_yclass_rpc_session_ensure_remote_id(rpc_ctx->session, method_slot);
        YETTY_RETURN_IF_ERR(yetty_ycore_void, remote_id_r, "yetty_ygrid_reset_content: ensure_remote_id failed");
        uint32_t remote_id = remote_id_r.value;
/* Byte-exact wire layout — #pragma pack matches the first-party
 * convention (yvnc/ydvnc/libvterm) and compiles on MSVC, unlike a GNU
 * packed attribute. */
...
#pragma pack(pop)
        ...
        struct yetty_ycore_size_result rpc_call_r = yetty_yclass_rpc_call(
            rpc_ctx->session, YETTY_YCLASS_RPC_OP_CALL, remote_id, &wire_args, sizeof(wire_args),
            resp_buf, sizeof(resp_buf));
        YETTY_RETURN_IF_ERR(yetty_ycore_void, rpc_call_r, "yetty_ygrid_reset_content: RPC call failed");
        size_t response_len = rpc_call_r.value;
        if (response_len < 1) return YETTY_ERR(yetty_ycore_void, "yetty_ygrid_reset_content: short RPC response");
        if (resp_buf[0] != 0) return YETTY_ERR(yetty_ycore_void, "yetty_ygrid_reset_content: remote impl returned error");
        return YETTY_OK_VOID();
    } else {
        struct yetty_yclass_ptr_result object_class_r =
            yetty_yclass_object_class(obj);
        YETTY_RETURN_IF_ERR(yetty_ycore_void, object_class_r, "yetty_ygrid_reset_content: object_class failed");
        struct yetty_yclass_impl_t_result dispatch_impl_r =
            yetty_yclass_dispatch_lookup(object_class_r.value, method_slot);
        YETTY_RETURN_IF_ERR(yetty_ycore_void, dispatch_impl_r, "yetty_ygrid_reset_content: dispatch_lookup failed");
        return ((yetty_ygrid_reset_content_fn)dispatch_impl_r.value)(ctx, obj);
    }
}
