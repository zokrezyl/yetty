 ...
 *                A real impl needs a yimage complex-prim atlas upload.
 *   - text     : TEXT_SPAN flyweight prim (font_id = -1 = canvas default).
 */
...
{
	if (s->stroke_type == PLOT_OP_TYPE_NONE) return 0;
...
static bool aabb_visible(const struct yetty_ynetsurf *ns,
			 int x0, int y0, int x1, int y1)
{
	if (x1 < ns->cur_clip.x0 || x0 > ns->cur_clip.x1) return false;
	if (y1 < ns->cur_clip.y0 || y0 > ns->cur_clip.y1) return false;
	return true;
}
...
{
	struct yetty_ynetsurf *ns = ctx->priv;
	if (!aabb_visible(ns, r->x0, r->y0, r->x1, r->y1)) return NSERROR_OK;
	...
	(void)yetty_ysdf_add_box(ns->cur_buf, ns->z_counter++,
				 fill_rgba(s), stroke_rgba(s), stroke_w(s),
				 &box);
	return NSERROR_OK;
}

static nserror p_disc(const struct redraw_context *ctx, const plot_style_t *s,
		      int x, int y, int radius)
{
	...
	(void)yetty_ysdf_add_circle(ns->cur_buf, ns->z_counter++,
				    fill_rgba(s), stroke_rgba(s), stroke_w(s), &c);
	return NSERROR_OK;
}

static nserror p_line(const struct redraw_context *ctx, const plot_style_t *s,
		      const struct rect *l)
{
	struct yetty_ynetsurf *ns = ctx->priv;
	...
	if (!aabb_visible(ns, xmin, ymin, xmax, ymax)) return NSERROR_OK;

	struct yetty_ysdf_segment seg = {
		.start_x = l->x0, .start_y = l->y0,
		.end_x = l->x1,   .end_y = l->y1,
	};
	uint32_t col = stroke_rgba(s);
	if (col == 0) col = to_rgba(s->stroke_colour);
	(void)yetty_ysdf_add_segment(ns->cur_buf, ns->z_counter++,
				     0, col, stroke_w(s), &seg);
	return NSERROR_OK;
}

static nserror p_arc(const struct redraw_context *ctx, const plot_style_t *s,
		     int x, int y, int radius, int angle1, int angle2)
{
	...
	struct yetty_ysdf_arc arc = {
		...
		.aperture_x = sinf(ah), .aperture_y = cosf(ah),
		.radius = radius, .thickness = stroke_w(s),
	};
	(void)ac; /* arc orientation is implicit in aperture vector */
	(void)yetty_ysdf_add_arc(ns->cur_buf, ns->z_counter++,
				 0, stroke_rgba(s), stroke_w(s), &arc);
	return NSERROR_OK;
}
...
static void fan_triangulate(struct yetty_ynetsurf *ns,
			    const float *xs, const float *ys, unsigned n,
			    uint32_t fill, uint32_t stroke, float sw)
{
	if (n < 3) return;
	for (unsigned i = 1; i + 1 < n; i++) {
		struct yetty_ysdf_triangle t = {
			.vertex_a_x = xs[0],   .vertex_a_y = ys[0],
			.vertex_b_x = xs[i],   .vertex_b_y = ys[i],
			.vertex_c_x = xs[i+1], .vertex_c_y = ys[i+1],
		};
		(void)yetty_ysdf_add_triangle(ns->cur_buf, ns->z_counter++,
					      fill, stroke, sw, &t);
	}
}

static nserror p_polygon(const struct redraw_context *ctx,
			 const plot_style_t *s, const int *p, unsigned int n)
{
	struct yetty_ynetsurf *ns = ctx->priv;
...
{
	if (t == NULL) { *ox = x; *oy = y; return; }
	*ox = t[0] * x + t[2] * y + t[4];
	*oy = t[1] * x + t[3] * y + t[5];
}

static nserror p_path(const struct redraw_context *ctx, const plot_style_t *s,
		      const float *p, unsigned int n, const float t[6])
{
	struct yetty_ynetsurf *ns = ctx->priv;
	...
	while (i < n) {
		...
		if (cmd == PLOTTER_PATH_MOVE) {
			...
			for (int k = 1; k <= BEZIER_STEPS; k++) {
				...
				float v = 1.0f - u;
				float bx = v*v*v*x0 + 3*v*v*u*c1x +
					   3*v*u*u*c2x + u*u*u*ex;
				float by = v*v*v*y0 + 3*v*v*u*c1y +
					   3*v*u*u*c2y + u*u*u*ey;
				PUSH(bx, by);
			}
			cx = ex; cy = ey;
		} else if (cmd == PLOTTER_PATH_CLOSE) {
			if (cnt > 0) PUSH(xs[0], ys[0]);
		} else {
			break;  /* unknown -> bail */
		}
	}
		...
		if (aabb_visible(ns, (int)xmin, (int)ymin, (int)xmax, (int)ymax)) {
			...
			if (s->stroke_type != PLOT_OP_TYPE_NONE) {
				...
				for (unsigned k = 1; k < cnt; k++) {
					...
					(void)yetty_ysdf_add_segment(
						ns->cur_buf, ns->z_counter++,
						0, col, sw, &seg);
				}
			}
		}
...
static nserror p_bitmap(const struct redraw_context *ctx, struct bitmap *bm,
			int x, int y, int width, int height,
			colour bg, bitmap_flags_t flags)
{
	(void)flags; (void)bg;
	struct yetty_ynetsurf *ns = ctx->priv;
	if (!aabb_visible(ns, x, y, x + width, y + height)) return NSERROR_OK;
