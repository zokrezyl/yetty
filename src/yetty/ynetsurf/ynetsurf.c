 ...
 * commands into a yetty_ydraw_core_draw_list.
 */
...
int yetty_ynetsurf_schedule_run(struct yetty_ynetsurf *ns)
{
    ...
    for (struct yetty_ynetsurf_schedule_entry *e = ns->schedule_head; e != NULL; e = e->next) {
...
static struct gui_window *ns_win_create(struct browser_window *bw, struct gui_window *existing,
...
static void ns_bm_set_opaque(void *bm, bool o)
{
    ((struct ns_bitmap *)bm)->opaque = o;
}
static bool ns_bm_get_opaque(void *bm)
{
    return ((struct ns_bitmap *)bm)->opaque;
}
static unsigned char *ns_bm_get_buffer(void *bm)
{
    return ((struct ns_bitmap *)bm)->pixels;
}
static size_t ns_bm_get_rowstride(void *bm)
{
    return (size_t)((struct ns_bitmap *)bm)->width * 4;
}
static int ns_bm_get_width(void *bm)
{
    return ((struct ns_bitmap *)bm)->width;
}
static int ns_bm_get_height(void *bm)
{
    return ((struct ns_bitmap *)bm)->height;
}
static void ns_bm_modified(void *bm)
{
    (void)bm;
}
...
static nserror ns_layout_width(const struct plot_font_style *fstyle, const char *string,
...
static nserror ns_layout_position(const struct plot_font_style *fstyle, const char *string,
...
static nserror ns_layout_split(const struct plot_font_style *fstyle, const char *string,
                               size_t length, int x, size_t *char_offset, int *actual_x)
{
    int gw = px_per_glyph(fstyle);
    if (gw < 1) {
...
{
    if (g_ynetsurf != NULL) {
        return YETTY_ERR(yetty_ynetsurf_ptr, "ynetsurf singleton already created");
    }
    ...
    if (err != NSERROR_OK) {
        free(ns);
        g_ynetsurf = NULL;
        return YETTY_ERR(yetty_ynetsurf_ptr, "netsurf_register failed");
    }
    ...
    if (err != NSERROR_OK) {
        free(ns);
        g_ynetsurf = NULL;
        return YETTY_ERR(yetty_ynetsurf_ptr, "nsoption_init failed");
    }
    ...
    if (err != NSERROR_OK) {
        free(ns);
        g_ynetsurf = NULL;
        return YETTY_ERR(yetty_ynetsurf_ptr, "netsurf_init failed");
    }
...
struct yetty_ycore_void_result yetty_ynetsurf_navigate(struct yetty_ynetsurf *ns, const char *url)
{
    ...
    if (ns->gw == NULL) {
        ...
        err = browser_window_create(BW_CREATE_HISTORY, nsu, NULL, NULL, &bw);
    } else {
        err = browser_window_navigate(ns->gw->bw, nsu, NULL, BW_NAVIGATE_HISTORY, NULL, NULL, NULL);
    }
    nsurl_unref(nsu);
                                                     ...
                                                     struct yetty_ydraw_core_draw_list *buf)
{
    ...
    if (!browser_window_has_content(ns->gw->bw)) {
        return YETTY_ERR(yetty_ycore_void, "browser_window has no content yet");
    }
    if (!browser_window_redraw_ready(ns->gw->bw)) {
        return YETTY_ERR(yetty_ycore_void, "browser_window not redraw-ready");
    }
    ...
    bool ok = browser_window_redraw(ns->gw->bw, -ns->gw->scroll_x, -ns->gw->scroll_y, &clip, &ctx);
    ns->gw->needs_redraw = false;
    ns->cur_buf = NULL;
    if (!ok) {
        return YETTY_ERR(yetty_ycore_void, "browser_window_redraw returned false");
    }
    return YETTY_OK_VOID();
}
...
{
    if (ns == NULL || ns->gw == NULL || ns->gw->bw == NULL) {
        return;
    }
    browser_window_mouse_track(ns->gw->bw, BROWSER_MOUSE_HOVER, x + ns->gw->scroll_x,
                               y + ns->gw->scroll_y);
}
...
void yetty_ynetsurf_mouse_click(struct yetty_ynetsurf *ns, int x, int y, int button)
{
    ...
    browser_mouse_state st = button == 2 ? BROWSER_MOUSE_PRESS_2 | BROWSER_MOUSE_CLICK_2
                                         : BROWSER_MOUSE_PRESS_1 | BROWSER_MOUSE_CLICK_1;
    browser_window_mouse_click(ns->gw->bw, st, x + ns->gw->scroll_x, y + ns->gw->scroll_y);
}
