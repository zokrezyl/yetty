#!/usr/bin/env python3
"""
ywasm-gen — emits the per-method shims for the WebGPU-over-OSC bridge.

Run by hand whenever the METHODS table below changes:

    python3 src/yetty/ywasm/generate.py

Output is written in place into the repo (committed source, not a
build artifact):

    include/yetty/ywasm/methods.gen.h
    src/yetty/ywasm/client-stubs.gen.c
    src/yetty/ywasm/server-dispatch.gen.c
...
METHODS = [
    ...
    # (yetty_ywasm_client_present_frame) lives in client.c because it
    # orchestrates BULK emit + CMD emit; the codegen stub would be
    # incomplete.
    {
        "name": "yetty_ywasm_present_frame",
        "id": 100,
        "args": [
...
def emit_struct_codec_body(name: str, struct_fields: dict[str, list[tuple[str, str]]],
    ...
    # ----- encoder ----------------------------------------------------
    body.append(f"void ywasm_encode_{name}(const {name} *src,")
    ...
    for f in fields:
        ...
        if k in ("scalar", "handle", "struct_pod"):
            ...
            body.append(f"    ywasm_encode_{f['ctype']}(&src->{fn}, out);")
        elif k == "stringview":
            body.append("    {")
    ...
    for f in fields:
        ...
        if k == "handle":
            ...
            body.append("        } else {")
            body.append("            char *_buf = (char *)ywasm_arena_alloc(arena, (size_t)_len + 1u);")
            ...
            body.append(f"            {f['ctype']} *_p = "
                        f"({f['ctype']} *)ywasm_arena_alloc(arena, sizeof({f['ctype']}));")
            ...
            body.append("        if (_n > 0) {")
            body.append(f"            _arr = ({it_t} *)ywasm_arena_alloc(arena, _n * sizeof({it_t}));")
...
def emit_client_struct_decoder_body(name: str, struct_fields, handle_types, aliases,
    ...
    fields = classify_struct_fields(name, struct_fields)
    body: list[str] = []
    body.append(f"int ywasm_client_decode_{name}(const uint8_t **src, size_t *rem,")
    ...
    for f in fields:
        ...
        if k in ("scalar", "handle", "struct_pod"):
            ...
            body.append(f"    if (!ywasm_client_decode_{f['ctype']}(src, rem, &out->{fn})) return 0;")
        elif k == "stringview":
            body.append("    {")
            body.append("        uint64_t _len = 0;")
            body.append("        if (*rem < sizeof(_len)) return 0;")
            ...
            if k == "ptr_pod":
                ...
                body.append(f"            if (!ywasm_client_decode_{f['ctype']}(src, rem, _p)) "
                            "{ free(_p); return 0; }")
            body.append(f"            out->{fn} = _p;")
            body.append(f"        }} else {{ out->{fn} = NULL; }}")
            body.append("    }")
            ...
            body.append("        for (size_t _i = 0; _i < _n; ++_i) {")
            if k == "array_nonpod":
                body.append(f"            if (!ywasm_client_decode_{it_t}(src, rem, &_arr[_i])) "
                            "{ free(_arr); return 0; }")
            else:
...
def emit_client_struct_freer_body(name: str, struct_fields, handle_types, aliases,
    ...
    fields = classify_struct_fields(name, struct_fields)
    body: list[str] = []
    body.append(f"void ywasm_client_free_{name}({name} *s)")
    body.append("{")
    ...
    for f in fields:
        ...
        if k == "stringview":
            ...
            body.append(f"    ywasm_client_free_{f['ctype']}(&s->{fn});")
        elif k in ("ptr_pod", "ptr_nonpod"):
            if k == "ptr_nonpod":
                body.append(f"    if (s->{fn}) ywasm_client_free_{f['ctype']}(s->{fn});")
...
def auto_spec_for(name: str, ret_type: str, args: list[tuple[str, str]],
    ...
    for kind, ctype, an in classified:
        if kind == "handle":
            ...
            # generated ywasm_encode_<Struct>. No slot in args struct.
            wire_args.append({
    ...
    if not ok_for_real_body:
        ...
        body = (
            "        (void)a;\n"
            f"        yerror(\"ywasm: stub for {name} (needs: {reason_str})\");\n"
    ...
    if has_var:
        body_lines.append("        struct ywasm_arena _arena = {0};")
    ...
    for kind, ctype, an in classified:
        if kind == "handle":
            local = f"_h_{an}"
            body_lines.append(
                f"        {ctype} {local} = ({ctype})ywasm_server_handle_get(ctx, a->{an});")
            body_lines.append(
            ...
            body_lines.append(f"        if (_rem < 1) {{ ywasm_arena_free(&_arena); "
                              f"return YETTY_YWASM_REPLY_VALIDATION_ERROR; }}")
            ...
            body_lines.append("            if (_rem < sizeof(_len)) { ywasm_arena_free(&_arena); "
                              "return YETTY_YWASM_REPLY_VALIDATION_ERROR; }")
            body_lines.append("            memcpy(&_len, _p, sizeof(_len));")
            body_lines.append("            _p += sizeof(_len); _rem -= sizeof(_len);")
            body_lines.append("            if (_len > _rem) { ywasm_arena_free(&_arena); "
                              "return YETTY_YWASM_REPLY_VALIDATION_ERROR; }")
            body_lines.append("            if (_len > 0) {")
            body_lines.append("                char *_buf = (char *)ywasm_arena_alloc(&_arena, "
                              "(size_t)_len + 1u);")
            body_lines.append("                if (!_buf) { ywasm_arena_free(&_arena); "
                              "return YETTY_YWASM_REPLY_INTERNAL; }")
            ...
            body_lines.append(f"        if (_rem < sizeof(_ac_{an})) {{ ywasm_arena_free(&_arena); "
                              f"return YETTY_YWASM_REPLY_VALIDATION_ERROR; }}")
            ...
            body_lines.append("            return YETTY_YWASM_REPLY_VALIDATION_ERROR;")
            body_lines.append("        struct ywasm_async_cb *_cb =")
            body_lines.append("            (struct ywasm_async_cb *)malloc(sizeof(*_cb));")
            body_lines.append("        if (!_cb) {")
            if has_var:
                body_lines.append("            ywasm_arena_free(&_arena);")
            body_lines.append("            return YETTY_YWASM_REPLY_OUT_OF_MEMORY;")
            body_lines.append("        }")
            ...
            body_lines.append(f"            _arr_{an} = ({ctype} *)ywasm_arena_alloc(&_arena, "
                              f"(size_t){count_var} * sizeof({ctype}));")
            body_lines.append(f"            if (!_arr_{an}) {{ ywasm_arena_free(&_arena); "
            ...
            body_lines.append(f"            if (_rem < _nb) {{ ywasm_arena_free(&_arena); "
                              f"return YETTY_YWASM_REPLY_VALIDATION_ERROR; }}")
            body_lines.append(f"            _arr_{an} = ({ctype} *)ywasm_arena_alloc(&_arena, _nb);")
            body_lines.append(f"            if (!_arr_{an}) {{ ywasm_arena_free(&_arena); "
                              f"return YETTY_YWASM_REPLY_INTERNAL; }}")
            ...
            body_lines.append(f"            if (_rem < (size_t){count_var}) {{ "
                              f"ywasm_arena_free(&_arena); "
                              f"return YETTY_YWASM_REPLY_VALIDATION_ERROR; }}")
            body_lines.append(f"            void *_buf = ywasm_arena_alloc(&_arena, "
                              f"(size_t){count_var});")
            body_lines.append(f"            if (!_buf) {{ ywasm_arena_free(&_arena); "
                              f"return YETTY_YWASM_REPLY_INTERNAL; }}")
    ...
    if has_cb:
        ...
        body_lines.append("            ywasm_server_emit_reply(ctx, req_id, method_id,")
        body_lines.append("                                    YETTY_YWASM_REPLY_OK,")
        ...
        body_lines.append(f"        {call_expr};")
        body_lines.append(f"        {cleanup}return YETTY_YWASM_REPLY_OK;")
    elif ret_kind == "value" and value_return_supported:
        ...
        body_lines.append("            return YETTY_YWASM_REPLY_INTERNAL;")
        body_lines.append("        struct yetty_ycore_void_result _r =")
        body_lines.append(
...
def _emit_auto_trampoline(method_name: str, cb_type: str,
    ...
    sig = f"static {rt} ywasm_trampoline_{method_name}({', '.join(params)})"
    lines = [sig, "{"]
    ...
    lines.append(f"    struct ywasm_async_cb *d = (struct ywasm_async_cb *){u1};")
    if first_arg:
        fat, fan = first_arg
        ...
        lines.append("        ywasm_server_emit_reply(d->ctx, d->req_id, d->method_id,")
        lines.append("                                YETTY_YWASM_REPLY_OK,")
        lines.append("                                &_payload, sizeof(_payload));")
        lines.append("    if (YETTY_IS_ERR(e)) yetty_ycore_error_destroy(e.error);")
    else:
        lines.append("    struct yetty_ycore_void_result e =")
        lines.append("        ywasm_server_emit_reply(d->ctx, d->req_id, d->method_id,")
        lines.append("                                YETTY_YWASM_REPLY_OK, NULL, 0);")
        lines.append("    if (YETTY_IS_ERR(e)) yetty_ycore_error_destroy(e.error);")
...
def merge_with_auto(curated: list[dict], header_path: pathlib.Path) -> list[dict]:
    ...
    if not header_path.exists():
        sys.stderr.write(
            f"ywasm-gen: webgpu.h not found at {header_path}; emitting curated table only.\n"
        )
        return curated
...
def emit_methods_h(methods: list[dict]) -> str:
    lines: list[str] = []
    lines.append("/* Generated by src/yetty/ywasm/generate.py — DO NOT EDIT. */")
    lines.append("#ifndef YETTY_YWASM_METHODS_GEN_H")
    lines.append("#define YETTY_YWASM_METHODS_GEN_H")
    lines.append("")
    ...
    lines.append("#endif")
    lines.append("")
    lines.append("enum yetty_ywasm_method_id {")
    lines.append("    YETTY_YWASM_METHOD_INVALID = 0,")
    for m in methods:
        lines.append(f"    YETTY_YWASM_METHOD_{m['name']} = {m['id']},")
    lines.append("};")
    lines.append("")
    for m in methods:
        lines.append(f"struct ywasm_args_{m['name']} {{")
        for a in m["args"]:
    ...
    lines.append("struct yetty_ywasm_client;")
    lines.append("typedef void (*yetty_ywasm_reply_cb)(void *user, uint32_t status, uint32_t method_id,")
    ...
    for m in methods:
        ...
        if m.get("free_members"):
            ...
            lines.append(f"void ywasm_client_{m['name']}(struct yetty_ywasm_client *c, "
                         f"{struct} {an});")
            continue
        ...
        for a in iter_args:
            ...
            if kind == "pointer_to_pod" or kind == "pointer_to_encodable":
                ...
                params.append(f"{to_c_type(a['ctype'])} {a['name']}")
        if cb_already:
            params.append("yetty_ywasm_reply_cb cb")
        ...
        lines.append(f"{ret_t} ywasm_client_{m['name']}({', '.join(params)});")
    lines.append("")
    lines.append("#ifdef __cplusplus")
    lines.append("}")
    lines.append("#endif")
    lines.append("")
    lines.append("#endif /* YETTY_YWASM_METHODS_GEN_H */")
    lines.append("")
    return "\n".join(lines)


def _emit_codec_runtime_decls(lines: list[str], codec_meta: dict | None, side: str) -> None:
    """Forward-declarations for ywasm_encode_<Struct> / ywasm_decode_<Struct>.
    Client also gets ywasm_client_decode_<Struct> (malloc-based) and
    ywasm_client_free_<Struct> (frees malloc'd inner pointers, used by
    ...
    for n in codec_meta["encodable"]:
        if side == "client":
            ...
            lines.append(f"int  ywasm_client_decode_{n}(const uint8_t **src, size_t *rem, {n} *out);")
            lines.append(f"void ywasm_client_free_{n}({n} *s);")
        else:
...
def emit_client_c(methods: list[dict], codec_meta: dict | None = None) -> str:
    ...
    lines.append("#include <yetty/ywasm/client.h>")
    lines.append("#include <yetty/ywasm/methods.gen.h>")
    lines.append("")
    ...
    for m in methods:
        ...
        if m.get("free_members"):
            struct = m["free_members"]
            an = m["free_members_arg"]
            sig = f"void ywasm_client_{m['name']}(struct yetty_ywasm_client *c, {struct} {an})"
            lines.append(sig)
            lines.append("{")
            lines.append("    (void)c;")
            lines.append(f"    ywasm_client_free_{struct}(&{an});")
            lines.append("}")
            lines.append("")
            continue
        ...
        if m["returns"] == "handle":
            ...
            ret_t = "struct yetty_ycore_void_result"
        sig = f"{ret_t} ywasm_client_{m['name']}({', '.join(params)})"
        lines.append(sig)
        lines.append("{")
        lines.append(f"    struct ywasm_args_{m['name']} args = {{0}};")
        if m["returns"] == "handle":
            out = next(a for a in m["args"] if a.get("wgpu_kind") == "out_handle")
            lines.append(f"    uint64_t h = yetty_ywasm_client_alloc_handle(c);")
        ...
        if var_args:
            ...
            for a in var_args:
                ...
                if k == "pointer_to_encodable":
                    lines.append(f"    {{ uint8_t _f = {an} ? 1u : 0u; "
                                 ...
                                 f"if (YETTY_IS_ERR(_r)) yetty_ycore_error_destroy(_r.error); }}")
                    lines.append(f"    if ({an}) ywasm_encode_{a['ctype']}({an}, &_body);")
                elif k == "stringview":
                    lines.append("    {")
            ...
            if has_out_struct:
                ...
                lines.append("    { struct yetty_ycore_void_result _r = "
                             f"yetty_ywasm_client_send_cmd_blocking_dyn(c, "
                             f"YETTY_YWASM_METHOD_{m['name']}, "
                ...
                lines.append(f"    {ret_ct} _result = ({ret_ct}){{0}};")
                lines.append("    { struct yetty_ycore_void_result _r = "
                             f"yetty_ywasm_client_send_cmd_blocking(c, "
                             f"YETTY_YWASM_METHOD_{m['name']}, "
                             "_body.data, _body.size, "
...
def emit_server_c(methods: list[dict], codec_meta: dict | None = None) -> str:
    ...
    for m in methods:
        lines.append(f"    case YETTY_YWASM_METHOD_{m['name']}: {{")
        lines.append(f"        if (body_len < sizeof(struct ywasm_args_{m['name']}))")
        lines.append(f"            return YETTY_YWASM_REPLY_VALIDATION_ERROR;")
        lines.append(
            f"        const struct ywasm_args_{m['name']} *a = "
            f"(const struct ywasm_args_{m['name']} *)body;"
        )
        ...
        if body_src:
            ...
            lines.append("        (void)a;")
            lines.append("        return YETTY_YWASM_REPLY_OK;")
        lines.append("    }")
    lines.append("    default:")
    lines.append("        return YETTY_YWASM_REPLY_UNKNOWN_METHOD;")
    lines.append("    }")
    lines.append("}")
    lines.append("")
    return "\n".join(lines)
...
def main(argv: list[str]) -> int:
    ...
    # script (src/yetty/ywasm/generate.py). Header lives in include/,
    # .c files alongside the generator.
    here = pathlib.Path(__file__).resolve().parent
    repo = here.parent.parent.parent  # src/yetty/ywasm → repo root
    default_methods_h   = repo / "include" / "yetty" / "ywasm" / "methods.gen.h"
    ...
    sys.stderr.write(
        f"ywasm-gen: {len(METHODS)} curated + {auto_n} auto-discovered = "
