...
The output of the pipeline is a `yetty_ypaint_core_buffer*` populated by
...
| `YL_BOX_INLINE_TEXT`               | `yetty_ypaint_core_buffer_add_text(buf, x, baseline_y, &txt, font_size, fg, z, font_id, rotation)` |
| `YL_BOX_INLINE_IMAGE`              | `yetty_yimage_uniforms_serialize` + `yetty_ypaint_core_buffer_add_prim(buf, prim_buf, len)` (or grey ysdf placeholder if decode failed) |

The paint pass tracks the maximum `(x+w, y+h)` of every emitted prim and
calls `yetty_ypaint_core_buffer_set_scene_bounds` before returning.
...
`yetty_ypaint_core` and `yetty_ysdf` are local yetty libs (the GPU side).
