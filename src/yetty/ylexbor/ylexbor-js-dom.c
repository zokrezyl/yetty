...
struct wrap_cache_entry { void *key; JSValue val; };
...
static JSValue wrap_cache_lookup(JSContext *ctx, void *key)
{
	...
	for (size_t i = 0; i < WRAP_CACHE_BUCKETS; i++) {
		...
		if (!g_wrap_cache[idx].key) return JS_UNDEFINED;
				  ...
				  int argc, JSValueConst *argv)
{
	...
	if (!name) {
		...
		const lxb_char_t *v = lxb_dom_element_get_attribute(el,
			(const lxb_char_t *)raw, nlen, &vlen);
				 ...
				 int argc, JSValueConst *argv)
{
	...
	if (parent == child) return JS_DupValue(ctx, argv[0]);
	for (lxb_dom_node_t *n = parent; n; n = n->parent) {
		if (n == child) return JS_DupValue(ctx, argv[0]);
	}
	...
	if (child->parent) lxb_dom_node_remove(child);
...
static lxb_dom_node_t *coerce_to_node(JSContext *ctx, JSValueConst v,
				      lxb_dom_document_t *doc)
{
	...
	if (el) {
		lxb_dom_text_t *t = lxb_dom_document_create_text_node(doc,
			(const lxb_char_t *)s, l);
		if (t) lxb_dom_node_insert_child(
			lxb_dom_interface_node(el),
			lxb_dom_interface_node(t));
	}
...
static void detach(lxb_dom_node_t *n) { if (n && n->parent) lxb_dom_node_remove(n); }
...
static JSValue js_el_before(JSContext *ctx, JSValueConst this_val,
			    int argc, JSValueConst *argv)
{
	...
	for (int i = 0; i < argc; i++) {
		...
		if (!n) continue;
		if (node_would_cycle(self->parent, n)) continue;
		detach(n);
...
static JSValue js_el_after(JSContext *ctx, JSValueConst this_val,
			   int argc, JSValueConst *argv)
{
	lxb_dom_node_t *self = unwrap_node(this_val);
	if (!self || !self->parent) return JS_UNDEFINED;
...
static JSValue js_el_replaceWith(JSContext *ctx, JSValueConst this_val,
				 int argc, JSValueConst *argv)
{
	lxb_dom_node_t *self = unwrap_node(this_val);
	if (!self || !self->parent) return JS_UNDEFINED;
...
static JSValue js_el_prepend(JSContext *ctx, JSValueConst this_val,
			     int argc, JSValueConst *argv)
{
	lxb_dom_node_t *parent = unwrap_node(this_val);
	...
	for (int i = 0; i < argc; i++) {
		lxb_dom_node_t *n = coerce_to_node(ctx, argv[i], doc);
		if (!n) continue;
		if (node_would_cycle(parent, n)) continue;
		detach(n);
		if (first) lxb_dom_node_insert_before(first, n);
...
static JSValue js_el_append(JSContext *ctx, JSValueConst this_val,
			    int argc, JSValueConst *argv)
{
	lxb_dom_node_t *parent = unwrap_node(this_val);
	if (!parent) return JS_UNDEFINED;
...
static int xml_name_start_char(unsigned char c)
{
	...
	if (c == '_' || c == ':') return 1;
	if (c >= 0x80) return 1;	/* high Unicode — accept optimistically */
	return 0;
}
static int xml_name_char(unsigned char c)
{
	if (xml_name_start_char(c)) return 1;
	if (c >= '0' && c <= '9') return 1;
	if (c == '-' || c == '.') return 1;
	return 0;
}
...
{
	if (l == 0) return 0;
	if (!xml_name_start_char((unsigned char)s[0])) return 0;
	for (size_t i = 1; i < l; i++) {
		if (!xml_name_char((unsigned char)s[i])) return 0;
	}
	return 1;
}
...
{
	if (out_colon) *out_colon = NULL;
	if (l == 0) return 0;
	if (!xml_name_start_char((unsigned char)s[0])) return 0;
	if (s[0] == ':') return 0;	/* leading ':' rejected */
	const char *colon = NULL;
	for (size_t i = 1; i < l; i++) {
		unsigned char c = (unsigned char)s[i];
		if (c == ':') {
			if (colon) return 0;	/* second colon — invalid QName */
			if (i + 1 >= l) return 0;	/* trailing colon */
			if (!xml_name_start_char((unsigned char)s[i + 1])) return 0;
			colon = s + i;
		} else if (!xml_name_char(c)) {
			return 0;
		}
	}
	if (out_colon) *out_colon = colon;
	return colon ? 2 : 1;
}
				    ...
				    int argc, JSValueConst *argv)
{
	...
	if (!xml_name_valid(tag, tlen)) {
		...
		return JS_ThrowTypeError(ctx,
			"Document.createElement: invalid name");
	}
 ...
 * this function pass. */
static JSValue js_doc_createElementNS(JSContext *ctx, JSValueConst this_val,
				      int argc, JSValueConst *argv)
{
	...
	if (qkind == 0) {
		...
		return JS_ThrowTypeError(ctx,
			"Document.createElementNS: invalid qualified name");
	}
	...
	int is_xml_prefix   = has_prefix && prefix_len == 3 &&
			      memcmp(qname, "xml", 3) == 0;
	int is_xmlns_prefix = has_prefix && prefix_len == 5 &&
			      memcmp(qname, "xmlns", 5) == 0;
	int is_xmlns_qname  = (qlen == 5 && memcmp(qname, "xmlns", 5) == 0);
	int ns_is_xml   = !ns_is_null && ns_len == sizeof(XML_NS)   - 1 &&
			  memcmp(ns, XML_NS,   ns_len) == 0;
	int ns_is_xmlns = !ns_is_null && ns_len == sizeof(XMLNS_NS) - 1 &&
			  memcmp(ns, XMLNS_NS, ns_len) == 0;
	int ns_err = 0;
	if (has_prefix && ns_is_null) ns_err = 1;
	else if (is_xml_prefix && !ns_is_xml) ns_err = 1;
	else if ((is_xmlns_prefix || is_xmlns_qname) && !ns_is_xmlns) ns_err = 1;
	else if (ns_is_xmlns && !is_xmlns_prefix && !is_xmlns_qname) ns_err = 1;
	if (ns_err) {
		if (ns) JS_FreeCString(ctx, ns);
		JS_FreeCString(ctx, qname);
		return JS_ThrowTypeError(ctx,
			"Document.createElementNS: namespace mismatch");
	}
...
static JSValue js_doc_createDocumentFragment(JSContext *ctx,
	JSValueConst this_val, int argc, JSValueConst *argv)
{
	(void)argc; (void)argv;
				    ...
				    int argc, JSValueConst *argv)
{
	...
	size_t l = 0;
	const char *s = NULL;
	if (argc >= 1) s = JS_ToCStringLen(ctx, &l, argv[0]);
	lxb_dom_comment_t *c = lxb_dom_document_create_comment(doc,
		(const lxb_char_t *)(s ? s : ""), l);
...
static JSValue js_el_delegate_get(JSContext *ctx, JSValueConst this_val)
{
	...
	const char *def =
		...
		"  };"
		"  return Object.create(proto, { element: { value: elStub, writable: true, configurable: true, enumerable: true } });"
		"})()";
	return JS_Eval(ctx, def, strlen(def),
		"<delegate-stub>", JS_EVAL_TYPE_GLOBAL);
}

/* Generic empty-object getter — used for `dataset` etc. */
static JSValue js_el_empty_obj_get(JSContext *ctx, JSValueConst this_val)
{ (void)this_val; return JS_NewObject(ctx); }
...
{ (void)this_val; return JS_NewArray(ctx); }
...
{
	struct { char *buf; size_t len, cap; } *acc = vctx;
	if (acc->len + len + 1 > acc->cap) {
		size_t nc = acc->cap ? acc->cap * 2 : 256;
		while (nc < acc->len + len + 1) nc *= 2;
		char *p = realloc(acc->buf, nc);
		if (!p) return LXB_STATUS_ERROR_MEMORY_ALLOCATION;
		acc->buf = p; acc->cap = nc;
	}
...
static JSValue js_el_innerHTML_get(JSContext *ctx, JSValueConst this_val)
{
	...
	struct { char *buf; size_t len, cap; } acc = {0};
	for (lxb_dom_node_t *c = n->first_child; c; c = c->next) {
...
static JSValue js_el_innerHTML_set(JSContext *ctx, JSValueConst this_val,
				   JSValueConst val)
{
	lxb_dom_node_t *n = unwrap_node(this_val);
	if (!n) return JS_UNDEFINED;
	...
	while (n->first_child) lxb_dom_node_remove(n->first_child);
	...
	if (n->type == LXB_DOM_NODE_TYPE_ELEMENT) {
		...
		(void)lxb_html_element_inner_html_set(htmlel,
			(const lxb_char_t *)s, slen);
	} else {
		(void)doc;
	}
...
static int chardata_splice(lxb_dom_character_data_t *cd,
			   size_t offset, size_t count,
			   const char *ins, size_t ins_len)
{
	const char *cur = (const char *)cd->data.data;
	size_t cur_len = cd->data.length;
	if (offset > cur_len) offset = cur_len;
	if (offset + count > cur_len) count = cur_len - offset;
	size_t new_len = cur_len - count + ins_len;
	char *buf = malloc(new_len + 1);
	if (!buf) return 0;
	if (offset)             memcpy(buf, cur, offset);
	if (ins_len)            memcpy(buf + offset, ins, ins_len);
	if (cur_len - offset - count)
		memcpy(buf + offset + ins_len,
		       cur + offset + count,
		       cur_len - offset - count);
	buf[new_len] = '\0';
	lxb_status_t st = lxb_dom_character_data_replace(cd,
		(const lxb_char_t *)buf, new_len, 0, 0);
	free(buf);
	return st == LXB_STATUS_OK;
}

static JSValue js_cd_data_set(JSContext *ctx, JSValueConst this_val,
			      JSValueConst v)
{
	lxb_dom_character_data_t *cd = as_chardata(this_val);
	if (!cd) return JS_UNDEFINED;
	...
	const char *s = JS_ToCStringLen(ctx, &l, v);
	if (!s) return JS_UNDEFINED;
	(void)lxb_dom_character_data_replace(cd,
		(const lxb_char_t *)s, l, 0, 0);
	JS_FreeCString(ctx, s);
	mark_dirty(ctx);
	return JS_UNDEFINED;
}
...
{
	lxb_dom_character_data_t *cd = as_chardata(this_val);
	if (!cd) return JS_NewInt32(ctx, 0);
	return JS_NewInt32(ctx, (int32_t)cd->data.length);
}

static JSValue js_cd_appendData(JSContext *ctx, JSValueConst this_val,
				int argc, JSValueConst *argv)
{
	if (argc < 1) return JS_UNDEFINED;
	lxb_dom_character_data_t *cd = as_chardata(this_val);
...
static JSValue js_cd_insertData(JSContext *ctx, JSValueConst this_val,
				int argc, JSValueConst *argv)
{
	if (argc < 2) return JS_UNDEFINED;
	lxb_dom_character_data_t *cd = as_chardata(this_val);
...
static JSValue js_cd_deleteData(JSContext *ctx, JSValueConst this_val,
				int argc, JSValueConst *argv)
{
	if (argc < 2) return JS_UNDEFINED;
	lxb_dom_character_data_t *cd = as_chardata(this_val);
...
static JSValue js_cd_replaceData(JSContext *ctx, JSValueConst this_val,
				 int argc, JSValueConst *argv)
{
...
static JSValue js_cd_substringData(JSContext *ctx, JSValueConst this_val,
				   int argc, JSValueConst *argv)
{
	if (argc < 2) return JS_NewString(ctx, "");
	lxb_dom_character_data_t *cd = as_chardata(this_val);
	if (!cd) return JS_NewString(ctx, "");
	int32_t offset = 0, count = 0;
	JS_ToInt32(ctx, &offset, argv[0]);
	JS_ToInt32(ctx, &count, argv[1]);
	if (offset < 0 || (size_t)offset > cd->data.length)
	...
	return JS_NewStringLen(ctx,
		(const char *)cd->data.data + offset, (size_t)count);
}
...
static JSValue js_el_nodeType_get(JSContext *ctx, JSValueConst this_val)
{
	...
	switch (n->type) {
	case LXB_DOM_NODE_TYPE_ELEMENT:                return JS_NewInt32(ctx, 1);
	case LXB_DOM_NODE_TYPE_TEXT:                   return JS_NewInt32(ctx, 3);
	case LXB_DOM_NODE_TYPE_PROCESSING_INSTRUCTION: return JS_NewInt32(ctx, 7);
	case LXB_DOM_NODE_TYPE_COMMENT:                return JS_NewInt32(ctx, 8);
	case LXB_DOM_NODE_TYPE_DOCUMENT:               return JS_NewInt32(ctx, 9);
	case LXB_DOM_NODE_TYPE_DOCUMENT_TYPE:          return JS_NewInt32(ctx, 10);
	case LXB_DOM_NODE_TYPE_DOCUMENT_FRAGMENT:      return JS_NewInt32(ctx, 11);
...
static JSValue js_el_nodeName_get(JSContext *ctx, JSValueConst this_val)
{
	...
	if (n) {
		...
		if (n->type == LXB_DOM_NODE_TYPE_DOCUMENT_FRAGMENT) return JS_NewString(ctx, "#document-fragment");
	}
	lxb_dom_element_t *el = unwrap_element(this_val);
	if (!el) return JS_NewString(ctx, "");
	size_t len = 0;
...
static JSValue style_method_getPropertyValue(JSContext *ctx,
	JSValueConst this_val, int argc, JSValueConst *argv)
{
	...
	if (!attr) { JS_FreeCString(ctx, name); return JS_NewString(ctx, ""); }
	char *buf = malloc(alen + 1);
	if (!buf) { JS_FreeCString(ctx, name); return JS_NewString(ctx, ""); }
...
static JSValue style_method_setProperty(JSContext *ctx,
	JSValueConst this_val, int argc, JSValueConst *argv)
{
	if (argc < 2) return JS_UNDEFINED;
	lxb_dom_element_t *el = JS_GetOpaque(this_val, class_style_id);
	...
	if (k && v) {
		...
		size_t cur_len = 0;
		const lxb_char_t *cur = lxb_dom_element_get_attribute(el,
			(const lxb_char_t *)"style", 5, &cur_len);
		...
		if (buf) {
			...
			buf[off++] = ':';
			memcpy(buf + off, v, vlen); off += vlen;
			buf[off++] = ';';
			lxb_dom_element_set_attribute(el,
...
static JSValue style_method_removeProperty(JSContext *ctx,
	JSValueConst this_val, int argc, JSValueConst *argv)
{
...
static JSValue style_method_getPropertyPriority(JSContext *ctx,
	JSValueConst this_val, int argc, JSValueConst *argv)
{
...
static JSValue style_method_item(JSContext *ctx,
	JSValueConst this_val, int argc, JSValueConst *argv)
{
	(void)this_val; (void)argc; (void)argv;
	return JS_NewString(ctx, "");
...
static JSValue style_method_for(JSContext *ctx, JSValueConst obj,
				const char *name)
{
	struct {
		const char *name;
	...
	for (size_t i = 0; i < sizeof(table)/sizeof(table[0]); i++) {
		if (strcmp(name, table[i].name) == 0) {
			...
			JSValue fn = JS_NewCFunction(ctx, table[i].fn,
				name, table[i].argc);
				  ...
				  JSAtom prop, JSValueConst receiver)
{
	...
	if (strcmp(name, "cssText") == 0) {
		...
		const lxb_char_t *attr = lxb_dom_element_get_attribute(el,
			(const lxb_char_t *)"style", 5, &alen);
		return attr
			? JS_NewStringLen(ctx, (const char *)attr, alen)
			: JS_NewString(ctx, "");
	}
...
static int classlist_throw_if_invalid(JSContext *ctx,
				      const char *s, size_t l, int *out_throw)
{
	if (l == 0) {
	...
	for (size_t i = 0; i < l; i++) {
		...
		if (c == ' ' || c == '\t' || c == '\n' ||
			...
			JS_ThrowTypeError(ctx,
				"DOMTokenList: token contains ASCII whitespace");
			*out_throw = 1;
			return 1;
		}
	}
...
static int classlist_count_tokens(const char *list, size_t llen,
				  const char **out_starts, size_t *out_lens,
				  int max)
{
	int n = 0;
	const char *p = list, *end = list + llen;
	while (p < end && n < max) {
		...
		while (p < end && *p != ' ' && *p != '\t' && *p != '\n' &&
		       *p != '\f' && *p != '\r') p++;
		if (p > s) {
			if (out_starts) out_starts[n] = s;
			if (out_lens)   out_lens[n]   = (size_t)(p - s);
			n++;
		}
	}
	return n;
...
static JSValue js_classlist_value_set(JSContext *ctx, JSValueConst this_val,
				      JSValueConst v)
{
	lxb_dom_element_t *el = JS_GetOpaque(this_val, class_classlist_id);
	if (!el) return JS_UNDEFINED;
	...
	if (!vs) return JS_UNDEFINED;
	lxb_dom_element_set_attribute(el,
		(const lxb_char_t *)"class", 5,
		(const lxb_char_t *)vs, vlen);
	JS_FreeCString(ctx, vs);
	mark_dirty(ctx);
	return JS_UNDEFINED;
}

static JSValue js_classlist_item(JSContext *ctx, JSValueConst this_val,
				 int argc, JSValueConst *argv)
{
	if (argc < 1) return JS_NULL;
	lxb_dom_element_t *el = JS_GetOpaque(this_val, class_classlist_id);
	...
	if (idx >= n) return JS_NULL;
	return JS_NewStringLen(ctx, starts[idx], lens[idx]);
}

static JSValue js_classlist_replace(JSContext *ctx, JSValueConst this_val,
				    int argc, JSValueConst *argv)
{
	...
	lxb_dom_element_t *el = JS_GetOpaque(this_val, class_classlist_id);
	if (!el) { JS_FreeCString(ctx, o); JS_FreeCString(ctx, nw); return JS_FALSE; }
	size_t alen;
	const lxb_char_t *a = lxb_dom_element_get_attribute(el,
		(const lxb_char_t *)"class", 5, &alen);
	if (!a) { JS_FreeCString(ctx, o); JS_FreeCString(ctx, nw); return JS_FALSE; }
	if (!has_class((const char *)a, alen, o, olen)) {
		JS_FreeCString(ctx, o); JS_FreeCString(ctx, nw);
		return JS_FALSE;
	}
	...
	while (p < end) {
		...
		if (tl == olen && memcmp(s, o, olen) == 0 && !replaced) {
			memcpy(buf + out, nw, nlen); out += nlen;
			replaced = 1;
		} else {
			memcpy(buf + out, s, tl); out += tl;
		}
...
static JSValue js_classlist_supports(JSContext *ctx, JSValueConst this_val,
				     int argc, JSValueConst *argv)
{
	(void)this_val; (void)argc; (void)argv;
	...
	return JS_ThrowTypeError(ctx,
		"DOMTokenList for 'class' has no supported tokens");
}

static JSValue js_classlist_toString(JSContext *ctx, JSValueConst this_val,
				     int argc, JSValueConst *argv)
{
	(void)argc; (void)argv;
...
static char *attr_normalize_name(JSContext *ctx, JSValueConst v, size_t *out_len)
{
	...
	char *buf = malloc(l + 1);
	if (!buf) { JS_FreeCString(ctx, s); return NULL; }
	for (size_t i = 0; i < l; i++) {
		char c = s[i];
		buf[i] = (c >= 'A' && c <= 'Z') ? (char)(c + 32) : c;
	}
...
static JSValue js_impl_hasFeature(JSContext *ctx, JSValueConst this_val,
				  int argc, JSValueConst *argv)
{
	(void)ctx; (void)this_val; (void)argc; (void)argv;
	return JS_TRUE;
...
static JSValue js_impl_createDocument(JSContext *ctx, JSValueConst this_val,
				      int argc, JSValueConst *argv)
{
	(void)this_val; (void)argc; (void)argv;
...
static JSValue js_impl_createDocumentType(JSContext *ctx, JSValueConst this_val,
					  int argc, JSValueConst *argv)
{
	(void)this_val; (void)argc; (void)argv;
	return JS_NewObject(ctx);
...
static JSValue js_impl_createHTMLDocument(JSContext *ctx, JSValueConst this_val,
					  int argc, JSValueConst *argv)
{
	(void)this_val; (void)argc; (void)argv;
	return JS_NewObject(ctx);
...
void yetty_ylexbor_js_dom_reset(struct yetty_ylexbor *r)
{
	...
	if (r && r->js_ctx) wrap_cache_clear((JSContext *)r->js_ctx);
	else {
