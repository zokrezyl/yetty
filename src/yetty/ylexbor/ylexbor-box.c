        ...
        if (d->disp == YL_DISP_BLOCK) {
            ...
            if (cstyle && cstyle_len > 0) {
                ...
                const lxb_char_t *cls =
                    lxb_dom_element_get_attribute(el, (const lxb_char_t *)"class", 5, &cls_len);
            ...
            int got = 0;
            if (cstyle &&
                read_inline_color(r, (const lxb_char_t *)cstyle, cstyle_len, "color", 5, &c)) {
                s.fg = c;
                got = 1;
            }
            if (!got && istyle && read_inline_color(r, istyle, istylen, "color", 5, &c)) {
                s.fg = c;
            }
            style_to_box(b, &s);

            got = 0;
            if (cstyle && (read_inline_color(r, (const lxb_char_t *)cstyle, cstyle_len,
                                             "background-color", 16, &c) ||
                           read_inline_color(r, (const lxb_char_t *)cstyle, cstyle_len,
                                             "background", 10, &c))) {
                b->bg = c;
                got = 1;
            }
            if (!got && istyle &&
                (read_inline_color(r, istyle, istylen, "background-color", 16, &c) ||
                 read_inline_color(r, istyle, istylen, "background", 10, &c))) {
                b->bg = c;
                got = 1;
            }
            ...
            if (cstyle) {
                read_inline_box_lengths((const lxb_char_t *)cstyle, cstyle_len, "padding", 7,
                                        s.font_size, pct_basis, &b->padding_top, &b->padding_right,
                                        &b->padding_bottom, &b->padding_left);
                read_inline_box_lengths((const lxb_char_t *)cstyle, cstyle_len, "margin", 6,
                                        s.font_size, pct_basis, &b->margin_top, &b->margin_right,
                                        &b->margin_bottom, &b->margin_left);
            }
            if (istyle) {
                read_inline_box_lengths(istyle, istylen, "padding", 7, s.font_size, pct_basis,
                                        &b->padding_top, &b->padding_right, &b->padding_bottom,
                                        &b->padding_left);
                read_inline_box_lengths(istyle, istylen, "margin", 6, s.font_size, pct_basis,
                                        &b->margin_top, &b->margin_right, &b->margin_bottom,
                                        &b->margin_left);
            }
            /* Width / max-width / min-width — clamp the box's
			 * content area in the layout pass. */
            if (cstyle) {
                read_inline_length((const lxb_char_t *)cstyle, cstyle_len, "width", 5, s.font_size,
                                   pct_basis, &b->css_width);
                read_inline_length((const lxb_char_t *)cstyle, cstyle_len, "max-width", 9,
                                   s.font_size, pct_basis, &b->css_max_width);
                read_inline_length((const lxb_char_t *)cstyle, cstyle_len, "min-width", 9,
                                   s.font_size, pct_basis, &b->css_min_width);
                read_inline_length((const lxb_char_t *)cstyle, cstyle_len, "height", 6, s.font_size,
                                   pct_basis, &b->css_height);
            }
            if (istyle) {
                read_inline_length(istyle, istylen, "width", 5, s.font_size, pct_basis,
                                   &b->css_width);
                read_inline_length(istyle, istylen, "max-width", 9, s.font_size, pct_basis,
                                   &b->css_max_width);
                read_inline_length(istyle, istylen, "min-width", 9, s.font_size, pct_basis,
                                   &b->css_min_width);
                read_inline_length(istyle, istylen, "height", 6, s.font_size, pct_basis,
                                   &b->css_height);
            }
            ...
            if (cstyle) {
                if (read_inline_keyword((const lxb_char_t *)cstyle, cstyle_len, "margin-left", 11,
                                        auto_choices) == 0) {
                    b->margin_left_auto = true;
                }
                if (read_inline_keyword((const lxb_char_t *)cstyle, cstyle_len, "margin-right", 12,
                                        auto_choices) == 0) {
                    b->margin_right_auto = true;
                }
            }
            if (istyle) {
                if (read_inline_keyword(istyle, istylen, "margin-left", 11, auto_choices) == 0) {
                    b->margin_left_auto = true;
                }
                if (read_inline_keyword(istyle, istylen, "margin-right", 12, auto_choices) == 0) {
                    b->margin_right_auto = true;
                }
            }
			 ...
			 * value's tokens; if it appears, set both auto flags. */
            {
                ...
                const char *mv = NULL;
                if (cstyle) {
                    mv = find_inline_decl((const lxb_char_t *)cstyle, cstyle_len, "margin", 6,
                                          &mvlen);
                }
                if (!mv && istyle) {
                    mv = find_inline_decl(istyle, istylen, "margin", 6, &mvlen);
                }
                if (mv && mvlen >= 4) {
			 ...
			 * content area. */
            static const char *const ta_choices[] = {"left", "center", "right", "justify", NULL};
            int ta = -1;
            if (cstyle) {
                ta = read_inline_keyword((const lxb_char_t *)cstyle, cstyle_len, "text-align", 10,
                                         ta_choices);
            }
            if (ta < 0 && istyle) {
                ta = read_inline_keyword(istyle, istylen, "text-align", 10, ta_choices);
            }
            ...
            if (cstyle && memmem(cstyle, cstyle_len, "border", 6)) {
                ...
                ydebug("css tag=%d cstyle=\"%.*s\"", (int)el->node.local_name, (int)cstyle_len,
                       cstyle);
            }
            ...
            for (int side = 0; side < 4; side++) {
                ...
                if (cstyle) {
                    read_inline_length((const lxb_char_t *)cstyle, cstyle_len, SIDE_W[side], kl,
                                       s.font_size, pct_basis, side_w[side]);
                }
                if (istyle) {
                    read_inline_length(istyle, istylen, SIDE_W[side], kl, s.font_size, pct_basis,
                                       side_w[side]);
                }
                ...
                if ((cstyle && read_inline_color(r, (const lxb_char_t *)cstyle, cstyle_len,
                                                 SIDE_C[side], kl, &sc)) ||
                    (istyle && read_inline_color(r, istyle, istylen, SIDE_C[side], kl, &sc))) {
            ...
            float bw = 0;
            if (b->border_top == 0 && b->border_right == 0 && b->border_bottom == 0 &&
                b->border_left == 0) {
                size_t mvlen = 0;
                const char *mv = NULL;
                if (cstyle) {
                    mv = find_inline_decl((const lxb_char_t *)cstyle, cstyle_len, "border", 6,
                                          &mvlen);
                }
                if (!mv && istyle) {
                    mv = find_inline_decl(istyle, istylen, "border", 6, &mvlen);
                }
                if (mv && mvlen) {
                    size_t qi = 0;
                    while (qi < mvlen) {
                        while (qi < mvlen && (mv[qi] == ' ' || mv[qi] == '\t')) {
                            qi++;
                        }
                        size_t qs = qi;
                        while (qi < mvlen && mv[qi] != ' ' && mv[qi] != '\t') {
						 ...
						 * doesn't parse. */
                        float bw_try = 0;
                        if (parse_css_length(mv + qs, qi - qs, s.font_size, pct_basis, &bw_try) &&
                            bw_try > 0 && bw == 0) {
                            bw = bw_try;
                            continue;
                        }
                        struct yetty_ylexbor_color cc = {0};
                        if (parse_css_color(mv + qs, qi - qs, &cc) && cc.a != 0 &&
                            side_color.a == 0) {
                            side_color = cc;
                        }
            ...
            read_inline_length((const lxb_char_t *)cstyle ?: (const lxb_char_t *)"",
                               cstyle ? cstyle_len : 0, "border-radius", 13, s.font_size, pct_basis,
                               &b->border_radius);
            if (istyle) {
                read_inline_length(istyle, istylen, "border-radius", 13, s.font_size, pct_basis,
                                   &b->border_radius);
            }
            if (bw > 0 && b->border_top == 0) {
                b->border_top = b->border_right = b->border_bottom = b->border_left = bw;
            }
            if (b->border_color.a == 0 && side_color.a != 0) {
                b->border_color = side_color;
            } else if ((b->border_top || b->border_right || b->border_bottom || b->border_left) &&
                       b->border_color.a == 0) {
