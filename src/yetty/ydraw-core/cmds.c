...
static const struct yetty_ydraw_core_prim_base_ops cmd_base_ops = {
    .size = cmd_prim_size,
    .aabb = cmd_prim_aabb,
};

struct yetty_ydraw_core_prim_base_ops_ptr_result yetty_ydraw_core_cmd_handler(uint32_t prim_type)
{
    if (prim_type <= YETTY_YDRAW_CMD_END) {
        return YETTY_OK(yetty_ydraw_core_prim_base_ops_ptr, &cmd_base_ops);
    }
