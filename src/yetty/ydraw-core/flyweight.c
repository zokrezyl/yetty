...
struct yetty_ydraw_core_handler_reg {
    uint32_t type_min;
    uint32_t type_max;
    yetty_ydraw_prim_handler_fn handler;
};

struct yetty_ydraw_core_flyweight_registry {
    yetty_ydraw_prim_handler_fn default_handler;
    struct yetty_ydraw_core_handler_reg handlers[FLYWEIGHT_MAX_HANDLERS];
    ...
    void)
{
    struct yetty_ydraw_core_flyweight_registry *reg =
        calloc(1, sizeof(struct yetty_ydraw_core_flyweight_registry));
    if (!reg) {
        return YETTY_ERR(yetty_ydraw_core_flyweight_registry_ptr, "calloc failed");
    }
    return YETTY_OK(yetty_ydraw_core_flyweight_registry_ptr, reg);
}

void yetty_ydraw_core_flyweight_registry_destroy(struct yetty_ydraw_core_flyweight_registry *reg)
{
    free(reg);
}

void yetty_ydraw_core_flyweight_registry_set_default(
    struct yetty_ydraw_core_flyweight_registry *reg, yetty_ydraw_prim_handler_fn handler)
{
    if (reg) {
...
{
    static struct yetty_ydraw_core_prim_flyweight fw;

    if (!reg) {
        return YETTY_ERR(yetty_ydraw_core_prim_flyweight_ptr, "registry is NULL");
    }
    if (!prim_data) {
        return YETTY_ERR(yetty_ydraw_core_prim_flyweight_ptr, "prim_data is NULL");
    }

    ydebug("flyweight_registry_get: prim_type=0x%08x handler_count=%zu", prim_type,
           reg->handler_count);

    // Try default handler first (fast path for SDF types)
    if (reg->default_handler) {
        struct yetty_ydraw_core_prim_base_ops_ptr_result ops_res = reg->default_handler(prim_type);
        if (YETTY_IS_OK(ops_res)) {
            ...
            return YETTY_OK(yetty_ydraw_core_prim_flyweight_ptr, &fw);
        }
    }
    ...
    for (size_t i = 0; i < reg->handler_count; i++) {
        ...
        if (prim_type >= reg->handlers[i].type_min && prim_type <= reg->handlers[i].type_max) {
            struct yetty_ydraw_core_prim_base_ops_ptr_result ops_res =
                reg->handlers[i].handler(prim_type);
            if (YETTY_IS_OK(ops_res)) {
                ydebug("flyweight_registry_get: handler[%zu] matched prim_type=0x%08x", i,
                       prim_type);
                fw.data = prim_data;
                fw.ops = ops_res.value;
                return YETTY_OK(yetty_ydraw_core_prim_flyweight_ptr, &fw);
    ...
    return YETTY_ERR(yetty_ydraw_core_prim_flyweight_ptr, "no handler for prim_type");
}
