    ...
    void)
{
    ...
    if (!reg) {
        return YETTY_ERR(yetty_ydraw_flyweight_registry_ptr, "calloc failed");
    }
    return YETTY_OK(yetty_ydraw_flyweight_registry_ptr, reg);
}

void yetty_ydraw_flyweight_registry_destroy(struct yetty_ydraw_flyweight_registry *reg)
{
    free(reg);
}
...
struct yetty_ydraw_drawable_flyweight_ptr_result yetty_ydraw_flyweight_registry_get(
    const struct yetty_ydraw_flyweight_registry *reg, uint32_t prim_type,
    const uint32_t *prim_data)
{
    static struct yetty_ydraw_drawable_flyweight fw;

    if (!reg) {
        return YETTY_ERR(yetty_ydraw_drawable_flyweight_ptr, "registry is NULL");
    }
    if (!prim_data) {
        return YETTY_ERR(yetty_ydraw_drawable_flyweight_ptr, "prim_data is NULL");
    }
    ...
    if (reg->default_handler) {
        struct yetty_ydraw_drawable_base_ops_ptr_result ops_res = reg->default_handler(prim_type);
    ...
    return YETTY_ERR(yetty_ydraw_drawable_flyweight_ptr, "no handler for prim_type");
}
