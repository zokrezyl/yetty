...
{
    size_t size = yetty_ydraw_complex_record_size(prim);
...
{
    if (yetty_ydraw_is_complex(drawable_type)) {
...
{
    const struct yetty_ydraw_complex_record *prim = data;
    return sizeof(struct yetty_ydraw_complex_record) + prim->header.payload_size;
}
...
#define COMPLEX_BOUNDS_SIZE 16 /* 4 floats */
...
struct rectangle_result yetty_ydraw_complex_record_aabb(const void *data)
{
    ...
    const struct yetty_ydraw_complex_record *prim = data;
    if (prim->header.payload_size < COMPLEX_BOUNDS_SIZE) {
