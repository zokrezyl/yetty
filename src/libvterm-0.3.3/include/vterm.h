...
{
    return p.row >= r.start_row && p.row < r.end_row && p.col >= r.start_col && p.col < r.end_col;
}
...
/* yetty: modified for direct buffer upload - 16 bytes (4 u32). GPU shaders
 * (text-layer.wgsl, shader-glyph-layer.wgsl) stride by sizeof/4 — keep the
 * struct a whole number of u32 and bump the shader stride if it grows. */
#pragma pack(push, 1)
typedef struct {
...
typedef VTermResolvedGlyph (*VTermGlyphResolver)(const uint32_t *chars, int count, int bold,
...
/* yetty: direct cursor/scroll control for ydraw integration */
