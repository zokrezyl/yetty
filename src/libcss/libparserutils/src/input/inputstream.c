...
static inline parserutils_error parserutils_inputstream_strip_bom(uint16_t *mibenum,
...
parserutils_error parserutils_inputstream_refill_buffer(parserutils_inputstream_private *stream)
{
    ...
    if (stream->done_first_chunk == false) {
        ...
        if (stream->csdetect != NULL) {
            error = stream->csdetect(stream->raw->data, stream->raw->length, &stream->mibenum,
                                     &stream->encsrc);
            if (error != PARSERUTILS_OK) {
