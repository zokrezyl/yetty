...
css_error emitToken(css_lexer *lexer, css_token_type type, css_token **token)
{
    ...
    case CSS_TOKEN_STRING:
        ...
        if (t->data.len > 0 &&
            (t->data.data[t->data.len - 1] == '"' || t->data.data[t->data.len - 1] == '\'')) {
            t->data.len -= 1;
        }
        break;
...
css_error Comment(css_lexer *lexer, css_token **token)
{
    ...
    case InComment:
        ...
        while (1) {
            perror =
                parserutils_inputstream_peek(lexer->input, lexer->bytesReadForToken, &cptr, &clen);
            if (perror != PARSERUTILS_OK && perror != PARSERUTILS_EOF) {
                return css_error_from_parserutils_error(perror);
            }
...
css_error String(css_lexer *lexer, css_token **token)
{
    ...
    return emitToken(lexer, error == CSS_INVALID ? CSS_TOKEN_INVALID_STRING : CSS_TOKEN_STRING,
                     token);
}
...
css_error consumeEscape(css_lexer *lexer, bool nl)
{
    ...
    if (!lexer->escapeSeen) {
        if (lexer->bytesReadForToken > 1) {
            ...
            /* -1 to skip '\\' */
            perror = parserutils_buffer_append(lexer->unescapedTokenData, sdata,
                                               lexer->bytesReadForToken - 1);
            if (perror != PARSERUTILS_OK) {
                return css_error_from_parserutils_error(perror);
            }
    ...
    if (nl && c == '\r') {
        perror = parserutils_inputstream_peek(lexer->input, lexer->bytesReadForToken + clen, &cptr,
                                              &clen);
        if (perror != PARSERUTILS_OK && perror != PARSERUTILS_EOF) {
            return css_error_from_parserutils_error(perror);
        }
...
css_error consumeUnicode(css_lexer *lexer, uint32_t ucs)
{
    ...
    if (ucs > 0x10FFFF || ucs <= 0x0008 || ucs == 0x000B || (0x000E <= ucs && ucs <= 0x001F) ||
        (0x007F <= ucs && ucs <= 0x009F) || (0xD800 <= ucs && ucs <= 0xDFFF) ||
    ...
    if (perror == PARSERUTILS_OK && *cptr == '\r') {
        ...
        perror =
            parserutils_inputstream_peek(lexer->input, lexer->bytesReadForToken + 1, &cptr, &clen);
        if (perror != PARSERUTILS_OK && perror != PARSERUTILS_EOF) {
            /* Rewind what we've read */
            lexer->bytesReadForToken = bytesReadInit;
            return css_error_from_parserutils_error(perror);
