...
extern void parserutils_vector_dump(parserutils_vector *vector, const char *prefix,
                                    void (*printer)(void *item));
#ifdef DEBUG_STACK
...
static css_error css__parser_create_internal(const char *charset, css_charset_source cs_source,
                                             parser_state initial, css_parser **parser);
 ...
 *         CSS_NOMEM on memory exhaustion
 */
css_error css__parser_create_for_inline_style(const char *charset, css_charset_source cs_source,
                                              css_parser **parser)
{
 ...
 *         CSS_NOMEM on memory exhaustion
 */
css_error css__parser_create_for_media_query(const char *charset, css_charset_source cs_source,
                                             css_parser **parser)
{
 ...
 *         CSS_NOMEM on memory exhaustion
 */
css_error css__parser_create_internal(const char *charset, css_charset_source cs_source,
                                      parser_state initial, css_parser **parser)
{
    css_parser *p;
    parserutils_error perror;
...
css_error parseStart(css_parser *parser)
{
    ...
    case Initial:
        ...
        if (parser->event != NULL) {
            parser->event(CSS_PARSER_START_STYLESHEET, NULL, parser->event_pw);
        }
...
#endif
    if (parser->event != NULL) {
        parser->event(CSS_PARSER_END_STYLESHEET, NULL, parser->event_pw);
    }
...
css_error parseRuleset(css_parser *parser)
{
...
#endif
            if (parser->event != NULL) {
                if (parser->event(CSS_PARSER_START_RULESET, NULL, parser->event_pw) ==
                    CSS_INVALID) {
    ...
    case Brace:
        ...
        if (parser->parseError == false && parser->event != NULL) {
            if (parser->event(CSS_PARSER_START_RULESET, parser->tokens, parser->event_pw) ==
                CSS_INVALID) {
                parser->parseError = true;
            }
        }
...
#endif
        if (parser->event != NULL) {
            if (parser->event(CSS_PARSER_START_ATRULE, parser->tokens, parser->event_pw) ==
                CSS_INVALID) {
                parser_state to = {sMalformedAtRule, Initial};
...
#endif
        if (parser->event != NULL) {
            parser->event(CSS_PARSER_START_BLOCK, NULL, parser->event_pw);
        }
...
#endif
                    if (parser->event != NULL) {
                        parser->event(CSS_PARSER_BLOCK_CONTENT, parser->tokens, parser->event_pw);
                    }

                    discard_tokens(parser);

                    return transition(parser, to, subsequent);
                } else if (lwc_string_length(token->idata) == 1 &&
                           lwc_string_data(token->idata)[0] == ';') {
...
#endif
                    if (parser->event != NULL) {
                        parser->event(CSS_PARSER_END_BLOCK_CONTENT, parser->tokens,
                                      parser->event_pw);
                    }
...
#endif
        if (parser->event != NULL) {
            parser->event(CSS_PARSER_DECLARATION, parser->tokens, parser->event_pw);
        }
        break;
    }
...
css_error parseAny(css_parser *parser)
{
    ...
    case Initial:
        ...
        if (token->type != CSS_TOKEN_IDENT && token->type != CSS_TOKEN_NUMBER &&
            token->type != CSS_TOKEN_PERCENTAGE && token->type != CSS_TOKEN_DIMENSION &&
            token->type != CSS_TOKEN_STRING && token->type != CSS_TOKEN_CHAR &&
            token->type != CSS_TOKEN_URI && token->type != CSS_TOKEN_HASH &&
            token->type != CSS_TOKEN_UNICODE_RANGE && token->type != CSS_TOKEN_INCLUDES &&
            token->type != CSS_TOKEN_DASHMATCH && token->type != CSS_TOKEN_PREFIXMATCH &&
            token->type != CSS_TOKEN_SUFFIXMATCH && token->type != CSS_TOKEN_SUBSTRINGMATCH &&
        ...
        if (token->type == CSS_TOKEN_FUNCTION) {
            ...
            parserutils_stack_push(parser->open_items,
                                   &(lwc_string_data(token->idata)[0] == '(' ? ")" : "]")[0]);
            state->substate = WS;
        } else {
            state->substate = WS2;
        }
    ...
    case AfterAny0: {
        ...
        if (token->type == CSS_TOKEN_CHAR && lwc_string_length(token->idata) == 1 &&
            lwc_string_data(token->idata)[0] ==
                ((uint8_t *)parserutils_stack_get_current(parser->open_items))[0]) {
...
css_error parseMalformedDeclaration(css_parser *parser)
{
    ...
    case Go:
        while (1) {
            ...
            if (len != 1 || (data[0] != '{' && data[0] != '}' && data[0] != '[' && data[0] != ']' &&
...
css_error parseMalformedSelector(css_parser *parser)
{
    ...
    case Go:
        while (1) {
            ...
            if (want == '{' && match != NULL &&
                parserutils_stack_get_current(parser->open_items) == NULL) {
                break;
            }
        }
    }
...
css_error parseInlineStyle(css_parser *parser)
{
    ...
    case Initial:
        ...
        if (parser->event != NULL) {
            ...
            /* 2) begin ruleset */
            parser->event(CSS_PARSER_START_RULESET, NULL, parser->event_pw);
        }
