...
static css_error mq_parse_condition(lwc_string **strings, const parserutils_vector *vector,
                                    int32_t *ctx, bool permit_or, css_mq_cond **cond);

static css_error mq_parse_ratio(const parserutils_vector *vector, int32_t *ctx,
...
static css_error mq_parse_range__convert_to_level_4(css_mq_feature *feature)
{
    ...
    if (name[0] == 'm' && name[3] == '-') {
        if (name[1] == 'i' && name[2] == 'n') {
            if (lwc_intern_substring(feature->name, 4, lwc_string_length(feature->name) - 4,
...
static css_error mq_parse_range(lwc_string **strings, const parserutils_vector *vector,
                                int32_t *ctx, const css_token *name_or_value,
                                css_mq_feature **feature)
{
    ...
    if (name_or_value->type == CSS_TOKEN_NUMBER &&
        ...
        value_is_ratio = true;
               ...
               lwc_string_caseless_isequal(name_or_value->idata, strings[INFINITE], &match) ==
                   lwc_error_ok &&
               match == false) {
...
static css_error mq_parse_media_feature(lwc_string **strings, const parserutils_vector *vector,
                                        int32_t *ctx, css_mq_feature **feature)
{
 ...
 */
static css_error mq_parse_consume_any_value(lwc_string **strings, const parserutils_vector *vector,
                                            int32_t *ctx, bool until, const char until_char)
{
    const css_token *token;
    css_error error;
...
static css_error mq_parse_general_enclosed(lwc_string **strings, const parserutils_vector *vector,
                                           int32_t *ctx)
{
    const css_token *token;
    css_error error;
...
static css_error mq_parse_media_in_parens(lwc_string **strings, const parserutils_vector *vector,
                                          int32_t *ctx, css_mq_cond_or_feature **cond_or_feature)
{
    const css_token *token;
    bool match;
    int old_ctx;
    ...
    if (tokenIsChar(token, '(') ||
        (token->type == CSS_TOKEN_IDENT &&
         lwc_string_caseless_isequal(token->idata, strings[NOT], &match) == lwc_error_ok &&
         match)) {
        css_mq_cond *cond;
    ...
    while (token != NULL && tokenIsChar(token, ')') == false && tokenIsChar(token, ',') == false) {
        ...
        if (token != NULL && tokenIsChar(token, ')') == false && tokenIsChar(token, ',') == false) {
            if (token->type != CSS_TOKEN_IDENT) {
                css__mq_cond_destroy(result);
                return CSS_INVALID;
            } else if (lwc_string_caseless_isequal(token->idata, strings[AND], &match) ==
                           lwc_error_ok &&
                       match) {
                if (op != 0 && op != AND) {
                    css__mq_cond_destroy(result);
                ...
                op = AND;
            } else if (lwc_string_caseless_isequal(token->idata, strings[OR], &match) ==
                           lwc_error_ok &&
                       match) {
...
static css_error mq_parse_type(lwc_string **strings, lwc_string *type, uint64_t *result)
{
    ...
    } else if (lwc_string_caseless_isequal(type, strings[BRAILLE], &match) == lwc_error_ok &&
               match) {
        *result = CSS_MEDIA_BRAILLE;
    } else if (lwc_string_caseless_isequal(type, strings[EMBOSSED], &match) == lwc_error_ok &&
               match) {
        *result = CSS_MEDIA_EMBOSSED;
    } else if (lwc_string_caseless_isequal(type, strings[HANDHELD], &match) == lwc_error_ok &&
               match) {
    ...
    } else if (lwc_string_caseless_isequal(type, strings[PROJECTION], &match) == lwc_error_ok &&
               match) {
        *result = CSS_MEDIA_PROJECTION;
    } else if (lwc_string_caseless_isequal(type, strings[SCREEN], &match) == lwc_error_ok &&
               match) {
        *result = CSS_MEDIA_SCREEN;
    } else if (lwc_string_caseless_isequal(type, strings[SPEECH], &match) == lwc_error_ok &&
               match) {
...
static css_error mq_parse_media_query(lwc_string **strings, const parserutils_vector *vector,
                                      int32_t *ctx, css_mq_query **query)
{
    const css_token *token;
    ...
    if (lwc_string_caseless_isequal(token->idata, strings[NOT], &match) == lwc_error_ok && match) {
        ...
        consumeWhitespace(vector, ctx);
        token = parserutils_vector_iterate(vector, ctx);
    } else if (lwc_string_caseless_isequal(token->idata, strings[ONLY], &match) == lwc_error_ok &&
               match) {
        consumeWhitespace(vector, ctx);
        token = parserutils_vector_iterate(vector, ctx);
    }
    ...
    if (token != NULL) {
        if (token->type != CSS_TOKEN_IDENT ||
            lwc_string_caseless_isequal(token->idata, strings[AND], &match) != lwc_error_ok ||
            match == false) {
            free(result);
            return CSS_INVALID;
        }
...
css_error css__mq_parse_media_list(lwc_string **strings, const parserutils_vector *vector,
...
css_error css_parse_media_query(lwc_string **strings, const uint8_t *mq, size_t len,
