                                  ...
                                  css_style *result)
{
    ...
    do {
        ...
        if ((token->type == CSS_TOKEN_IDENT) &&
            (lwc_string_caseless_isequal(token->idata, c->strings[THIN], &match) == lwc_error_ok &&
             match)) {
            side_val[side_count] = BORDER_WIDTH_THIN;
            parserutils_vector_iterate(vector, ctx);
            error = CSS_OK;
        } else if ((token->type == CSS_TOKEN_IDENT) &&
                   (lwc_string_caseless_isequal(token->idata, c->strings[MEDIUM], &match) ==
                        lwc_error_ok &&
                    match)) {
            ...
            error = CSS_OK;
                   ...
                   (lwc_string_caseless_isequal(token->idata, c->strings[THICK], &match) ==
                        lwc_error_ok &&
                    match)) {
            parserutils_vector_iterate(vector, ctx);
            error = CSS_OK;
            ...
            error = css__parse_unit_specifier(c, vector, ctx, UNIT_PX, &side_length[side_count],
