...
static css_error parse_system_font(css_language *c, css_style *result, css_system_font *system_font)
{
    ...
    case CSS_FONT_VARIANT_NORMAL:
        error =
            css__stylesheet_style_appendOPV(result, CSS_PROP_FONT_VARIANT, 0, FONT_VARIANT_NORMAL);
        break;

    case CSS_FONT_VARIANT_SMALL_CAPS:
        error = css__stylesheet_style_appendOPV(result, CSS_PROP_FONT_VARIANT, 0,
    ...
    case CSS_FONT_WEIGHT_NORMAL:
        error =
            css__stylesheet_style_appendOPV(result, CSS_PROP_FONT_WEIGHT, 0, FONT_WEIGHT_NORMAL);
        break;
    ...
    case CSS_FONT_WEIGHT_BOLDER:
        error =
            css__stylesheet_style_appendOPV(result, CSS_PROP_FONT_WEIGHT, 0, FONT_WEIGHT_BOLDER);
        break;

    case CSS_FONT_WEIGHT_LIGHTER:
        error =
            css__stylesheet_style_appendOPV(result, CSS_PROP_FONT_WEIGHT, 0, FONT_WEIGHT_LIGHTER);
        break;
    ...
    error = css__stylesheet_style_vappend(result, 2, system_font->size.size,
    ...
    error = css__stylesheet_style_vappend(result, 2, system_font->line_height.size,
    ...
    /* font family */
    if ((lwc_string_caseless_isequal(system_font->family, c->strings[SERIF], &match) ==
             lwc_error_ok &&
         match)) {
        error = css__stylesheet_style_appendOPV(result, CSS_PROP_FONT_FAMILY, 0, FONT_FAMILY_SERIF);
    } else if ((lwc_string_caseless_isequal(system_font->family, c->strings[SANS_SERIF], &match) ==
                    lwc_error_ok &&
                match)) {
        error = css__stylesheet_style_appendOPV(result, CSS_PROP_FONT_FAMILY, 0,
                                                FONT_FAMILY_SANS_SERIF);
    } else if ((lwc_string_caseless_isequal(system_font->family, c->strings[CURSIVE], &match) ==
                    lwc_error_ok &&
                match)) {
        error =
            css__stylesheet_style_appendOPV(result, CSS_PROP_FONT_FAMILY, 0, FONT_FAMILY_CURSIVE);
    } else if ((lwc_string_caseless_isequal(system_font->family, c->strings[FANTASY], &match) ==
                    lwc_error_ok &&
                match)) {
        error =
            css__stylesheet_style_appendOPV(result, CSS_PROP_FONT_FAMILY, 0, FONT_FAMILY_FANTASY);
    } else if ((lwc_string_caseless_isequal(system_font->family, c->strings[MONOSPACE], &match) ==
                    lwc_error_ok &&
                match)) {
        error =
            css__stylesheet_style_appendOPV(result, CSS_PROP_FONT_FAMILY, 0, FONT_FAMILY_MONOSPACE);
    } else {
        uint32_t snumber;
        ...
        error =
            css__stylesheet_style_appendOPV(result, CSS_PROP_FONT_FAMILY, 0, FONT_FAMILY_STRING);
        if (error != CSS_OK) {
            return error;
        }
                          ...
                          css_style *result)
{
    ...
    for (svw = 0; svw < 3; svw++) {
        ...
        if ((style) && (error = css__parse_font_style(c, vector, ctx, style_style)) == CSS_OK) {
            style = false;
                   ...
                   (error = css__parse_font_variant(c, vector, ctx, variant_style)) == CSS_OK) {
            variant = false;
        } else if ((weight) &&
                   (error = css__parse_font_weight(c, vector, ctx, weight_style)) == CSS_OK) {
            weight = false;
        }
    ...
    if (style) {
        error =
            css__stylesheet_style_appendOPV(style_style, CSS_PROP_FONT_STYLE, 0, FONT_STYLE_NORMAL);
        if (error != CSS_OK) {
            goto css__parse_font_cleanup;
        }
    }
    ...
    if (variant) {
        error = css__stylesheet_style_appendOPV(variant_style, CSS_PROP_FONT_VARIANT, 0,
                                                FONT_VARIANT_NORMAL);
        if (error != CSS_OK) {
            goto css__parse_font_cleanup;
        }
    ...
    if (weight) {
        error = css__stylesheet_style_appendOPV(weight_style, CSS_PROP_FONT_WEIGHT, 0,
                                                FONT_WEIGHT_NORMAL);
        if (error != CSS_OK) {
            goto css__parse_font_cleanup;
        }
    ...
    if (line_height) {
        error = css__stylesheet_style_appendOPV(line_height_style, CSS_PROP_LINE_HEIGHT, 0,
                                                LINE_HEIGHT_NORMAL);
        if (error != CSS_OK) {
            goto css__parse_font_cleanup;
        }
