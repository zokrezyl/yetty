                             ...
                             css_style *result)
{
    ...
    if (flag_value != FLAG_VALUE__NONE) {
        error = css_stylesheet_style_flag_value(result, flag_value, CSS_PROP_CONTENT);
               ...
               (lwc_string_caseless_isequal(token->idata, c->strings[NORMAL], &match) ==
                    lwc_error_ok &&
                match)) {
        error = css__stylesheet_style_appendOPV(result, CSS_PROP_CONTENT, 0, CONTENT_NORMAL);
    } else if ((token->type == CSS_TOKEN_IDENT) &&
               (lwc_string_caseless_isequal(token->idata, c->strings[NONE], &match) ==
                    lwc_error_ok &&
                match)) {
...
#define CSS_APPEND(CSSVAL)                                                                         \
    css__stylesheet_style_append(result, first ? buildOPV(CSS_PROP_CONTENT, 0, CSSVAL) : CSSVAL)
        ...
        while (token != NULL) {
            if ((token->type == CSS_TOKEN_IDENT) &&
                (lwc_string_caseless_isequal(token->idata, c->strings[OPEN_QUOTE], &match) ==
                     lwc_error_ok &&
                 match)) {
                ...
                error = CSS_APPEND(CONTENT_CLOSE_QUOTE);
                       ...
                       (lwc_string_caseless_isequal(token->idata, c->strings[NO_OPEN_QUOTE],
                                                    &match) == lwc_error_ok &&
                        match)) {
                error = CSS_APPEND(CONTENT_NO_OPEN_QUOTE);
            } else if (token->type == CSS_TOKEN_IDENT &&
                       (lwc_string_caseless_isequal(token->idata, c->strings[NO_CLOSE_QUOTE],
                                                    &match) == lwc_error_ok &&
                        match)) {
                ...
                error =
                    css__stylesheet_string_add(c->sheet, lwc_string_ref(token->idata), &snumber);
                if (error != CSS_OK) {
                    *ctx = orig_ctx;
                    return error;
                }
                ...
                error = css__stylesheet_style_append(result, uri_snumber);
                       ...
                       (lwc_string_caseless_isequal(token->idata, c->strings[ATTR], &match) ==
                            lwc_error_ok &&
                        match)) {
                uint32_t snumber;
                ...
                if (token == NULL || tokenIsChar(token, ')') == false) {
                    *ctx = orig_ctx;
                    return CSS_INVALID;
                       ...
                       (lwc_string_caseless_isequal(token->idata, c->strings[COUNTER], &match) ==
                            lwc_error_ok &&
                        match)) {
                lwc_string *name;
                uint32_t snumber;
                ...
                error = css__stylesheet_style_append(result, snumber);
                       ...
                       (lwc_string_caseless_isequal(token->idata, c->strings[COUNTERS], &match) ==
                            lwc_error_ok &&
                        match)) {
                lwc_string *name;
                lwc_string *sep;
