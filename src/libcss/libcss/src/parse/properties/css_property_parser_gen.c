...
void output_header(FILE *outputf, const char *descriptor, struct keyval *parser_id, bool is_generic)
{
    fprintf(outputf,
            ...
            "		css_style *result%s)\n"
            "{\n",
            descriptor, parser_id->key,
            is_generic ? " * \\param op	 Bytecode OpCode for CSS property to encode\n" : "",
            parser_id->key, is_generic ? ", enum css_properties_e op" : "");
}

void output_token_type_check(FILE *outputf, bool do_token_check, struct keyval_list *IDENT,
                             struct keyval_list *URI, struct keyval_list *NUMBER)
{
    fprintf(outputf,
...
void output_ident_list(FILE *outputf, struct keyval *parseid, struct keyval_list *kvlist)
{
    ...
    fprintf(outputf,
            ...
            "\t\t\tcss_fixed num;\n"
            "\t\t\tint pctx;\n\n"
            "\t\t\terror = css__stylesheet_string_add(c->sheet, lwc_string_ref(token->idata), "
            "&snumber);\n"
            "\t\t\tif (error != CSS_OK) {\n"
            "\t\t\t\t*ctx = orig_ctx;\n"
            "\t\t\t\treturn error;\n"
...
{
    fprintf(outputf, "	if (error != CSS_OK)\n"
                     "		*ctx = orig_ctx;\n"
                     "	\n"
                     "	return error;\n"
                     "}\n\n");
