...
#define MAP_ENTRIES 52
    ...
    const struct {
        int stringid;
        int value;
    } mapping[MAP_ENTRIES] = {{DISC, LIST_STYLE_TYPE_DISC},
    ...
    for (midx = 0; midx < MAP_ENTRIES; midx++) {
        if ((lwc_string_caseless_isequal(ident->idata, c->strings[mapping[midx].stringid],
                                         &match) == lwc_error_ok &&
             match)) {
 ...
 */
css_error css__parse_border_side(css_language *c, const parserutils_vector *vector, int32_t *ctx,
                                 css_style *result, enum border_side_e side)
{
    ...
    do {
		 ...
		 */
        if ((color) &&
            (error = css__parse_border_side_color(c, vector, ctx, color_style,
                                                  CSS_PROP_BORDER_TOP_COLOR + side)) == CSS_OK) {
            color = false;
        } else if ((style) &&
                   (error = css__parse_border_side_style(
                        c, vector, ctx, style_style, CSS_PROP_BORDER_TOP_STYLE + side)) == CSS_OK) {
            style = false;
        } else if ((width) &&
                   (error = css__parse_border_side_width(
                        c, vector, ctx, width_style, CSS_PROP_BORDER_TOP_WIDTH + side)) == CSS_OK) {
            width = false;
        }
    ...
    if (style) {
        error = css__stylesheet_style_appendOPV(style_style, CSS_PROP_BORDER_TOP_STYLE + side, 0,
                                                BORDER_STYLE_NONE);
        if (error != CSS_OK) {
            goto css__parse_border_side_cleanup;
        }
    ...
    if (width) {
        error = css__stylesheet_style_appendOPV(width_style, CSS_PROP_BORDER_TOP_WIDTH + side, 0,
                                                BORDER_WIDTH_MEDIUM);
        if (error != CSS_OK) {
            goto css__parse_border_side_cleanup;
        }
    ...
    if (color) {
        error = css__stylesheet_style_appendOPV(color_style, CSS_PROP_BORDER_TOP_COLOR + side, 0,
 ...
 */
static void HSL_to_RGB(css_fixed hue, css_fixed sat, css_fixed lit, uint8_t *r, uint8_t *g,
                       uint8_t *b)
{
 ...
 *                 If the input is invalid, then \a *ctx remains unchanged.
 */
css_error css__parse_colour_specifier(css_language *c, const parserutils_vector *vector,
                                      int32_t *ctx, uint16_t *value, uint32_t *result)
{
    int32_t orig_ctx = *ctx;
    const css_token *token;
    bool match;
    ...
    if (token == NULL || (token->type != CSS_TOKEN_IDENT && token->type != CSS_TOKEN_HASH &&
    ...
    if (token->type == CSS_TOKEN_IDENT) {
        if ((lwc_string_caseless_isequal(token->idata, c->strings[TRANSPARENT], &match) ==
                 lwc_error_ok &&
             match)) {
            *value = COLOR_TRANSPARENT;
            *result = 0; /* black transparent */
            return CSS_OK;
        } else if ((lwc_string_caseless_isequal(token->idata, c->strings[CURRENTCOLOR], &match) ==
                        lwc_error_ok &&
                    match)) {
        ...
        if ((lwc_string_caseless_isequal(token->idata, c->strings[RGB], &match) == lwc_error_ok &&
             match)) {
            colour_channels = 3;
        } else if ((lwc_string_caseless_isequal(token->idata, c->strings[RGBA], &match) ==
                        lwc_error_ok &&
                    match)) {
            colour_channels = 4;
        }
        if ((lwc_string_caseless_isequal(token->idata, c->strings[HSL], &match) == lwc_error_ok &&
             match)) {
            colour_channels = 5;
        } else if ((lwc_string_caseless_isequal(token->idata, c->strings[HSLA], &match) ==
                        lwc_error_ok &&
                    match)) {
            colour_channels = 6;
        }
...
css_error css__parse_hash_colour(lwc_string *data, uint32_t *result)
{
    ...
    /* CSS Color Level 4 allows a 4th channel (alpha) in both the short
     * (#RGBA) and long (#RRGGBBAA) hex forms. Modern design systems rely on
     * these heavily — e.g. `#0000` as a transparent override and 8-digit
     * translucent tokens (GitHub's `--header-bgColor: #151b23f2`). Without
     * them the whole declaration is dropped, so an earlier opaque rule wins
     * (GitHub's header rendered as white bars). */
    bool all_hex = true;
    for (size_t hex_i = 0; hex_i < len; hex_i++) {
        if (!isHex(input[hex_i])) {
            all_hex = false;
            break;
        }
    }
    if (all_hex && (len == 3 || len == 4)) {
        r = charToHex(input[0]);
        g = charToHex(input[1]);
        b = charToHex(input[2]);
        r |= (r << 4);
        ...
        *unit = default_unit;
        ...
        if (c->sheet->quirks_allowed) {
            ...
            if (token != NULL && token->type == CSS_TOKEN_IDENT) {
                error = css__parse_unit_keyword(lwc_string_data(token->idata),
                                                lwc_string_length(token->idata), &temp_unit);
                if (error == CSS_OK) {
                    c->sheet->quirks_used = true;
                    *ctx = temp_ctx;
                    *unit = temp_unit;
 ...
 */
css_error css__ident_list_or_string_to_string(
    css_language *c, const parserutils_vector *vector, int32_t *ctx,
                                    ...
                                    lwc_string **result)
{
    ...
    while (token != NULL && (token->type == CSS_TOKEN_IDENT || token->type == CSS_TOKEN_S)) {
        if (token->type == CSS_TOKEN_IDENT) {
            ...
            perror =
                parserutils_buffer_append(buffer, (const uint8_t *)lwc_string_data(token->idata),
...
css_error css__comma_list_to_style(css_language *c, const parserutils_vector *vector, int32_t *ctx,
                                   bool (*reserved)(css_language *c, const css_token *ident),
                                   css_code_t (*get_value)(css_language *c, const css_token *token,
                                                           bool first),
                                   css_style *result)
{
    int32_t orig_ctx = *ctx;
    ...
    while (token != NULL) {
        ...
        if (token != NULL && tokenIsChar(token, ',')) {
            ...
            token = parserutils_vector_peek(vector, *ctx);
            if (token == NULL ||
                (token->type != CSS_TOKEN_IDENT && token->type != CSS_TOKEN_STRING)) {
                error = CSS_INVALID;
                goto cleanup;
            }
        } else {
