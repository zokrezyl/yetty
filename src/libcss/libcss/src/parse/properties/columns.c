                             ...
                             css_style *result)
{
    ...
    do {
        ...
        if ((width) && (error = css__parse_column_width(c, vector, ctx, width_style)) == CSS_OK) {
            width = false;
                   ...
                   (error = css__parse_column_count(c, vector, ctx, count_style)) == CSS_OK) {
            count = false;
        }
    ...
    if (width) {
        error = css__stylesheet_style_appendOPV(width_style, CSS_PROP_COLUMN_WIDTH, 0,
                                                COLUMN_WIDTH_AUTO);
        if (error != CSS_OK) {
            goto css__parse_columns_cleanup;
        }
    ...
    if (count) {
        error = css__stylesheet_style_appendOPV(count_style, CSS_PROP_COLUMN_COUNT, 0,
                                                COLUMN_COUNT_AUTO);
        if (error != CSS_OK) {
            goto css__parse_columns_cleanup;
        }
