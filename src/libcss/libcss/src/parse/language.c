...
/* Event handlers */
static css_error language_handle_event(css_parser_event type, const parserutils_vector *tokens,
                                       void *pw);
...
static css_error parseClass(css_language *c, const parserutils_vector *vector, int32_t *ctx,
                            css_selector_detail *specific);
static css_error parseAttrib(css_language *c, const parserutils_vector *vector, int32_t *ctx,
                             css_selector_detail *specific);
static css_error parseNth(css_language *c, const parserutils_vector *vector, int32_t *ctx,
                          css_selector_detail_value *value);
static css_error parsePseudo(css_language *c, const parserutils_vector *vector, int32_t *ctx,
                             bool in_not, css_selector_detail *specific);
static css_error parseSpecific(css_language *c, const parserutils_vector *vector, int32_t *ctx,
                               bool in_not, css_selector_detail *specific);
static css_error parseAppendSpecific(css_language *c, const parserutils_vector *vector,
                                     int32_t *ctx, css_selector **parent);
static css_error parseSelectorSpecifics(css_language *c, const parserutils_vector *vector,
                                        int32_t *ctx, css_selector **parent);
static css_error parseTypeSelector(css_language *c, const parserutils_vector *vector, int32_t *ctx,
                                   css_qname *qname);
static css_error parseSimpleSelector(css_language *c, const parserutils_vector *vector,
                                     int32_t *ctx, css_selector **result);
static css_error parseCombinator(css_language *c, const parserutils_vector *vector, int32_t *ctx,
                                 css_combinator *result);
static css_error parseSelector(css_language *c, const parserutils_vector *vector, int32_t *ctx,
                               css_selector **result);
static css_error parseSelectorList(css_language *c, const parserutils_vector *vector,
                                   css_rule *rule);

/* Declaration parsing */
static css_error parseProperty(css_language *c, const css_token *property,
...
css_error handleStartAtRule(css_language *c, const parserutils_vector *vector)
{
    ...
    if (lwc_string_caseless_isequal(atkeyword->idata, c->strings[CHARSET], &match) ==
            lwc_error_ok &&
        match) {
    ...
    } else if (lwc_string_caseless_isequal(atkeyword->idata, c->strings[LIBCSS_IMPORT], &match) ==
                   lwc_error_ok &&
               match) {
    ...
    } else if (lwc_string_caseless_isequal(atkeyword->idata, c->strings[NAMESPACE], &match) ==
                   lwc_error_ok &&
               match) {
        if (c->state <= NAMESPACE_PERMITTED) {
            ...
            if (token == NULL ||
                (token->type != CSS_TOKEN_STRING && token->type != CSS_TOKEN_URI)) {
                return CSS_INVALID;
            }
    ...
    } else if (lwc_string_caseless_isequal(atkeyword->idata, c->strings[MEDIA], &match) ==
                   lwc_error_ok &&
               match) {
        css_mq_query *media = NULL;
        ...
        c->state = HAD_RULE;
    } else if (lwc_string_caseless_isequal(atkeyword->idata, c->strings[FONT_FACE], &match) ==
                   lwc_error_ok &&
               match) {
        ...
        c->state = HAD_RULE;
    } else if (lwc_string_caseless_isequal(atkeyword->idata, c->strings[PAGE], &match) ==
                   lwc_error_ok &&
               match) {
...
css_error addNamespace(css_language *c, lwc_string *prefix, lwc_string *uri)
{
    if (prefix == NULL) {
        ...
        for (idx = 0; idx < c->num_namespaces; idx++) {
            if (lwc_string_isequal(c->namespaces[idx].prefix, prefix, &match) == lwc_error_ok &&
                match) {
                break;
            }
        }
        ...
        if (idx == c->num_namespaces) {
            ...
            css_namespace *ns =
                realloc(c->namespaces, sizeof(css_namespace) * (c->num_namespaces + 1));
...
css_error parseClass(css_language *c, const parserutils_vector *vector, int32_t *ctx,
                     css_selector_detail *specific)
{
    css_qname qname;
    ...
    return css__stylesheet_selector_detail_init(c->sheet, CSS_SELECTOR_CLASS, &qname, detail_value,
                                                CSS_SELECTOR_DETAIL_VALUE_STRING, false, specific);
}

css_error parseAttrib(css_language *c, const parserutils_vector *vector, int32_t *ctx,
                      css_selector_detail *specific)
{
    css_qname qname;
    css_selector_detail_value detail_value;
    ...
    return css__stylesheet_selector_detail_init(c->sheet, type, &qname, detail_value,
                                                CSS_SELECTOR_DETAIL_VALUE_STRING, false, specific);
}

css_error parseNth(css_language *c, const parserutils_vector *vector, int32_t *ctx,
                   css_selector_detail_value *value)
{
    const css_token *token;
    bool match;
    ...
    if (token->type == CSS_TOKEN_IDENT &&
        lwc_string_caseless_isequal(token->idata, c->strings[ODD], &match) == lwc_error_ok &&
        match) {
        /* Odd */
        value->nth.a = 2;
        value->nth.b = 1;
    } else if (token->type == CSS_TOKEN_IDENT &&
               lwc_string_caseless_isequal(token->idata, c->strings[EVEN], &match) ==
                   lwc_error_ok &&
               match) {
        /* Even */
        value->nth.a = 2;
        ...
        if (token->type == CSS_TOKEN_IDENT) {
            ...
            if (len > 0) {
                ...
                if (len > 1) {
                    ...
                    /* -n-b */
                    b = css__number_from_string((const uint8_t *)data + 1, len - 1, true,
                                                &consumed);
                    if (consumed != len - 1) {
                        return CSS_INVALID;
                    }
            ...
            if (len - (++consumed) > 0) {
                ...
                if (len - (++consumed) > 0) {
                    ...
                    b = css__number_from_string((const uint8_t *)data + bstart, len - bstart, true,
                                                &consumed);
...
css_error parsePseudo(css_language *c, const parserutils_vector *vector, int32_t *ctx, bool in_not,
                      css_selector_detail *specific)
{
    static const struct {
        int index;
    ...
    for (lut_idx = 0; lut_idx < N_ELEMENTS(pseudo_lut); lut_idx++) {
        if ((lwc_string_caseless_isequal(qname.name, c->strings[pseudo_lut[lut_idx].index],
                                         &match) == lwc_error_ok) &&
            match) {
    ...
    return css__stylesheet_selector_detail_init(c->sheet, type, &qname, detail_value, value_type,
                                                negate, specific);
}

css_error parseSpecific(css_language *c, const parserutils_vector *vector, int32_t *ctx,
                        bool in_not, css_selector_detail *specific)
{
    css_error error;
    const css_token *token;
    ...
    if (token->type == CSS_TOKEN_HASH) {
        ...
        error =
            css__stylesheet_selector_detail_init(c->sheet, CSS_SELECTOR_ID, &qname, detail_value,
                                                 CSS_SELECTOR_DETAIL_VALUE_STRING, false, specific);
        if (error != CSS_OK) {
            return error;
        }
...
css_error parseAppendSpecific(css_language *c, const parserutils_vector *vector, int32_t *ctx,
                              css_selector **parent)
{
    css_error error;
    css_selector_detail specific;
...
css_error parseSelectorSpecifics(css_language *c, const parserutils_vector *vector, int32_t *ctx,
                                 css_selector **parent)
{
    css_error error;
    const css_token *token;
    ...
    while ((token = parserutils_vector_peek(vector, *ctx)) != NULL && token->type != CSS_TOKEN_S &&
           tokenIsChar(token, '+') == false && tokenIsChar(token, '>') == false &&
           tokenIsChar(token, '~') == false && tokenIsChar(token, ',') == false) {
...
css_error parseTypeSelector(css_language *c, const parserutils_vector *vector, int32_t *ctx,
                            css_qname *qname)
{
    const css_token *token;
    css_error error;
...
css_error parseSimpleSelector(css_language *c, const parserutils_vector *vector, int32_t *ctx,
                              css_selector **result)
{
...
css_error parseCombinator(css_language *c, const parserutils_vector *vector, int32_t *ctx,
                          css_combinator *result)
{
    const css_token *token;
...
css_error parseSelector(css_language *c, const parserutils_vector *vector, int32_t *ctx,
                        css_selector **result)
{
    css_error error;
    const css_token *token = NULL;
                        ...
                        const parserutils_vector *vector, int32_t *ctx, css_rule *rule)
{
    ...
    for (i = FIRST_PROP; i <= LAST_PROP; i++) {
        ...
        if (lwc_string_caseless_isequal(property->idata, c->strings[i], &match) == lwc_error_ok &&
            match) {
            break;
        }
    }
