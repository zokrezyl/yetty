...
static inline bool mq_match_feature_range_length_op1(css_mq_feature_op op,
                                                     ...
                                                     const css_unit_ctx *unit_ctx)
{
    ...
    if (value->data.dim.unit != UNIT_PX) {
        v = css_unit_len2px_mq(unit_ctx, value->data.dim.len,
...
static inline bool mq_match_feature_range_length_op2(css_mq_feature_op op,
                                                     const css_mq_value *value,
                                                     const css_fixed client_len,
                                                     const css_unit_ctx *unit_ctx)
{
...
static inline bool mq_match_feature_eq_ident_op1(css_mq_feature_op op, const css_mq_value *value,
                                                 const lwc_string *client_value)
{
    ...
    switch (op) {
    case CSS_MQ_FEATURE_OP_EQ:
        return (lwc_string_isequal(value->data.ident, client_value, &is_match) == lwc_error_ok) &&
               is_match;
    default:
        return false;
    }
 ...
 */
static inline bool mq_match_feature(const css_mq_feature *feat, const css_unit_ctx *unit_ctx,
                                    const css_media *media, const css_select_strings *str)
{
    bool match;
    ...
    } else if (lwc_string_isequal(feat->name, str->height, &match) == lwc_error_ok &&
               match == true) {
    ...
    } else if (lwc_string_isequal(feat->name, str->prefers_color_scheme, &match) == lwc_error_ok &&
               match == true) {
 ...
 * \return true if condition matches, otherwise false.
 */
static inline bool mq_match_condition(const css_mq_cond *cond, const css_unit_ctx *unit_ctx,
                                      const css_media *media, const css_select_strings *str)
{
 ...
 */
static inline bool mq__list_match(const css_mq_query *m, const css_unit_ctx *unit_ctx,
                                  const css_media *media, const css_select_strings *str)
{
    for (; m != NULL; m = m->next) {
        /* Check type */
 ...
 */
static inline bool mq_rule_good_for_media(const css_rule *rule, const css_unit_ctx *unit_ctx,
                                          const css_media *media, const css_select_strings *str)
{
    bool applies = true;
