 ...
 */
static inline css_unit css_unit__map_viewport_units(const css_computed_style *style,
                                                    ...
                                                    const css_unit unit)
{
    switch (unit) {
    case CSS_UNIT_VI:
        return (style != NULL && get_writing_mode(style) != CSS_WRITING_MODE_HORIZONTAL_TB)
                   ? CSS_UNIT_VH
                   : CSS_UNIT_VW;
 ...
 */
static inline css_fixed css_unit__absolute_len2pt(const css_computed_style *style,
                                                  const css_fixed viewport_height,
                                                  const css_fixed viewport_width,
                                                  const css_fixed length, const css_unit unit)
{
    /* Length must not be relative */
    assert(unit != CSS_UNIT_EM && unit != CSS_UNIT_EX && unit != CSS_UNIT_CH &&
           unit != CSS_UNIT_REM);
...
css_fixed css_unit_font_size_len2pt(const css_computed_style *style, const css_unit_ctx *ctx,
                                    const css_fixed length, const css_unit unit)
{
    return css_unit__absolute_len2pt(style, ctx->viewport_height, ctx->viewport_width, length,
                                     unit);
}
 ...
 */
static inline css_fixed css_unit__font_size_px(const css_computed_style *style,
                                               ...
                                               const css_fixed viewport_width)
{
    ...
    if (font_unit != CSS_UNIT_PX) {
        font_length = css_unit__absolute_len2pt(style, viewport_height, viewport_width, font_length,
                                                font_unit);
...
static inline css_fixed css_unit__px_per_unit(
    const css_unit_len_measure measure, const css_computed_style *ref_style,
    const css_computed_style *root_style, const css_fixed font_size_default,
    const css_fixed font_size_minimum, const css_fixed viewport_height,
    const css_fixed viewport_width, const css_unit unit, void *pw)
{
    switch (css_unit__map_viewport_units(ref_style, viewport_height, viewport_width, unit)) {
    case CSS_UNIT_EM:
        return css_unit__font_size_px(ref_style, font_size_default, font_size_minimum,
    ...
    case CSS_UNIT_EX:
        ...
        return FMUL(css_unit__font_size_px(ref_style, font_size_default, font_size_minimum,
                                           viewport_height, viewport_width),
                    FLTTOFIX(0.6));
    ...
    case CSS_UNIT_REM:
        return css_unit__font_size_px(root_style, font_size_default, font_size_minimum,
                                      viewport_height, viewport_width);
...
css_fixed css_unit_len2px_mq(const css_unit_ctx *ctx, const css_fixed length, const css_unit unit)
{
    ...
    css_fixed px_per_unit = css_unit__px_per_unit(ctx->measure, NULL, NULL, ctx->font_size_default,
                                                  ctx->font_size_minimum, ctx->viewport_height,
                                                  ctx->viewport_width, unit, ctx->pw);
...
/* Exported function, documented in libcss/unit.h. */
css_fixed css_unit_len2css_px(const css_computed_style *style, const css_unit_ctx *ctx,
                              const css_fixed length, const css_unit unit)
{
    css_fixed px_per_unit = css_unit__px_per_unit(
        ctx->measure, style, ctx->root_style, ctx->font_size_default, ctx->font_size_minimum,
        ctx->viewport_height, ctx->viewport_width, unit, ctx->pw);
...
/* Exported function, documented in libcss/unit.h. */
css_fixed css_unit_len2device_px(const css_computed_style *style, const css_unit_ctx *ctx,
                                 const css_fixed length, const css_unit unit)
{
    css_fixed px_per_unit = css_unit__px_per_unit(
        ctx->measure, style, ctx->root_style, ctx->font_size_default, ctx->font_size_minimum,
    ...
    case CSS_FONT_SIZE_DIMENSION:
        ...
        case CSS_UNIT_PCT:
            size->data.length.value =
                FDIV(FMUL(size->data.length.value, ref_len.value), INTTOFIX(100));
            size->data.length.unit = ref_len.unit;
            break;
