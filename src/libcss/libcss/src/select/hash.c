...
static css_error _iterate_elements(const struct css_hash_selection_requirments *req,
                                   const css_selector **current, const css_selector ***next);
static css_error _iterate_classes(const struct css_hash_selection_requirments *req,
                                  const css_selector **current, const css_selector ***next);
static css_error _iterate_ids(const struct css_hash_selection_requirments *req,
                              const css_selector **current, const css_selector ***next);
static css_error _iterate_universal(const struct css_hash_selection_requirments *req,
                                    const css_selector **current, const css_selector ***next);
                                                ...
                                                const css_qname *qname, const lwc_string *uni)
{
    if (selector->data.qname.name != uni) {
        bool match;
        if (lwc_string_caseless_isequal(selector->data.qname.name, qname->name, &match) ==
                lwc_error_ok &&
            match == false) {
            return false;
        }
...
css_error css__selector_hash_create(css_selector_hash **hash)
{
    ...
    h->hash_size = sizeof(css_selector_hash) + DEFAULT_SLOTS * sizeof(hash_entry) +
                   DEFAULT_SLOTS * sizeof(hash_entry) + DEFAULT_SLOTS * sizeof(hash_entry);
                                  ...
                                  const css_selector ***matched)
{
    ...
    if (head->sel != NULL) {
        ...
        while (head != NULL) {
            ...
            lerror = lwc_string_isequal(req->qname.name->insensitive,
                                        head->sel->data.qname.name->insensitive, &match);
    ...
    if (head->sel != NULL) {
        ...
        while (head != NULL) {
            ...
            if (n != NULL) {
                ...
                if (match && RULE_HAS_BYTECODE(head)) {
                    if (css_bloom_in_bloom(head->sel_chain_bloom, req->node_bloom) &&
                        _chain_good_for_element_name(head->sel, &(req->qname),
                                                     req->str->universal) &&
    ...
    if (head->sel != NULL) {
        ...
        while (head != NULL) {
            if (RULE_HAS_BYTECODE(head) &&
                css_bloom_in_bloom(head->sel_chain_bloom, req->node_bloom) &&
                mq_rule_good_for_media(head->sel->rule, req->unit_ctx, req->media, req->str)) {
                /* Found a match */
                break;
            }
