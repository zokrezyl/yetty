...
css_error css__set_content_from_hint(const css_hint *hint, css_computed_style *style)
{
    ...
    for (item = hint->data.content; item != NULL && item->type != CSS_COMPUTED_CONTENT_NONE;
         item++) {
        switch (item->type) {
...
css_error css__compose_content(const css_computed_style *parent, const css_computed_style *child,
