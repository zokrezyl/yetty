...
_Static_assert(sizeof(JSValue) == 2 * sizeof(sljit_sw),
               "JIT assumes a two-word JSValue");
...
{
    switch (tag) {
    case JS_TAG_UNDEFINED: frame->stack_buf[slot] = JS_UNDEFINED; break;
    case JS_TAG_NULL:      frame->stack_buf[slot] = JS_NULL; break;
    default:               frame->stack_buf[slot] = js_bool(tag == 1); break;
    }
}
...
static sljit_sw qjs_jh_get_loc_check(QJSJitFrame *frame, sljit_sw dst_and_idx,
                                sljit_sw pc_off)
{
...
static sljit_sw qjs_jh_put_loc_check(QJSJitFrame *frame, sljit_sw idx_and_src,
                                sljit_sw pc_off)
{
...
static sljit_sw qjs_jh_to_bool_free(QJSJitFrame *frame, sljit_sw slot,
                                    sljit_sw unused)
{
...
static sljit_sw qjs_jh_poll_interrupts(QJSJitFrame *frame, sljit_sw pc_off,
                                  sljit_sw unused)
{
    (void)unused;
    frame->sf->cur_pc = frame->b->byte_code_buf + pc_off;
...
/* --- calls --- */
static sljit_sw qjs_jh_call(QJSJitFrame *frame, sljit_sw func_slot_argc,
                       sljit_sw pc_off)
{
    ...
    frame->sf->cur_pc = frame->b->byte_code_buf + pc_off;
    ret = JS_CallInternal(frame->ctx, base[0], JS_UNDEFINED, JS_UNDEFINED,
                          argc, vc(base + 1), 0);
...
{
    JSValue *base = sel == 0 ? frame->stack_buf :
                    sel == 1 ? frame->var_buf : frame->arg_buf;
...
static sljit_sw qjs_jh_poll_slow(QJSJitFrame *frame, sljit_sw pc_off,
                                 sljit_sw unused)
{
    (void)unused;
    frame->sf->cur_pc = frame->b->byte_code_buf + pc_off;
...
static sljit_sw qjs_jh_binary(QJSJitFrame *frame, sljit_sw sp_slot,
                              sljit_sw op_pc)
{
    ...
    if (both_int) {
        ...
        case OP_mul:
            ...
            break;
        case OP_and:  sp[-2] = js_int32(a & b); return 0;
        case OP_or:   sp[-2] = js_int32(a | b); return 0;
        case OP_xor:  sp[-2] = js_int32(a ^ b); return 0;
        case OP_shl:  sp[-2] = js_int32(a << (b & 0x1f)); return 0;
        case OP_sar:  sp[-2] = js_int32(a >> (b & 0x1f)); return 0;
        case OP_shr:  sp[-2] = js_uint32((uint32_t)a >> (b & 0x1f)); return 0;
        case OP_lt:   sp[-2] = js_bool(a < b);  return 0;
        case OP_lte:  sp[-2] = js_bool(a <= b); return 0;
        case OP_gt:   sp[-2] = js_bool(a > b);  return 0;
        case OP_gte:  sp[-2] = js_bool(a >= b); return 0;
        case OP_eq:
        case OP_strict_eq:  sp[-2] = js_bool(a == b); return 0;
        case OP_neq:
        case OP_strict_neq: sp[-2] = js_bool(a != b); return 0;
        default: break;   /* div/mod/pow: slow */
        }
    }

    frame->sf->cur_pc = frame->b->byte_code_buf + (op_pc & 0xfffff);
    switch (op) {
    case OP_add:  return js_add_slow(frame->ctx, sp);
    ...
    case OP_shr:  return js_shr_slow(frame->ctx, sp);
    case OP_lt: case OP_lte: case OP_gt: case OP_gte:
                  return js_relational_slow(frame->ctx, sp, op);
    case OP_eq:   return js_eq_slow(frame->ctx, sp, 0);
    case OP_neq:  return js_eq_slow(frame->ctx, sp, 1);
    case OP_strict_eq:  return js_strict_eq_slow(frame->ctx, sp, 0);
    case OP_strict_neq: return js_strict_eq_slow(frame->ctx, sp, 1);
    default: return -1;
    }
}
...
static sljit_sw qjs_jh_unary(QJSJitFrame *frame, sljit_sw sp_slot, sljit_sw op_pc)
{
    ...
    case OP_inc:
        if (tag == JS_TAG_INT) {
            ...
            if (val != INT32_MAX) { sp[-1] = js_int32(val + 1); return 0; }
        }
        break;
    case OP_dec:
        if (tag == JS_TAG_INT) {
            int val = JS_VALUE_GET_INT(op1);
            if (val != INT32_MIN) { sp[-1] = js_int32(val - 1); return 0; }
        }
        break;
    case OP_neg:
        if (tag == JS_TAG_INT) {
            int val = JS_VALUE_GET_INT(op1);
            if (val != 0 && val != INT32_MIN) { sp[-1] = js_int32(-val); return 0; }
        }
        break;
    case OP_plus:
        ...
        break;
    case OP_not:
        if (tag == JS_TAG_INT) { sp[-1] = js_int32(~JS_VALUE_GET_INT(op1)); return 0; }
        frame->sf->cur_pc = frame->b->byte_code_buf + (op_pc & 0xfffff);
...
static sljit_sw qjs_jh_post_incdec(QJSJitFrame *frame, sljit_sw sp_slot,
                                   sljit_sw op_pc)
{
    JSValue *sp = &frame->stack_buf[sp_slot];
    JSValue op1 = sp[-1];
    ...
    if (JS_VALUE_GET_TAG(op1) == JS_TAG_INT) {
        int val = JS_VALUE_GET_INT(op1);
        if (op == OP_post_inc) {
            if (val != INT32_MAX) { sp[0] = js_int32(val + 1); return 0; }
        } else {
            if (val != INT32_MIN) { sp[0] = js_int32(val - 1); return 0; }
        }
    }
    frame->sf->cur_pc = frame->b->byte_code_buf + (op_pc & 0xfffff);
...
static sljit_sw qjs_jh_incdec_loc(QJSJitFrame *frame, sljit_sw idx_isdec,
                                  sljit_sw pc_off)
{
    ...
    if (JS_VALUE_GET_TAG(op1) == JS_TAG_INT) {
        int val = JS_VALUE_GET_INT(op1);
        if (!is_dec && val != INT32_MAX) { frame->var_buf[idx] = js_int32(val + 1); return 0; }
        if (is_dec && val != INT32_MIN) { frame->var_buf[idx] = js_int32(val - 1); return 0; }
    }
    frame->sf->cur_pc = frame->b->byte_code_buf + pc_off;
    op1 = js_dup(op1);
...
static sljit_sw qjs_jh_add_loc(QJSJitFrame *frame, sljit_sw idx_and_sp,
                               sljit_sw pc_off)
{
    ...
    if (JS_VALUE_GET_TAG(*pv) == JS_TAG_STRING) {
        ...
        if (JS_IsException(top)) return -1;
    ...
    }
    {
        ...
        ops[0] = js_dup(*pv);
        ops[1] = top;
        if (js_add_slow(frame->ctx, ops + 2)) return -1;
...
static sljit_sw qjs_jh_get_field(QJSJitFrame *frame, sljit_sw top, sljit_sw pc_off)
{
    ...
    if (JS_IsException(val)) return -1;   /* obj stays live; unwind frees */
...
static sljit_sw qjs_jh_get_field2(QJSJitFrame *frame, sljit_sw top, sljit_sw pc_off)
{
    ...
    frame->sf->cur_pc = frame->b->byte_code_buf + pc_off;
    val = JS_GetPropertyInternal(frame->ctx, obj, atom, obj, false);
    if (JS_IsException(val)) return -1;   /* obj stays live */
...
static sljit_sw qjs_jh_put_array_el(QJSJitFrame *frame, sljit_sw top, sljit_sw pc_off)
{
    int ret;
    frame->sf->cur_pc = frame->b->byte_code_buf + pc_off;
    ret = JS_SetPropertyValue(frame->ctx, frame->stack_buf[top - 2],
                              frame->stack_buf[top - 1], frame->stack_buf[top],
                              JS_PROP_THROW_STRICT);       /* consumes idx + val */
    frame->stack_buf[top] = JS_UNDEFINED;
    frame->stack_buf[top - 1] = JS_UNDEFINED;
...
{
    set_value(frame->ctx, frame->var_refs[idx]->pvalue,
              js_dup(frame->stack_buf[src]));
}

static sljit_sw qjs_jh_get_var_ref_check(QJSJitFrame *frame, sljit_sw dst_idx,
                                         sljit_sw pc_off)
{
...
static sljit_sw qjs_jh_put_var_ref_check(QJSJitFrame *frame, sljit_sw idx_src,
                                         sljit_sw pc_off)
{
...
static sljit_sw qjs_jh_call_constructor(QJSJitFrame *frame,
                                        sljit_sw func_slot_argc, sljit_sw pc_off)
{
    int func_slot = (int)(func_slot_argc >> 20);
    int argc = (int)(func_slot_argc & 0xfffff);
    ...
    int i;
    frame->sf->cur_pc = frame->b->byte_code_buf + pc_off;
    ret = JS_CallConstructorInternal(frame->ctx, base[0], base[1], argc,
                                     vc(base + 2), 0);
    if (unlikely(JS_IsException(ret)))
...
static sljit_sw qjs_jh_call_method(QJSJitFrame *frame, sljit_sw this_slot_argc,
                                   sljit_sw pc_off)
{
    ...
    int i;
    frame->sf->cur_pc = frame->b->byte_code_buf + pc_off;
    ret = JS_CallInternal(frame->ctx, base[1], base[0], JS_UNDEFINED, argc,
                          vc(base + 2), 0);
    if (unlikely(JS_IsException(ret)))
...
static void qjs_jit_fail(struct qjs_jit_ctx *jc) { jc->failed = 1; }
...
static void qjs_jit_add_pending(struct qjs_jit_ctx *jc,
                                struct sljit_jump *jump, int target)
{
    if (!jump) { qjs_jit_fail(jc); return; }
    if (jc->pend_count >= jc->pend_cap) { qjs_jit_fail(jc); return; }
...
static void qjs_jit_call_fss(struct qjs_jit_ctx *jc, void *helper,
                             sljit_sw imm1, sljit_sw imm2)
{
    ...
    qjs_jit_check(jc, sljit_emit_icall(c, SLJIT_CALL, SLJIT_ARGS3V(P, W, W),
                                       SLJIT_IMM, (sljit_sw)helper));
}

/* value-returning helper(frame, imm1, imm2) -> sljit_sw in R0 */
static void qjs_jit_call_fss_ret(struct qjs_jit_ctx *jc, void *helper,
                                 sljit_sw imm1, sljit_sw imm2)
{
    struct sljit_compiler *c = jc->compiler;
    qjs_jit_check(jc, sljit_emit_op1(c, SLJIT_MOV, SLJIT_R0, 0, JIT_FRAME, 0));
    qjs_jit_check(jc, sljit_emit_op1(c, SLJIT_MOV, SLJIT_R1, 0, SLJIT_IMM, imm1));
    qjs_jit_check(jc, sljit_emit_op1(c, SLJIT_MOV, SLJIT_R2, 0, SLJIT_IMM, imm2));
    qjs_jit_check(jc, sljit_emit_icall(c, SLJIT_CALL, SLJIT_ARGS3(W, P, W, W),
                                       SLJIT_IMM, (sljit_sw)helper));
}
...
static void qjs_jit_emit_unwind_return(struct qjs_jit_ctx *jc, int depth)
{
    ...
    qjs_jit_check(jc, sljit_emit_return(jc->compiler, SLJIT_MOV,
...
static void qjs_jit_emit_return_slot(struct qjs_jit_ctx *jc, int slot)
{
    ...
    qjs_jit_check(jc, sljit_emit_icall(c, SLJIT_CALL, SLJIT_ARGS3V(P, P, W),
                                       SLJIT_IMM, (sljit_sw)qjs_jh_return));
...
{
    struct sljit_compiler *c = jc->compiler;
    struct sljit_jump *ok = sljit_emit_cmp(c, SLJIT_SIG_GREATER_EQUAL,
                                           SLJIT_R0, 0, SLJIT_IMM, 0);
    if (!ok) { qjs_jit_fail(jc); return; }
    qjs_jit_emit_unwind_return(jc, pre_op_depth);
    {
        struct sljit_label *cont = sljit_emit_label(c);
        if (!cont) { qjs_jit_fail(jc); return; }
...
static void qjs_jit_store_int(struct qjs_jit_ctx *jc, int slot, sljit_s32 reg)
{
    ...
    qjs_jit_check(jc, sljit_emit_op1(c, SLJIT_MOV, SLJIT_MEM1(JIT_STACK), TAG_W(slot), SLJIT_IMM, JS_TAG_INT));
}
...
static void qjs_jit_store_bool(struct qjs_jit_ctx *jc, int slot, sljit_s32 reg)
{
    struct sljit_compiler *c = jc->compiler;
    qjs_jit_check(jc, sljit_emit_op1(c, SLJIT_MOV, SLJIT_MEM1(JIT_STACK), VAL_W(slot), reg, 0));
    qjs_jit_check(jc, sljit_emit_op1(c, SLJIT_MOV, SLJIT_MEM1(JIT_STACK), TAG_W(slot), SLJIT_IMM, JS_TAG_BOOL));
}
...
{
    struct sljit_compiler *c = jc->compiler;
    qjs_jit_check(jc, sljit_emit_op1(c, SLJIT_MOV, SLJIT_MEM1(JIT_STACK), VAL_W(slot), SLJIT_IMM, val));
    qjs_jit_check(jc, sljit_emit_op1(c, SLJIT_MOV, SLJIT_MEM1(JIT_STACK), TAG_W(slot), SLJIT_IMM, tag));
}

/* Copy a 16-byte JSValue between slots of two cached bases (no refcount). */
static void qjs_jit_emit_copy(struct qjs_jit_ctx *jc, sljit_s32 dbase, int dslot,
                              sljit_s32 sbase, int sslot)
{
    struct sljit_compiler *c = jc->compiler;
...
static void qjs_jit_emit_incref(struct qjs_jit_ctx *jc, sljit_s32 base, int slot)
{
    ...
    if (skip && cont) sljit_set_label(skip, cont); else qjs_jit_fail(jc);
}
...
static void qjs_jit_emit_free(struct qjs_jit_ctx *jc, sljit_s32 base, int slot,
                              int sel)
{
    struct sljit_compiler *c = jc->compiler;
    struct sljit_jump *skip;
...
static void qjs_jit_emit_binary(struct qjs_jit_ctx *jc, int op, int depth, int pos)
{
    ...
    if (!qjs_jit_binary_inlinable(op)) {
        qjs_jit_call_fss_ret(jc, (void *)qjs_jh_binary, depth,
                             ((sljit_sw)op << 20) | pos);
    ...
    qjs_jit_check(jc, sljit_emit_op1(c, SLJIT_MOV_S32, SLJIT_R2, 0, SLJIT_MEM1(JIT_STACK), VAL_W(s1)));
    qjs_jit_check(jc, sljit_emit_op1(c, SLJIT_MOV_S32, SLJIT_R3, 0, SLJIT_MEM1(JIT_STACK), VAL_W(s2)));

    switch (op) {
    case OP_add:
        qjs_jit_check(jc, sljit_emit_op2(c, SLJIT_ADD32 | SLJIT_SET(SLJIT_OVERFLOW), SLJIT_R4, 0, SLJIT_R2, 0, SLJIT_R3, 0));
        overflow = sljit_emit_jump(c, SLJIT_OVERFLOW);
        qjs_jit_store_int(jc, s1, SLJIT_R4); break;
    case OP_sub:
        qjs_jit_check(jc, sljit_emit_op2(c, SLJIT_SUB32 | SLJIT_SET(SLJIT_OVERFLOW), SLJIT_R4, 0, SLJIT_R2, 0, SLJIT_R3, 0));
        overflow = sljit_emit_jump(c, SLJIT_OVERFLOW);
        qjs_jit_store_int(jc, s1, SLJIT_R4); break;
    case OP_and:
    ...
    default: {  /* comparisons */
        ...
        switch (op) {
        case OP_lt:  set_flag = SLJIT_SET(SLJIT_SIG_LESS);    read_cond = SLJIT_SIG_LESS; break;
        case OP_gte: set_flag = SLJIT_SET(SLJIT_SIG_LESS);    read_cond = SLJIT_SIG_GREATER_EQUAL; break;
        case OP_gt:  set_flag = SLJIT_SET(SLJIT_SIG_GREATER); read_cond = SLJIT_SIG_GREATER; break;
        case OP_lte: set_flag = SLJIT_SET(SLJIT_SIG_GREATER); read_cond = SLJIT_SIG_LESS_EQUAL; break;
        case OP_eq: case OP_strict_eq:   set_flag = SLJIT_SET_Z; read_cond = SLJIT_EQUAL; break;
        default:                         set_flag = SLJIT_SET_Z; read_cond = SLJIT_NOT_EQUAL; break;
        }
        (void)cond;
    ...
    if (!not_int || !done || !slow_lbl) { qjs_jit_fail(jc); return; }
    sljit_set_label(not_int, slow_lbl);
    if (overflow) sljit_set_label(overflow, slow_lbl);
    qjs_jit_call_fss_ret(jc, (void *)qjs_jh_binary, depth,
                         ((sljit_sw)op << 20) | pos);
    qjs_jit_check_exception(jc, depth);

    done_lbl = sljit_emit_label(c);
    if (done_lbl) sljit_set_label(done, done_lbl); else qjs_jit_fail(jc);
}

/* Emit inc/dec on the stack top (slot) or a local var (base=JIT_VAR).
   Inline int fast path with overflow guard; else the helper. */
static void qjs_jit_emit_incdec(struct qjs_jit_ctx *jc, int is_dec,
                                sljit_s32 base, int slot,
                                void *helper, sljit_sw helper_a, sljit_sw pc_off,
                                int depth)
{
    struct sljit_compiler *c = jc->compiler;
    ...
    qjs_jit_check(jc, sljit_emit_op2(c, (is_dec ? SLJIT_SUB32 : SLJIT_ADD32) | SLJIT_SET(SLJIT_OVERFLOW),
    ...
    qjs_jit_check(jc, sljit_emit_op1(c, SLJIT_MOV, SLJIT_MEM1(base), TAG_W(slot), SLJIT_IMM, JS_TAG_INT));
    done = sljit_emit_jump(c, SLJIT_JUMP);

    slow_lbl = sljit_emit_label(c);
    if (!not_int || !overflow || !done || !slow_lbl) { qjs_jit_fail(jc); return; }
    sljit_set_label(not_int, slow_lbl);
...
static void qjs_jit_emit_put_var(struct qjs_jit_ctx *jc, sljit_s32 base,
                                 int idx, int top, void *helper)
{
    struct sljit_compiler *c = jc->compiler;
    ...
    if (!heap || !done || !heap_lbl) { qjs_jit_fail(jc); return; }
...
static void qjs_jit_emit_set_var(struct qjs_jit_ctx *jc, sljit_s32 base,
                                 int idx, int top, void *helper)
{
    struct sljit_compiler *c = jc->compiler;
    ...
    struct sljit_label *heap_lbl, *done_lbl;
    qjs_jit_check(jc, sljit_emit_op1(c, SLJIT_MOV, SLJIT_R0, 0, SLJIT_MEM1(base), TAG_W(idx)));
    heap_old = sljit_emit_cmp(c, SLJIT_GREATER_EQUAL, SLJIT_R0, 0, SLJIT_IMM, (sljit_sw)JS_TAG_FIRST);
    qjs_jit_check(jc, sljit_emit_op1(c, SLJIT_MOV, SLJIT_R0, 0, SLJIT_MEM1(JIT_STACK), TAG_W(top)));
    heap_new = sljit_emit_cmp(c, SLJIT_GREATER_EQUAL, SLJIT_R0, 0, SLJIT_IMM, (sljit_sw)JS_TAG_FIRST);
    qjs_jit_emit_copy(jc, base, idx, JIT_STACK, top);   /* both int: plain copy, stack kept */
    done = sljit_emit_jump(c, SLJIT_JUMP);
    heap_lbl = sljit_emit_label(c);
    if (!heap_old || !heap_new || !done || !heap_lbl) { qjs_jit_fail(jc); return; }
...
static void qjs_jit_emit_branch(struct qjs_jit_ctx *jc, int cond_true, int top,
                                int target)
{
    struct sljit_compiler *c = jc->compiler;
    ...
    qjs_jit_check(jc, sljit_emit_op1(c, SLJIT_MOV_S32, SLJIT_R1, 0, SLJIT_MEM1(JIT_STACK), VAL_W(top)));
    past = sljit_emit_jump(c, SLJIT_JUMP);

    slow_lbl = sljit_emit_label(c);
    if (!slow || !past || !slow_lbl) { qjs_jit_fail(jc); return; }
    ...
    if (!test_lbl) { qjs_jit_fail(jc); return; }
    sljit_set_label(past, test_lbl);
    brj = sljit_emit_cmp(c, cond_true ? SLJIT_NOT_EQUAL : SLJIT_EQUAL,
...
static void qjs_jit_emit_backedge_poll(struct qjs_jit_ctx *jc, int pos, int depth)
{
    ...
    qjs_jit_check_exception(jc, depth);
    cont = sljit_emit_label(c);
    if (no_poll && cont) sljit_set_label(no_poll, cont); else qjs_jit_fail(jc);
}
...
/* Signed operand-stack delta and control-flow shape of a supported op.
   ...
   return/return_undef. */
static int qjs_jit_op_delta(uint8_t op, const uint8_t *pc, int pos,
                            int *branch_target, int *is_uncond,
                            int *is_terminal)
{
    *branch_target = -1;
    *is_uncond = 0;
    *is_terminal = 0;
    ...
    case OP_push_7: case OP_push_const: case OP_push_const8:
    ...
    case OP_get_loc2: case OP_get_loc3: case OP_get_loc_check:
    case OP_get_arg: case OP_get_arg0: case OP_get_arg1: case OP_get_arg2:
    case OP_get_arg3: case OP_dup:
        return 1;
    case OP_get_loc0_loc1:
        return 2;
    case OP_drop:
    case OP_put_loc: case OP_put_loc8: case OP_put_loc0: case OP_put_loc1:
    case OP_put_loc2: case OP_put_loc3: case OP_put_loc_check:
    case OP_put_arg: case OP_nip:
        return -1;
    case OP_set_loc: case OP_set_loc8: case OP_set_loc0: case OP_set_loc1:
    case OP_set_loc2: case OP_set_loc3: case OP_set_loc_uninitialized:
    case OP_set_arg: case OP_swap: case OP_nop:
        return 0;
    ...
    case OP_strict_neq: case OP_add_loc:
        return -1;
    /* unary in place: pop 1, push 1 */
    case OP_inc: case OP_dec: case OP_neg: case OP_plus: case OP_not:
    case OP_lnot:
        return 0;
    ...
    case OP_goto16:
        ...
        *is_uncond = 1;
        return 0;
    case OP_call0: return -0;
    case OP_call1: return -1;
    case OP_call2: return -2;
    case OP_call3: return -3;
    case OP_call:  return -(int)get_u16(pc);
    case OP_call_method: return -((int)get_u16(pc) + 1); /* this+func+args -> result */
    case OP_call_constructor: return -((int)get_u16(pc) + 1); /* func+nt+args -> result */
    case OP_tail_call:
        *is_terminal = 1; return 0;   /* returns the call result */
    case OP_tail_call_method:
        *is_terminal = 1; return 0;
    case OP_return:       *is_terminal = 1; return -1;
    case OP_return_undef: *is_terminal = 1; return 0;
    /* Stage 4: property access */
    case OP_get_field: case OP_get_length: case OP_get_array_el2:
    ...
    case OP_get_var_ref2: case OP_get_var_ref3: case OP_get_var_ref_check:
        return 1;
    case OP_put_var_ref: case OP_put_var_ref0: case OP_put_var_ref1:
    case OP_put_var_ref2: case OP_put_var_ref3: case OP_put_var_ref_check:
        return -1;
...
static bool qjs_jit_op_supported(uint16_t op)
{
    ...
    case OP_push_7: case OP_push_const: case OP_push_const8:
    case OP_undefined: case OP_null: case OP_push_false: case OP_push_true:
    case OP_drop: case OP_nip: case OP_dup: case OP_swap:
    case OP_get_loc: case OP_get_loc8: case OP_get_loc0: case OP_get_loc1:
    case OP_get_loc2: case OP_get_loc3: case OP_get_loc_check:
    case OP_get_loc0_loc1:
    case OP_put_loc: case OP_put_loc8: case OP_put_loc0: case OP_put_loc1:
    ...
    case OP_set_loc2: case OP_set_loc3: case OP_set_loc_uninitialized:
    case OP_get_arg: case OP_get_arg0: case OP_get_arg1: case OP_get_arg2:
    case OP_get_arg3: case OP_put_arg: case OP_set_arg:
    case OP_if_true: case OP_if_false: case OP_if_true8: case OP_if_false8:
    case OP_goto: case OP_goto8: case OP_goto16:
    case OP_call: case OP_call0: case OP_call1: case OP_call2: case OP_call3:
    ...
    case OP_strict_neq: case OP_add_loc:
    case OP_inc: case OP_dec: case OP_neg: case OP_plus: case OP_not:
    case OP_lnot: case OP_inc_loc: case OP_dec_loc:
    case OP_post_inc: case OP_post_dec:
    /* Stage 4: property / element access, closures, method calls */
    case OP_get_field: case OP_get_field2: case OP_get_length:
    case OP_put_field: case OP_get_array_el: case OP_get_array_el2:
    case OP_put_array_el:
    case OP_get_var_ref: case OP_get_var_ref0: case OP_get_var_ref1:
    case OP_get_var_ref2: case OP_get_var_ref3: case OP_get_var_ref_check:
    case OP_put_var_ref: case OP_put_var_ref0: case OP_put_var_ref1:
    ...
    case OP_set_var_ref: case OP_set_var_ref0: case OP_set_var_ref1:
    case OP_set_var_ref2: case OP_set_var_ref3:
    case OP_call_method: case OP_tail_call: case OP_tail_call_method:
...
static bool qjs_jit_compute_depths(struct qjs_jit_ctx *jc)
{
    ...
    while (wl_count > 0) {
        ...
        delta = qjs_jit_op_delta(op, bytecode + pos + 1, pos,
                                 &branch_target, &is_uncond, &is_terminal);
        ...
        /* successors */
        if (branch_target >= 0) {
            if (branch_target < 0 || branch_target >= len ||
                bytecode[branch_target] >= OP_COUNT) {
                js_free_rt(jc->rt, worklist);
                return false;
            }
...
static sljit_sw qjs_jit_pack2(int a, int b) { return ((sljit_sw)a << 20) | b; }

static void qjs_jit_lower_op(struct qjs_jit_ctx *jc, uint8_t op,
                             const uint8_t *pc, int pos)
{
    ...
    case OP_push_i32:  qjs_jit_emit_imm(jc, depth, (int32_t)get_u32(pc), JS_TAG_INT); break;
    case OP_push_i8:   qjs_jit_emit_imm(jc, depth, get_i8(pc), JS_TAG_INT); break;
    case OP_push_i16:  qjs_jit_emit_imm(jc, depth, get_i16(pc), JS_TAG_INT); break;
    case OP_push_minus1: qjs_jit_emit_imm(jc, depth, -1, JS_TAG_INT); break;
    case OP_push_0: case OP_push_1: case OP_push_2: case OP_push_3:
    case OP_push_4: case OP_push_5: case OP_push_6: case OP_push_7:
        qjs_jit_emit_imm(jc, depth, op - OP_push_0, JS_TAG_INT); break;
    case OP_push_const:  qjs_jit_call_fss(jc, (void *)qjs_jh_dup_const, depth, get_u32(pc)); break;
    case OP_push_const8: qjs_jit_call_fss(jc, (void *)qjs_jh_dup_const, depth, pc[0]); break;
    case OP_undefined:   qjs_jit_emit_imm(jc, depth, 0, JS_TAG_UNDEFINED); break;
    case OP_null:        qjs_jit_emit_imm(jc, depth, 0, JS_TAG_NULL); break;
    case OP_push_false:  qjs_jit_emit_imm(jc, depth, 0, JS_TAG_BOOL); break;
    case OP_push_true:   qjs_jit_emit_imm(jc, depth, 1, JS_TAG_BOOL); break;

    /* --- stack manipulation --- */
    case OP_drop: qjs_jit_emit_free(jc, JIT_STACK, top, 0); break;
    case OP_dup:  qjs_jit_emit_copy(jc, JIT_STACK, depth, JIT_STACK, top);
                  qjs_jit_emit_incref(jc, JIT_STACK, depth); break;
    case OP_nip:  qjs_jit_emit_free(jc, JIT_STACK, top - 1, 0);
                  qjs_jit_emit_copy(jc, JIT_STACK, top - 1, JIT_STACK, top); break;
    case OP_swap: qjs_jit_call_fss(jc, (void *)qjs_jh_swap, top - 1, top); break;

    /* --- locals (inline copy + refcount) --- */
    case OP_get_loc:  qjs_jit_emit_copy(jc, JIT_STACK, depth, JIT_VAR, get_u16(pc));
                      qjs_jit_emit_incref(jc, JIT_STACK, depth); break;
    case OP_get_loc8: qjs_jit_emit_copy(jc, JIT_STACK, depth, JIT_VAR, pc[0]);
                      qjs_jit_emit_incref(jc, JIT_STACK, depth); break;
    ...
    case OP_get_loc0_loc1:
        ...
        qjs_jit_emit_incref(jc, JIT_STACK, depth + 1); break;
    case OP_put_loc:  qjs_jit_emit_put_var(jc, JIT_VAR, get_u16(pc), top, (void *)qjs_jh_put_loc); break;
    case OP_put_loc8: qjs_jit_emit_put_var(jc, JIT_VAR, pc[0], top, (void *)qjs_jh_put_loc); break;
    case OP_put_loc0: case OP_put_loc1: case OP_put_loc2: case OP_put_loc3:
        qjs_jit_emit_put_var(jc, JIT_VAR, op - OP_put_loc0, top, (void *)qjs_jh_put_loc); break;
    case OP_set_loc:  qjs_jit_emit_set_var(jc, JIT_VAR, get_u16(pc), top, (void *)qjs_jh_set_loc); break;
    case OP_set_loc8: qjs_jit_emit_set_var(jc, JIT_VAR, pc[0], top, (void *)qjs_jh_set_loc); break;
    case OP_set_loc0: case OP_set_loc1: case OP_set_loc2: case OP_set_loc3:
        qjs_jit_emit_set_var(jc, JIT_VAR, op - OP_set_loc0, top, (void *)qjs_jh_set_loc); break;
    case OP_set_loc_uninitialized:
        qjs_jit_call_fss(jc, (void *)qjs_jh_set_loc_uninit, get_u16(pc), 0); break;
    case OP_get_loc_check:
        qjs_jit_call_fss_ret(jc, (void *)qjs_jh_get_loc_check,
                             qjs_jit_pack2(depth, get_u16(pc)), pos);
        qjs_jit_check_exception(jc, depth);
        break;
    case OP_put_loc_check:
        qjs_jit_call_fss_ret(jc, (void *)qjs_jh_put_loc_check,
                             qjs_jit_pack2(get_u16(pc), top), pos);
        qjs_jit_check_exception(jc, depth);
        break;

    /* --- args --- */
    case OP_get_arg:  qjs_jit_call_fss(jc, (void *)qjs_jh_get_arg, depth, get_u16(pc)); break;
    case OP_get_arg0: case OP_get_arg1: case OP_get_arg2: case OP_get_arg3:
        qjs_jit_call_fss(jc, (void *)qjs_jh_get_arg, depth, op - OP_get_arg0); break;
    case OP_put_arg:  qjs_jit_call_fss(jc, (void *)qjs_jh_put_arg, get_u16(pc), top); break;
    case OP_set_arg:  qjs_jit_call_fss(jc, (void *)qjs_jh_set_arg, get_u16(pc), top); break;
    ...
    case OP_call: case OP_call0: case OP_call1: case OP_call2: case OP_call3: {
        ...
        qjs_jit_call_fss_ret(jc, (void *)qjs_jh_call,
                             qjs_jit_pack2(func_slot, argc), pos);
        qjs_jit_check_exception(jc, depth);
        break;
    }
    case OP_call_method: {
        ...
        qjs_jit_call_fss_ret(jc, (void *)qjs_jh_call_method,
                             qjs_jit_pack2(this_slot, argc), pos);
        qjs_jit_check_exception(jc, depth);
        break;
    }
    case OP_call_constructor: {
        ...
        qjs_jit_call_fss_ret(jc, (void *)qjs_jh_call_constructor,
                             qjs_jit_pack2(func_slot, argc), pos);
        qjs_jit_check_exception(jc, depth);
        break;
    }
    ...
    case OP_get_field:
        ...
        qjs_jit_check_exception(jc, depth); break;
    case OP_get_length:
    ...
    /* --- Stage 4: closure variables --- */
    case OP_get_var_ref:
        qjs_jit_call_fss(jc, (void *)qjs_jh_get_var_ref, depth, get_u16(pc)); break;
    case OP_get_var_ref0: case OP_get_var_ref1:
    case OP_get_var_ref2: case OP_get_var_ref3:
        qjs_jit_call_fss(jc, (void *)qjs_jh_get_var_ref, depth, op - OP_get_var_ref0); break;
    case OP_put_var_ref:
        qjs_jit_call_fss(jc, (void *)qjs_jh_put_var_ref, get_u16(pc), top); break;
    case OP_put_var_ref0: case OP_put_var_ref1:
    case OP_put_var_ref2: case OP_put_var_ref3:
        qjs_jit_call_fss(jc, (void *)qjs_jh_put_var_ref, op - OP_put_var_ref0, top); break;
    case OP_set_var_ref:
        qjs_jit_call_fss(jc, (void *)qjs_jh_set_var_ref, get_u16(pc), top); break;
    case OP_set_var_ref0: case OP_set_var_ref1:
    case OP_set_var_ref2: case OP_set_var_ref3:
        qjs_jit_call_fss(jc, (void *)qjs_jh_set_var_ref, op - OP_set_var_ref0, top); break;
    case OP_get_var_ref_check:
        qjs_jit_call_fss_ret(jc, (void *)qjs_jh_get_var_ref_check,
                             qjs_jit_pack2(depth, get_u16(pc)), pos);
        qjs_jit_check_exception(jc, depth); break;
    case OP_put_var_ref_check:
        qjs_jit_call_fss_ret(jc, (void *)qjs_jh_put_var_ref_check,
                             qjs_jit_pack2(get_u16(pc), top), pos);
        qjs_jit_check_exception(jc, depth); break;
    ...
    case OP_strict_neq:
        ...
        break;
    case OP_add_loc:
        qjs_jit_call_fss_ret(jc, (void *)qjs_jh_add_loc,
                             ((sljit_sw)pc[0] << 20) | depth, pos);
        qjs_jit_check_exception(jc, depth);
        break;

    /* --- Stage 3: unary in place (inline int fast path) --- */
    case OP_inc: case OP_dec:
        qjs_jit_emit_incdec(jc, op == OP_dec, JIT_STACK, top,
                            (void *)qjs_jh_unary, depth,
                            ((sljit_sw)op << 20) | pos, depth);
        break;
    case OP_neg: case OP_plus: case OP_not: case OP_lnot:
        qjs_jit_call_fss_ret(jc, (void *)qjs_jh_unary, depth,
                             ((sljit_sw)op << 20) | pos);
        qjs_jit_check_exception(jc, depth);
        break;
    case OP_post_inc: case OP_post_dec:
        qjs_jit_call_fss_ret(jc, (void *)qjs_jh_post_incdec, depth,
                             ((sljit_sw)op << 20) | pos);
        qjs_jit_check_exception(jc, depth);
        break;
    case OP_inc_loc: case OP_dec_loc:
        qjs_jit_emit_incdec(jc, op == OP_dec_loc, JIT_VAR, pc[0],
                            (void *)qjs_jh_incdec_loc,
...
static void qjs_jit_compile(JSContext *ctx, JSFunctionBytecode *b)
{
    ...
    qjs_jit_check(&jc, sljit_emit_enter(compiler, 0, SLJIT_ARGS2(W, P, P),
                                        6, 6, 0));
    qjs_jit_check(&jc, sljit_emit_op1(compiler, SLJIT_MOV, JIT_STACK, 0,
                                      SLJIT_MEM1(JIT_FRAME), offsetof(QJSJitFrame, stack_buf)));
    qjs_jit_check(&jc, sljit_emit_op1(compiler, SLJIT_MOV, JIT_VAR, 0,
                                      SLJIT_MEM1(JIT_FRAME), offsetof(QJSJitFrame, var_buf)));
    qjs_jit_check(&jc, sljit_emit_op1(compiler, SLJIT_MOV, JIT_ARG, 0,
                                      SLJIT_MEM1(JIT_FRAME), offsetof(QJSJitFrame, arg_buf)));
    qjs_jit_check(&jc, sljit_emit_op1(compiler, SLJIT_MOV, JIT_CTX, 0,
                                      SLJIT_MEM1(JIT_FRAME), offsetof(QJSJitFrame, ctx)));
...
{
    if (compiled)    *compiled = rt->jit.compiled;
    if (unsupported) *unsupported = rt->jit.unsupported;
    if (failed)      *failed = rt->jit.failed;
    if (jit_calls)   *jit_calls = rt->jit.jit_calls;
    return 0;
}
