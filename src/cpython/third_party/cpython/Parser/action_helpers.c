...
expr_ty _PyPegen_join_names_with_dot(Parser *p, expr_ty first_name, expr_ty second_name)
{
    assert(first_name != NULL && second_name != NULL);
    PyObject *uni = PyUnicode_FromFormat("%U.%U", first_name->v.Name.id, second_name->v.Name.id);
    if (!uni) {
        return NULL;
    }
...
alias_ty _PyPegen_alias_for_star(Parser *p, int lineno, int col_offset, int end_lineno,
...
CmpopExprPair *_PyPegen_cmpop_expr_pair(Parser *p, cmpop_ty cmpop, expr_ty expr)
{
    assert(expr != NULL);
...
static asdl_expr_seq *_set_seq_context(Parser *p, asdl_expr_seq *seq, expr_context_ty ctx)
{
    Py_ssize_t len = asdl_seq_LEN(seq);
    if (len == 0) {
        return NULL;
...
{
    return _PyAST_Tuple(_set_seq_context(p, e->v.Tuple.elts, ctx), ctx, EXTRA_EXPR(e, e));
}
...
static expr_ty _set_subscript_context(Parser *p, expr_ty e, expr_context_ty ctx)
{
    return _PyAST_Subscript(e->v.Subscript.value, e->v.Subscript.slice, ctx, EXTRA_EXPR(e, e));
}

static expr_ty _set_attribute_context(Parser *p, expr_ty e, expr_context_ty ctx)
{
    return _PyAST_Attribute(e->v.Attribute.value, e->v.Attribute.attr, ctx, EXTRA_EXPR(e, e));
}

static expr_ty _set_starred_context(Parser *p, expr_ty e, expr_context_ty ctx)
{
    return _PyAST_Starred(_PyPegen_set_expr_context(p, e->v.Starred.value, ctx), ctx,
                          EXTRA_EXPR(e, e));
}
...
KeyPatternPair *_PyPegen_key_pattern_pair(Parser *p, expr_ty key, pattern_ty pattern)
{
...
NameDefaultPair *_PyPegen_name_default_pair(Parser *p, arg_ty arg, expr_ty value, Token *tc)
{
...
SlashWithDefault *_PyPegen_slash_with_default(Parser *p, asdl_arg_seq *plain_names,
...
StarEtc *_PyPegen_star_etc(Parser *p, arg_ty vararg, asdl_seq *kwonlyargs, arg_ty kwarg)
{
...
static int _make_posonlyargs(Parser *p, asdl_arg_seq *slash_without_default,
                             SlashWithDefault *slash_with_default, asdl_arg_seq **posonlyargs)
{
    if (slash_without_default != NULL) {
        ...
        *posonlyargs = (asdl_arg_seq *)_PyPegen_join_sequences(
            p, (asdl_seq *)slash_with_default->plain_names, (asdl_seq *)slash_with_default_names);
    } else {
...
static int _make_posargs(Parser *p, asdl_arg_seq *plain_names, asdl_seq *names_with_default,
...
static int _make_posdefaults(Parser *p, SlashWithDefault *slash_with_default,
                             asdl_seq *names_with_default, asdl_expr_seq **posdefaults)
{
    if (slash_with_default != NULL && names_with_default != NULL) {
        ...
        *posdefaults = (asdl_expr_seq *)_PyPegen_join_sequences(
            p, (asdl_seq *)slash_with_default_values, (asdl_seq *)names_with_default_values);
...
arguments_ty _PyPegen_make_arguments(Parser *p, asdl_arg_seq *slash_without_default,
                                     ...
                                     StarEtc *star_etc)
{
    ...
    return _PyAST_arguments(posonlyargs, posargs, vararg, kwonlyargs, kwdefaults, kwarg,
                            posdefaults, p->arena);
}
...
arguments_ty _PyPegen_empty_arguments(Parser *p)
{
    ...
    return _PyAST_arguments(posonlyargs, posargs, NULL, kwonlyargs, kwdefaults, NULL, posdefaults,
                            p->arena);
}
...
stmt_ty _PyPegen_function_def_decorators(Parser *p, asdl_expr_seq *decorators, stmt_ty function_def)
{
    assert(function_def != NULL);
    if (function_def->kind == AsyncFunctionDef_kind) {
        return _PyAST_AsyncFunctionDef(
            function_def->v.AsyncFunctionDef.name, function_def->v.AsyncFunctionDef.args,
            function_def->v.AsyncFunctionDef.body, decorators,
            function_def->v.AsyncFunctionDef.returns, function_def->v.AsyncFunctionDef.type_comment,
    ...
    return _PyAST_FunctionDef(
        ...
        function_def->v.FunctionDef.type_comment, function_def->v.FunctionDef.type_params,
        function_def->lineno, function_def->col_offset, function_def->end_lineno,
        function_def->end_col_offset, p->arena);
}

/* Construct a ClassDef equivalent to class_def, but with decorators */
stmt_ty _PyPegen_class_def_decorators(Parser *p, asdl_expr_seq *decorators, stmt_ty class_def)
{
    assert(class_def != NULL);
    return _PyAST_ClassDef(class_def->v.ClassDef.name, class_def->v.ClassDef.bases,
...
KeywordOrStarred *_PyPegen_keyword_or_starred(Parser *p, void *element, int is_keyword)
{
...
asdl_keyword_seq *_PyPegen_seq_delete_starred_exprs(Parser *p, asdl_seq *kwargs)
{
...
mod_ty _PyPegen_make_module(Parser *p, asdl_stmt_seq *a)
{
    ...
    if (num > 0) {
        ...
        for (Py_ssize_t i = 0; i < num; i++) {
            ...
            type_ignore_ty ti =
                _PyAST_TypeIgnore(p->type_ignore_comments.items[i].lineno, tag, p->arena);
            if (ti == NULL) {
                return NULL;
            }
...
arg_ty _PyPegen_add_type_comment_to_arg(Parser *p, arg_ty a, Token *tc)
{
    ...
    return _PyAST_arg(a->arg, a->annotation, tco, a->lineno, a->col_offset, a->end_lineno,
                      a->end_col_offset, p->arena);
}
...
static ResultTokenWithMetadata *result_token_with_metadata(Parser *p, void *result,
                                                           PyObject *metadata)
{
...
ResultTokenWithMetadata *_PyPegen_check_fstring_conversion(Parser *p, Token *conv_token,
                                                           expr_ty conv)
{
    ...
    if (PyUnicode_GET_LENGTH(conv->v.Name.id) > 1 ||
        ...
        RAISE_SYNTAX_ERROR_KNOWN_LOCATION(
            conv, "%c-string: invalid conversion character %R: expected 's', 'r', or 'a'",
            TOK_GET_STRING_PREFIX(p->tok), conv->v.Name.id);
        return NULL;
    }
...
ResultTokenWithMetadata *_PyPegen_setup_full_format_spec(Parser *p, Token *colon,
                                                         asdl_expr_seq *spec, int lineno,
                                                         ...
                                                         int end_col_offset, PyArena *arena)
{
    ...
    for (Py_ssize_t i = 0; i < n_items; i++) {
        ...
        non_empty_count +=
            !(item->kind == Constant_kind && PyUnicode_CheckExact(item->v.Constant.value) &&
    ...
    if (non_empty_count != n_items) {
        ...
        for (Py_ssize_t i = 0; i < n_items; i++) {
            expr_ty item = asdl_seq_GET(spec, i);
            if (item->kind == Constant_kind && PyUnicode_CheckExact(item->v.Constant.value) &&
                PyUnicode_GET_LENGTH(item->v.Constant.value) == 0) {
                continue;
            }
    ...
    if (n == 0 || (n == 1 && asdl_seq_GET(spec, 0)->kind == Constant_kind)) {
        res = _PyAST_JoinedStr(spec, lineno, col_offset, end_lineno, end_col_offset, p->arena);
    } else {
        res = _PyPegen_concatenate_strings(p, spec, lineno, col_offset, end_lineno, end_col_offset,
                                           arena);
    }
    if (!res) {
        return NULL;
...
const char *_PyPegen_get_expr_name(expr_ty e)
{
    ...
    default:
        PyErr_Format(PyExc_SystemError, "unexpected expression in assignment %d (line %d)", e->kind,
                     e->lineno);
        return NULL;
    }
}
...
expr_ty _PyPegen_collect_call_seqs(Parser *p, asdl_expr_seq *a, asdl_seq *b, int lineno,
                                   ...
                                   PyArena *arena)
{
    ...
    if (b == NULL) {
        return _PyAST_Call(_PyPegen_dummy_name(p), a, NULL, lineno, col_offset, end_lineno,
    ...
    return _PyAST_Call(_PyPegen_dummy_name(p), args, keywords, lineno, col_offset, end_lineno,
                       end_col_offset, arena);
}
                                       ...
                                       asdl_comprehension_seq *comprehensions)
{
    ...
    return RAISE_SYNTAX_ERROR_KNOWN_RANGE((expr_ty)asdl_seq_GET(args->v.Call.args, len - 1),
...
static expr_ty _PyPegen_decode_fstring_part(Parser *p, int is_raw, expr_ty constant, Token *token)
{
    ...
    return _PyAST_Constant(str, NULL, constant->lineno, constant->col_offset, constant->end_lineno,
                           constant->end_col_offset, p->arena);
}

static asdl_expr_seq *_get_resized_exprs(Parser *p, Token *a, asdl_expr_seq *raw_expressions,
                                         Token *b, enum string_kind_t string_kind)
{
    ...
    for (Py_ssize_t i = 0; i < n_items; i++) {
        ...
        if (item->kind == JoinedStr_kind) {
            ...
            assert((string_kind == TSTRING && second->kind == Interpolation_kind) ||
...
expr_ty _PyPegen_template_str(Parser *p, Token *a, asdl_expr_seq *raw_expressions, Token *b)
{
    ...
    return _PyAST_TemplateStr(resized_exprs, a->lineno, a->col_offset, b->end_lineno,
...
expr_ty _PyPegen_joined_str(Parser *p, Token *a, asdl_expr_seq *raw_expressions, Token *b)
{
    ...
    return _PyAST_JoinedStr(resized_exprs, a->lineno, a->col_offset, b->end_lineno,
                            b->end_col_offset, p->arena);
}
...
expr_ty _PyPegen_decoded_constant_from_token(Parser *p, Token *tok)
{
    ...
    return _PyAST_Constant(str, NULL, tok->lineno, tok->col_offset, tok->end_lineno,
...
static int _get_interpolation_conversion(Parser *p, Token *debug,
...
static PyObject *_strip_interpolation_expr(PyObject *exprstr)
{
...
expr_ty _PyPegen_interpolation(Parser *p, expr_ty expression, Token *debug,
                               ...
                               Token *closing_brace, int lineno, int col_offset, int end_lineno,
                               int end_col_offset, PyArena *arena)
{
    ...
    expr_ty interpolation = _PyAST_Interpolation(expression, final_exprstr, conversion_val,
                                                 format ? (expr_ty)format->result : NULL, lineno,
    ...
    expr_ty debug_text = _PyAST_Constant(debug_metadata, NULL, lineno, col_offset + 1,
...
expr_ty _PyPegen_formatted_value(Parser *p, expr_ty expression, Token *debug,
                                 ResultTokenWithMetadata *conversion,
                                 ResultTokenWithMetadata *format, Token *closing_brace, int lineno,
                                 int col_offset, int end_lineno, int end_col_offset, PyArena *arena)
{
    int conversion_val = _get_interpolation_conversion(p, debug, conversion, format);

    expr_ty formatted_value =
        _PyAST_FormattedValue(expression, conversion_val, format ? (expr_ty)format->result : NULL,
...
static expr_ty _build_concatenated_bytes(Parser *p, asdl_expr_seq *strings, int lineno,
                                         int col_offset, int end_lineno, int end_col_offset,
                                         PyArena *arena)
{
...
static expr_ty _build_concatenated_unicode(Parser *p, asdl_expr_seq *strings, int lineno,
                                           int col_offset, int end_lineno, int end_col_offset,
                                           PyArena *arena)
{
    Py_ssize_t len = asdl_seq_LEN(strings);
    ...
    for (Py_ssize_t i = 0; i < len; i++) {
        ...
        if (PyUnicodeWriter_WriteStr(writer, current_elem->v.Constant.value)) {
    ...
    return _PyAST_Constant(final, kind, lineno, col_offset, end_lineno, end_col_offset, arena);
}

static asdl_expr_seq *_build_concatenated_str(Parser *p, asdl_expr_seq *strings, int lineno,
                                              int col_offset, int end_lineno, int end_col_offset,
                                              PyArena *arena)
{
    Py_ssize_t len = asdl_seq_LEN(strings);
    ...
    for (Py_ssize_t i = 0; i < n_flattened_elements; i++) {
        ...
        if (elem->kind == Constant_kind && PyUnicode_CheckExact(elem->v.Constant.value) &&
...
static expr_ty _build_concatenated_joined_str(Parser *p, asdl_expr_seq *strings, int lineno,
                                              int col_offset, int end_lineno, int end_col_offset,
                                              PyArena *arena)
{
    asdl_expr_seq *values =
        _build_concatenated_str(p, strings, lineno, col_offset, end_lineno, end_col_offset, arena);
    return _PyAST_JoinedStr(values, lineno, col_offset, end_lineno, end_col_offset, p->arena);
}

expr_ty _PyPegen_concatenate_tstrings(Parser *p, asdl_expr_seq *strings, int lineno, int col_offset,
                                      int end_lineno, int end_col_offset, PyArena *arena)
{
    asdl_expr_seq *values =
        _build_concatenated_str(p, strings, lineno, col_offset, end_lineno, end_col_offset, arena);
    return _PyAST_TemplateStr(values, lineno, col_offset, end_lineno, end_col_offset, arena);
}

expr_ty _PyPegen_concatenate_strings(Parser *p, asdl_expr_seq *strings, int lineno, int col_offset,
                                     int end_lineno, int end_col_offset, PyArena *arena)
{
    Py_ssize_t len = asdl_seq_LEN(strings);
    assert(len > 0);
    ...
    if (!f_string_found) {
        if (len == 1) {
            ...
            return _build_concatenated_bytes(p, strings, lineno, col_offset, end_lineno,
                                             end_col_offset, arena);
        } else {
            return _build_concatenated_unicode(p, strings, lineno, col_offset, end_lineno,
                                               end_col_offset, arena);
        }
    }

    return _build_concatenated_joined_str(p, strings, lineno, col_offset, end_lineno,
                                          end_col_offset, arena);
}

stmt_ty _PyPegen_checked_future_import(Parser *p, identifier module, asdl_alias_seq *names,
                                       int level, int lineno, int col_offset, int end_lineno,
                                       int end_col_offset, PyArena *arena)
{
    ...
    return _PyAST_ImportFrom(module, names, level, lineno, col_offset, end_lineno, end_col_offset,
                             arena);
}
