...
int _PyPegen_lookahead_with_string(int, expr_ty(func)(Parser *, const char *), Parser *,
                                   const char *);
...
void *_PyPegen_raise_error_known_location(Parser *p, PyObject *errtype, Py_ssize_t lineno,
...
static inline void *RAISE_ERROR_KNOWN_LOCATION(Parser *p, PyObject *errtype, Py_ssize_t lineno,
                                               Py_ssize_t col_offset, Py_ssize_t end_lineno,
                                               Py_ssize_t end_col_offset, const char *errmsg, ...)
{
    va_list va;
...
#define RAISE_SYNTAX_ERROR_KNOWN_RANGE(a, b, msg, ...)                                             \
    RAISE_ERROR_KNOWN_LOCATION(p, PyExc_SyntaxError, (a)->lineno, (a)->col_offset,                 \
                               (b)->end_lineno, (b)->end_col_offset, msg, ##__VA_ARGS__)
#define RAISE_SYNTAX_ERROR_KNOWN_LOCATION(a, msg, ...)                                             \
    RAISE_ERROR_KNOWN_LOCATION(p, PyExc_SyntaxError, (a)->lineno, (a)->col_offset,                 \
                               (a)->end_lineno, (a)->end_col_offset, msg, ##__VA_ARGS__)
#define RAISE_SYNTAX_ERROR_STARTING_FROM(a, msg, ...)                                              \
    RAISE_ERROR_KNOWN_LOCATION(p, PyExc_SyntaxError, (a)->lineno, (a)->col_offset, CURRENT_POS,    \
                               CURRENT_POS, msg, ##__VA_ARGS__)
...
Py_LOCAL_INLINE(void *) _RAISE_SYNTAX_ERROR_INVALID_TARGET(Parser *p, TARGETS_TYPE type, void *e)
{
    ...
    if (invalid_target != NULL) {
        ...
        return RAISE_SYNTAX_ERROR_KNOWN_LOCATION(invalid_target, msg,
...
#define EXTRA_EXPR(head, tail)                                                                     \
    head->lineno, (head)->col_offset, (tail)->end_lineno, (tail)->end_col_offset, p->arena
...
Py_LOCAL_INLINE(void *) INVALID_VERSION_CHECK(Parser *p, int version, char *msg, void *node)
{
    if (node == NULL) {
        p->error_indicator = 1; // Inline CHECK_CALL
        return NULL;
...
arguments_ty _PyPegen_make_arguments(Parser *, asdl_arg_seq *, SlashWithDefault *, asdl_arg_seq *,
                                     asdl_seq *, StarEtc *);
...
expr_ty _PyPegen_interpolation(Parser *, expr_ty, Token *, ResultTokenWithMetadata *,
                               ResultTokenWithMetadata *, Token *, int, int, int, int, PyArena *);
expr_ty _PyPegen_formatted_value(Parser *, expr_ty, Token *, ResultTokenWithMetadata *,
                                 ResultTokenWithMetadata *, Token *, int, int, int, int, PyArena *);
...
expr_ty _PyPegen_collect_call_seqs(Parser *, asdl_expr_seq *, asdl_seq *, int lineno,
...
ResultTokenWithMetadata *_PyPegen_setup_full_format_spec(Parser *, Token *, asdl_expr_seq *, int,
                                                         int, int, int, PyArena *);
...
stmt_ty _PyPegen_checked_future_import(Parser *p, identifier module, asdl_alias_seq *, int, int,
                                       int, int, int, PyArena *);
...
mod_ty _PyPegen_run_parser_from_file_pointer(FILE *, int, PyObject *, const char *, const char *,
                                             const char *, PyCompilerFlags *, int *, PyObject **,
                                             PyArena *);
