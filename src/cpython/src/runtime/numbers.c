...
static void format_double_repr(double d, char *out, size_t outsz)
{
    if (isnan(d)) {
    ...
    if (isinf(d)) {
        snprintf(out, outsz, "%sinf", d < 0 ? "-" : "");
        return;
    }
    if (d == 0.0) {
        snprintf(out, outsz, "%s0.0", signbit(d) ? "-" : "");
        return;
    }
...
static int detect_base(const char *p, int base, const char **after_prefix)
{
    int detected = base;
    if (base == 0) {
...
PyObject *PyLong_FromString(const char *str, char **pend, int base)
{
    ...
    while (ndec > 1 && dec[ndec - 1] == 0) {
        ndec--; /* strip leading zeros */
    }

    char *out = malloc(ndec + 2);
    size_t oi = 0;
    if (neg && !(ndec == 1 && dec[0] == 0)) {
        out[oi++] = '-';
    }
    for (size_t i = 0; i < ndec; i++) {
        out[oi++] = (char)('0' + dec[ndec - 1 - i]);
    }
    out[oi] = '\0';
    free(dec);
...
unsigned long PyOS_strtoul(const char *str, char **ptr, int base)
{
    ...
    for (; *p; p++) {
        ...
        if (result > (ULONG_MAX - (unsigned long)d) / (unsigned long)detected) {
            overflow = 1;
        }
