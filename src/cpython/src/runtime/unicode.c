...
static void
normalize_encoding(const char *enc, char *out, size_t outsz)
{
    size_t oi = 0;
    if (enc) {
        for (; *enc && oi + 1 < outsz; enc++) {
            ...
            if (c >= 'A' && c <= 'Z') c = c - 'A' + 'a';
            if ((c >= 'a' && c <= 'z') || (c >= '0' && c <= '9')) out[oi++] = (char)c;
        }
    }
    out[oi] = '\0';
}
...
static PyObject *
str_from(const char *bytes, Py_ssize_t size)
{
    if (size < 0) {
...
PyObject *PyUnicode_FromString(const char *str) { return str_from(str, -1); }
...
const char *PyUnicode_AsUTF8(PyObject *unicode) { return PyUnicode_AsUTF8AndSize(unicode, NULL); }
...
PyObject *PyUnicode_InternFromString(const char *str) { return str_from(str, -1); }
void _PyUnicode_InternImmortal(void *interp, PyObject **p) { (void)interp; (void)p; }
...
PyObject *
PyUnicode_Decode(const char *s, Py_ssize_t size, const char *encoding, const char *errors)
{
    char norm[32];
    normalize_encoding(encoding, norm, sizeof norm);
    /* Latin-1 / ISO-8859-1: each byte is code point U+00..U+FF -> UTF-8. */
    if (!strcmp(norm, "latin1") || !strcmp(norm, "iso88591") ||
        !strcmp(norm, "l1") || !strcmp(norm, "latin") || !strcmp(norm, "cp819") ||
        !strcmp(norm, "8859") || !strcmp(norm, "iso8859") || !strcmp(norm, "iso885911")) {
        char *out = malloc((size_t)size * 2 + 1);
        if (out == NULL) { PyErr_NoMemory(); return NULL; }
        Py_ssize_t oi = 0;
...
static Py_ssize_t
put_utf8(char *out, Py_ssize_t oi, unsigned cp)
{
    if (cp < 0x80) {
        out[oi++] = (char)cp;
    } else if (cp < 0x800) {
...
PyObject *
_PyUnicode_DecodeUnicodeEscapeInternal2(const char *string, Py_ssize_t length, const char *errors,
                                        Py_ssize_t *consumed, int *first_invalid_escape_char,
...
int
_PyUnicode_ScanIdentifier(PyObject *u)
{
...
{ return ch == ' ' || (ch >= 0x09 && ch <= 0x0d) || ch == 0x1c || ch == 0x1d || ch == 0x1e || ch == 0x1f; }
...
static int
writer_reserve(PyUnicodeWriter *w, size_t extra)
{
...
PyUnicodeWriter_WriteChar(PyUnicodeWriter *writer, Py_UCS4 ch)
{
    ...
    char tmp[4];
    int n;
    if (ch < 0x80) { tmp[0] = (char)ch; n = 1; }
    else if (ch < 0x800) {
        tmp[0] = (char)(0xc0 | (ch >> 6)); tmp[1] = (char)(0x80 | (ch & 0x3f)); n = 2;
    } else if (ch < 0x10000) {
        tmp[0] = (char)(0xe0 | (ch >> 12)); tmp[1] = (char)(0x80 | ((ch >> 6) & 0x3f));
        tmp[2] = (char)(0x80 | (ch & 0x3f)); n = 3;
    } else {
        tmp[0] = (char)(0xf0 | (ch >> 18)); tmp[1] = (char)(0x80 | ((ch >> 12) & 0x3f));
        tmp[2] = (char)(0x80 | ((ch >> 6) & 0x3f)); tmp[3] = (char)(0x80 | (ch & 0x3f)); n = 4;
    }
...
PyUnicode_FromFormatV(const char *format, va_list vargs)
{
    ...
    const char *p = format;
    while (*p) {
        if (*p != '%') { PyUnicodeWriter_WriteChar(w, (unsigned char)*p++); continue; }
        ...
        switch (conv) {
        case 'U': case 'S': case 'R': case 'A': case 'V': {
            PyObject *o = va_arg(vargs, PyObject *);
            if (conv == 'V') { const char *fallback = va_arg(vargs, const char *);
                if ((!o || !PyUnicode_Check(o)) && fallback) {
                    PyUnicodeWriter_WriteUTF8(w, fallback, -1); break; } }
            if (o && PyUnicode_Check(o)) PyUnicodeWriter_WriteUTF8(w, o->data, o->len);
            else if (o) { PyObject *s = PyObject_Str(o); if (s) PyUnicodeWriter_WriteUTF8(w, s->data, s->len); }
            break;
        }
        case 's': {
        ...
        case 'd': case 'i': case 'u': case 'x': case 'X': case 'o': {
            ...
            char real[40];
            size_t ri = 0;
            for (size_t k = 0; k + 1 < si && spec[k] != conv; k++) real[ri++] = spec[k];
            if (length_mod == 'z') { real[ri++] = 'z'; }
            else if (length_mod == 'l') { real[ri++] = 'l'; }
            else if (length_mod == 'L') { real[ri++] = 'l'; real[ri++] = 'l'; }
            real[ri++] = conv; real[ri] = '\0';
            if (length_mod == 'z') {
        ...
        case '%': PyUnicodeWriter_WriteChar(w, '%'); break;
        case '\0': break;
        default: PyUnicodeWriter_WriteUTF8(w, spec, -1); break;
        }
    }
