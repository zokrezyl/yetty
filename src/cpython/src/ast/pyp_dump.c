...
static unsigned utf8_next(const char *s, Py_ssize_t n, Py_ssize_t i, int *adv)
{
    ...
    if ((c & 0xe0) == 0xc0) {
        ...
        *adv = 1;
        return c;
    } /* invalid lead: treat as raw byte */
    if (i + len > n) {
        *adv = 1;
        return c;
    }
...
static void emit_repr(const char *s, Py_ssize_t n)
{
    ...
    while (i < n) {
        ...
        if (cp == (unsigned)quote || cp == '\\') {
            putc('\\', pyp_dump_fp);
            putc((int)cp, pyp_dump_fp);
        } else if (cp == '\t') {
            fputs("\\t", pyp_dump_fp);
        } else if (cp == '\n') {
            fputs("\\n", pyp_dump_fp);
            ...
            fprintf(pyp_dump_fp, "\\x%02x", cp);
        } else if (cp < 0x7f) {
            putc((int)cp, pyp_dump_fp);
        } else if (pyp_is_printable(cp)) {
            for (int k = 0; k < adv; k++) {
                putc((unsigned char)s[i + k], pyp_dump_fp);
            }
        } else if (cp < 0x100) {
            fprintf(pyp_dump_fp, "\\x%02x", cp);
        } else if (cp < 0x10000) {
            fprintf(pyp_dump_fp, "\\u%04x", cp);
        } else {
            fprintf(pyp_dump_fp, "\\U%08x", cp);
        }
        i += adv;
    }
    putc(quote, pyp_dump_fp);
...
void pyp_emit_identifier(PyObject *o)
{
    if (o == NULL) {
...
static void emit_bytes_repr(const char *s, Py_ssize_t n)
{
    int has_single = 0, has_double = 0;
    for (Py_ssize_t i = 0; i < n; i++) {
        if (s[i] == '\'') {
    ...
    for (Py_ssize_t i = 0; i < n; i++) {
        ...
        if (c == (unsigned char)quote || c == '\\') {
            ...
            putc((int)c, pyp_dump_fp);
        } else {
            fprintf(pyp_dump_fp, "\\x%02x", c);
        }
    }
    putc(quote, pyp_dump_fp);
}

void pyp_emit_constant(PyObject *o)
{
    if (o == NULL) {
        fputs("None", pyp_dump_fp);
        return;
    ...
    case PYP_NONE:
        ...
        break;
    case PYP_BOOL:
        fputs(o->ival ? "True" : "False", pyp_dump_fp);
        break;
    case PYP_ELLIPSIS:
        fputs("Ellipsis", pyp_dump_fp);
        break;
    case PYP_LONG:
        fputs(o->data ? o->data : "0", pyp_dump_fp);
        break;
    case PYP_FLOAT:
        fputs(o->data ? o->data : "0.0", pyp_dump_fp);
        break;
    case PYP_COMPLEX:
        fputs(o->data ? o->data : "0j", pyp_dump_fp);
        break;
    case PYP_STR:
        emit_repr(o->data, o->len);
        break;
    case PYP_BYTES:
        putc('b', pyp_dump_fp);
        emit_bytes_repr(o->data, o->len);
        break;
    default:
        fputs("<const>", pyp_dump_fp);
    }
}
