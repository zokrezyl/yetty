...
# Links against the prebuilt openssl-1.1.1w tarball — openssh 9.x only
# accepts OpenSSL 1.x/3.x (SSH doesn't use TLS; OpenSSL provides crypto
# primitives, not a TLS layer, so the 1.1.1w build is correct here).
#
# Unsupported platforms (android, ios, tvos, webasm, windows) emit an
# UNSUPPORTED placeholder tarball for shape parity with other 3rdparty libs.
# The consumer-side openssh.cmake is guarded (NOT ANDROID / NOT YETTY_IOS /
# ...) so those tarballs are never fetched at build time.
#
# Required env:
...
mkdir -p "$WORK_DIR" "$OUTPUT_DIR" "$CACHE_DIR" "$STAGE"

#-----------------------------------------------------------------------------
# Placeholder for platforms where openssh is not built.
# Emits an UNSUPPORTED tarball so the release has shape parity with other
# 3rdparty libs. The consumer cmake is guarded and never fetches these.
#-----------------------------------------------------------------------------
case "$TARGET_PLATFORM" in
android-arm64-v8a|android-x86_64|\
ios-arm64|ios-x86_64|tvos-arm64|tvos-x86_64|\
webasm|windows-x86_64)
    echo "openssh: $TARGET_PLATFORM is not built (UNSUPPORTED placeholder)" >&2
    cat > "$STAGE/UNSUPPORTED" <<EOF
openssh $VERSION is not built for $TARGET_PLATFORM.

Reason:
  android  — yetty on Android cannot exec() a child ssh process
  ios/tvos — Apple sandbox forbids exec()
  webasm   — no PTY/exec semantics in the browser
  windows  — openssh ships as a Windows system component

The consumer-side openssh.cmake is guarded for these platforms and will
never attempt to fetch this tarball.
EOF
    tar -C "$STAGE" -czf "$TARBALL" .
    echo "openssh $VERSION ($TARGET_PLATFORM) — placeholder tarball written:"
    ls -lh "$TARBALL"
    exit 0
    ;;
esac
...
#-----------------------------------------------------------------------------
# Fetch helper (retry-friendly).
#-----------------------------------------------------------------------------
fetch() {
    local url="$1" cache="$2" descr="$3"
    if [ ! -f "$cache" ]; then
        echo "==> downloading $descr"
        curl -fL --retry 8 --retry-delay 5 --retry-all-errors -o "$cache.part.$$" "$url"
        mv "$cache.part.$$" "$cache"
    fi
}

#-----------------------------------------------------------------------------
# Fetch openssh source + prebuilt openssl.
#-----------------------------------------------------------------------------
fetch "$OPENSSH_URL"  "$OPENSSH_TARBALL" "openssh ${VERSION}"
fetch "$OSSL_TAR_URL" "$OSSL_TARBALL"    "openssl ${OSSL_VERSION} (${TARGET_PLATFORM})"
...
linux-x86_64)
    : # native gcc — no extra args
    ;;

linux-aarch64)
    # Runs natively on ubuntu-24.04-arm; no cross-compiler needed.
    : # native gcc
    ;;
...
linux-riscv64)
    ...
    # Pre-answer AC_TRY_RUN tests that cannot run on the x86_64 host.
...
macos-arm64)
    : # native clang, Xcode CommandLineTools
    ;;
