...
CFLAGS  += -arch \$(ARCH) -isysroot \$(SDKROOT) ${_MIN_FLAG} -DAPPLE_IOS
LDFLAGS += -arch \$(ARCH) -isysroot \$(SDKROOT) ${_MIN_FLAG}
TVOS_MK
    EXTRA_ARGS=(
        OS=tvos
        "ARCH=${_OH264_ARCH}"
...
windows-x86_64)
    ...
    # (chocolatey: `choco install make`) under a vcvarsall'd shell so
    # cl.exe produces MSVC-ABI openh264.lib for yetty.exe linking.
