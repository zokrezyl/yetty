#!/usr/bin/env bash
# quickjs-ng 3rdparty wrapper. MIT-licensed JavaScript engine
# (quickjs-ng fork of Bellard's QuickJS). Pure C, libc-only — so this is
# the simple zlib/lexbor-shaped wrapper, no transitive prebuilt deps.
set -euo pipefail
...
case "$TARGET_PLATFORM" in
    ...
    windows-x86_64)
        ...
        # is on PATH. Every lib that links into yetty.exe needs MSVC ABI.
...
cd "$(dirname "$0")/../../.."
exec nix develop ".#$SHELL_NAME" --command bash build-tools/3rdparty/quickjs/_build.sh "$@"
