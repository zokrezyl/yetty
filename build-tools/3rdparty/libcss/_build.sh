...
webasm|windows-x86_64)
    # NetSurf's buildsystem is a GNU-make build that assumes a Unix
    # toolchain (POSIX shell, sed, awk). Same blocker as the netsurf
    # placeholder branch — these targets emit an UNSUPPORTED-marker
    # tarball; the consumer-side libcss.cmake detects it and skips,
    # so the rest of yetty configures unaffected. Real cross builds
    # for Android/iOS/etc. can be added later once the upstream make
    # rules are taught to honour CC/HOST for these triples.
...
NetSurf's buildsystem.mk is a GNU-make build that assumes a Unix-style
toolchain. Cross-compile for Android/iOS/tvOS/webasm/windows-MSVC needs
work in the upstream Makefile (or a CMake reimplementation). For now,
the consumer-side build-tools/cmake/libs/libcss.cmake detects this
UNSUPPORTED marker and skips libcss silently — ybrowser falls back to
its built-in lexbor-CSS path on these platforms.
EOF
