#!/usr/bin/env bash
# lexbor 3rdparty wrapper. Apache-2.0 HTML/CSS parsing + DOM + selector
# engine. Pure C, libc-only (its `posix`/`windows_nt` ports are part of
# its own source tree), so this is the simple zlib-shaped wrapper —
# no transitive prebuilt deps.
set -euo pipefail
: "${TARGET_PLATFORM:?TARGET_PLATFORM is required}"

case "$TARGET_PLATFORM" in
    linux-x86_64|linux-aarch64|linux-riscv64|\
    macos-x86_64|macos-arm64|\
    android-arm64-v8a|android-x86_64|\
    ios-arm64|ios-x86_64|tvos-arm64|tvos-x86_64|\
    webasm)
        SHELL_NAME="3rdparty-${TARGET_PLATFORM}"
        ;;
    windows-x86_64)
        # Native MSVC: caller must have vcvarsall'd the shell so cl.exe
        # is on PATH. qemu is the lone mingw build — every lib that
        # links into yetty.exe needs MSVC ABI.
        if ! command -v cl >/dev/null 2>&1 && ! command -v cl.exe >/dev/null 2>&1; then
            echo "error: windows-x86_64 requires MSVC cl on PATH (vcvarsall x64)" >&2
            exit 1
        fi
        exec bash "$(dirname "$0")/_build.sh" "$@"
        ;;
    *) echo "unknown TARGET_PLATFORM: $TARGET_PLATFORM" >&2; exit 1 ;;
esac

if [ "${USE_NIX:-1}" = "0" ]; then
    exec bash "$(dirname "$0")/_build.sh" "$@"
fi
cd "$(dirname "$0")/../../.."
exec nix develop ".#$SHELL_NAME" --command bash build-tools/3rdparty/lexbor/_build.sh "$@"
