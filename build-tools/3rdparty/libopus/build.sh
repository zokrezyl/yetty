#!/usr/bin/env bash
# libopus 3rdparty wrapper: picks the right nix shell for $TARGET_PLATFORM
...
# `lib-libopus-<version>` → tarball `libopus-<target>-<version>.tar.gz`.
...
case "$TARGET_PLATFORM" in
    ...
    webasm)
        ...
        ;;
    windows-x86_64)
        # Native MSVC: caller must have vcvarsall'd the shell so cl.exe is on
        # PATH. Every lib that links into yetty.exe needs MSVC ABI.
...
cd "$(dirname "$0")/../../.."
exec nix develop ".#$SHELL_NAME" --command bash build-tools/3rdparty/libopus/_build.sh "$@"
