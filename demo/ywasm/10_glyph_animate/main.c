 ...
 *   BufferUnmap
 *   yetty_ywasm_client_present_frame
 ...
 *
 * Run via:  ./yetty -e demo-ywasm-10-glyph-animate
 * Trace at: /tmp/ywasm-demo-10-glyph-animate.trace
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static int load_glyph(struct yetty_ywasm_client *c, uint64_t device, uint64_t ubo,
                      ...
                      struct glyph_rt *out, FILE *trace)
{
    ...
    out->module = ywasm_client_wgpuDeviceCreateShaderModule(c, device, &smd);
    free(wgsl);
    ...
    pd.fragment = &fs;
    out->pipeline = ywasm_client_wgpuDeviceCreateRenderPipeline(c, device, &pd);

    out->bgl = ywasm_client_wgpuRenderPipelineGetBindGroupLayout(c, out->pipeline, 0);
    ...
    bgd.entryCount = 1;
    bgd.entries = &entry;
    out->bg = ywasm_client_wgpuDeviceCreateBindGroup(c, device, &bgd);
...
static void unload_glyph(struct yetty_ywasm_client *c, struct glyph_rt *g)
{
    if (g->bg)       (void)ywasm_client_wgpuBindGroupRelease(c, g->bg);
    if (g->bgl)      (void)ywasm_client_wgpuBindGroupLayoutRelease(c, g->bgl);
    if (g->pipeline) (void)ywasm_client_wgpuRenderPipelineRelease(c, g->pipeline);
    if (g->module)   (void)ywasm_client_wgpuShaderModuleRelease(c, g->module);
    memset(g, 0, sizeof(*g));
}

/* Render one frame at `time`, read pixels back, present. Returns 1 on
 * success, 0 if the round-trip failed. */
static int render_frame(struct yetty_ywasm_client *c, uint64_t instance,
                        ...
                        uint8_t *pixels, size_t pixel_count)
{
    ...
    (void)ywasm_client_wgpuQueueWriteBuffer(c, queue, ubo, 0, ubo_data, sizeof(ubo_data));

    uint64_t enc = ywasm_client_wgpuDeviceCreateCommandEncoder(c, device);
    ...
    uint64_t pass = ywasm_client_wgpuCommandEncoderBeginRenderPass(c, enc, &rp);
    (void)ywasm_client_wgpuRenderPassEncoderSetPipeline(c, pass, g->pipeline);
    (void)ywasm_client_wgpuRenderPassEncoderSetBindGroup(c, pass, 0, g->bg, 0, NULL);
    (void)ywasm_client_wgpuRenderPassEncoderDraw(c, pass, 3, 1, 0, 0);
    (void)ywasm_client_wgpuRenderPassEncoderEnd(c, pass);
    (void)ywasm_client_wgpuRenderPassEncoderRelease(c, pass);
    ...
    (void)ywasm_client_wgpuCommandEncoderCopyTextureToBuffer(c, enc, &src, &dst, &copy_size);

    uint64_t cb = ywasm_client_wgpuCommandEncoderFinish(c, enc);
    uint64_t cbs[1] = { cb };
    (void)ywasm_client_wgpuQueueSubmit(c, queue, 1, (WGPUCommandBuffer const *)cbs);

    s_map_done = 0;
    s_map_payload = 0;
    (void)ywasm_client_wgpuBufferMapAsync(c, readback, WGPUMapMode_Read, 0,
    ...
    if (ok) {
        (void)ywasm_client_wgpuBufferReadMappedRange(c, readback, 0, pixels, pixel_count);
        (void)ywasm_client_wgpuBufferUnmap(c, readback);
        struct yetty_ycore_void_result pr =
            yetty_ywasm_client_present_frame(c, W, H, pixels, pixel_count);
        if (pr.ok != 1) ok = 0;
    }

    (void)ywasm_client_wgpuCommandBufferRelease(c, cb);
    (void)ywasm_client_wgpuCommandEncoderRelease(c, enc);
    return ok;
}
...
#define LOG(...) do { if (trace) fprintf(trace, __VA_ARGS__); } while (0)
    ...
    for (int i = 0; i < 200 && !yetty_ywasm_client_connected(c); ++i) {
        (void)yetty_ywasm_client_pump(c); demo_sleep_ms(10);
    }
    ...
    uint64_t tex = ywasm_client_wgpuDeviceCreateTexture(c, device, &td);
    ...
    uint64_t view = ywasm_client_wgpuTextureCreateView(c, tex, &tvd);

    WGPUBufferDescriptor ubd = {0};
    ubd.usage = WGPUBufferUsage_CopyDst | WGPUBufferUsage_Uniform;
    ubd.size = 16;
    uint64_t ubo = ywasm_client_wgpuDeviceCreateBuffer(c, device, &ubd);
    ...
    uint64_t readback = ywasm_client_wgpuDeviceCreateBuffer(c, device, &rbd);
...
cleanup_buffers:
    (void)ywasm_client_wgpuBufferRelease(c, readback);
    (void)ywasm_client_wgpuBufferRelease(c, ubo);
    (void)ywasm_client_wgpuTextureViewRelease(c, view);
    (void)ywasm_client_wgpuTextureRelease(c, tex);
