 ...
 *
 * Run via:  ./yetty -e demo-ywasm-09-glyph-render
 * Trace at: /tmp/ywasm-demo-09-glyph-render.trace
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
#define LOG(...) do { if (trace) fprintf(trace, __VA_ARGS__); } while (0)
    ...
    if (cr.ok != 1) { LOG("09: client_create failed\n"); return 1; }
    ...
    uint64_t module = ywasm_client_wgpuDeviceCreateShaderModule(c, device, &smd);
    ...
    pd.fragment = &fs;
    uint64_t pipeline = ywasm_client_wgpuDeviceCreateRenderPipeline(c, device, &pd);
    ...
    uint64_t bgl = ywasm_client_wgpuRenderPipelineGetBindGroupLayout(c, pipeline, 0);
    ...
    bgd.entryCount = 1;
    bgd.entries = &entry;
    uint64_t bg = ywasm_client_wgpuDeviceCreateBindGroup(c, device, &bgd);
    ...
    uint64_t pass = ywasm_client_wgpuCommandEncoderBeginRenderPass(c, encoder, &rp);
    (void)ywasm_client_wgpuRenderPassEncoderSetPipeline(c, pass, pipeline);
    (void)ywasm_client_wgpuRenderPassEncoderSetBindGroup(c, pass, 0, bg, 0, NULL);
    ...
    (void)ywasm_client_wgpuCommandEncoderCopyTextureToBuffer(c, encoder, &src, &dst, &copy_size);
    ...
    for (int i = 0; i < 500 && !s_map_done; ++i) {
        (void)ywasm_client_wgpuDeviceTick(c, device);
        (void)ywasm_client_wgpuInstanceProcessEvents(c, instance);
        (void)yetty_ywasm_client_pump(c); demo_sleep_ms(10);
    }
    ...
    WGPUStatus rs = ywasm_client_wgpuBufferReadMappedRange(c, readback, 0,
                                                           pixels, pixel_count);
...
cleanup_pipeline:
    (void)ywasm_client_wgpuBindGroupRelease(c, bg);
    (void)ywasm_client_wgpuBindGroupLayoutRelease(c, bgl);
    (void)ywasm_client_wgpuCommandBufferRelease(c, cb);
    (void)ywasm_client_wgpuCommandEncoderRelease(c, encoder);
    (void)ywasm_client_wgpuRenderPipelineRelease(c, pipeline);
    ...
    (void)ywasm_client_wgpuShaderModuleRelease(c, module);
