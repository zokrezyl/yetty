...
#define LOG(...) do { if (trace) fprintf(trace, __VA_ARGS__); } while (0)
    ...
    uint64_t adapter = ywasm_client_wgpuInstanceRequestAdapter(c, instance, on_adapter, &st);
    ...
    uint64_t device = ywasm_client_wgpuAdapterRequestDevice(c, adapter, on_device, &st);
...
cleanup_instance:
     ...
     * yetty log before we tear the PTY down. */
    for (int i = 0; i < 100; ++i) {
        (void)yetty_ywasm_client_pump(c);
        demo_sleep_ms(10);
    }
