 ...
 *
 * Run via:  ./yetty -e demo-ywasm-08-glyph-shaders
 * Trace at: /tmp/ywasm-demo-08-glyph-shaders.trace
 */
#include <dirent.h>
#include <stdio.h>
#include <stdlib.h>
...
#define LOG(...) do { if (trace) fprintf(trace, __VA_ARGS__); } while (0)
    ...
    if (cr.ok != 1) {
        LOG("08_glyph_shaders: client_create failed\n");
        return 1;
    }
    ...
    for (int i = 0; i < n_names; ++i) {
        ...
        uint64_t mod = ywasm_client_wgpuDeviceCreateShaderModule(c, device, &desc);
        if (mod) {
            ++ok;
            LOG("  [%2d] %s (id=0x%04x, %zu bytes WGSL) → module=%lu\n",
                i, names[i], glyph_id, wgsl_len, (unsigned long)mod);
            (void)ywasm_client_wgpuShaderModuleRelease(c, mod);
        } else {
            ++fail;
