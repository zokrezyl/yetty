#!/usr/bin/env python3
"""ygui2 from Python — the static hello. RUNNABLE inside a yetty pane:

    PYTHONPATH=bindings/python python3 demo/ffi/ygui2/python/hello.py
...
actually changed. Ctrl-C quits (also `q` while no text input holds focus).
"""
...
column.label(text="ygui2 — hello from Python", fg="#74C5A5", basis=24)
...
column.label(text="clean frames ship zero bytes — watch the pane stay silent",
             fg="#9FA7A8", basis=20)
row = column.row(basis=24, gap=10)
...
column.statusbar(left="hello.py — static frame", right="Ctrl-C: quit", basis=24)
