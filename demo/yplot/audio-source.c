...
static int parse_args(int argc, char **argv, struct opts *o)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (strncmp(a, "--frames=", 9) == 0) {
            ...
            if (parse_double(a + 7, &o->freq) < 0) {
                return -1;
            }
        } else if (strncmp(a, "--sample-rate=", 14) == 0) {
            if (parse_int(a + 14, &o->sample_rate) < 0) {
                return -1;
            }
        } else if (strncmp(a, "--window=", 9) == 0) {
            if (parse_int(a + 9, &o->window_samples) < 0) {
                return -1;
            }
        } else if (strncmp(a, "--plot-id=", 10) == 0) {
            if (parse_int(a + 10, &o->plot_id) < 0) {
                return -1;
            }
...
static void generate_frame(float *out, int n, double t0, double sample_rate, double freq)
{
    for (int i = 0; i < n; i++) {
        ...
        double v = sin(2.0 * M_PI * freq * t) + 0.30 * sin(2.0 * M_PI * 2.0 * freq * t) +
...
static struct yetty_ycore_void_result emit_create_envelope(const struct opts *o, double window_s)
{
    ...
    struct yetty_ycore_void_result er = yetty_ydraw_drawable_list_end_group(dl, (uint32_t)mk.value);
    if (YETTY_IS_ERR(er)) {
        yetty_ydraw_drawable_list_destroy(dl);
                                                           ...
                                                           int count)
{
    ...
    struct yetty_ycore_void_result cu =
        yetty_ydraw_drawable_list_add_cmd_update(dl, (uint32_t)plot_id, payload, payload_size);
    free(payload);
    if (YETTY_IS_ERR(cu)) {
        yetty_ydraw_drawable_list_destroy(dl);
