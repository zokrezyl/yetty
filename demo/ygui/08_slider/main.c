...
static struct yetty_ygui_old_widget* g_value_label = NULL;

static void on_change(struct yetty_ygui_old_widget* w, float value, void* u) {
    (void)w; (void)u;
    char buf[32];
    snprintf(buf, sizeof(buf), "Volume: %d%%", (int)value);
    yetty_ygui_old_widget_label_set_text(g_value_label, buf);
}
...
int main(void) {
    ...
    { struct ygui_engine_ptr_result _eng_r = yetty_ygui_old_engine_create((struct yetty_ygui_old_engine_args){.name = "slider-demo"});
    ...
    yetty_ygui_old_engine_label(g_engine, "title", 50, 30, "Volume Control");
    g_value_label = yetty_ygui_old_engine_label(g_engine, "value", 50, 70, "Volume: 50%");

    struct yetty_ygui_old_widget* sl = yetty_ygui_old_engine_slider(g_engine, "volume", 50, 110, 300, 30, 0, 100, 50);
    yetty_ygui_old_widget_slider_on_change(sl, on_change, NULL);
