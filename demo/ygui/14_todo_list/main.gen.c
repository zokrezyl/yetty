...
struct yetty_yclass_ptr_result yetty_demoygui_14_todo_list_class_get(void)
{
    ...
    if (YETTY_IS_ERR(parent_class_r)) {
        yerror("yetty_demoygui_14_todo_list_class_get: parent accessor failed: %s",
               parent_class_r.error.msg);
        return YETTY_ERR(yetty_yclass_ptr,
                         "yetty_demoygui_14_todo_list_class_get: parent accessor failed",
                         parent_class_r);
    }
    struct yetty_yclass_ptr_result register_class_r = yetty_yclass_register(
        &desc, ops, sizeof(ops) / sizeof(ops[0]), parent_class_r.value, NULL, 0);
    if (YETTY_IS_ERR(register_class_r)) {
        yerror("yetty_demoygui_14_todo_list_class_get: class_register failed: %s",
               register_class_r.error.msg);
        return YETTY_ERR(yetty_yclass_ptr,
                         "yetty_demoygui_14_todo_list_class_get: class_register failed",
                         register_class_r);
    }
    ...
    struct yetty_yclass_object *obj)
{
    ...
    if (YETTY_IS_ERR(class_r)) {
        return YETTY_ERR(yetty_demoygui_14_todo_list_ptr,
                         "yetty_demoygui_14_todo_list_from: class accessor", class_r);
    }
    struct yetty_yclass_void_ptr_result slice_r = yetty_yclass_object_data(obj, class_r.value);
    if (YETTY_IS_ERR(slice_r)) {
        return YETTY_ERR(yetty_demoygui_14_todo_list_ptr,
                         "yetty_demoygui_14_todo_list_from: object_data", slice_r);
    }
    return YETTY_OK(yetty_demoygui_14_todo_list_ptr,
                    (struct yetty_demoygui_14_todo_list *)slice_r.value);
}
    ...
    struct yetty_demoygui_14_todo_list *data)
{
    ...
    YETTY_RETURN_IF_ERR(yetty_yclass_object_ptr, class_r,
                        "yetty_demoygui_14_todo_list_to: class accessor");
    struct yetty_ycore_size_result offset_r =
        yetty_yclass_object_data_offset(class_r.value, class_r.value);
    YETTY_RETURN_IF_ERR(yetty_yclass_object_ptr, offset_r,
                        "yetty_demoygui_14_todo_list_to: data offset");
    ...
    struct yetty_yclass_ctx *ctx)
{
    ...
    if (YETTY_IS_ERR(class_accessor_r)) {
        return YETTY_ERR(yetty_yclass_object_ptr,
                         "yetty_demoygui_14_todo_list_create: class accessor failed",
                         class_accessor_r);
    }
     ...
     * response isn't silently swallowed. */
    {
        ...
        if (YETTY_IS_ERR(translate_class_r)) {
            yetty_ycore_error_print(stderr,
                                    "yetty_demoygui_14_todo_list_create: translate_class (degraded "
                                    "— will lazy-resolve)",
    ...
    if (!proxy) {
        return YETTY_ERR(yetty_yclass_object_ptr,
                         "yetty_demoygui_14_todo_list_create: calloc(proxy) failed");
    }
