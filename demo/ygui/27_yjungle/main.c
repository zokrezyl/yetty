...
static struct yetty_ygui_old_widget *g_jungle = NULL;
...
{
    if (g_jungle) {
        yetty_ygui_old_widget_remove(g_jungle);
        g_jungle = NULL;
    }
    ...
    g_jungle = yetty_ygui_old_engine_yjungle(g_engine, "jungle", 0, 0, w, h,
                                         &cfg, g_seed);
    if (g_jungle) {
        yetty_ygui_old_widget_apply_css(g_jungle, "flex: 1 0 0; align-self: stretch;");
        yetty_ygui_old_widget_add_child(g_outer, g_jungle);
    }
}
...
int main(int argc, char **argv)
{
    ...
    struct ygui_engine_ptr_result eng_r = yetty_ygui_old_engine_create(
        (struct yetty_ygui_old_engine_args){.name = "yjungle-demo"});
