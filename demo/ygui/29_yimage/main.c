...
static struct yetty_ygui_old_widget *g_img = NULL;
...
{
    if (g_img) {
        yetty_ygui_old_widget_remove(g_img);
        g_img = NULL;
    }
    g_img = yetty_ygui_old_engine_yimage_from_file(g_engine, "img", 0, 0, w, h, g_path);
    if (g_img) {
        yetty_ygui_old_widget_apply_css(g_img, "flex: 1 0 0; align-self: stretch;");
        yetty_ygui_old_widget_add_child(g_outer, g_img);
    }
}
...
int main(int argc, char **argv)
{
    ...
    struct ygui_engine_ptr_result eng_r = yetty_ygui_old_engine_create(
        (struct yetty_ygui_old_engine_args){.name = "yimage-demo"});
