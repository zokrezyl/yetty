 ...
 * ypdf widget via yetty_ygui_old_widget_scrollbar_bind. The scrollbar then
...
static struct yetty_ygui_old_widget *g_pdf = NULL;
static struct yetty_ygui_old_widget *g_scroll = NULL;
...
static void on_key(struct yetty_ygui_old_engine *e, uint32_t key, int mods, void *u)
{
    ...
    float viewport = yetty_ygui_old_widget_ypdf_viewport_height(g_pdf);
    float line = 60.0f;
    switch (key) {
    case KEY_UP:        yetty_ygui_old_widget_ypdf_scroll_by(g_pdf, -line); break;
    case KEY_DOWN:      yetty_ygui_old_widget_ypdf_scroll_by(g_pdf,  line); break;
    case KEY_PAGE_UP:   yetty_ygui_old_widget_ypdf_scroll_by(g_pdf, -viewport * 0.9f); break;
    case KEY_PAGE_DOWN: yetty_ygui_old_widget_ypdf_scroll_by(g_pdf,  viewport * 0.9f); break;
    case KEY_HOME:      yetty_ygui_old_widget_ypdf_scroll_to(g_pdf, 0.0f); break;
    case KEY_END:
        yetty_ygui_old_widget_ypdf_scroll_to(g_pdf,
                                         yetty_ygui_old_widget_ypdf_max_scroll(g_pdf));
        break;
    default: break;
    }
}
...
int main(int argc, char **argv)
{
    ...
    struct ygui_engine_ptr_result eng_r = yetty_ygui_old_engine_create(
        (struct yetty_ygui_old_engine_args){.name = "ypdf-demo"});
    ...
    g_outer = yetty_ygui_old_engine_hbox(g_engine, "outer", 0, 0, 100, 100);
    ...
    g_pdf = yetty_ygui_old_engine_ypdf_from_file(g_engine, "pdf", 0, 0, cw - sbar_w, ch, path);
    if (!g_pdf) {
    ...
    yetty_ygui_old_widget_apply_css(g_pdf, "flex: 1 0 0; align-self: stretch;");
    yetty_ygui_old_widget_add_child(g_outer, g_pdf);

    g_scroll = yetty_ygui_old_engine_vscrollbar(g_engine, "vbar", 0, 0, sbar_w, ch);
    yetty_ygui_old_widget_apply_css(g_scroll, "align-self: stretch;");
    yetty_ygui_old_widget_add_child(g_outer, g_scroll);
    ...
    yetty_ygui_old_widget_scrollbar_bind(g_scroll, g_pdf);

    fprintf(stderr, "ypdf-demo: %d pages, content_h=%.0fpx\n",
            yetty_ygui_old_widget_ypdf_page_count(g_pdf),
            yetty_ygui_old_widget_ypdf_content_height(g_pdf));
