...
YETTY_MAYBE_UNUSED
static yetty_yguiapp_build_fn yetty_demoygui_25_ypdf_yetty_yguiapp_build_check = build;

struct yetty_yclass_ptr_result yetty_demoygui_25_ypdf_class_get(void)
{
    static const struct yetty_yclass *cls = NULL;
    if (cls) return YETTY_OK(yetty_yclass_ptr, cls);
    ydebug("registering class=yetty_demoygui_25_ypdf");

    static const struct yetty_yclass_descriptor desc = {
        .name = "yetty_demoygui_25_ypdf",
        .type = YETTY_YCLASS_TYPE_REGULAR,
        .data_size = sizeof(struct yetty_demoygui_25_ypdf),
        .data_align = _Alignof(struct yetty_demoygui_25_ypdf),
    };
    ...
    if (YETTY_IS_ERR(parent_class_r)) {
        yerror("yetty_demoygui_25_ypdf_class_get: parent accessor failed: %s", parent_class_r.error.msg);
        return YETTY_ERR(yetty_yclass_ptr, "yetty_demoygui_25_ypdf_class_get: parent accessor failed", parent_class_r);
    }
    struct yetty_yclass_ptr_result register_class_r =
        yetty_yclass_register(&desc, ops, sizeof(ops) / sizeof(ops[0]),
                              parent_class_r.value, NULL, 0);
    if (YETTY_IS_ERR(register_class_r)) {
        yerror("yetty_demoygui_25_ypdf_class_get: class_register failed: %s", register_class_r.error.msg);
        return YETTY_ERR(yetty_yclass_ptr, "yetty_demoygui_25_ypdf_class_get: class_register failed", register_class_r);
    }
...
{
    struct yetty_yclass_ptr_result class_r = yetty_demoygui_25_ypdf_class_get();
    if (YETTY_IS_ERR(class_r))
        return YETTY_ERR(yetty_demoygui_25_ypdf_ptr, "yetty_demoygui_25_ypdf_from: class accessor", class_r);
    struct yetty_yclass_void_ptr_result slice_r =
        yetty_yclass_object_data(obj, class_r.value);
    if (YETTY_IS_ERR(slice_r))
        return YETTY_ERR(yetty_demoygui_25_ypdf_ptr, "yetty_demoygui_25_ypdf_from: object_data", slice_r);
    return YETTY_OK(yetty_demoygui_25_ypdf_ptr, (struct yetty_demoygui_25_ypdf *)slice_r.value);
}
...
struct yetty_yclass_object_ptr_result yetty_demoygui_25_ypdf_to(struct yetty_demoygui_25_ypdf *data)
{
    ...
    struct yetty_yclass_ptr_result class_r = yetty_demoygui_25_ypdf_class_get();
    YETTY_RETURN_IF_ERR(yetty_yclass_object_ptr, class_r, "yetty_demoygui_25_ypdf_to: class accessor");
    struct yetty_ycore_size_result offset_r =
        yetty_yclass_object_data_offset(class_r.value, class_r.value);
    YETTY_RETURN_IF_ERR(yetty_yclass_object_ptr, offset_r, "yetty_demoygui_25_ypdf_to: data offset");
...
struct yetty_yclass_object_ptr_result yetty_demoygui_25_ypdf_create(struct yetty_yclass_ctx *ctx)
{
    ydebug("class=yetty_demoygui_25_ypdf");
    /* Touch the local accessor first — registers the class's slots in
     * slot_table so subsequent name→local-slot lookups succeed.
     * Without this, translate_class on a fresh remote-only session
     * would have no local slots to map remote ids onto. */
    struct yetty_yclass_ptr_result class_accessor_r = yetty_demoygui_25_ypdf_class_get();
    if (YETTY_IS_ERR(class_accessor_r))
        return YETTY_ERR(yetty_yclass_object_ptr,
                         "yetty_demoygui_25_ypdf_create: class accessor failed", class_accessor_r);
     ...
     * response isn't silently swallowed. */
    {
        struct yetty_ycore_void_result translate_class_r =
            yetty_yclass_rpc_session_translate_class(ctx->session, "yetty_demoygui_25_ypdf");
        if (YETTY_IS_ERR(translate_class_r)) {
            yetty_ycore_error_print(stderr,
                "yetty_demoygui_25_ypdf_create: translate_class (degraded — will lazy-resolve)",
    ...
    uint64_t handle = 0;
    const char *class_name = "yetty_demoygui_25_ypdf";
    ...
    if (YETTY_IS_ERR(create_call_r))
        return YETTY_ERR(yetty_yclass_object_ptr,
                         "yetty_demoygui_25_ypdf_create: CREATE call failed", create_call_r);
    if (create_call_r.value != sizeof(handle) || !handle)
        return YETTY_ERR(yetty_yclass_object_ptr,
                         "yetty_demoygui_25_ypdf_create: CREATE returned no/invalid handle");
    ...
    if (!proxy)
        return YETTY_ERR(yetty_yclass_object_ptr, "yetty_demoygui_25_ypdf_create: calloc(proxy) failed");
