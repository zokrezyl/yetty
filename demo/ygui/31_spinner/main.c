...
static void on_int_change(struct yetty_ygui_widget *w, float value, void *u)
{
    (void)w; (void)u;
    char buf[64];
...
static void on_float_change(struct yetty_ygui_widget *w, float value, void *u)
{
    (void)w; (void)u;
    char buf[64];
...
int main(void)
{
    ...
    struct ygui_engine_ptr_result eng_r = yetty_ygui_engine_create(
        (struct yetty_ygui_engine_args){.name = "spinner-demo"});
    ...
    struct yetty_ygui_widget *spin_int =
        yetty_ygui_engine_spinner(engine, "spin_int", 0, 0, 180, 32, 1, 100, 1, 42);
    yetty_ygui_widget_spinner_on_change(spin_int, on_int_change, NULL);
    yetty_ygui_widget_add_child(g_outer, spin_int);

    struct yetty_ygui_widget *spin_float =
        yetty_ygui_engine_spinner(engine, "spin_float", 0, 0, 180, 32, 0.0f, 10.0f, 0.25f, 2.5f);
    yetty_ygui_widget_spinner_set_precision(spin_float, 2);
    yetty_ygui_widget_spinner_on_change(spin_float, on_float_change, NULL);
    yetty_ygui_widget_add_child(g_outer, spin_float);

    g_status = yetty_ygui_engine_label(engine, "status", 0, 0, "Ready");
