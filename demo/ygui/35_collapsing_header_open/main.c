                                ...
                                struct yetty_ygui_old_widget *tab_panel)
{
    ...
    yetty_ygui_old_widget_add_child(
        sec, yetty_ygui_old_engine_toggle(engine, "el_toggle", 24, 0, 240, 26, "Notifications", 1));
    yetty_ygui_old_widget_add_child(
        sec, yetty_ygui_old_engine_progress(engine, "el_prog", 24, 0, 320, 16, 0.65f));
    struct yetty_ygui_old_widget *prog_indet =
        yetty_ygui_old_engine_progress(engine, "el_prog_indet", 24, 0, 320, 16, 0.0f);
    yetty_ygui_old_widget_progress_set_indeterminate(prog_indet, 1);
    yetty_ygui_old_widget_add_child(sec, prog_indet);
    yetty_ygui_old_widget_add_child(
        sec, yetty_ygui_old_engine_textarea(engine, "el_ta", 24, 0, 420, 120,
                                        "Multi-line text area.\nClick to focus, then type.\n"));
}

/* Tab #2 — a single label so the tabbar has a sibling to switch to. */
static void build_other_tab(struct yetty_ygui_old_engine *engine,
                             struct yetty_ygui_old_widget *tab_panel)
{
    struct yetty_ygui_old_widget *label = yetty_ygui_old_engine_label(
        engine, "other_label", 24, 24,
        "This is the second tab. Click 'Elements' to see the open section.");
    yetty_ygui_old_widget_apply_css(label, "align-self: flex-start;");
    yetty_ygui_old_widget_add_child(tab_panel, label);
}
...
int main(void)
{
    ...
    struct ygui_engine_ptr_result eng_r =
        yetty_ygui_old_engine_create((struct yetty_ygui_old_engine_args){.name = "demo35-elements"});
