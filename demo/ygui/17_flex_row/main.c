...
static struct yetty_ygui_old_widget *g_row = NULL;
...
static void add_child(void)
{
    ...
    struct yetty_ygui_old_widget *b = yetty_ygui_old_engine_button(g_engine, id, 0, 0, 96, 56, label);
    yetty_ygui_old_widget_add_child(g_row, b);
                      ...
                      float prev_h, void *u)
{
    ...
    (void)prev_h;
    (void)u;
    yetty_ygui_old_widget_set_size(g_row, new_w, new_h - HEADER_H);
}
...
static void on_key(struct yetty_ygui_old_engine *e, uint32_t key, int mods, void *u)
{
    ...
    case 'J':
        ...
        yetty_ygui_old_widget_set_justify_content(g_row, (ygui_justify_t)g_justify);
        break;
    case 'a':
    case 'A':
        g_align = g_align >= YETTY_YGUI_OLD_ALIGN_STRETCH ? YETTY_YGUI_OLD_ALIGN_START : g_align + 1;
        yetty_ygui_old_widget_set_align_items(g_row, (ygui_align_t)g_align);
        break;
    default:
        dirty = 0;
        break;
...
int main(void)
{
    ...
    struct ygui_engine_ptr_result eng_r =
        yetty_ygui_old_engine_create((struct yetty_ygui_old_engine_args){.name = "flex-row"});
    ...
    g_row = yetty_ygui_old_engine_hbox(g_engine, "row", 0, HEADER_H, 100, 100);
    yetty_ygui_old_widget_set_padding(g_row, 16, 16, 16, 16);
    yetty_ygui_old_widget_set_gap(g_row, 12);
    yetty_ygui_old_widget_set_align_items(g_row, (ygui_align_t)g_align);
    yetty_ygui_old_widget_set_justify_content(g_row, (ygui_justify_t)g_justify);
    ...
    /* Snap to current canvas dims in case OSC 777780 already arrived. */
    {
        ...
        if (YETTY_IS_OK(sr) && sr.value.width > 0 && sr.value.height > 0) {
            yetty_ygui_old_widget_set_size(g_row, sr.value.width, sr.value.height - HEADER_H);
