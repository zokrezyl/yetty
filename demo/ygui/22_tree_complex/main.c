...
static void on_volume(struct yetty_ygui_old_widget *w, float v, void *u)
{
    (void)w; (void)u;
    status("Volume: %d%%", (int)v);
}

static void on_brightness(struct yetty_ygui_old_widget *w, float v, void *u)
{
    (void)w; (void)u;
    status("Brightness: %d%%", (int)v);
}

static void on_mute(struct yetty_ygui_old_widget *w, int checked, void *u)
{
    (void)w; (void)u;
    status("Mute: %s", checked ? "on" : "off");
}

static void on_dark(struct yetty_ygui_old_widget *w, int checked, void *u)
{
    (void)w; (void)u;
...
static struct yetty_ygui_old_widget *
make_field_row(const char *id, const char *label_text, struct yetty_ygui_old_widget *control)
{
    char hbox_id[128], lbl_id[128];
    snprintf(hbox_id, sizeof(hbox_id), "row_%s", id);
    snprintf(lbl_id, sizeof(lbl_id), "lbl_%s", id);

    struct yetty_ygui_old_widget *row = yetty_ygui_old_engine_hbox(g_engine, hbox_id, 0, 0, 0, 32);
    yetty_ygui_old_widget_apply_css(row,
                                 "padding: 4px 8px; gap: 12px;"
                                 "justify-content: space-between; align-items: center;");
    struct yetty_ygui_old_widget *lbl = yetty_ygui_old_engine_label(g_engine, lbl_id, 0, 0, label_text);
    yetty_ygui_old_widget_add_child(row, lbl);
    yetty_ygui_old_widget_add_child(row, control);
    return row;
}
...
int main(void)
{
    ...
    struct ygui_engine_ptr_result eng_r =
        yetty_ygui_old_engine_create((struct yetty_ygui_old_engine_args){.name = "tree-complex"});
    ...
    yetty_ygui_old_widget_add_child(
        g_outer,
        yetty_ygui_old_engine_label(g_engine, "title", 0, 0,
                                 "Settings tree — sliders, checkboxes, "
                                 "buttons live inside tree_node children"));

    g_status = yetty_ygui_old_engine_label(g_engine, "status", 0, 0, "Interact with any control");
    ...
    yetty_ygui_old_widget_apply_css(g_tree,
                                 "padding: 6px; gap: 4px; flex: 1 0 0;");
    yetty_ygui_old_widget_add_child(g_outer, g_tree);

    /* Audio section */
    struct yetty_ygui_old_widget *audio = yetty_ygui_old_engine_tree_node(g_engine, "audio", "Audio");
    yetty_ygui_old_widget_add_child(g_tree, audio);

    struct yetty_ygui_old_widget *volume = yetty_ygui_old_engine_slider(
        g_engine, "vol", 0, 0, 180, 22, 0, 100, 75);
    yetty_ygui_old_widget_slider_on_change(volume, on_volume, NULL);
    yetty_ygui_old_widget_add_child(yetty_ygui_old_widget_tree_node_children(audio),
                                 make_field_row("volume", "Volume", volume));

    struct yetty_ygui_old_widget *mute = yetty_ygui_old_engine_checkbox(
        g_engine, "mute", 0, 0, 24, 22, "", 0);
    yetty_ygui_old_widget_checkbox_on_change(mute, on_mute, NULL);
    yetty_ygui_old_widget_add_child(yetty_ygui_old_widget_tree_node_children(audio),
                                 make_field_row("mute", "Mute", mute));

    /* Display section */
    struct yetty_ygui_old_widget *display = yetty_ygui_old_engine_tree_node(g_engine, "display", "Display");
    yetty_ygui_old_widget_add_child(g_tree, display);

    struct yetty_ygui_old_widget *brightness = yetty_ygui_old_engine_slider(
        g_engine, "bri", 0, 0, 180, 22, 0, 100, 60);
    yetty_ygui_old_widget_slider_on_change(brightness, on_brightness, NULL);
    yetty_ygui_old_widget_add_child(yetty_ygui_old_widget_tree_node_children(display),
                                 make_field_row("brightness", "Brightness", brightness));

    struct yetty_ygui_old_widget *dark = yetty_ygui_old_engine_checkbox(
        g_engine, "dark", 0, 0, 24, 22, "", 1);
    yetty_ygui_old_widget_checkbox_on_change(dark, on_dark, NULL);
    yetty_ygui_old_widget_add_child(yetty_ygui_old_widget_tree_node_children(display),
                                 make_field_row("dark", "Dark theme", dark));

    /* Nested: an "Advanced" subsection inside Display with another control. */
    struct yetty_ygui_old_widget *advanced = yetty_ygui_old_engine_tree_node(
        g_engine, "adv", "Advanced");
    yetty_ygui_old_widget_add_child(yetty_ygui_old_widget_tree_node_children(display), advanced);
    struct yetty_ygui_old_widget *reset = yetty_ygui_old_engine_button(
        g_engine, "reset", 0, 0, 100, 28, "Reset");
    yetty_ygui_old_widget_button_on_click(reset, on_reset, NULL);
    yetty_ygui_old_widget_add_child(yetty_ygui_old_widget_tree_node_children(advanced),
                                 make_field_row("reset", "Defaults", reset));

    /* About section: leaf labels only. */
    struct yetty_ygui_old_widget *about = yetty_ygui_old_engine_tree_node(g_engine, "about", "About");
    yetty_ygui_old_widget_add_child(g_tree, about);
    yetty_ygui_old_widget_add_child(yetty_ygui_old_widget_tree_node_children(about),
                                 yetty_ygui_old_engine_label(g_engine, "ver", 0, 0,
                                                          "ygui v0.2.0"));
    yetty_ygui_old_widget_add_child(yetty_ygui_old_widget_tree_node_children(about),
                                 yetty_ygui_old_engine_label(g_engine, "lic", 0, 0,
                                                          "MIT-licensed"));

    /* Open the obvious sections by default so the controls are visible. */
    yetty_ygui_old_widget_tree_node_set_expanded(audio, 1);
    yetty_ygui_old_widget_tree_node_set_expanded(display, 1);
