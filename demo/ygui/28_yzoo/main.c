...
static struct yetty_ygui_widget *g_zoo = NULL;
...
{
    if (g_zoo) {
        yetty_ygui_widget_remove(g_zoo);
        g_zoo = NULL;
    }
    struct yetty_yzoo_config cfg = yetty_yzoo_config_default();
    g_zoo = yetty_ygui_engine_yzoo(g_engine, "zoo", 0, 0, w, h, &cfg, g_seed);
    if (g_zoo) {
        yetty_ygui_widget_apply_css(g_zoo, "flex: 1 0 0; align-self: stretch;");
        yetty_ygui_widget_add_child(g_outer, g_zoo);
    }
}
...
int main(int argc, char **argv)
{
    ...
    struct ygui_engine_ptr_result eng_r = yetty_ygui_engine_create(
        (struct yetty_ygui_engine_args){.name = "yzoo-demo"});
