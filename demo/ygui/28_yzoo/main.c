 ...
 * Demo 28: yzoo widget.
 *
 * Renders one yzoo snapshot at time=0 — control-point markers and the
 * curves connecting them — into a ygui canvas. The leading CMD_ZERO
 * the producer emits is stripped inside the widget so the RICH render
 * path can paint the primitives directly.
 *
...
static struct yetty_ygui_old_widget *g_zoo = NULL;
static struct yetty_ygui_old_engine *g_engine = NULL;
static uint32_t g_seed = 0;

static void rebuild_zoo(float w, float h)
{
    if (g_zoo) {
        yetty_ygui_old_widget_remove(g_zoo);
        g_zoo = NULL;
    }
    struct yetty_yzoo_config cfg = yetty_yzoo_config_default();
    g_zoo = yetty_ygui_old_engine_yzoo(g_engine, "zoo", 0, 0, w, h, &cfg, g_seed);
    if (g_zoo) {
        yetty_ygui_old_widget_apply_css(g_zoo, "flex: 1 0 0; align-self: stretch;");
        yetty_ygui_old_widget_add_child(g_outer, g_zoo);
    }
}
...
int main(int argc, char **argv)
{
    ...
    struct ygui_engine_ptr_result eng_r = yetty_ygui_old_engine_create(
        (struct yetty_ygui_old_engine_args){.name = "yzoo-demo"});
