...
static struct yetty_ygui_old_widget *g_ta = NULL;
static struct yetty_ygui_old_widget *g_status = NULL;

static void on_text_change(struct yetty_ygui_old_widget *w, const char *text, void *u)
{
    (void)w; (void)u;
    char buf[128];
...
static void on_key(struct yetty_ygui_old_engine *e, uint32_t key, int mods, void *u)
{
    ...
    if ((key == 'q' || key == 'Q')) {
        ...
        struct yetty_ygui_old_widget *ta = yetty_ygui_old_engine_find(e, "ta");
        if (!ta || !(yetty_ygui_old_widget_get_flags(ta) & YETTY_YGUI_OLD_FLAG_FOCUSED)) {
...
int main(void)
{
    ...
    struct ygui_engine_ptr_result eng_r = yetty_ygui_old_engine_create(
        (struct yetty_ygui_old_engine_args){.name = "textarea-demo"});
    ...
    yetty_ygui_old_widget_apply_css(g_outer, "padding: 16; gap: 12; align-items: stretch;");
    ...
    g_ta = yetty_ygui_old_engine_textarea(engine, "ta", 0, 0, 0, 0,
        "Hello, world!\n\nThis is a multi-line text area.\nType, hit Enter, arrow around.\n");
    yetty_ygui_old_widget_apply_css(g_ta, "flex: 1 0 0; align-self: stretch;");
    yetty_ygui_old_widget_textarea_on_change(g_ta, on_text_change, NULL);
    yetty_ygui_old_widget_add_child(g_outer, g_ta);

    g_status = yetty_ygui_old_engine_label(engine, "status", 0, 0, "0 bytes");
    yetty_ygui_old_widget_add_child(g_outer, g_status);
    on_text_change(g_ta, yetty_ygui_old_widget_textarea_get_text(g_ta), NULL);
