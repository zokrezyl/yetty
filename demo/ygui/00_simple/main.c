...
int main(void)
{
    ...
    struct ygui_engine_ptr_result eng_r = yetty_ygui_old_engine_create(
        (struct yetty_ygui_old_engine_args){.name = "simple-tabbar"});
    ...
    struct yetty_ygui_old_widget *tabbar =
        yetty_ygui_old_engine_tabbar(g_engine, "tabs", 0, 0, 0, 0);
    yetty_ygui_old_widget_apply_css(tabbar, "flex: 1 0 0; align-items: stretch;");
    yetty_ygui_old_widget_add_child(g_outer, tabbar);

    struct yetty_ygui_old_widget *tab_plot = yetty_ygui_old_widget_tabbar_add_tab(tabbar, "Plot");
    ...
    yetty_ygui_old_widget_apply_css(plot, "flex: 1 0 0; align-self: stretch;");
    yetty_ygui_old_widget_add_child(tab_plot, plot);
    ...
    yetty_ygui_old_widget_set_size(g_outer, W, H);
