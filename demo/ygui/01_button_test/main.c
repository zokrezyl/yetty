...
static struct yetty_ygui_old_widget* g_status_label = NULL;
static int g_click_count = 0;

static void on_button1_click(struct yetty_ygui_old_widget* widget, void* userdata) {
    (void)userdata;
    g_click_count++;
    char buf[64];
    snprintf(buf, sizeof(buf), "Button 1 clicked! Count: %d", g_click_count);
    yetty_ygui_old_widget_label_set_text(g_status_label, buf);
    fprintf(stderr, "[DEMO] Button 1 clicked (id=%s)\n", yetty_ygui_old_widget_id(widget));
}

static void on_button2_click(struct yetty_ygui_old_widget* widget, void* userdata) {
    (void)userdata;
    g_click_count++;
    char buf[64];
...
static void on_button3_click(struct yetty_ygui_old_widget* widget, void* userdata) {
    (void)userdata;
    g_click_count++;
    char buf[64];
...
int main(void) {
    ...
    { struct ygui_engine_ptr_result _eng_r = yetty_ygui_old_engine_create((struct yetty_ygui_old_engine_args){.name = "btn-test"});
    ...
    /* Title */
    yetty_ygui_old_engine_label(g_engine, "title", 30, 20, "Button Click Test");

    /* Status label */
    g_status_label = yetty_ygui_old_engine_label(g_engine, "status", 30, 250, "Click any button...");

    /* Button 1 - at x=30 */
    fprintf(stderr, "[DEMO] Creating button1 at x=30, y=70, w=120, h=40\n");
    struct yetty_ygui_old_widget* btn1 = yetty_ygui_old_engine_button(g_engine, "button1", 30, 70, 120, 40, "Button 1");
    yetty_ygui_old_widget_button_on_click(btn1, on_button1_click, NULL);

    /* Button 2 - at x=180 */
    fprintf(stderr, "[DEMO] Creating button2 at x=180, y=70, w=120, h=40\n");
    struct yetty_ygui_old_widget* btn2 = yetty_ygui_old_engine_button(g_engine, "button2", 180, 70, 120, 40, "Button 2");
    yetty_ygui_old_widget_button_on_click(btn2, on_button2_click, NULL);

    /* Button 3 - at x=330 */
    fprintf(stderr, "[DEMO] Creating button3 at x=330, y=70, w=120, h=40\n");
    struct yetty_ygui_old_widget* btn3 = yetty_ygui_old_engine_button(g_engine, "button3", 330, 70, 120, 40, "Button 3");
    yetty_ygui_old_widget_button_on_click(btn3, on_button3_click, NULL);

    /* Info labels showing expected hit areas */
    yetty_ygui_old_engine_label(g_engine, "info1", 30, 130, "Btn1: x=[30,150)");
    yetty_ygui_old_engine_label(g_engine, "info2", 30, 160, "Btn2: x=[180,300)");
    yetty_ygui_old_engine_label(g_engine, "info3", 30, 190, "Btn3: x=[330,450)");

    /* Quit button */
    struct yetty_ygui_old_widget* quit_btn = yetty_ygui_old_engine_button(g_engine, "quit", 380, 250, 80, 30, "Quit");
