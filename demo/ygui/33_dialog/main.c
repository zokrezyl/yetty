...
static void on_dialog_button(struct yetty_ygui_old_widget *dlg, int idx, void *u)
{
    (void)dlg; (void)u;
...
static struct yetty_ygui_old_widget *g_dlg = NULL;

static void on_open_dialog(struct yetty_ygui_old_widget *btn, void *u)
{
    (void)btn; (void)u;
    yetty_ygui_old_widget_popup_set_open(g_dlg, 1);
}
...
int main(void)
{
    ...
    struct ygui_engine_ptr_result eng_r = yetty_ygui_old_engine_create(
        (struct yetty_ygui_old_engine_args){.name = "dialog-demo"});
    ...
    struct yetty_ygui_old_widget *btn = yetty_ygui_old_engine_button(engine, "open", 0, 0, 140, 36,
                                                              "Save…");
    yetty_ygui_old_widget_button_on_click(btn, on_open_dialog, NULL);
    yetty_ygui_old_widget_add_child(g_outer, btn);

    g_status = yetty_ygui_old_engine_label(engine, "status", 0, 0,
                                       "Click \"Save…\" to open the dialog.");
    yetty_ygui_old_widget_add_child(g_outer, g_status);

    const char *btns[] = {"Cancel", "Save", "Discard"};
    struct yetty_ygui_old_dialog_args args = {
        .id = "save_q",
        .title = "Save changes?",
    ...
    g_dlg = yetty_ygui_old_engine_dialog(engine, &args);
