...
struct yetty_yclass_ptr_result yetty_demoygui_43_tree_showcase_class_get(void)
{
    ...
    if (YETTY_IS_ERR(parent_class_r)) {
        yerror("yetty_demoygui_43_tree_showcase_class_get: parent accessor failed: %s", parent_class_r.error.msg);
        return YETTY_ERR(yetty_yclass_ptr, "yetty_demoygui_43_tree_showcase_class_get: parent accessor failed", parent_class_r);
    }
    struct yetty_yclass_ptr_result register_class_r =
        yetty_yclass_register(&desc, ops, sizeof(ops) / sizeof(ops[0]),
                              parent_class_r.value, NULL, 0);
    if (YETTY_IS_ERR(register_class_r)) {
        yerror("yetty_demoygui_43_tree_showcase_class_get: class_register failed: %s", register_class_r.error.msg);
        return YETTY_ERR(yetty_yclass_ptr, "yetty_demoygui_43_tree_showcase_class_get: class_register failed", register_class_r);
    }
    cls = register_class_r.value;
    return register_class_r;
}

struct yetty_demoygui_43_tree_showcase_ptr_result yetty_demoygui_43_tree_showcase_from(struct yetty_yclass_object *obj)
{
    struct yetty_yclass_ptr_result class_r = yetty_demoygui_43_tree_showcase_class_get();
    if (YETTY_IS_ERR(class_r))
        return YETTY_ERR(yetty_demoygui_43_tree_showcase_ptr, "yetty_demoygui_43_tree_showcase_from: class accessor", class_r);
    struct yetty_yclass_void_ptr_result slice_r =
        yetty_yclass_object_data(obj, class_r.value);
    if (YETTY_IS_ERR(slice_r))
        return YETTY_ERR(yetty_demoygui_43_tree_showcase_ptr, "yetty_demoygui_43_tree_showcase_from: object_data", slice_r);
    return YETTY_OK(yetty_demoygui_43_tree_showcase_ptr, (struct yetty_demoygui_43_tree_showcase *)slice_r.value);
}

struct yetty_yclass_object_ptr_result yetty_demoygui_43_tree_showcase_to(struct yetty_demoygui_43_tree_showcase *data)
{
    if (!data)
        return YETTY_OK(yetty_yclass_object_ptr, NULL);
    struct yetty_yclass_ptr_result class_r = yetty_demoygui_43_tree_showcase_class_get();
    YETTY_RETURN_IF_ERR(yetty_yclass_object_ptr, class_r, "yetty_demoygui_43_tree_showcase_to: class accessor");
    struct yetty_ycore_size_result offset_r =
        yetty_yclass_object_data_offset(class_r.value, class_r.value);
    YETTY_RETURN_IF_ERR(yetty_yclass_object_ptr, offset_r, "yetty_demoygui_43_tree_showcase_to: data offset");
    return YETTY_OK(yetty_yclass_object_ptr,
                    (struct yetty_yclass_object *)((char *)data - offset_r.value));
}


struct yetty_yclass_object_ptr_result yetty_demoygui_43_tree_showcase_create(struct yetty_yclass_ctx *ctx);
struct yetty_yclass_object_ptr_result yetty_demoygui_43_tree_showcase_create(struct yetty_yclass_ctx *ctx)
{
    ...
    if (YETTY_IS_ERR(class_accessor_r))
        return YETTY_ERR(yetty_yclass_object_ptr,
                         "yetty_demoygui_43_tree_showcase_create: class accessor failed", class_accessor_r);
     ...
     * response isn't silently swallowed. */
    {
        struct yetty_ycore_void_result translate_class_r =
            yetty_yclass_rpc_session_translate_class(ctx->session, "yetty_demoygui_43_tree_showcase");
        if (YETTY_IS_ERR(translate_class_r)) {
            yetty_ycore_error_print(stderr,
                "yetty_demoygui_43_tree_showcase_create: translate_class (degraded — will lazy-resolve)",
    ...
    if (YETTY_IS_ERR(create_call_r))
        return YETTY_ERR(yetty_yclass_object_ptr,
                         "yetty_demoygui_43_tree_showcase_create: CREATE call failed", create_call_r);
    ...
    if (!proxy)
        return YETTY_ERR(yetty_yclass_object_ptr, "yetty_demoygui_43_tree_showcase_create: calloc(proxy) failed");
