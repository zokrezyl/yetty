...
static struct yetty_ygui_widget* g_coord_label = NULL;
static struct yetty_ygui_widget* g_hit_label = NULL;
...
static void on_test_click(struct yetty_ygui_widget* widget, void* userdata) {
    (void)userdata;
    fprintf(stderr, "[COORD_DEBUG] *** BUTTON CLICKED: %s ***\n", yetty_ygui_widget_id(widget));
    yetty_ygui_widget_label_set_text(g_hit_label, "HIT: Button clicked!");
}
...
int main(void) {
    ...
    /* Title */
    yetty_ygui_engine_label(g_engine, "title", 20, 15, "Coordinate Debug");

    /* Info showing expected button position */
    char info[128];
    snprintf(info, sizeof(info), "Button: x=%d y=%d w=%d h=%d", BTN_X, BTN_Y, BTN_W, BTN_H);
    yetty_ygui_engine_label(g_engine, "info", 20, 45, info);

    /* The test button */
    fprintf(stderr, "[DEMO] Creating test button at x=%d, y=%d, w=%d, h=%d\n",
            BTN_X, BTN_Y, BTN_W, BTN_H);
    struct yetty_ygui_widget* test_btn = yetty_ygui_engine_button(g_engine, "test_btn", BTN_X, BTN_Y, BTN_W, BTN_H, "TEST BUTTON");
    yetty_ygui_widget_button_on_click(test_btn, on_test_click, NULL);

    /* Labels to show coordinates */
    g_coord_label = yetty_ygui_engine_label(g_engine, "coords", 20, 170, "Click near button...");
    g_hit_label = yetty_ygui_engine_label(g_engine, "hit", 20, 195, "");

    /* Quit button */
    struct yetty_ygui_widget* quit_btn = yetty_ygui_engine_button(g_engine, "quit", 300, 200, 80, 35, "Quit");
