 ...
 *
 * Run via:  ./yetty -e demo-yrdawn-07-submit-chain
 * Trace at: /tmp/yrdawn-demo-07-submit-chain.trace
 */
#include <stdio.h>
#include <stdint.h>
#include <unistd.h>
...
#define LOG(...) do { if (trace) fprintf(trace, __VA_ARGS__); } while (0)
    ...
    if (!canvas) {
        LOG("07_submit_chain: bringup failed\n");
        return 1;
    }
    ...
    uint64_t cbs[1] = { cb };
    (void)yrdawn_client_wgpuQueueSubmit(c, queue, 1,
