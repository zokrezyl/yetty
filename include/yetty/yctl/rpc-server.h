#ifndef YETTY_YCTL_RPC_SERVER_H
#define YETTY_YCTL_RPC_SERVER_H
...
/* Result types */
YETTY_YRESULT_DECLARE(yetty_rpc_server_ptr, struct yetty_yctl_server *);
...
#define YETTY_YCTL_HANDLER_OK_DATA(d, l)                                                           \
    ((struct yetty_rpc_handler_result){.ok = 1, .value = {.data = (d), .len = (l)}})

/* Create success result with bool */
#define YETTY_YCTL_HANDLER_OK_BOOL(v)                                                              \
    ((struct yetty_rpc_handler_result){.ok = 1,                                                    \
                                       .value = {.data = NULL, .len = 0, .bool_value = (v)}})

/* Create error result */
#define YETTY_YCTL_HANDLER_ERR(msg) ((struct yetty_rpc_handler_result){.ok = 0, .error = (msg)})
...
typedef struct yetty_rpc_handler_result (*yetty_rpc_handler_fn)(
    const struct yetty_yctl_message *msg, void *userdata);
