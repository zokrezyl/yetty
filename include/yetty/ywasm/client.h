/*
 * ywasm/client.h — pure-C side of the WebGPU-over-OSC bridge.
 *
 ...
 */
#ifndef YETTY_YWASM_CLIENT_H
#define YETTY_YWASM_CLIENT_H
...
YETTY_YRESULT_DECLARE(yetty_ywasm_client_ptr, struct yetty_ywasm_client *);
 ...
 * status is enum yetty_ywasm_reply_status. The slot is freed before
 * the callback runs. */
 ...
 * kind is enum yetty_ywasm_event_kind; body is the kind-specific tail. */
typedef void (*yetty_ywasm_event_cb)(void *user, uint32_t kind, uint64_t device_handle,
                                     const uint8_t *body, size_t body_len);

/* Fires when a SC_KEY input frame arrives. kind is enum
 * yetty_ywasm_input_key_kind; key/mods/codepoint match the wire struct. */
typedef void (*yetty_ywasm_input_key_cb)(void *user, uint32_t kind, int32_t key,
                                         int32_t mods, uint32_t codepoint);

/* Fires when the layer's pane pixel size changes. */
typedef void (*yetty_ywasm_input_resize_cb)(void *user, float width, float height);
...
/* Monotonic u64 handle allocator. Never returns YETTY_YWASM_HANDLE_NULL.
...
/* Ship a single RGBA8 frame to the local yetty's ywasm-layer. The
