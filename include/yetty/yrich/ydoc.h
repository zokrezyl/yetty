...
struct yetty_ycore_void_result yetty_yrich_paragraph_run_get(struct yetty_yclass_object *obj, size_t index, int32_t *out_start, int32_t *out_end, uint32_t *out_format, uint32_t *out_color);
/* Append one styled run — loader-side; ranges must arrive sorted and
 * non-overlapping (the writer emits them that way). */
struct yetty_ycore_void_result yetty_yrich_paragraph_add_run(struct yetty_yclass_object *obj, int32_t start, int32_t end, uint32_t format, uint32_t color);
