...
struct yetty_ydraw_core_font_prim_view {
    int32_t font_id;
...
int yetty_ydraw_core_font_prim_parse(const uint32_t *prim,
                                      struct yetty_ydraw_core_font_prim_view *out YETTY_ANNOT_OUT);

/* Flyweight base ops handler. Returns ops only for type FONT. Register
 * via yetty_ydraw_core_flyweight_registry_add(reg, FONT, FONT, handler). */
