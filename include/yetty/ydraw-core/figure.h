...
struct yetty_ydraw_raw_figure_ops {
    void (*destroy)(struct yetty_ydraw_raw_figure *self);

    struct yetty_yrender_gpu_resource_set_result (*get_gpu_resource_set)(
        struct yetty_ydraw_raw_figure *self);
};

struct yetty_ydraw_raw_figure {
    const struct yetty_ydraw_raw_figure_ops *ops;
    uint32_t type;
    uint32_t size;
};
