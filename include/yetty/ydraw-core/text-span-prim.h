...
struct yetty_ydraw_core_text_span_prim_view {
    float x, y;
    float font_size;
    float rotation;
    uint32_t color;
...
int yetty_ydraw_core_text_span_prim_parse(
    const uint32_t *prim, struct yetty_ydraw_core_text_span_prim_view *out YETTY_ANNOT_OUT);
