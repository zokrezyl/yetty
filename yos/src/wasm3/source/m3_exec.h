...
#ifdef DEBUG
  #define d_outOfBounds { fprintf(stderr, "OUT OF BOUNDS: mem_len=%zu operand=%llu\n", (size_t)_mem->length, (unsigned long long)operand); \
...
d_m3Op  (CallRawFunction)
{
    ...
    // Threads hack: with shared linear memory the M3MemoryHeader.runtime back
    // pointer always points at the *master* runtime. Going through it from a
    // sibling thread would let two threads stomp on each other's stack-swap
    // and corrupt master->stack. Use the per-call function's own runtime.
    IM3Runtime runtime = ctx.function->module->runtime;
...
// memory.init segment_idx, memory_idx — copies bytes from a passive data
// segment into linear memory.  Stack: [dst, src, size] (size on top).
// Compiled bytecode after the op:  src_slot, dst_slot, segment_pointer.
d_m3Op  (MemInit)
{
    u32 size       = (u32) _r0;
    u64 src        = slot (u32);
    u64 destination = slot (u32);
    M3DataSegment * seg = (M3DataSegment *) immediate (void *);

    if (size > seg->size)                              d_outOfBoundsMemOp (src, size);
    if (src + size > seg->size)                        d_outOfBoundsMemOp (src, size);
    if (destination + size > _mem->length)             d_outOfBoundsMemOp (destination, size);

    if (size > 0)
        memcpy (m3MemData (_mem) + destination, seg->data + src, size);

    nextOp ();
}


// data.drop segment_idx — marks the passive data segment as dropped (size 0).
// Subsequent memory.init on this segment must trap unless `size==0 && src==0`.
d_m3Op  (DataDrop)
{
    M3DataSegment * seg = (M3DataSegment *) immediate (void *);
    seg->size = 0;
...
d_m3Op  (Entry)
{
    ...
    if (true)
#else
    // Threads hack: with sibling runtimes the linear-memory header is shared,
    // so `_mem->maxStack` may belong to a different thread's stack. Derive
    // the per-runtime stack bound from function->module->runtime instead.
    void * _m3_runtimeStackEnd =
        (void *) ((m3slot_t *) function->module->runtime->stack
                  + function->module->runtime->numStackSlots);
    if (LIKELY ((void *) (_sp + function->maxStackSlots) < _m3_runtimeStackEnd))
#endif
    {
#if defined(DEBUG)
        function->hits++;
...
//---------------------------------------------------------------------------------------------------------------------
// Atomic memory ops (threads proposal, 0xFE prefix)
//
// All atomic ops use the all-slots form: every wasm-stack operand is materialised
// into a slot before the op runs (the compiler uses PreserveRegisterIfOccupied
// to force this). The bytecode stream after the op pointer is:
//     [value slots in stack-top-first order] [memory offset u32]
// or for fence:
//     (nothing)
//
// Wasm requires natural alignment for atomic accesses; an unaligned address
// traps. We reuse the OOB trap code rather than introducing a new constant.
//---------------------------------------------------------------------------------------------------------------------

#include "m3_atomic.h"

// reuse the existing OOB trap macro for misalignment as well
#define d_atomicMisaligned d_outOfBounds

#define d_m3AtomicLoad(WASM_T, MEM_T)                                       \
d_m3Op (WASM_T##_AtomicLoad_##MEM_T)                                        \
{                                                                           \
    u64 operand = slot (u32);                                               \
    u32 offset  = immediate (u32);                                          \
    operand += offset;                                                      \
                                                                            \
    if (m3MemCheck (operand + sizeof (MEM_T) <= _mem->length))              \
    {                                                                       \
        if ((operand & (sizeof (MEM_T) - 1)) != 0) d_atomicMisaligned;      \
        MEM_T * p = (MEM_T *)(m3MemData (_mem) + operand);                  \
        MEM_T v = __atomic_load_n (p, __ATOMIC_SEQ_CST);                    \
        _r0 = (WASM_T) v;                                                   \
        nextOp ();                                                          \
    } else d_outOfBounds;                                                   \
}

#define d_m3AtomicStore(WASM_T, MEM_T)                                      \
d_m3Op (WASM_T##_AtomicStore_##MEM_T)                                       \
{                                                                           \
    WASM_T value = slot (WASM_T);                                           \
    u64 operand  = slot (u32);                                              \
    u32 offset   = immediate (u32);                                         \
    operand += offset;                                                      \
                                                                            \
    ...
    {                                                                       \
        if ((operand & (sizeof (MEM_T) - 1)) != 0) d_atomicMisaligned;      \
        MEM_T * p = (MEM_T *)(m3MemData (_mem) + operand);                  \
        __atomic_store_n (p, (MEM_T) value, __ATOMIC_SEQ_CST);              \
        nextOp ();                                                          \
    } else d_outOfBounds;                                                   \
}

#define d_m3AtomicRMW(WASM_T, MEM_T, OP_NAME, FETCH_FN)                     \
d_m3Op (WASM_T##_Atomic##OP_NAME##_##MEM_T)                                 \
{                                                                           \
    WASM_T value = slot (WASM_T);                                           \
    u64 operand  = slot (u32);                                              \
    u32 offset   = immediate (u32);                                         \
    ...
    {                                                                       \
        if ((operand & (sizeof (MEM_T) - 1)) != 0) d_atomicMisaligned;      \
        MEM_T * p = (MEM_T *)(m3MemData (_mem) + operand);                  \
        MEM_T old = FETCH_FN (p, (MEM_T) value, __ATOMIC_SEQ_CST);          \
        _r0 = (WASM_T) old;                                                 \
        nextOp ();                                                          \
    } else d_outOfBounds;                                                   \
}

#define d_m3AtomicCmpxchg(WASM_T, MEM_T)                                    \
d_m3Op (WASM_T##_AtomicCmpxchg_##MEM_T)                                     \
{                                                                           \
    WASM_T replacement = slot (WASM_T);                                     \
    WASM_T expected    = slot (WASM_T);                                     \
    u64    operand     = slot (u32);                                        \
    u32    offset      = immediate (u32);                                   \
    operand += offset;                                                      \
                                                                            \
    ...
    {                                                                       \
        if ((operand & (sizeof (MEM_T) - 1)) != 0) d_atomicMisaligned;      \
        MEM_T * p = (MEM_T *)(m3MemData (_mem) + operand);                  \
        MEM_T exp = (MEM_T) expected;                                       \
        (void) __atomic_compare_exchange_n (                                \
            p, &exp, (MEM_T) replacement,                                   \
            0 /*strong*/, __ATOMIC_SEQ_CST, __ATOMIC_SEQ_CST);              \
        _r0 = (WASM_T) exp; /* old value (matches expected on success) */   \
        nextOp ();                                                          \
    } else d_outOfBounds;                                                   \
}

// Loads
d_m3AtomicLoad (u32, u32)
d_m3AtomicLoad (u32, u8)
d_m3AtomicLoad (u32, u16)
d_m3AtomicLoad (u64, u64)
d_m3AtomicLoad (u64, u8)
d_m3AtomicLoad (u64, u16)
d_m3AtomicLoad (u64, u32)

// Stores
d_m3AtomicStore (u32, u32)
d_m3AtomicStore (u32, u8)
d_m3AtomicStore (u32, u16)
d_m3AtomicStore (u64, u64)
d_m3AtomicStore (u64, u8)
d_m3AtomicStore (u64, u16)
d_m3AtomicStore (u64, u32)

// RMW: add / sub / and / or / xor / xchg, all i32 + i64 widths.
// (Sub uses fetch_sub; xchg uses exchange — covered with one extra macro below.)
#define d_m3AllRMW_W(WASM_T, MEM_T)                                                          \
    d_m3AtomicRMW (WASM_T, MEM_T, RmwAdd,  __atomic_fetch_add)                               \
    d_m3AtomicRMW (WASM_T, MEM_T, RmwSub,  __atomic_fetch_sub)                               \
    d_m3AtomicRMW (WASM_T, MEM_T, RmwAnd,  __atomic_fetch_and)                               \
    d_m3AtomicRMW (WASM_T, MEM_T, RmwOr,   __atomic_fetch_or)                                \
    d_m3AtomicRMW (WASM_T, MEM_T, RmwXor,  __atomic_fetch_xor)                               \
    d_m3Op (WASM_T##_AtomicRmwXchg_##MEM_T)                                                  \
    {                                                                                        \
        WASM_T value = slot (WASM_T);                                                        \
        u64 operand  = slot (u32);                                                           \
        u32 offset   = immediate (u32);                                                      \
        ...
        {                                                                                    \
            if ((operand & (sizeof (MEM_T) - 1)) != 0) d_atomicMisaligned;                   \
            MEM_T * p = (MEM_T *)(m3MemData (_mem) + operand);                               \
            MEM_T old = __atomic_exchange_n (p, (MEM_T) value, __ATOMIC_SEQ_CST);            \
            _r0 = (WASM_T) old;                                                              \
            nextOp ();                                                                       \
        } else d_outOfBounds;                                                                \
    }                                                                                        \
    d_m3AtomicCmpxchg (WASM_T, MEM_T)

d_m3AllRMW_W (u32, u32)
d_m3AllRMW_W (u32, u8)
d_m3AllRMW_W (u32, u16)
d_m3AllRMW_W (u64, u64)
d_m3AllRMW_W (u64, u8)
d_m3AllRMW_W (u64, u16)
d_m3AllRMW_W (u64, u32)

// memory.atomic.wait32: stack(addr i32, expected i32, timeout i64) -> i32
d_m3Op (Memory_AtomicWait32)
{
    i64 timeout  = slot (i64);
    u32 expected = slot (u32);
    u64 operand  = slot (u32);
    u32 offset   = immediate (u32);
    operand += offset;

    if (m3MemCheck (operand + sizeof (u32) <= _mem->length))
    {
        if ((operand & 3) != 0) d_atomicMisaligned;
        u32 * p = (u32 *)(m3MemData (_mem) + operand);
        i32 r = m3Atomic_Wait32 ((volatile u32 *) p, expected, timeout);
        _r0 = (u32) r;
        nextOp ();
    } else d_outOfBounds;
}

// memory.atomic.wait64: stack(addr i32, expected i64, timeout i64) -> i32
d_m3Op (Memory_AtomicWait64)
{
    i64 timeout  = slot (i64);
    u64 expected = slot (u64);
    u64 operand  = slot (u32);
    u32 offset   = immediate (u32);
    operand += offset;

    if (m3MemCheck (operand + sizeof (u64) <= _mem->length))
    {
        if ((operand & 7) != 0) d_atomicMisaligned;
        u64 * p = (u64 *)(m3MemData (_mem) + operand);
        i32 r = m3Atomic_Wait64 ((volatile u64 *) p, expected, timeout);
        _r0 = (u32) r;
        nextOp ();
    } else d_outOfBounds;
}

// memory.atomic.notify: stack(addr i32, count i32) -> i32 (woken)
d_m3Op (Memory_AtomicNotify)
{
    u32 count    = slot (u32);
    u64 operand  = slot (u32);
    u32 offset   = immediate (u32);
    ...
    if (m3MemCheck (operand + sizeof (u32) <= _mem->length))
    {
        if ((operand & 3) != 0) d_atomicMisaligned;
        u32 * p = (u32 *)(m3MemData (_mem) + operand);
        u32 woken = m3Atomic_Notify (p, count);
        _r0 = woken;
        nextOp ();
    } else d_outOfBounds;
}

// atomic.fence: full SeqCst barrier. No operands, no immediates beyond the
// 0-byte the parser already consumed.
d_m3Op (Atomic_Fence)
{
