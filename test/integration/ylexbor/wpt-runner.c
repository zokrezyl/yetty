...
static char *splice_harness(const char *html, size_t hlen,
			     const char *harness, size_t hslen,
			     size_t *out_len)
{
	...
	if (!out) return NULL;
	size_t off = 0;
	memcpy(out + off, html, insert_after); off += insert_after;
	memcpy(out + off, prefix, plen);       off += plen;
	memcpy(out + off, harness, hslen);     off += hslen;
	memcpy(out + off, suffix, slen);       off += slen;
...
static char *read_results_json(struct yetty_ylexbor *r)
{
	...
	const char *expr =
		...
		"})()";
	JSValue v = JS_Eval(ctx, expr, strlen(expr),
		"<wpt-runner-readback>", JS_EVAL_TYPE_GLOBAL);
	if (JS_IsException(v)) {
...
struct counts { int pass, fail, timeout, notrun, total; };

static int dispatch_results(const char *json, int tap,
			    struct counts *cnt, int tap_offset,
			    const char *test_path)
{
	...
	while (p < end) {
		...
		if      (strcmp(status, "PASS")    == 0) cnt->pass++;
		else if (strcmp(status, "FAIL")    == 0) cnt->fail++;
		else if (strcmp(status, "TIMEOUT") == 0) cnt->timeout++;
		else                                     cnt->notrun++;
		cnt->total++;
		...
		if (tap) {
			 ...
			 * grep-able. */
			fprintf(stdout, "{\"src\":\"%s\",\"r\":",
				test_path ? test_path : "<inline>");
...
static int run_one(const char *html_path, const char *harness,
		   size_t harness_len, int boot_budget_ms,
		   int tap, struct counts *cnt, int tap_offset)
{
	size_t hlen = 0;
	...
	struct yetty_ycore_void_result lr =
		yetty_ylexbor_load_html(r, spliced, splen);
	if (YETTY_IS_ERR(lr)) {
	...
	while (boot_budget_ms > 0) {
		...
		if (elapsed >= boot_budget_ms) break;
	}
...
static int run_one_in_subprocess(const char *self_path,
				 ...
				 int tap, struct counts *cnt)
{
	...
	pid_t pid = fork();
	if (pid < 0) { close(pipe_fd[0]); close(pipe_fd[1]); return -1; }
	if (pid == 0) {
		...
		char *args[] = {
			(char *)self_path,
			tap ? "--tap" : "--json",
			"--harness", (char *)harness_path,
			"--boot-budget-ms", budget_buf,
			"--html", (char *)test_path,
			NULL,
		};
	...
	while (1) {
		...
		if (tap) {
			if (strncmp(line, "TAP version", 11) == 0) continue;
			if (strncmp(line, "1..",         3) == 0) continue;
			...
			if (strncmp(line, "ok ", 3) == 0) {
				tag = "ok"; after = line + 3; is_pass = 1;
			} else if (strncmp(line, "not ok ", 7) == 0) {
				tag = "not ok"; after = line + 7; is_pass = 0;
			}
			if (tag) {
				...
				while (alen > 0 && (after[alen - 1] == '\n' ||
						    after[alen - 1] == '\r')) alen--;
				if (is_pass) { cnt->pass++; local_pass++; }
				else         { cnt->fail++; local_fail++; }
				cnt->total++;
			...
			if (strstr(line, "\"summary\":true")) continue;
			fputs(line, stdout);
			if (strstr(line, "\"status\":\"PASS\"")) {
				cnt->pass++; local_pass++; cnt->total++;
			} else if (strstr(line, "\"status\":\"FAIL\"")) {
				cnt->fail++; local_fail++; cnt->total++;
			}
		}
	}
	free(line);
...
static int walk_suite(const char *dir, const char *harness,
		      size_t harness_len, int boot_budget_ms,
		      int tap, struct counts *cnt)
{
	(void)harness; (void)harness_len;
	DIR *d = opendir(dir);
	if (!d) {
	...
	for (size_t i = 0; i < n; i++) {
		...
		if (stat(path, &st) == 0) {
			if (S_ISDIR(st.st_mode)) {
				walk_suite(path, harness, harness_len,
					   boot_budget_ms, tap, cnt);
			} else if (has_html_suffix(names[i])) {
				run_one_in_subprocess(g_self_path,
					g_harness_path, path,
					boot_budget_ms, tap, cnt);
			}
		}
...
int main(int argc, char **argv)
{
	...
	for (int i = 1; i < argc; i++) {
		const char *a = argv[i];
		if      (!strcmp(a, "--html")    && i + 1 < argc) html_path  = argv[++i];
		else if (!strcmp(a, "--suite")   && i + 1 < argc) suite_path = argv[++i];
		else if (!strcmp(a, "--harness") && i + 1 < argc) harness_path = argv[++i];
		else if (!strcmp(a, "--boot-budget-ms") && i + 1 < argc)
			boot_budget_ms = atoi(argv[++i]);
		else if (!strcmp(a, "--tap"))  tap = 1;
		else if (!strcmp(a, "--json")) tap = 0;
		else if (!strcmp(a, "-h") || !strcmp(a, "--help")) {
			usage(argv[0]); return 0;
		}
		else { usage(argv[0]); return 2; }
	}
	if (!html_path && !suite_path) { usage(argv[0]); return 2; }
	...
	if (html_path) {
		(void)run_one(html_path, harness, harness_len,
			      boot_budget_ms, tap, &cnt, 0);
	}
	if (suite_path) {
		(void)walk_suite(suite_path, harness, harness_len,
				 boot_budget_ms, tap, &cnt);
	}
	if (tap) {
		fprintf(stdout, "1..%d\n", cnt.total);
