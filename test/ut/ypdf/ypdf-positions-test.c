 ...
 * NUL-terminated. */
static int text_contains(const struct yetty_ydraw_core_text_span_prim_view *view,
                         const char *needle)
{
...
int main(void)
{
    ...
    struct yetty_ydraw_core_flyweight_registry_ptr_result rfr = yetty_ydraw_flyweight_create();
    REQUIRE(rfr.ok, "flyweight_create failed");
    struct yetty_ydraw_core_flyweight_registry *reg = rfr.value;
    ...
    for (;;) {
        ...
        if (t == YETTY_YDRAW_TYPE_TEXT_SPAN) {
            ...
            if (yetty_ydraw_core_text_span_prim_parse(it.fw.data, &v) == 0) {
                if (dump_count++ < 320) {
                    char snippet[64];
    ...
    for (size_t li = 0; li < n_lines; li++) {
        ...
        struct yetty_ydraw_core_primitive_iter it3 = ir2.value;
        float ypdf_left = 1e9f, ypdf_right_estimate = -1e9f;
        int found = 0;
        for (;;) {
            if (it3.fw.data[0] == YETTY_YDRAW_TYPE_TEXT_SPAN) {
                struct yetty_ydraw_core_text_span_prim_view v;
                if (yetty_ydraw_core_text_span_prim_parse(it3.fw.data, &v) == 0 &&
    ...
    for (;;) {
        if (it4.fw.data[0] == YETTY_YDRAW_TYPE_FONT) {
            ...
            if (yetty_ydraw_core_font_prim_parse(it4.fw.data, &fv) == 0 && fv.font_id >= 0 &&
    ...
    for (;;) {
        if (it5.fw.data[0] == YETTY_YDRAW_TYPE_TEXT_SPAN) {
            struct yetty_ydraw_core_text_span_prim_view v;
            if (yetty_ydraw_core_text_span_prim_parse(it5.fw.data, &v) == 0 && v.text_len > 0 &&
    ...
    if (ir5.ok) {
        ...
        for (;;) {
            if (dit.fw.data[0] == YETTY_YDRAW_TYPE_TEXT_SPAN) {
                struct yetty_ydraw_core_text_span_prim_view dv;
                if (yetty_ydraw_core_text_span_prim_parse(dit.fw.data, &dv) == 0 &&
