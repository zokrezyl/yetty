 ...
 * NUL-terminated. */
static int text_contains(const struct yetty_ydraw_text_span_prim_view *view,
                         const char *needle)
{
...
int main(void)
{
    ...
    struct yetty_ydraw_primitive_iter_result ir =
        yetty_ydraw_draw_list_prim_first(out->buffer, reg);
    ...
    for (;;) {
        ...
        if (t == YETTY_YDRAW_TYPE_TEXT_SPAN) {
            ...
            if (yetty_ydraw_text_span_prim_parse(it.fw.data, &v) == 0) {
                if (dump_count++ < 320) {
                    char snippet[64];
        ...
        struct yetty_ydraw_primitive_iter_result nx =
            yetty_ydraw_draw_list_prim_next(out->buffer, reg, &it);
        if (!nx.ok) break;
        it = nx.value;
    }
    ...
    for (size_t li = 0; li < n_lines; li++) {
        ...
        for (;;) {
            if (it3.fw.data[0] == YETTY_YDRAW_TYPE_TEXT_SPAN) {
                struct yetty_ydraw_text_span_prim_view v;
                if (yetty_ydraw_text_span_prim_parse(it3.fw.data, &v) == 0 &&
            ...
            struct yetty_ydraw_primitive_iter_result nx3 =
                yetty_ydraw_draw_list_prim_next(out->buffer, reg, &it3);
            if (!nx3.ok) break;
            it3 = nx3.value;
        }
    ...
    for (;;) {
        if (it4.fw.data[0] == YETTY_YDRAW_TYPE_FONT) {
            ...
            if (yetty_ydraw_font_prim_parse(it4.fw.data, &fv) == 0 && fv.font_id >= 0 &&
        ...
        struct yetty_ydraw_primitive_iter_result nx4 =
            yetty_ydraw_draw_list_prim_next(out->buffer, reg, &it4);
        if (!nx4.ok) break;
        it4 = nx4.value;
    }
    ...
    for (;;) {
        if (it5.fw.data[0] == YETTY_YDRAW_TYPE_TEXT_SPAN) {
            struct yetty_ydraw_text_span_prim_view v;
            if (yetty_ydraw_text_span_prim_parse(it5.fw.data, &v) == 0 && v.text_len > 0 &&
        ...
        struct yetty_ydraw_primitive_iter_result nx5 =
            yetty_ydraw_draw_list_prim_next(out->buffer, reg, &it5);
        if (!nx5.ok) break;
        it5 = nx5.value;
    }
    ...
    if (ir5.ok) {
        ...
        for (;;) {
            if (dit.fw.data[0] == YETTY_YDRAW_TYPE_TEXT_SPAN) {
                struct yetty_ydraw_text_span_prim_view dv;
                if (yetty_ydraw_text_span_prim_parse(dit.fw.data, &dv) == 0 &&
            ...
            struct yetty_ydraw_primitive_iter_result nxd =
                yetty_ydraw_draw_list_prim_next(out->buffer, reg, &dit);
            if (!nxd.ok) break;
            dit = nxd.value;
        }
    }
