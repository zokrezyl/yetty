                                        ...
                                        size_t _resp_max)
{
    ...
    memcpy(&_a, _body, sizeof(_a));
    struct ctx _local = {0};
    struct str_result _r = yanimal_animal_speak(
        &_local, (struct object *)rpc_handle_resolve(_a.obj_handle), _a.volume);
    if (_resp_max < 1) {
        return 0;
    }
    if (YETTY_IS_ERR(_r)) {
                                      ...
                                      size_t _resp_max)
{
    ...
    struct yetty_ycore_int_result _r =
        yanimal_animal_eat(&_local, (struct object *)rpc_handle_resolve(_a.obj_handle), _a.amount);
    if (_resp_max < 1) {
        return 0;
    }
    if (YETTY_IS_ERR(_r)) {
