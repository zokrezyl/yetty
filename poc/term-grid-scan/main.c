...
static void grid_mutate(struct cell *grid, size_t n, size_t mutate_count, uint32_t threshold,
                        uint32_t *prng)
{
...
static size_t scan_avx2(const struct cell *cells, size_t n, uint32_t *out_idx)
{
    ...
    size_t i = 0;
    for (; i < n_aligned; i += 8) {
        __m256i v = _mm256_i32gather_epi32((const int *)(base + i * 3), indices, 4);
...
static size_t scan_avx2_v2(const struct cell *cells, size_t n, uint32_t *out_idx)
{
    ...
    for (; i < n_aligned; i += 8) {
        ...
        uint32_t bits = ((mA) & 0x01u) |      /* mA bit 0 -> bit 0 (g0) */
...
static struct stats run_one(const char *name, scan_fn fn, struct cell *grid, size_t n,
                            uint32_t *positions, uint64_t *ns_buf, const struct config *cfg,
                            uint32_t threshold)
{
    ...
    printf("%-8s avg=%8.2f us  min=%8.2f  p50=%8.2f  p99=%8.2f  max=%8.2f  "
           ...
           name, s.avg_us, s.min_us, s.p50_us, s.p99_us, s.max_us, s.cells_per_sec / 1e6,
           measured_density);
    return s;
}
...
static int parse_args(int argc, char **argv, struct config *cfg)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (!strcmp(a, "--rows")) {
            cfg->rows = (uint32_t)strtoul(v, NULL, 10);
        } else if (!strcmp(a, "--cols")) {
            cfg->cols = (uint32_t)strtoul(v, NULL, 10);
        } else if (!strcmp(a, "--frames")) {
            cfg->frames = (uint32_t)strtoul(v, NULL, 10);
        } else if (!strcmp(a, "--warmup")) {
            cfg->warmup = (uint32_t)strtoul(v, NULL, 10);
        } else if (!strcmp(a, "--shader-density")) {
            cfg->density = strtod(v, NULL);
        } else if (!strcmp(a, "--mutate-pct")) {
            cfg->mutate_pct = strtod(v, NULL);
        } else if (!strcmp(a, "--seed")) {
            cfg->seed = (uint32_t)strtoul(v, NULL, 10);
...
int main(int argc, char **argv)
{
    ...
    printf("grid=%ux%u (%zu cells, %.2f MB)  density=%.4f  mutate=%.2f%%  "
           ...
           cfg.rows, cfg.cols, n, (double)(n * sizeof(struct cell)) / (1024.0 * 1024.0),
           cfg.density, cfg.mutate_pct * 100.0, cfg.frames, cfg.warmup, cfg.seed);
    printf(
        "variant   ----time per frame----                                   throughput     hits\n");
