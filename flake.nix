    ...
    flake-utils.lib.eachDefaultSystem (system:
      ...
      in {
        devShells = {
          ...
          # NetSurf-all 3.11 (TARGET=monkey) needs more system deps than
          # the generic linux-x86_64 shell carries — it isn't just a CMake
          # project. Header-time deps come from libdom's expat binding
          # (expat) and netsurf core's image content handlers (libxml2,
          # libjpeg, libpng, libwebp). Curl + openssl are link-only at
          # core build time but the Makefile probes them via pkg-config,
          # so they stay in buildInputs too. flex/bison are required by
          # libcss's autogenerated parser tables.
          "3rdparty-netsurf" = pkgs.mkShell {
            buildInputs = with pkgs; [
              gnumake perl python3 flex bison
              gcc binutils
              curl gnutar xz gzip
              pkg-config
